...
namespace utopia {
    inline double volume(LibMeshFunctionSpace &V, int block = -1) {
        ...
        utopia::assemble(integral(interpolate(x, u), block), volume);
    ...
    void SemiGeometricMultigrid::generate_coarse_meshes(const libMesh::MeshBase &fine_mesh,
    ...
    std::unique_ptr<libMesh::MeshBase> SemiGeometricMultigrid::generate_box_mesh(const libMesh::MeshBase &fine_mesh,
                                                                                 const std::size_t n_levels) {
        ...
        switch (dim) {
            case 2: {
                const int n_segments =
                    std::max(2, int(std::round(std::sqrt(fine_mesh.n_nodes() / std::pow(4, n_coarse_spaces)))));
                ...
                libMesh::MeshTools::Generation::build_square(
                    *m, nx, ny, bb.min()(0), bb.max()(0), bb.min()(1), bb.max()(1), libMesh::QUAD4);
            ...
            case 3: {
                const int n_segments =
                    std::max(2, int(std::round(std::cbrt(fine_mesh.n_nodes() / std::pow(8, n_coarse_spaces)))));
                ...
                libMesh::MeshTools::Generation::build_cube(*m,
                                                           nx,
                                                           ny,
                                                           nz,
                                                           bb.min()(0),
    ...
    void SemiGeometricMultigrid::init(const libMesh::System &es, const std::size_t n_levels) {
        ...
        for (std::size_t i = 0; i < n_coarse_spaces; ++i) {
            ...
            std::cout << "level: " << i << " n_dofs: " << equation_systems[i]->get_system(0).get_dof_map().n_dofs()
        ...
        for (std::size_t i = 1; i < n_coarse_spaces; ++i) {
            ...
            bool success = assemble_volume_transfer(comm,
                                                    make_ref(*meshes[i - 1]),
                                                    make_ref(*meshes[i]),
    ...
    void SemiGeometricMultigrid::clamp_operator(USparseMatrix &T) const {
        ...
        sum_T_cols.transform_values(
            [this](const Scalar &val) -> Scalar { return (std::abs(val) < clamp_tol_) ? 1.0 : 0.0; });
        ...
        if (sum_T_cols.comm().rank() == 0) {
            std::cout << original_sum << " - " << removed_coarse_sum << " (>) " << (original_sum > removed_coarse_sum)
                      << std::endl;

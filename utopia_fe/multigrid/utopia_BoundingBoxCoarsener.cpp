...
namespace utopia {
	...
	public:
		...
		static void build_mesh_from_box(
			const moonolith::AABB<2, double> &box,
		...
		static void build_mesh_from_box(
			const moonolith::AABB<3, double> &box,
			...
			libMesh::MeshBase &coarse_mesh)
		{
			...
			for(int i = 0; i < comm.size(); ++i) {
				...
				for(auto m_ptr : sub_meshes) {
					...
					for(auto n_it = m_ptr->active_nodes_begin(); n_it != m_ptr->active_nodes_end(); ++n_it) {
						auto n_ptr = coarse_mesh.add_point(**n_it, node_id_offset + (*n_it)->id(), coarse_mesh.processor_id());
						++node_count;
					}
	...
	BoundingBoxCoarsener::BoundingBoxCoarsener()
	: impl_(std::make_shared<BoundingBoxCoarsener::Impl>())
	{ }
	...
	const std::vector< std::pair<int, int> > &BoundingBoxCoarsener::get_tags() const
	{
		return impl_->tags;
	}

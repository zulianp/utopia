...
namespace utopia {
    ...
    public:
        ...
        FESpaceAdapter(const std::shared_ptr<libMesh::MeshBase> &master_slave,
                       const std::shared_ptr<libMesh::DofMap> &original_dofmap,
                       const unsigned int var_num,
                       const std::vector<std::pair<int, int>> &tags);

        inline std::shared_ptr<libMesh::MeshBase> &mesh() { return mesh_; }

        inline const std::shared_ptr<libMesh::MeshBase> &mesh() const { return mesh_; }
        ...
        inline std::vector<ElementDofMap> &dof_map() { return dof_maps_; }

        inline const std::vector<ElementDofMap> &dof_map() const { return dof_maps_; }

        inline std::vector<long> &variable_number() { return var_number_; }

        inline const std::vector<long> &variable_number() const { return var_number_; }

        inline std::vector<long> &variable_order() { return var_order_; }

        inline const std::vector<long> &variable_order() const { return var_order_; }

        inline std::vector<long> &variable_type() { return var_type_; }

        inline const std::vector<long> &variable_type() const { return var_type_; }

        inline std::vector<ElementDofMap> &subdomain_id() { return subdomain_id_; }

        inline const std::vector<ElementDofMap> &subdomain_id() const { return subdomain_id_; }

        inline std::vector<ElementDofMap> &side_set_id() { return side_set_id_; }

        inline const std::vector<ElementDofMap> &side_set_id() const { return side_set_id_; }

        inline std::vector<ElementDofMap> &face_set_id_global() { return face_set_id_global_; }

        inline const std::vector<ElementDofMap> &face_set_id_global() const { return face_set_id_global_; }

        inline std::vector<ElementDofMap> &n_face_nodes() { return n_face_nodes_; }

        inline const std::vector<ElementDofMap> &n_face_nodes() const { return n_face_nodes_; }

        inline std::vector<ElementDofMap> &side_set_id_tag() { return side_set_id_tag_; }

        inline const std::vector<ElementDofMap> &side_set_id_tag() const { return side_set_id_tag_; }

        inline std::vector<moonolith::Integer> &ownershipRangesFaceID() { return ownershipRangesFaceID_; }

        inline const std::vector<moonolith::Integer> &ownershipRangesFaceID() const { return ownershipRangesFaceID_; }
    ...
    private:
        ...
        static bool is_tagged_contact_boundary(const libMesh::MeshBase &mesh,
                                               const libMesh::Elem *elem,
                                               const int side_elem,
                                               const std::vector<std::pair<int, int>> &tags);

        static void copy_global_dofs(libMesh::MeshBase &mesh,
                                     const std::shared_ptr<libMesh::DofMap> &original_dofmap,
                                     const unsigned int var_num,
    ...
    template <class Iterator>
    static void write_element_selection(const Iterator &begin,
                                        const Iterator &end,
    ...
    static void read_spaces(moonolith::InputStream &is,
                            FESpaceAdapter &utopiamesh,
                            const libMesh::Parallel::Communicator &comm_mesh) {
        read_space(is,
                   utopiamesh.mesh(),
                   utopiamesh.dof_map(),

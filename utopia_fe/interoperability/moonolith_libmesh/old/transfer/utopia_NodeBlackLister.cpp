...
namespace utopia {
    ...
    void ElementBlackList::read(Input &in) { black_lister_.read(in); }

    void ElementBlackList::init(const libMesh::MeshBase &mesh) { init(black_lister_, mesh); }
    ...
    ElementBlackList::ElementBlackList(const bool is_boundary_mesh) : is_boundary_mesh_(is_boundary_mesh) {}

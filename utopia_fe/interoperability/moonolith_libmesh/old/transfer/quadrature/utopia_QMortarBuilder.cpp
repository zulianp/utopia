...
namespace utopia {
    ...
    inline bool intersect_convex_polygon_with_polyline(const SizeType n_vertices_1,
            ...
            for (SizeType j = 0; j < n_input_points; ++j) {
                ...
                if (is_inside[j]) {
                    ...
                    if ((is_inside[j] != ON_EDGE) && (!is_inside[jp1])) {
                        n_output_points = Intersector::append_point(
                            n_output_points, Intersector::intersect_lines(e1, e2, s, e), output);
                    }
                } else if (is_inside[jp1]) {
                    n_output_points =
                        Intersector::append_point(n_output_points, Intersector::intersect_lines(e1, e2, s, e), output);
    ...
    bool QMortarBuilder1::build(const Elem &trial,
                                FEType trial_type,
                                const Elem &test,
                                FEType test_type,
                                QMortar &q_trial,
    ...
    bool QMortarBuilder2::build(const Elem &trial,
                                FEType trial_type,
                                const Elem &test,
                                FEType test_type,
                                QMortar &q_trial,
        ...
        if (test.dim() == 1) {
            ...
            SizeType n_res_pts;
            if (intersect_convex_polygon_with_polyline(trial_pts.get_values().size() / 2,
    ...
    bool QMortarBuilderShell2::build(const Elem &trial,
                                     FEType trial_type,
                                     const Elem &test,
                                     FEType test_type,
                                     QMortar &q_trial,
    ...
    bool QMortarBuilderShell2::intersect() {
        ...
        Intersector::apply_affine_transform_3(
            Ainv, binv, trial_pts.m(), &trial_pts.get_values()[0], &ref_trial_pts.get_values()[0]);

        Intersector::apply_affine_transform_3(
            Ainv, binv, test_pts.m(), &test_pts.get_values()[0], &ref_test_pts.get_values()[0]);
        ...
        Intersector::apply_affine_transform_3(
            A, b, ref_intersection_slave.m(), &ref_intersection_slave.get_values()[0], &intersection.get_values()[0]);

        return true;
    }

    class QMortarBuilder3::Impl {
    public:
        ...
        Impl() : total_intersection_volume(0.), composite_ir(3) {}
        ...
        bool build(const Elem &trial,
            ...
            if (intersect_3D(trial_poly, test_poly, intersection)) {
                ...
                if (vol2surf) {
                    ...
                    std::copy(intersection.points,
                              intersection.points + intersection.n_nodes * intersection.n_dims,
                              &shell_poly.get_values()[0]);
        ...
        inline double get_total_intersection_volume() const { return total_intersection_volume; }
    };

    QMortarBuilder3::QMortarBuilder3() : impl_(utopia::make_unique<Impl>()) {}
    QMortarBuilder3::~QMortarBuilder3() {}

    bool QMortarBuilder3::build(const Elem &trial,
                                FEType trial_type,
                                const Elem &test,
                                FEType test_type,
                                QMortar &q_trial,
                                QMortar &q_test) {
        return impl_->build(trial, trial_type, test, test_type, q_trial, q_test);
    }

    bool QMortarBuilder3::build_vol_2_surf(const Elem &trial,
                                           FEType trial_type,
                                           const Elem &test,
                                           FEType test_type,
                                           QMortar &q_trial,
    ...
    {
        return impl_->build_vol_2_surf(trial, trial_type, test, test_type, q_trial, q_test);
    }

    double QMortarBuilder3::get_total_intersection_volume() const { return impl_->get_total_intersection_volume(); }
}  // namespace utopia

...
namespace utopia {
    ...
    namespace private_ {
        ...
        bool PetrovGalerkinAssembler::assemble(const Elem &master,
                                               FEType master_type,
                                               const Elem &slave,
                                               FEType slave_type,
            ...
            if (assembler_->assemble(master, master_type, slave, slave_type, elemmat_)) {
                ...
                for (std::size_t i = 0; i < elemmat_.size(); ++i) {
                    ...
                    auto partial_sum =
                        std::accumulate(mat_i.get_values().begin(), mat_i.get_values().end(), libMesh::Real(0.0));
        ...
        void PetrovGalerkinAssembler::initialize(const moonolith::Communicator &comm,
        ...
        void PetrovGalerkinAssembler::finalize_form(std::size_t buffer_num,
            ...
            if (qb_assembler) {
                ...
                if (comm_.is_root()) {
                    std::cout << "sum(B): " << volumes[0] << ", vol(I): " << volumes[1]
                              << "\nn_candidates: " << (isect_stats[0] + isect_stats[1])
                              << ", n_intersections: " << isect_stats[0] << ", n_false_positives: " << isect_stats[1]
                              << std::endl;
                }

                // moonolith::root_describe("vol(I_local) : " + std::to_string(total_intersection_volume), comm,
                // std::cout);
            }
        }

...
#endif  // UTOPIA_ENABLE_LIBMESH_DEPRECATED
    ...
    public:
        ...
        static bool apply(LibMeshFunctionSpaceAdapter &master,
...

            {
                ...
                c.stop();
                moonolith::logger() << "init: " << c << std::endl;
            ...
            c.stop();
            moonolith::logger() << "time MarsMeshTransfer::assemble: " << c << std::endl;
            moonolith::logger() << "vol: " << vol << " sum(B): " << sum_mat_B << " sum(D): " << sum_mat_D << std::endl;
            return vol > 0.0;
        }
    };

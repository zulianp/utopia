...
namespace utopia {
    FESpaceAdapter::FESpaceAdapter(const std::shared_ptr<MeshBase> &master_slave,
    ...
    void FESpaceAdapter::copy_global_dofs(MeshBase &mesh,
        ...
        for (; e_it_s != e_end_s; ++e_it_s, ++local_element_id) {
                ...
                for (int side_elem = 0; side_elem < elem->n_sides(); side_elem++) {
                    {
                        if (is_tagged_contact_boundary(mesh, elem, side_elem, tags) && check_side_id_one_tag) {
                            side_set_id[local_element_id].global.push_back(
                                utopia::libmesh::boundary_id(mesh.get_boundary_info(), elem, side_elem));
            ...
            subdomain_id[local_element_id].global.insert(subdomain_id[local_element_id].global.end(),
                                                         elem->subdomain_id());
        ...
        std::partial_sum(ownershipRangesFaceID.begin(), ownershipRangesFaceID.end(), ownershipRangesFaceID.begin());
            ...
            if (elem_new->on_boundary()) {
                for (int jj = 0; jj < face_set_id[local_element_id].global.size(); jj++) {
                    ...
                    if (i != -1) {
                        ...
                        face_set_id_global[local_element_id].global.insert(
                            face_set_id_global[local_element_id].global.end(), global_id);
                    } else {
                        face_set_id_global[local_element_id].global.insert(
                            face_set_id_global[local_element_id].global.end(), -1);
                    }
                }
            }
        }

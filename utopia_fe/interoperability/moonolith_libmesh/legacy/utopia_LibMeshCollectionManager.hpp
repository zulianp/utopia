...
namespace utopia {
    ...
    public:
        ...
        static const Elem &elem(const FunctionSpace &space, const Integer idx) {
            return *utopia::libmesh::elem_ptr(space.mesh(), idx);
        }
        ...
        static Integer handle(const FunctionSpace &space, const ElementIter &e_it) { return (*e_it)->id(); }
        ...
        static void fill_bound(const FunctionSpace &space, const Integer handle, Bound &bound, const double blow_up) {
            ...
            const auto &e = *utopia::libmesh::elem_ptr(mesh, handle);
        ...
        static void serialize(const FunctionSpace &space,
            ...
            for (Iterator it = begin; it != end; ++it) {
                ...
                const libMesh::Elem *elem = utopia::libmesh::elem_ptr(mesh, global_element_id);
            ...
            for (auto node_id : nodeIds) {
                const libMesh::Point &p = *utopia::libmesh::node_ptr(mesh, node_id);

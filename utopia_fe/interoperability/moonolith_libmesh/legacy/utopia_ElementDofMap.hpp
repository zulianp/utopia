...
namespace utopia {
    ...
    template <typename T>
    inline void write_vector(const std::vector<T> &v, moonolith::OutputStream &os) {
        int n = v.size();
        os << n;
        os.write(&v[0], n);
    }
    ...
    template <typename T>
    inline void read_vector(std::vector<T> &v, moonolith::InputStream &is) {
        int n;
        is >> n;
        v.resize(n);
        is.read(&v[0], n);
    ...
    public:
        ...
        inline bool empty() const { return global.empty(); }

        inline std::size_t n_dofs() const { return global.size(); }
        inline long dof(const std::size_t idx) const {
            assert(idx < global.size());
            return global[idx];
        }

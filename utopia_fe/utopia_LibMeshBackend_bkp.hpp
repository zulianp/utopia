...
public: 
	...
	void dof_indices_of_current_element(std::vector<SizeType> &v)
	{
		...
		std::copy(dof_indices_.begin(), dof_indices_.end(), v.begin());
			...
			template<class SystemType>
	LibMeshFEFunction(const std::string &name, const libMesh::Order &order, const libMesh::FEFamily &type, LibMeshFEContext<SystemType> &context)
	: mesh_(context.mesh),
	...
	LibMeshFEFunction(const std::shared_ptr<libMesh::MeshBase> &mesh, 
		const int var_num,
		const std::shared_ptr<libMesh::DofMap> &dof_map)
	: mesh_(mesh), var_num_(var_num), dof_map_(dof_map)
	{}
...
public: 
	...
	inline const libMesh::DofMap &dof_map() const { return *dof_map_; }
		...
		template<class SystemType>
	LibMeshVecFEFunction(const std::string &name, const libMesh::Order &order, const libMesh::FEFamily &type, LibMeshFEContext<SystemType> &context)
	: mesh_(context.mesh),
	var_num_( context.system.add_variable(name, order, type) ),
	dof_map_(make_ref(context.system.get_dof_map()))
	{}
	...
	LibMeshVecFEFunction(const std::shared_ptr<libMesh::MeshBase> &mesh, 
		const int var_num,
		const std::shared_ptr<libMesh::DofMap> &dof_map)
	: mesh_(mesh), var_num_(var_num), dof_map_(dof_map)
	{}
...
public:
	...
	void eval(std::vector< std::vector<Scalar> > &values) const
	{	
		if(!global_vec_) {
			auto &p = fe_.get_fe().get_xyz();
			auto &fun = fe_.get_fe().get_phi();

			values.resize(fun.size());
			for(SizeType i = 0; i < values.size(); ++i) {
				values[i].resize(fun[i].size());
			}

			for(SizeType j = 0; j < p.size(); ++j) {
				Scalar val = 0.;
				coeff_.eval(p[j], val);

				for(SizeType i = 0; i < fun.size(); ++i) {
					values[i][j] = fun[i][j] * val;
				}
			}
			...
			for(SizeType i = 0; i < fun.size(); ++i) {
				for(SizeType j = 0; j < fun[i].size(); ++j) {
					values[i][j] = fun[i][j] * vec_values[i];
				}
			}
		}
	}
...
public:
	...
	inline static std::vector< std::vector<DenseVectorT> > eval(const Divergence< MatrixFE<FE, ROWS, COLS> > &expr, const Context &)
	{
		...
		template<class Left, class Right, class Operation>
	inline static std::vector< std::vector<libMesh::Real> > eval(const Binary<Left, Right, Operation> &expr, const Context &ctx)
	{
		std::vector< std::vector<libMesh::Real> > result;
	...
	inline static void apply_binary_op(const std::vector<T> &left, const std::vector<T> &right, const Operation &op, std::vector<T> &result)
	{
		...
		template<typename T, class Operation>
	inline static void apply_binary_op(const std::vector< std::vector<T> > &left, const std::vector< std::vector<T> > &right, const Operation &op, std::vector< std::vector<T> > &result)
	{
		result.resize(left.size());

		for(uint i = 0; i < left.size(); ++i) {
			result[i].resize(left[i].size());
			for(uint j = 0; j < left[i].size(); ++j) {
				result[i][j] = Operation::template Fun<T>()(left[i][j], right[i][j]);
			}
		}
	}
	...
	template<class CoeffLeft, class VLeft, 
	class CoeffRight, class VRight, 
	typename T, class Operation>
	inline static void apply_binary_op(const Interpolate<CoeffLeft, VLeft> &left, 
		const Interpolate<CoeffRight, VRight> &right, 
		const Operation &op, std::vector< std::vector<T> > &result)
	{
		auto &&e_left  = eval(left);
		auto &&e_right = eval(right);

		result.resize(e_left.size());
		for(uint i = 0; i < e_left.size(); ++i) {
			result[i].resize(e_left[i].size());
			for(uint j = 0; j < e_left[i].size(); ++j) {
				result[i][j] = Operation::template Fun<T>()(e_left[i][j], e_right[i][j]);
			}
		}
	}
	...
	inline static std::vector< std::vector<libMesh::Real> > eval(const Interpolate<Coeff, LibMeshFEFunction> &expr, const Context &)
	{
		std::vector< std::vector<libMesh::Real> > result;
		expr.eval(result);
		return result;
	...
	inline static auto eval(const Unary<Expr, Reciprocal<T> > &expr, const Context &ctx) -> typename std::remove_reference< decltype(eval(expr.expr(), ctx)) >::type
	{
		...
		for(uint i = 0; i < in.size(); ++i) {
			result[i].resize(in[i].size());
			for(uint j = 0; j < in[i].size(); ++j) {
				result[i][j] = expr.operation().numerator()/in[i][j];
			}
		}
	...
	{	
		...
		template<class Operation>
	inline static void apply_unary_transform(const std::vector< std::vector<libMesh::Real> > &in, std::vector< std::vector<libMesh::Real> > &out, Operation op)
	{	
		out.resize(in.size());
		for(uint i = 0; i < in.size(); ++i) {
			out[i].resize(in[i].size());
			for(uint j = 0; j < in[i].size(); ++j) {
				out[i][j] = Operation::template Fun<libMesh::Real>()(in[i][j]);
			}
		}
	}
		...
		const Multiply<Left, Right> &expr, const Context &ctx) 
	{
		...
		for(uint i = 0; i < right.size(); ++i) {
			...
			for(uint j = 0; j < right[i].size(); ++j) {
				mult_at_quad_point(i, j, left, right[i][j], result[i][j]);
			}
		}
	...
	inline static void mult_at_quad_point(
		const SizeType shape_fun_i,
		const SizeType quad_index,
	...
	inline static void mult_at_quad_point(
		const SizeType shape_fun_i,
		const SizeType quad_index,
		const std::vector< std::vector<libMesh::Real> > &coeff, 
		const libMesh::FEBase::OutputGradient &v, 
		libMesh::FEBase::OutputGradient &result)
	{
		result = v;
		result *= coeff[shape_fun_i][quad_index];
	}
		...
		libMesh::DenseVector<libMesh::Real> &vec)
	{
		...
		for (uint i = 0; i < test.size(); i++) {
			for (uint j = 0; j < coef.size(); j++) {
				for (uint qp = 0; qp < n_quad_points; qp++) {
					vec(i) += JxW[qp] *  contract(test[i][qp], coef[j][qp]);
				}
			}
		}
	}
	...
	inline static std::vector< std::vector<T> > eval(const ConstantCoefficient<T, Order> &expr, const Context &ctx){
		std::vector< std::vector<T> > result(1);
		result[0].resize(ctx.points->size());
		std::fill(result[0].begin(), result[0].end(), expr[0]);
		return result;
	}
		...
		)
	{
		...
		mat.add_matrix(diag_block, dof_u, dof_u);
		mat.add_matrix(off_diag_block, dof_u, dof_sigma);
	}

		// template<int ROWS, int COLS, class Matrix>
		// static void constrain_adj_mixed_matrix_and_add(
		// 							libMesh::Elem *el, DenseMatrixT &diag_block, DenseMatrixT &off_diag_block, 
		// 							LibMeshVecFEFunction &u, MatrixFE<LibMeshFEFunction, ROWS, COLS> &sigma,
		// 							Matrix &mat)
		// {
		// 	using namespace libMesh;
		...
		)
	{
		...
		mat.add_matrix(diag_block, dof_sigma, dof_sigma);
		mat.add_matrix(off_diag_block, dof_sigma, dof_u);
	}
	...
	void mixed_vector_add(libMesh::Elem *el, const DenseVectorT &vec_1, const DenseVectorT &vec_2, 
		LibMeshVecFEFunction &u, MatrixFE<LibMeshFEFunction, ROWS, COLS> &sigma, Vector &vec)
	{
		using namespace libMesh;
		std::vector<dof_id_type> dof_u, dof_sigma;
		u.dof_map().dof_indices(el, dof_u, u.var_num());
		...
		collect_indices(el, sigma, dof_sigma);
		vec.add_vector(vec_1, dof_u);
		vec.add_vector(vec_2, dof_sigma);
	}
};

	template<class Expr, class ElementMatrix, class DofIndexTrial, class DofIndexTest>
void add_matrix(Wrapper<Expr, 2> &expr, const ElementMatrix &el_mat, const DofIndexTrial &dof_trial, const DofIndexTest &dof_test)
{	
...
void add_vector(Wrapper<Expr, 1> &expr, const ElementVector &el_vec, const DofIndexTest &dof_test)
{	
	int i = 0;
	for(auto test : dof_test) {
	...
	const Integral<Expr> &expr, Wrapper<Matrix, 2> &result)
{
	...
	for(auto e_it = e_begin; e_it != e_end; ++e_it) {
		...
		add_matrix(result, mat, dof_indices_trial, dof_indices_test);
	}
}
	...
	Wrapper<Vector, 1> &result)
{
	...
	for(auto e_it = e_begin; e_it != e_end; ++e_it) {
		...
		add_vector(result, vec, dof_indices_test);
	}
}
...
void block_local_assemble(const Integral<BilinearForm_11> &b_form_11, 
	...
	template<class TrialFunction, class TestFunction, class Expr, class Matrix>
void assemble(TrialFunction &trial, TestFunction &test, 
	const Integral<Expr> &expr, Matrix &result)
{
	using namespace libMesh;
	LibMeshBackend backend;
	...
	for(auto e_it = e_begin; e_it != e_end; ++e_it) {
		...
		test.dof_map().constrain_element_matrix(mat, dof_indices_test, dof_indices_trial);
		result.add_matrix(mat, dof_indices_test, dof_indices_trial);
	}
}
	...
	libMesh::NumericVector<libMesh::Real> &result)
{
	...
	for(auto e_it = e_begin; e_it != e_end; ++e_it) {
		...
		test.dof_map().constrain_element_vector(vec, dof_indices_test);
		result.add_vector(vec, dof_indices_test);
	}
}



	template<class TrialFunction, class TestFunction, class BilinearForm, class LinearForm, class Matrix>
void assemble(TrialFunction &trial, 
	TestFunction  &test, 
	const Integral<BilinearForm> &bilinear_form,
	const Integral<LinearForm>   &linear_form,
	Matrix &mat,
	libMesh::NumericVector<libMesh::Real> &rhs)
{
	using namespace libMesh;
	LibMeshBackend backend;
	...
	for(auto e_it = e_begin; e_it != e_end; ++e_it) {
		...
		mat.add_matrix(el_mat, dof_indices_test, dof_indices_trial);
		rhs.add_vector(el_vec, dof_indices_test);
	}
}
}

...
namespace utopia {
    ...
    inline static void transport_stabilization(const USparseMatrix &A, USparseMatrix &S) {
    ...
    // const_cast<StoreTransferOperators&>(_fe_problem.getUserObject<StoreTransferOperators>(_userobject_name_T)).setTransferOperator()=
    // std::make_shared<USparseMatrix>(T);
    ...
    // const_cast<StoreTransferOperators&>(_fe_problem.getUserObject<StoreTransferOperators>(_userobject_name_1)).setTransferOperator()=
    // std::make_shared<USparseMatrix>(S_m_cons);

    // USparseMatrix A_sc = mass_lumped_mp * inv_dt + transpose(T) * mass_lumped_fp * inv_dt * T + A_m_tot_cons +
    // S_m_cons;

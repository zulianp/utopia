...
namespace utopia {
    ...
    void SteadyFractureFlowSimulation::assemble_systems() {
        ...
        auto eq_f =
            inner(fracture_network->diffusion_tensor * grad(u_s), ctx_fun(fracture_network->sampler) * grad(v_f)) * dX;
    ...
    void SteadyFractureFlowSimulation::init_coupling() {
        ...
        auto constant_function =
            std::dynamic_pointer_cast<UIConstantFunction<double>>(normal_hydraulic_conductivity_blocks);
        if (constant_function) {
            ...
            if (lagrange_multiplier->empty()) {
                ...
                std::cout << "normal-hydraulic-conductivity-blocks  in [" << min_kappa << ", " << max_kappa << "]"
                          << std::endl;
            } else {
                auto &V_l = lagrange_multiplier->space.subspace(0);
                UVector kappa;
    ...
    */
    ...
    bool SteadyFractureFlowSimulation::solve_cg_dual() {
        ...
        solver.update(make_ref(A_m), make_ref(A_f), make_ref(B), make_ref(D), make_ref(kappa_B_t), make_ref(D_t));

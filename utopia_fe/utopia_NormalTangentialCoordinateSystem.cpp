...
namespace utopia {
												   ...
												   DSMatrixd &mat)
	{
		...
		QGauss quad(n_dims, FIFTH);
		
		
		const SizeType local_dofs = dof_map.n_local_dofs();
		DVectord global_normal_vec = local_zeros(local_dofs * n_dims);
		...
		{ //synch-block begin
			...
			for(auto e_it = mesh.active_local_elements_begin();
				...
				for(uint side = 0; side < e.n_sides(); ++side) {
					...
					fe->reinit(&e, side, TOLERANCE, &quad.get_points(), &quad.get_weights());
					...
					for(uint i = 0; i < dof_indices.size(); ++i) {
						...
						for(uint d = 0; d < n_dims; ++d) {
							global_normal_vec.add(ind * n_dims + d, vec(i * n_dims + d));
						}
					}
				}
			}
		...
		mat = local_sparse(local_dofs * n_dims, local_dofs * n_dims, n_dims);
		...
		{ //synch-block begin
			...
			for(SizeType i = r.begin(); i < r.end(); i += n_dims) {
				...
				if(use_identity) {
					for(uint di = 0; di < n_dims; ++di) {
						mat.set((i + di), (i + di), H[di * n_dims + di]);
					}
				}
			}
		} //synch-block end

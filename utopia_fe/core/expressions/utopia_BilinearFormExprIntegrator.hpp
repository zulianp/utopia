...
namespace utopia {
    ...
    public:
        ...
        BiLinearFormExprIntegrator(const Expr &expr) : expr_(expr), space_(find_test_space<FunctionSpace>(expr)) {
            assert(space_);
        }
    ...
    template <class FunctionSpace, class Expr>
    std::unique_ptr<BilinearIntegrator<FunctionSpace>> bilinear_form(const std::shared_ptr<FunctionSpace> &fs,
                                                                     const Expr &expr) {
        return utopia::make_unique<BiLinearFormExprIntegrator<FunctionSpace, Expr>>(fs, expr);
    }

    template <class FS, class Expr>
    std::unique_ptr<BilinearIntegrator<FS>> bilinear_form(FunctionSpace<FS> &fs, const Expr &expr) {
        return utopia::make_unique<BiLinearFormExprIntegrator<FS, Expr>>(make_ref(fs.derived()), expr);
    }

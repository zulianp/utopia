...
namespace utopia {
    ...
    public:
        ...
        Dual(const Expr &expr) : expr_(expr) {}
    ...
    public:
        inline static int type(const Dual<Expr> &expr, const AssemblyContext &ctx) {
            return FunctionalTraits<Expr, AssemblyContext>::type(expr.expr(), ctx);
        }
        inline static int order(const Dual<Expr> &expr, const AssemblyContext &ctx) {

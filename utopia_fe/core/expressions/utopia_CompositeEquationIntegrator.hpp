...
namespace utopia {
    ...
    public:
        ...
        std::shared_ptr<BilinearIntegratorT> bilinear_integrator() const
        {
            return bilinear_integrator_;
        }

        std::shared_ptr<LinearIntegratorT> linear_integrator() const
        {
            return linear_integrator_;
        }

        void bilinear_integrator(const std::shared_ptr<BilinearIntegratorT> &b)
        {
            bilinear_integrator_ = b;
        }

        void linear_integrator(const std::shared_ptr<LinearIntegratorT> &l)
        {
            linear_integrator_ = l;
        }
        ...
        {
            return bilinear_integrator_->is_surface(); 
        }
    ...
    public:
            ...
            ElementVector &vec) override
        {
            ...
            for(auto integrator : integrators_) {
                ok = integrator->assemble(trial_r, test_r, ctx, mat, vec); assert(ok);
            }

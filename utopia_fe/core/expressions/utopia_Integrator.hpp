...
namespace utopia {
    ...
    public:
        ...
        virtual bool assemble(const Range &test_r, AssemblyContext<BACKEND_TAG> &ctx, ElementVector &result) = 0;

        bool assemble(AssemblyContext<BACKEND_TAG> &ctx, ElementVector &result) {
    ...
    public:
        inline static int type(const LinearIntegrator<FunctionSpace> &expr, const AssemblyContext &ctx) {
            return FunctionalTraits<FunctionSpace, AssemblyContext>::type(expr.space(), ctx);
        }
        inline static int order(const LinearIntegrator<FunctionSpace> &expr, const AssemblyContext &ctx) {
            return expr.order(ctx);
        }
    };
    ...
    public:
        ...
        virtual bool assemble(const Range &trial_r,
                              const Range &test_r,
                              AssemblyContext<BACKEND_TAG> &ctx,
                              ElementMatrix &result) = 0;

        bool assemble(AssemblyContext<BACKEND_TAG> &ctx, ElementMatrix &result) {
    ...
    public:
        inline static int type(const BilinearIntegrator<FunctionSpace> &expr, const AssemblyContext &ctx) {
            return FunctionalTraits<FunctionSpace, AssemblyContext>::type(expr.test(), ctx);
        }
        inline static int order(const BilinearIntegrator<FunctionSpace> &expr, const AssemblyContext &ctx) {
            return expr.order(ctx);
        }
    };
    ...
    public:
        ...
        bool assemble(AssemblyContext<BACKEND_TAG> &ctx, ElementMatrix &mat, ElementVector &rhs) {
            auto trial_r = this->trial_range(ctx);
            auto test_r = this->test_range(ctx);
    ...
    public:
        inline static int type(const EquationIntegrator<FunctionSpace> &expr, const AssemblyContext &ctx) {
            return FunctionalTraits<FunctionSpace, AssemblyContext>::type(expr.test(), ctx);
        }
        inline static int order(const EquationIntegrator<FunctionSpace> &expr, const AssemblyContext &ctx) {
            return expr.order(ctx);
        }
    };

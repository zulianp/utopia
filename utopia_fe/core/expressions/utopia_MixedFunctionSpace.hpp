...
namespace utopia {
    ...
    template <class InputTuple, class OutputTuple, std::size_t N>
    struct TupleFilter {
    ...
    template <class... Args>
    MixedFunctionSpace<TrialFunction<FunctionSpace<Args>>...> trials(
        MixedFunctionSpace<FunctionSpace<Args>...> &&mixed_space) {
        ...
        TupleFilter<decltype(mixed_space), decltype(ret), decltype(mixed_space)::tuple_size>::apply(
            mixed_space, ret, MakeTrial());

        return ret;
    }

    template <class... Args>
    MixedFunctionSpace<TestFunction<FunctionSpace<Args>>...> tests(
        MixedFunctionSpace<FunctionSpace<Args>...> &&mixed_space) {
        MixedFunctionSpace<TestFunction<FunctionSpace<Args>>...> ret;
        TupleFilter<decltype(mixed_space), decltype(ret), decltype(mixed_space)::tuple_size>::apply(
            mixed_space, ret, MakeTest());

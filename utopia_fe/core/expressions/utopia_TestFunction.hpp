...
namespace utopia {
    ...
    public:
        inline static int type(const TestFunction<Space> &expr, const AssemblyContext &ctx) {
            return FunctionalTraits<Space, AssemblyContext>::type(*expr.space_ptr(), ctx);
        }
        inline static int order(const TestFunction<Space> &expr, const AssemblyContext &ctx) {

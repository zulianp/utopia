...
namespace utopia {
    ...
    public:
        ...
        FEKernel(Fun fun, const unsigned int integration_order) : fun_(fun), integration_order_(integration_order) {}
        template <class... Args>
        void apply(Args &&... args) {
            fun_(std::forward<Args...>(args...));
        }

        std::string get_class() const override { return "FEKernel"; }

        unsigned int integration_order() const { return integration_order_; }

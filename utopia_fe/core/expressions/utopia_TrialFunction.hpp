...
namespace utopia {
    ...
    public:
        inline static int type(const TrialFunction<Space> &expr, const AssemblyContext &ctx) {
            return FunctionalTraits<Space, AssemblyContext>::type(*expr.space_ptr(), ctx);
        }
        inline static int order(const TrialFunction<Space> &expr, const AssemblyContext &ctx) {
            return FunctionalTraits<Space, AssemblyContext>::order(*expr.space_ptr(), ctx);
        }
    };
}  // namespace utopia

...
namespace utopia {
    ...
    public:
        ...
        enum Type { SURFACE = 0, VOLUME = 1, INNER_VOLUME_ONLY = 2 };
        ...
        inline int has_block_id() const { return block_id_ != -1; }

        inline bool has_integral_id() const { return integral_id_ != -1; }

        inline void set_integral_id(const int id) { integral_id_ = id; }

        inline bool is_surface() const { return (type_ == SURFACE); }

        inline bool is_volume() const { return (type_ == VOLUME); }

        inline bool is_inner_volume_only() const { return (type_ == INNER_VOLUME_ONLY); }
    ...
    public:
        inline static int type(const Integral<Expr> &expr, const AssemblyContext &ctx) {
            return FunctionalTraits<Expr, AssemblyContext>::type(expr.expr(), ctx);
        }
        inline static int order(const Integral<Expr> &expr, const AssemblyContext &ctx) {

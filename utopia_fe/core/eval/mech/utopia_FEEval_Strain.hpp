...
namespace utopia {
    ...
    public:
        ...
        inline static auto apply(const Expr &expr, AssemblyContext<Backend> &ctx) 
        -> decltype( FEBackend<Backend>::grad_t_plus_grad(
                            FEBackend<Backend>::apply(expr.left(), ctx),
    ...
    template<class C, class LeftFun, class RightFun, class Traits, int Backend, int IsQuadData>
    class FEEval<  Multiply<
                        Multiply<
                            BlockVar<double>,
                            Trace<StrainExpr<C, LeftFun, RightFun>>
                            >,
                        SymbolicTensor<Identity, 2>
                        >,
                        Traits, Backend, IsQuadData> 
    {
    public:
        ...
        inline static auto apply(const Expr &expr, AssemblyContext<Backend> &ctx) 
        -> decltype( 
            FEBackend<Backend>::grad_t_plus_grad(
                    1.,

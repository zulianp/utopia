...
namespace utopia {
		...
		inline static auto apply(
			...
			AssemblyContext<Backend> &ctx) -> decltype( 
			    		FEBackend<Backend>::apply_binary(
			    			FEEval<Left,  Traits, Backend, IsHackType>::apply(left, ctx),
	...
	public:
	    ...
	    inline static auto apply(
	    	const Number<T> &scale_factor,
	    	const SymmetrizedGrad &symm_grad,
	    	AssemblyContext<Backend> &ctx) -> decltype( 
	    	    		FEBackend<Backend>::grad_t_plus_grad(scale_factor, symm_grad.left().expr().expr(), symm_grad.right().expr(), ctx)
	    	    	)
	    {
	    	return FEBackend<Backend>::grad_t_plus_grad(scale_factor, symm_grad.left().expr().expr(), symm_grad.right().expr(), ctx);
	    } 
	};
	...
	public:
	    inline static auto apply(
	    	...
	    	const Integral<Right> &right,
	    	AssemblyContext<Backend> &ctx) -> decltype( 
	    	    		FEEval<Right, Traits, Backend, IsQuadData>::apply(right.expr(), ctx)
	    	    	)
	    {
	...
	public:
	    inline static auto apply(
	    	const Integral<Left> &left,
	    	const Number<Right> &right,
	    	AssemblyContext<Backend> &ctx) -> decltype( 
	    	    		FEEval<Left, Traits, Backend, IsQuadData>::apply(left, ctx)
	    	    	)
	    {

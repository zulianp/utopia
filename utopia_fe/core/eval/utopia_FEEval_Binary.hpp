...
namespace utopia {
    ...
    public:
        ...
        static auto apply(const Binary<Left, Right, Op> &expr, AssemblyContext<Traits::Backend> &ctx)
            -> decltype(FEBackend<Traits::Backend>::apply_binary(
    ...
    public:
        ...
        static auto apply(const BinExpr &expr, AssemblyContext<Backend> &ctx)
            -> decltype(FEBackend<Backend>::apply_binary(FEEval<Left, Traits, Backend, 1>::apply(expr.left(), ctx),
                                                         FEEval<Right, Traits, Backend, 1>::apply(expr.right(), ctx),
                                                         expr.operation(),
                                                         ctx)) {
            return FEBackend<Backend>::apply_binary(FEEval<Left, Traits, Backend, 1>::apply(expr.left(), ctx),
                                                    FEEval<Right, Traits, Backend, 1>::apply(expr.right(), ctx),
                                                    expr.operation(),
                                                    ctx);
        }
    ...
    public:
        ...
        inline static auto apply(const Expr &expr, AssemblyContext<Backend> &ctx)
            -> decltype(DelegateT::apply(expr, ctx)) {
    ...
    public:
        ...
        inline static auto apply(const Expr &expr, AssemblyContext<Backend> &ctx)
            -> decltype(FEEval<Integral<Left>, Traits, Backend, IsQuadData>::apply(expr.left())) {
    ...
    public:
        ...
        inline static auto apply(const Expr &expr, AssemblyContext<Backend> &ctx) -> decltype(
            FEBackend<Backend>::grad_t_plus_grad(1.0, expr.right().expr().expr(), expr.left().expr(), ctx)) {
            return FEBackend<Backend>::grad_t_plus_grad(1.0, expr.right().expr().expr(), expr.left().expr(), ctx);
        }
    };
    ...
    public:
        ...
        inline static auto apply(const Expr &expr, AssemblyContext<Backend> &ctx)
            -> decltype(FEEval<Left, Traits, Backend, IsQuadData>::apply(expr.left().expr(), ctx)) {
            return FEEval<Left, Traits, Backend, IsQuadData>::apply(expr.left().expr(), ctx) +
                   FEEval<Right, Traits, Backend, IsQuadData>::apply(expr.right().expr(), ctx);
        }
    };
    ...
    public:
        ...
        static auto apply(const Expr &expr, AssemblyContext<Backend> &ctx)
            -> decltype(FEEval<Right, Traits, Backend, IsQuadData>::apply(expr.right(), ctx)) {
            return FEBackend<Backend>::apply_binary(expr.left(), expr.right(), expr.operation(), ctx);
        }
    };
    ...
    public:
        ...
        static auto apply(const Expr &expr, AssemblyContext<Backend> &ctx)
            -> decltype(FEEval<Left, Traits, Backend, IsQuadData>::apply(expr.left(), ctx)) {
            return FEBackend<Backend>::apply_binary(expr.left(), expr.right(), expr.operation(), ctx);
        }
    };
    ...
    public:
        ...
        static auto apply(const Expr &expr, AssemblyContext<Backend> &ctx)
            -> decltype(FEEval<Left, Traits, Backend, IsQuadData>::apply(expr.left(), ctx)) {
            return FEBackend<Backend>::apply_binary(expr.right(), expr.left(), expr.operation(), ctx);
        }
    };
    ...
    public:
        ...
        static auto apply(const Expr &expr, AssemblyContext<Backend> &ctx)
            -> decltype(FEEval<Gradient<Right>, Traits, Backend, IsQuadData>::apply(expr.right(), ctx)) {
            return FEBackend<Backend>::apply_binary(
                expr.left(),
    ...
    public:
        ...
        static auto apply(const Expr &expr, AssemblyContext<Backend> &ctx)
            -> decltype(FEEval<Left, Traits, Backend, IsQuadData>::apply(expr.left(), ctx)) {
            return FEBackend<Backend>::apply_binary(FEEval<Left, Traits, Backend, IsQuadData>::apply(expr.left(), ctx),
                                                    expr.right(),
                                                    expr.operation(),
                                                    ctx);
        }

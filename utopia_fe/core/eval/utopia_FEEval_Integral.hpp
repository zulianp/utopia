...
namespace utopia {
    ...
    public:
        ...
        inline static auto apply(const Expr &expr, AssemblyContext<Backend> &ctx) -> decltype(
            FEBackend<Traits::Backend>::integrate(FEEval<Inner, Traits, Backend, IsQuadData>::apply(expr.expr(), ctx),
                                                  ctx)) {
            // Check subtree properties
            // TODO
            return FEBackend<Traits::Backend>::integrate(
                FEEval<Inner, Traits, Backend, IsQuadData>::apply(expr.expr(), ctx), ctx);
        }
    };
}  // namespace utopia

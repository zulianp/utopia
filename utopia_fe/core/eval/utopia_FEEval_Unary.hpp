...
namespace utopia {
    ...
    public:
        ...
        inline static auto apply(const Expr &expr, AssemblyContext<Backend> &ctx)
            -> decltype(FEBackend<Backend>::apply_unary(FEEval<Inner, Traits, Backend, IsQuadData>::apply(expr.expr(),
                                                                                                          ctx),
                                                        expr.operation(),
                                                        ctx)) {
            return FEBackend<Backend>::apply_unary(
                FEEval<Inner, Traits, Backend, IsQuadData>::apply(expr.expr(), ctx), expr.operation(), ctx);
        }
    };

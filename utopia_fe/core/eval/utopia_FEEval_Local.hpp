...
namespace utopia {
    ...
    class Eval<Gradient<TrialFunction<FiniteElement<Space>>>, Traits, Backend> {
    public:
        using Expr = Gradient<TrialFunction<FiniteElement<Space>>>;
    ...
    // FIXME (specialize and move to libmesh backend)
    template <class Space, class Traits, int Backend>
    class Eval<Divergence<TrialFunction<FiniteElement<Space>>>, Traits, Backend> {
    public:
        using Expr = Divergence<TrialFunction<FiniteElement<Space>>>;
        ...
        inline static void eval_div(const FiniteElement<ProductFunctionSpace<SubSpace>> &fe_in,
                                    FormTensor<T2, 0> &result) {
    ...
    // FIXME (specialize and move to libmesh backend)
    template <class Space, class Traits, int Backend>
    class Eval<TrialFunction<FiniteElement<Space>>, Traits, Backend> {
    public:
        using Expr = TrialFunction<FiniteElement<Space>>;
    ...
    public:
        Form(const MeasureT &measure, const Left &l, const Right &r) : measure_(measure), left_(l), right_(r) {}
        ...
        inline std::string get_class() const override {
            return "Form2<" + left().get_class() + ", " + right().get_class() + ">";
        }
    ...
    public:
        Form(const MeasureT &measure, const Expr &expr) : measure_(measure), expr_(expr) {}
    ...
    class Eval<Interpolate<Vector, TrialFunction<FiniteElement<ProductFunctionSpace<Space>, FEBackendTag>>>,
               Traits,
               Backend> {
    public:
        using Expr =
            utopia::Interpolate<Vector, TrialFunction<FiniteElement<ProductFunctionSpace<Space>, FEBackendTag>>>;
    ...
    class Eval<Gradient<Interpolate<Vector, TrialFunction<FiniteElement<Space>>>>, Traits, Backend> {
    public:
        using Expr = utopia::Gradient<Interpolate<Vector, TrialFunction<FiniteElement<Space>>>>;

        template <class Out>
        inline static void apply(const Expr &expr, Out &result) {
            const auto &fe = *expr.expr().fun().space_ptr();
            eval_grad(fe, expr.expr().coefficient(), result);
        }

        template <class T1, class VectorNonConst>
        static void gather_interp_values(const FiniteElement<ProductFunctionSpace<T1>> &fe,
        ...
        inline static void eval_grad(const FiniteElement<ProductFunctionSpace<T1>> &fe,
                                     const VectorNonConst &c,
    ...
    public:
        ...
        static void apply(const MultiTensor<Scalar, 0> &left,
        ...
        static void apply(const Scalar &left,
                          const MultiTensor<T, Order> &right,
                          const Multiplies &,
                          MultiTensor<T, Order> &result) {
            if (!result.is_alias(right)) {
        ...
        static void apply(const MultiTensor<Matrix, 2> &left,
        ...
        template <class Op>
        static void apply(const MultiTensor<T, Order> &left,
                          const MultiTensor<T, Order> &right,
                          const Op &op,
                          MultiTensor<T, Order> &result) {
    ...
    public:
        ...
        static void apply(const MultiTensor<MTType, MTOrder> &left,
                          const FormTensor<T, Order> &right,
                          const Op &op,
                          FormTensor<T, Order> &result) {
        ...
        template <class MTType, int MTOrder, class Op>
        static void apply(const FormTensor<MTType, MTOrder> &left,
                          const MultiTensor<T, Order> &right,
                          const Op &op,
                          FormTensor<T, Order> &result) {
        ...
        static void apply(const FormTensor<Scalar, 0> &left,
                          const FormTensor<T, Order> &right,
                          const Multiplies &,
                          FormTensor<T, Order> &result) {
            if (result.is_alias(right)) {
        ...
        static void apply(const FormTensor<Matrix, 2> &left,
                          const FormTensor<T, Order> &right,
                          const Multiplies &,
                          FormTensor<T, Order> &result) {
            if (result.is_alias(right)) {
        ...
        template <class Op>
        static void apply(const FormTensor<T, Order> &left,
                          const FormTensor<T, Order> &right,
                          const Op &op,
                          FormTensor<T, Order> &result) {
            const SizeType n = left.size();
    ...
    class Eval<Dot<Tensor<MultiTensor<Left, Order>, Order>, Tensor<FormTensor<Right, Order>, Order>>, Traits, Backend> {
    public:
        using Expr = utopia::Dot<Tensor<MultiTensor<Left, Order>, Order>, Tensor<FormTensor<Right, Order>, Order>>;

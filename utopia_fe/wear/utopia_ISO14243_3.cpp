...
namespace utopia {
    ...
    void ISO14243_3::read(Input &is) {
        ...
        std::string temp;
        is.get("flexion-extension-angle-axis", temp);
        flexion_extension_angle_axis_ = temp[0];
        is.get("axial-force-axis", temp);
        axial_force_axis_char = temp[0];
        is.get("ap-motion-axis", temp);
        ap_motion_axis_ = temp[0];
        is.get("tibial-rotation-axis", temp);
        tibial_rotation_axis_ = temp[0];
        ...
        is.get("femural-block", femural_block_);
        assert(femural_block_ != -1);
        is.get("tibial-block", tibial_block_);
        assert(tibial_block_ != -1);
        is.get("axial-force-side", axial_force_side_);
        assert(axial_force_side_ != -1);
    ...
    void ISO14243_3::displacement_and_forces(ProductFunctionSpace<LibMeshFunctionSpace> &space,
        ...
        auto l_form =
            surface_integral((1. / area) * inner(coeff(axial_force_), v[axial_force_axis_]), axial_force_side_);

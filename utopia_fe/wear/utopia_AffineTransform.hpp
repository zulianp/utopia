...
namespace utopia {
    ...
    private:
        static void mat_mult(const std::array<double, 3 * 3> &A,
        ...
        static void axpy(const double alpha, const std::array<double, 3> &x, std::array<double, 3> &y) {
            std::fill(std::begin(y), std::end(y), 0.);
        ...
        inline static double get(const std::size_t i, const std::size_t j, const std::array<double, 9> &mat) {
            return mat[i * 3 + j];
        }
    };
    ...
    public:
        ...
        void compose_right(const AffineTransform &t) { transforms.push_front(t); }

        void compose_left(const AffineTransform &t) { transforms.push_back(t); }

...
namespace utopia {
    ...
    bool ContactStress<FunctionSpaceT, Matrix, Vector>::assemble(const UVector &x, UVector &result) {
        ...
        ok = elast_->von_mises_stress(x_p1_, von_mises, 0);
        assert(ok);
        ok = elast_->normal_stress(x_p1_, normal_stress, 1);
        assert(ok);
    ...
    void ContactStress<FunctionSpaceT, Matrix, Vector>::init() {
        auto &stress_system =
            V_[0].equation_systems().template add_system<libMesh::LinearImplicitSystem>("p1_gradient");

        P1_ *= LibMeshFunctionSpace(stress_system,
                                    stress_system.add_variable("von_mises", libMesh::Order(1), libMesh::LAGRANGE));
        P1_ *= LibMeshFunctionSpace(stress_system,
                                    stress_system.add_variable("normal_stress", libMesh::Order(1), libMesh::LAGRANGE));

        for (auto i = 2; i < V_.n_subspaces(); ++i) {
            P1_ *= LibMeshFunctionSpace(
                stress_system,
                stress_system.add_variable("temp_" + std::to_string(i), libMesh::Order(1), libMesh::LAGRANGE));
        }
        ...
        bool ok =
            assemble_interpolation(V_.subspace(0).mesh(), P1_.subspace(0).dof_map(), V_.subspace(0).dof_map(), P1toV_);
        assert(ok);
        ok = assemble_projection(V_.subspace(0), P1_.subspace(0), B, D, true, V_.n_subspaces());
        assert(ok);
        // ok = assemble_interpolation(V_.subspace(0).mesh(), V_.subspace(0).dof_map(), P1_.subspace(0).dof_map(),
        // VtoP1_);

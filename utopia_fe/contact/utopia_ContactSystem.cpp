...
namespace utopia {
    ContactSystem::ContactSystem(const std::shared_ptr<libMesh::EquationSystems> &equation_systems,
                                 const int main_system_num)
    ...
    void ContactSystem::init() {
        ...
        if (dim > 2) var_num_.push_back(aux.add_variable("inc_z", order, libMesh::LAGRANGE));
        ...
        if (dim > 2) var_num_.push_back(aux.add_variable("vel_z", order, libMesh::LAGRANGE));
        ...
        if (dim > 2) var_num_.push_back(aux.add_variable("f_z", order, libMesh::LAGRANGE));
        ...
        if (dim > 2) var_num_.push_back(aux.add_variable("fext_z", order, libMesh::LAGRANGE));
        ...
        if (dim > 2) var_num_.push_back(aux.add_variable("n_z", order, libMesh::LAGRANGE));
        ...
        if (dim > 2) var_num_.push_back(aux.add_variable("wear_disp_z", order, libMesh::LAGRANGE));
    ...
    void ContactSystem::update_wear(const double dt,
    ...
    void ContactSystem::update(const MechanicsState &state, const ContactT &contact, const double dt) {

...
namespace utopia {
    ...
    void ContactTensors::convert(const USparseMatrix &perm,
        ...
        each_transform(
            out.is_contact, out.is_contact, [&n_local_contact_nodes](const SizeType i, const double val) -> double {
                if (val > 0) {
    ...
    public:
        inline MPI_Comm comm() const { return B.comm.get(); }
        ...
        void finalize(const LibMeshFunctionSpaceAdapter &adapter,
                ...
                for (auto i = r.begin(); i < r.end(); ++i) {
                    ...
                    if (a > 0.0) {
                        ...
                        if (!approxeq(ratio, 1.0, 1e-2)) {
                            ...
                            // std::cout << "=====================================\n";
                            // std::cout << i << ") " << volumes.get(i) << " == " << element_wise.area.get(i) <<
                            // std::endl;
            ...
            if (must_assemble_trafo) {
                DualBasis::build_global_trafo(adapter.mesh(),
            ...
            element_wise.convert(*adapter.permutation(), *adapter.vector_permutation(), *dof_wise);
        ...
        ContactDataBuffers(MPI_Comm comm)
            : B(comm), D(comm), gap(comm), normal(comm), area(comm), is_glue(comm, false) {
    ...
    public:
        ...
        void assemble_volumes(LibMeshFunctionSpaceAdapter &space, UVector &volumes) const {
            Chrono c;
            c.start();
        ...
        void init_biorth(const libMesh::Elem &el, const int el_order) {
            if (use_biorth && biorth_weights.get_values().empty()) {
                use_trafo = false;
                if (el_order == 2) {
                    DualBasis::build_trafo_and_weights(
                        el.type(), el_order, alpha, local_trafo, inv_local_trafo, biorth_weights);
                    use_trafo = true;
                } else {
        ...
        void local_assemble_aux(const moonolith::Vector<double, Dim> &normal, const moonolith::Storage<double> &gap) {
            b_elmat.zero();
            d_elmat.zero();
            gap_vec.zero();
            normal_vec.zero();
            ...
            if (use_biorth) {
                // P1 biorth
                mortar_assemble_weighted_biorth(*master_fe, *slave_fe, biorth_weights, b_elmat);

                if (use_trafo) {
                    // use trafo also for trial element
                    mortar_assemble_weighted_biorth(*slave_fe, local_trafo, *slave_fe, biorth_weights, d_elmat);

                } else {
                    mortar_assemble_weighted_biorth(*slave_fe, *slave_fe, biorth_weights, d_elmat);
                }

                integrate_scalar_function_weighted_biorth(*slave_fe, biorth_weights, gap, gap_vec);

                l2_project_normal_weighted_biorth(*slave_fe, biorth_weights, normal, normal_vec);
                ...
                integrate_scalar_function(*slave_fe, gap, gap_vec);
        ...
        void assemble(const Adapter &master,
            ...
            auto isect_area =
                std::accumulate(b_elmat.get_values().begin(), b_elmat.get_values().end(), libMesh::Real(0.0));
        ...
        bool apply(Adapter &master, Adapter &slave) {
            ...
            if (is_affine) {
                ...
                converter.init(
                    e_m, m_m.fe_type(0).order, e_s, m_s.fe_type(0).order, affine_contact.q_rule, shift_in_ref_el);
                ...
                converter.init(
                    e_m, m_m.fe_type(0).order, e_s, m_s.fe_type(0).order, warped_contact.q_rule, shift_in_ref_el);
    ...
    public:
        static bool apply(const ContactParams &params,
            ...
            if (params.side_set_search_radius) {
                ...
                algo.init(adapter, params.contact_pair_tags, params.side_set_search_radius);
            } else {
                algo.init(adapter, params.contact_pair_tags, params.search_radius);
            }

            ProjectionAlgorithm<Dim> contact_algo(contact_data, *params.is_glue);
            contact_algo.use_biorth = params.use_biorthogonal_basis;
            algo.compute(
                [&](const Adapter &master, const Adapter &slave) -> bool { return contact_algo.apply(master, slave); });
    ...
    bool ContactAssembler::assemble(libMesh::MeshBase &mesh, libMesh::DofMap &dof_map, const ContactParams &params) {
        Chrono overall_time;
        overall_time.start();
        ...
        if (is_volume) {
            ...
            adapter.extract_surface_init_for_contact(make_ref(mesh), dof_map, params.variable_number);
    ...
    bool ContactAssembler::init_no_contact(const libMesh::MeshBase &mesh, const libMesh::DofMap &dof_map) {
        if (!contact_tensors_) {
    ...
    void ContactAssembler::remove_mass(const UVector &in, UVector &out) const {
        if (!empty(contact_tensors_->inv_mass_vector)) {
            ...
            if (!empty(contact_tensors_->Q_inv)) {
                out = e_mul(contact_tensors_->inv_mass_vector, contact_tensors_->Q_inv * in);

...
namespace utopia {
    ...
    public:
        ...
        ContactSolver(const std::shared_ptr<FunctionSpaceT> &V,
        ...
        void set_tol(const Scalar tol) { tol_ = tol; }

        void set_material(const std::shared_ptr<ElasticMaterial<Matrix, Vector>> &material) { material_ = material; }
        ...
        void update_contact(const Vector &x) {
            ...
            if (bypass_contact_) {
                if (!contact_->initialized()) {
                    contact_->init_no_contact(utopia::make_ref(V_0.mesh()), utopia::make_ref(V_0.dof_map()));
                }
            ...
            contact_->assemble(utopia::make_ref(V_0.mesh()), utopia::make_ref(V_0.dof_map()), params_);
        ...
        bool solve_contact() {
...

            {
                ...
                for (int i = 0; i < max_outer_loops_; ++i) {
                    ...
                    if (diff < tol_) {
                        std::cout << "terminated at iteration " << i << " with diff " << diff << " <  " << tol_
                                  << std::endl;
                        break;
                    } else {
                        if (i + 1 == max_outer_loops_) {
                            std::cerr << "[Warning] contact solver failed to converge with " << max_outer_loops_
                                      << " loops under tolerance " << tol_ << std::endl;
                            return true;
                        }
                    }
        ...
        bool step() {
            ...
            if (contact_->has_glue()) {
                ...
                qp_solve(
                    Hc_, gc_, make_upper_bound_constraints(std::make_shared<Vector>(contact_->gap() - xc_)), inc_c_);
            }
        ...
        ElasticMaterial<Matrix, Vector> &material() { return *material_; }

        FunctionSpaceT &space() { return *V_; }

        const FunctionSpaceT &space() const { return *V_; }

        const Vector &displacement() const { return x_; }

        Vector &displacement() { return x_; }

        void debug_output(const bool val) { debug_output_ = val; }

        void export_results(const bool val) { export_results_ = val; }

        virtual void next_step() {}

        inline const std::shared_ptr<ExternalForce> &external_force_fun() const { return external_force_fun_; }
        ...
        void set_bypass_contact(const bool val) { bypass_contact_ = val; }
        void set_max_outer_loops(const int val) { max_outer_loops_ = val; }

        void set_max_non_linear_iterations(const int val) { max_non_linear_iterations_ = val; }

        void set_use_ssn(const bool val) { use_ssn_ = val; }

        void set_use_pg(const bool val) { use_pg_ = val; }

        void set_exit_on_contact_solve_failure(const bool val) { exit_on_contact_solve_failure_ = val; }

        void plot_gap(const bool val) { plot_gap_ = val; }

        virtual bool stress(const Vector &x, Vector &result) { return material_->stress(x, result); }
        ...
        inline void set_contact_stress(const std::shared_ptr<ContactStressT> &stress) { contact_stress_ = stress; }

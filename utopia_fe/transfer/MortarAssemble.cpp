...
int order_for_l2_integral(const int dim, const libMesh::Elem &master_el,
                          const int master_order, const libMesh::Elem &slave_el,
  ...
  if (dim == 2) {
    order = master_order * (is_quad(master_el.type()) ? 2 : 1) +
            slave_order * (is_quad(slave_el.type()) ? 2 : 1) *
                (slave_has_affine_map ? 1 : 2);
  } else if (dim == 3) {
    order = master_order * (is_hex(master_el.type()) ? 3 : 1) +
            slave_order * (is_hex(slave_el.type()) ? 3 : 1) *
                (slave_has_affine_map ? 1 : 2);
  } else {
...
void Transform2::transform_to_reference(const libMesh::Point &world,
                                        libMesh::Point &ref) const {
  ref = libMesh::FE<2, libMesh::LAGRANGE>::inverse_map(&elem_, world, 1e-10);
  assert((libMesh::FE<2, libMesh::LAGRANGE>::on_reference_element(
      ref, elem_.type(), 1e-3)));
  assert((libMesh::FE<2, libMesh::LAGRANGE>::map(&elem_, ref)
              .absolute_fuzzy_equals(world, 1e-8)));
}

void AffineTransform2::compute_affine_transformation(
    const libMesh::Elem &elem, libMesh::DenseMatrix<libMesh::Real> &A_inv,
...
void AffineTransform2::transform_to_reference(const libMesh::Point &world,
                                              libMesh::Point &ref) const {
...
void Transform3::transform_to_reference(const libMesh::Point &world,
                                        libMesh::Point &ref) const {
  ref = libMesh::FE<3, libMesh::LAGRANGE>::inverse_map(&elem_, world);
  // assert( (libMesh::FE<3, libMesh::LAGRANGE>::on_reference_element(ref,
  // elem_.type(), 1e-6)) );
  assert((libMesh::FE<3, libMesh::LAGRANGE>::map(&elem_, ref)
              .absolute_fuzzy_equals(world, 1e-8)));
}

void AffineTransform3::compute_affine_transformation(
    const libMesh::Elem &elem, libMesh::DenseMatrix<libMesh::Real> &A_inv,
    libMesh::DenseVector<libMesh::Real> &A_inv_m_b) {
  ...
  A_inv_m_b(1) = -1.0 * A_inv(1, 0) * p0(0) - A_inv(1, 1) * p0(1) -
                 1.0 * A_inv(1, 2) * p0(2);
  A_inv_m_b(2) = -1.0 * A_inv(2, 0) * p0(0) - A_inv(2, 1) * p0(1) -
                 1.0 * A_inv(2, 2) * p0(2);
}

void SideAffineTransform2::compute_affine_transformation(
    const libMesh::Elem &elem, const int side,
  ...
  libMesh::Point p0 =
      libMesh::FE<1, libMesh::LAGRANGE>::map(side_ptr.get(), ref_p0);
  libMesh::Point p1 =
      libMesh::FE<1, libMesh::LAGRANGE>::map(side_ptr.get(), ref_p1);
...
void SideAffineTransform3::compute_affine_transformation(
    const libMesh::Elem &elem, const int side,
    libMesh::DenseMatrix<libMesh::Real> &A_inv,
    libMesh::DenseVector<libMesh::Real> &A_inv_m_b) {
  static Intersector isector;
  ...
  libMesh::Point p0 =
      libMesh::FE<2, libMesh::LAGRANGE>::map(side_ptr.get(), ref_p0);
  libMesh::Point p1 =
      libMesh::FE<2, libMesh::LAGRANGE>::map(side_ptr.get(), ref_p1);
  libMesh::Point p2 =
      libMesh::FE<2, libMesh::LAGRANGE>::map(side_ptr.get(), ref_p2);
...
void AffineTransform3::transform_to_reference(const libMesh::Point &world,
                                              libMesh::Point &ref) const {
  ref(0) = A_inv_m_b_(0) + A_inv_(0, 0) * world(0) + A_inv_(0, 1) * world(1) +
           A_inv_(0, 2) * world(2);
  ref(1) = A_inv_m_b_(1) + A_inv_(1, 0) * world(0) + A_inv_(1, 1) * world(1) +
           A_inv_(1, 2) * world(2);
  ref(2) = A_inv_m_b_(2) + A_inv_(2, 0) * world(0) + A_inv_(2, 1) * world(1) +
           A_inv_(2, 2) * world(2);
}
...
void make_composite_quadrature_2D_non_affine(
    const libMesh::DenseMatrix<libMesh::Real> &polygon, const double weight,
    const int order, QMortar &c_ir) {
  std::vector<int> tri;
  // triangulate_polygon(polygon.m(), &polygon.get_values()[0], tri);
  // make_composite_quadrature_2D_from_tri_mesh(tri, polygon, weight, order,
  // c_ir);
}

void make_composite_quadrature_2D_from_tri_mesh(
    const std::vector<int> &tri,
    const libMesh::DenseMatrix<libMesh::Real> &points, const double weight,
    const int order, QMortar &c_ir) {
...
void make_composite_quadrature_2D(
    const libMesh::DenseMatrix<libMesh::Real> &polygon, const double weight,
    const int order, QMortar &c_ir) {
  libMesh::QGauss ir(2, libMesh::Order(order));
  ...
  double triangle[3 * 2] = {
      polygon.get_values()[0], polygon.get_values()[1], 0., 0., 0., 0.};
...
void make_composite_quadrature_3D(const Polyhedron &polyhedron,
                                  const double weight, const int order,
                                  QMortar &c_ir) {
  using std::min;
  ...
  const uint max_n_sub_inc =
      std::max(1u, MAX_QUAD_POINTS / (max_n_sub_els * ir.n_points()));
  ...
  if (n_sub_elements == 1) {
    ...
    const double w =
        fabs(isector.m_tetrahedron_volume(polyhedron.points) * scale);
    ...
    for (int begin_k = 0; begin_k < polyhedron.n_elements;) {
      const int end_k = min(begin_k + max_n_sub_inc,
                            static_cast<uint>(polyhedron.n_elements));
      ...
      const int n_quad_points =
          isector.make_quadrature_points_from_polyhedron_in_range_around_point(
              polyhedron, begin_k, end_k, scale, ir.n_points(),
              &ref_quad_points[0], &ref_quad_weights[0], barycenter_p,
...
void make_composite_quadrature_on_surf_2D(
    const libMesh::DenseMatrix<libMesh::Real> &line, const double weight,
    const int order, QMortar &c_ir) {
  using namespace libMesh;
...
void make_composite_quadrature_on_surf_3D(
    const libMesh::DenseMatrix<libMesh::Real> &polygon, const double weight,
    const int order, QMortar &c_ir) {
  Intersector isector;
  libMesh::QGauss ir(2, libMesh::Order(order));
  ...
  double triangle[3 * 3] = {polygon.get_values()[0],
                            polygon.get_values()[1],
                            polygon.get_values()[2],
                            0.,
                            0.,
                            0.,
                            0.,
...
void transform_to_reference(const Transform &trans, const int type,
                            const QMortar &global_ir, QMortar &ref_ir) {
...
void transform_to_reference_surf(const Transform &trans, const int type,
                                 const QMortar &global_ir, QMortar &ref_ir) {
...
template <class FE>
void mortar_assemble_aux(const FE &trial_fe, const FE &test_fe,
...
template <class FE>
void mortar_assemble_aux(const double &master_frac, const double &slave_frac,
                         const FE &trial_fe, const FE &test_fe,
                         libMesh::DenseMatrix<libMesh::Real> &elmat) {
  if (elmat.m() != test_fe.get_phi().size() ||
      elmat.n() != trial_fe.get_phi().size()) {
    elmat.resize(test_fe.get_phi().size(), trial_fe.get_phi().size());
  ...
  for (uint i = 0; i < n_test; ++i) {
    for (uint j = 0; j < n_trial; ++j) {
      for (uint qp = 0; qp < n_qp; ++qp) {
        elmat(i, j) +=
            contract(test[i][qp],  trial[j][qp])* master_frac * slave_frac * JxW[qp];
      }
    }
  }
}
...
//                          libMesh::DenseMatrix<libMesh::Real> &elmat)
// {
//     if(elmat.m() != test_fe.get_phi().size() ||  elmat.n() !=
//     trial_fe.get_phi().size()) {
//         elmat.resize(test_fe.get_phi().size(), trial_fe.get_phi().size());
//         elmat.zero();
//     }
...
template <class FE>
void mortar_assemble_biorth_aux(const FE &trial_fe, const FE &test_fe,
...
void mortar_assemble(const double &master_frac, const double &slave_frac,
                     const libMesh::FEBase &trial_fe,
                     const libMesh::FEBase &test_fe,
                     libMesh::DenseMatrix<libMesh::Real> &elmat) {
  mortar_assemble_aux(master_frac, slave_frac, trial_fe, test_fe, elmat);
}
...
void mortar_assemble_biorth(const libMesh::FEBase &trial_fe,
                            const libMesh::FEBase &test_fe, const int type,
                            libMesh::DenseMatrix<libMesh::Real> &elmat) {
...
void mortar_assemble_biorth_aux(
    const int dim, const FE &trial_fe, const FE &test_fe,
    const libMesh::Real &wii, const libMesh::Real &wij,
  ...
  for (uint qp = 0; qp < n_qp; ++qp) {
    for (uint i = 0; i < n_test; ++i) {
      ...
      for (uint j = 0; j < n_trial; ++j) {
        elmat(i, j) +=
            indicator(i) * contract(biorth_test, trial[j][qp]) * JxW[qp];
      }
    }
  }
}
...
void mortar_assemble_biorth(
    const int dim, const libMesh::FEBase &trial_fe,
    const libMesh::FEBase &test_fe, const int type,
    const libMesh::DenseVector<libMesh::Real> &indicator,
    libMesh::DenseMatrix<libMesh::Real> &elmat) {
  libMesh::Real w_ii, w_ij;
  biorthgonal_weights(type, w_ii, w_ij);
  mortar_assemble_biorth_aux(dim, trial_fe, test_fe, w_ii, w_ij, indicator,
                             elmat);
}

void mortar_assemble_biorth(
    const int dim, const libMesh::FEVectorBase &trial_fe,
    const libMesh::FEVectorBase &test_fe, const int type,
    const libMesh::DenseVector<libMesh::Real> &indicator,
    libMesh::DenseMatrix<libMesh::Real> &elmat) {
  libMesh::Real w_ii, w_ij;
  biorthgonal_weights(type, w_ii, w_ij);
...
void mortar_normal_and_gap_assemble(
    const uint dim, const libMesh::FEBase &test_fe,
    const libMesh::Point &surf_normal, const libMesh::Point &plane_normal,
  ...
  mortar_normal_and_gap_assemble(test_fe, surf_normal_v, plane_normal_v,
                                 plane_offset, normals, gap);
}
...
void mortar_normal_and_gap_assemble(
  ...
  for (uint i = 0; i < n_test; ++i) {
    for (uint qp = 0; qp < n_qp; ++qp) {
      ...
      Real isect = 0;
      isector.intersect_ray_with_plane(
          dim, 1, &p.get_values()[0], &surf_normal.get_values()[0],
          &plane_normal.get_values()[0], plane_offset, &isect);
...
void mortar_normal_and_gap_assemble(
    const uint dim, const libMesh::FEVectorBase &test_fe,
    const libMesh::Point &surf_normal, const libMesh::Point &plane_normal,
    const libMesh::Real &plane_offset,
    libMesh::DenseMatrix<libMesh::Real> &normals,
    libMesh::DenseVector<libMesh::Real> &gap) {
...
void mortar_normal_and_gap_assemble_biorth(
    const int type, const uint dim, const libMesh::FEBase &test_fe,
    const libMesh::Point &surf_normal, const libMesh::Point &plane_normal,
    const libMesh::Real &plane_offset,
    const libMesh::DenseVector<libMesh::Real> &indicator,
    libMesh::DenseMatrix<libMesh::Real> &normals,
  ...
  mortar_normal_and_gap_assemble_biorth(type, test_fe, surf_normal_v,
                                        plane_normal_v, plane_offset, indicator,
                                        normals, gap);
}

void mortar_normal_and_gap_assemble_biorth(
    const int type, const libMesh::FEBase &test_fe,
    const libMesh::DenseVector<libMesh::Real> &surf_normal,
    const libMesh::DenseVector<libMesh::Real> &plane_normal,
    const libMesh::Real &plane_offset,
    const libMesh::DenseVector<libMesh::Real> &indicator,
...
void mortar_normal_and_gap_assemble_biorth(
    const int type, const libMesh::FEVectorBase &test_fe,
    const libMesh::DenseVector<libMesh::Real> &surf_normal,
    const libMesh::DenseVector<libMesh::Real> &plane_normal,
    const libMesh::Real &plane_offset,
    const libMesh::DenseVector<libMesh::Real> &indicator,
  ...
  for (uint qp = 0; qp < n_qp; ++qp) {
    ...
    for (uint i = 0; i < n_test; ++i) {
      ...
      for (uint d = 0; d < dim; ++d) {
        normals.get_values()[i] +=
            indicator(i) * biorth_test(d) * surf_normal(d) * JxW[qp];
      }
    }
  }
}
...
void mortar_normal_and_gap_assemble_biorth(
    const int type, const uint dim, const libMesh::FEVectorBase &test_fe,
    const libMesh::Point &surf_normal, const libMesh::Point &plane_normal,
    const libMesh::Real &plane_offset,
    const libMesh::DenseVector<libMesh::Real> &indicator,
    libMesh::DenseMatrix<libMesh::Real> &normals,
...
bool intersect_2D(const libMesh::DenseMatrix<libMesh::Real> &poly1,
  ...
  if (!
      // isector.
      intersect_convex_polygons(poly1.m(), &poly1.get_values()[0], poly2.m(),
                                &poly2.get_values()[0], &n_vertices_result,
                                result_buffer, DEFAULT_TOLLERANCE)) {
    return false;
  }
  ...
  // plot_polygon(2, n_vertices_result, &intersection.get_values()[0],
  // "isect/poly");
  return true;
}

void make_polygon_from_quad4(const libMesh::Elem &e,
...
void make_polygon_from_quad8(const libMesh::Elem &e,
                             libMesh::DenseMatrix<libMesh::Real> &polygon) {
...
void make_polygon_from_tri3(const libMesh::Elem &e,
                            libMesh::DenseMatrix<libMesh::Real> &polygon) {
  polygon.resize(e.n_nodes(), 2);
...
void discretize_segmented_curve(const double x[N], const double y[N],
                                const int order, const libMesh::Elem &e,
                                libMesh::DenseMatrix<libMesh::Real> &polygon) {
  using namespace libMesh;
...
void make_polygon_from_curved_tri6(
...
void make_polygon_from_curved_quad8(
    const libMesh::Elem &e, libMesh::DenseMatrix<libMesh::Real> &polygon) {
  using namespace libMesh;
...
void make_polygon_from_tri6(const libMesh::Elem &e,
                            libMesh::DenseMatrix<libMesh::Real> &polygon) {
  polygon.resize(e.n_nodes() / 2, 2);
  for (int i = 0; i < e.n_nodes() / 2; ++i) {
    for (int j = 0; j < 2; ++j) {
...
void make_polygon(const libMesh::Elem &e,
                  libMesh::DenseMatrix<libMesh::Real> &polygon) {
...
void make_polygon_3(const libMesh::Elem &e,
                    libMesh::DenseMatrix<libMesh::Real> &polygon) {
...
void make_polyhedron_from_tet10(const libMesh::Elem &e,
...
// FIXME between -1, 1 ?
void make_polyhedron_from_hex27(const libMesh::Elem &e,
                                Polyhedron &polyhedron) {
  const int dim = 3;
  assert(e.dim() == 3);
  assert(e.n_nodes() == 27);
...
bool intersect_3D(const libMesh::Elem &el1, const libMesh::Elem &el2,
...
bool intersect_3D(const Polyhedron &poly1, const Polyhedron &poly2,
                  Polyhedron &intersection) {
  Intersector isector;
...
bool project_2D(const libMesh::DenseMatrix<libMesh::Real> &poly1,
  ...
  isector.apply_affine_transform_2(Ainv, binv, n_points_master,
                                   &poly1.get_values()[0], w.ref_points_master);
  ...
  isector.apply_affine_transform_2(A, b, 2, w.isect_master,
                                   &projection_1.get_values()[0]);

  // move back to global coordinates
  isector.apply_affine_transform_2(A, b, 2, w.isect_slave,
                                   &projection_2.get_values()[0]);
  return true;
}
...
bool project_3D(const libMesh::DenseMatrix<libMesh::Real> &polygon_1,
  ...
  libMesh::DenseMatrix<libMesh::Real> ref_polygon_1(polygon_1.m(),
                                                    polygon_1.n());
  libMesh::DenseMatrix<libMesh::Real> ref_polygon_2(polygon_2.m(),
                                                    polygon_2.n());
  ...
  isector.apply_affine_transform_3(Ainv, binv, polygon_1.m(),
  ...
  const int n_projection_points = isector.project_surface_poly_onto_ref_poly(
      dim, ref_polygon_1.m(), &ref_polygon_1.get_values()[0], polygon_2.m(),
      &clipper_2.get_values()[0], isect_1, isect_2);

  // plot_polygon(clipper_2.n(),     clipper_2.m(),
  // &clipper_2.get_values()[0], "clipper");
  // plot_polygon(ref_polygon_1.n(), ref_polygon_1.m(),
  // &ref_polygon_1.get_values()[0], "clipped");
  ...
  isector.apply_affine_transform_3(A, b, n_projection_points, isect_1,
                                   &projection_1.get_values()[0]);
  isector.apply_affine_transform_3(A, b, n_projection_points, isect_2,
                                   &projection_2.get_values()[0]);
  return true;
}

bool biorthgonal_weights(const int type, libMesh::Real &w_ii,
                         libMesh::Real &w_ij) {
  using namespace libMesh;
  ...
  switch (type) {
    ...
    default: {
      ...
      if (!error_msg_printed) {
        std::cerr
            << "[Error] biorthgonal weights not supported for element type: "
            << type << std::endl;
...
template <class FE>
void mortar_assemble_weights_aux(const FE &fe,
...
void mortar_assemble_weights(const libMesh::FEVectorBase &fe,
                             libMesh::DenseMatrix<libMesh::Real> &weights) {
  mortar_assemble_weights_aux(fe, weights);
}

void mortar_assemble_weights(const libMesh::FEBase &fe,
                             libMesh::DenseMatrix<libMesh::Real> &weights) {
  mortar_assemble_weights_aux(fe, weights);
}
...
void mortar_assemble_weighted_biorth(
    const libMesh::FEBase &trial_fe, const libMesh::FEBase &test_fe,
    const libMesh::DenseMatrix<libMesh::Real> &weights,
    libMesh::DenseMatrix<libMesh::Real> &elmat) {
...
void mortar_assemble_weighted_biorth(
    const libMesh::FEVectorBase &trial_fe, const libMesh::FEVectorBase &test_fe,
    const libMesh::DenseMatrix<libMesh::Real> &weights,
    libMesh::DenseMatrix<libMesh::Real> &elmat) {
  mortar_assemble_weighted_aux(trial_fe, test_fe, weights, elmat);
}
...
template <class FE>
void mortar_assemble_weighted_aux(
    const double &master_frac, const double &slave_frac, const FE &trial_fe,
    const FE &test_fe, const libMesh::DenseMatrix<libMesh::Real> &weights,
    libMesh::DenseMatrix<libMesh::Real> &elmat) {
  if (elmat.m() != test_fe.get_phi().size() ||
      elmat.n() != trial_fe.get_phi().size()) {
    elmat.resize(test_fe.get_phi().size(), trial_fe.get_phi().size());
  ...
  for (uint i = 0; i < n_test; ++i) {
    for (uint qp = 0; qp < n_qp; ++qp) {
      ...
      for (uint j = 0; j < n_trial; ++j) {
        // assert(  JxW[qp] >= 0.0 );
        elmat(i, j) +=
            contract(w_test, trial[j][qp]) * JxW[qp] * master_frac * slave_frac;
      }
    }
  }
}
...
void mortar_assemble_weighted_biorth(
    const double &master_frac, const double &slave_frac, const libMesh::FEBase &trial_fe,
    const libMesh::FEBase &test_fe,
    const libMesh::DenseMatrix<libMesh::Real> &weights,
    libMesh::DenseMatrix<libMesh::Real> &elmat) {
  mortar_assemble_weighted_aux(master_frac, slave_frac, trial_fe, test_fe,
                               weights, elmat);
}

void mortar_assemble_weighted_biorth(
    const double &master_frac, const double &slave_frac,
    const libMesh::FEVectorBase &trial_fe, const libMesh::FEVectorBase &test_fe,
    const libMesh::DenseMatrix<libMesh::Real> &weights,
    libMesh::DenseMatrix<libMesh::Real> &elmat) {
  mortar_assemble_weighted_aux(master_frac, slave_frac, trial_fe, test_fe,
...
void mortar_normal_and_gap_assemble_weighted_biorth(
    const libMesh::FEVectorBase &test_fe, const int dim,
    const libMesh::Point &surf_normal, const libMesh::Point &plane_normal,
    const libMesh::Real &plane_offset,
    const libMesh::DenseMatrix<libMesh::Real> &weights,
    libMesh::DenseMatrix<libMesh::Real> &normals,
  ...
  for (uint qp = 0; qp < n_qp; ++qp) {
    ...
    Real isect = 0;
    isector.intersect_ray_with_plane(dim, 1, &p.get_values()[0],
                                     &s_n.get_values()[0], &p_n.get_values()[0],
                                     plane_offset, &isect);
...
template <typename T>
void convert_point_to_vector(const int dim, const libMesh::Point &point,
                             std::vector<T> &point_vec) {
...
void mortar_normal_and_gap_assemble_weighted_biorth(
    const libMesh::FEBase &test_fe, const int dim,
    const libMesh::Point &surf_normal, const libMesh::Point &plane_normal,
    const libMesh::Real &plane_offset,
    const libMesh::DenseMatrix<libMesh::Real> &weights,
    libMesh::DenseMatrix<libMesh::Real> &normals,
    libMesh::DenseVector<libMesh::Real> &gap, const bool visdbg) {
  using namespace libMesh;
  ...
  for (uint i = 0; i < n_test; ++i) {
    for (uint qp = 0; qp < n_qp; ++qp) {
      ...
      Real isect = 0;
      isector.intersect_ray_with_plane(dim, 1, &p.get_values()[0],
                                       &surf_normal_v[0], &plane_normal_v[0],
                                       plane_offset, &isect);
      // assert(isect > 0);
      // if(visdbg) {

...
namespace utopia {
    ...
    public:
        ...
        inline Integer node_dof_from_hash(const Integer &hash) const { return dof_map(hash); }
        ...
        inline Integer element_hash(const Array &coord) const { return element_hash_from_index(coord); }
        ...
        void elements_in_range(const Vector &min, const Vector &max, Index &hashes) const {
            hashes.clear();
        ...
        inline Integer n_elements(const Integer dim) const { return dims[dim]; }
        ...
        inline Integer n_nodes(const Integer dim) const { return dims[dim] + 1; }

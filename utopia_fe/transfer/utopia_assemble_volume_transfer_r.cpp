...
namespace utopia {
    ...
    static bool Assemble(moonolith::Communicator &comm,
        ...
        auto local_spaces = std::make_shared<FESpacesRAdapter>(master,
                                                               slave,
                                                               dof_master,
                                                               dof_slave,
                                                               dof_reverse_master,
        ...
        auto read =
            [&spaces, &migrated_spaces, comm, &libmesh_comm_master, &libmesh_comm_slave](
    ...
    static void assemble_biorth_weights_from_space(const std::shared_ptr<MeshBase> &mesh,
        ...
        std::unique_ptr<libMesh::FEBase> biorth_elem = libMesh::FEBase::build(dim, dof_map->variable_type(var_num));

        auto &el = **mesh->active_local_elements_begin();

        const int order = order_for_l2_integral(
            dim, el, dof_map->variable(var_num).type().order, el, dof_map->variable(var_num).type().order);
    ...
    bool Assemble(moonolith::Communicator &comm,
        ...
        auto local_fun_spaces = std::make_shared<FESpacesRAdapter>(master,
                                                                   slave,
                                                                   dof_master,
                                                                   dof_slave,
                                                                   dof_reverse_master,
        ...
        auto fun = [&](const VElementAdapter<Dimensions> &master, const VElementAdapter<Dimensions> &slave) -> bool {
            // c.start();

            libMesh::DenseMatrix<libMesh::Real> biorth_weights;

            if (use_biorth_) {
                assemble_biorth_weights_from_space(slave_space, dof_slave, var_num_slave, biorth_weights);
            }
            ...
            const int order = order_for_l2_integral(
                dim, master_el, dof_master->variable(0).type().order, slave_el, dof_slave->variable(0).type().order);
            ...
            if (pair_intersected) {
                ...
                local_element_matrices_sum +=
                    std::accumulate(elemmat.get_values().begin(), elemmat.get_values().end(), libMesh::Real(0.0));
                local_element_matrices_sum_reverse += std::accumulate(
                    elemmat_reverse.get_values().begin(), elemmat_reverse.get_values().end(), libMesh::Real(0.0));
                ;
        ...
        std::partial_sum(
            ownershipRangesMaster_r.begin(), ownershipRangesMaster_r.end(), ownershipRangesMaster_r.begin());
        ...
        const SizeType local_range_slave_r =
            ownershipRangesSlave_r[comm.rank() + 1] - ownershipRangesSlave_r[comm.rank()];
        const SizeType local_range_master_r =
            ownershipRangesMaster_r[comm.rank() + 1] - ownershipRangesMaster_r[comm.rank()];
        ...
        // bbecsek
        PetscMatrix B_x_reverse =
            utopia::local_sparse(local_range_master_r, local_range_slave_r, mMaxRowEntries_reverse);
        //
        {
        ...
        // std::cout << "Participate size: " << particip_slave_dofs.size() << " on processor " << world_rank <<
        // std::endl;
...
// dof_reverse_master, dof_reverse_slave, from_var_num, to_var_num, from_var_num_r, to_var_num_r); 		int offset = 0; 		int
// space_num = 0; 		if(tags.empty()){ 			for(auto s : local_spaces->spaces()) { 				if(s) { 					bool first = true;
// 					libMesh::dof_id_type local_element_id = 0;
// 					for (auto it = s->active_local_elements_begin(); it != s->active_local_elements_end(); ++it,
// ++local_element_id) { 						auto elem = *it; 						Adapter a(*s, elem->id(), offset + local_element_id, space_num);
// 						assert(!local_spaces->dof_map(space_num)[local_element_id].empty());
// 						assert(!local_spaces->dof_map_reverse(space_num)[local_element_id].empty());
// 						a.set_dof_map(&local_spaces->dof_map(space_num)[local_element_id].global);
// 						a.set_dof_map_reverse(&local_spaces->dof_map_reverse(space_num)[local_element_id].global);
...
// 					libMesh::dof_id_type local_element_id = 0;
// 					for (auto it = s->active_local_elements_begin(); it != s->active_local_elements_end(); ++it,
// ++local_element_id) { 						auto elem = *it; 						if (predicate->select(elem->subdomain_id())){ 							Adapter a(*s, elem->id(), offset
// + local_element_id, elem->subdomain_id()); 							assert(!local_spaces->dof_map(space_num)[local_element_id].empty());
// 							assert(!local_spaces->dof_map_reverse(space_num)[local_element_id].empty());
// 							a.set_dof_map(&local_spaces->dof_map(space_num)[local_element_id].global);
// 							a.set_dof_map_reverse(&local_spaces->dof_map_reverse(space_num)[local_element_id].global);
// 							tree->insert(a);
...
// &dof_map, 												   const int var_num, 												   libMesh::DenseMatrix<libMesh::Real> &weights)
// 	{
// 		const int dim = mesh->mesh_dimension();
...
// 												el,
// dof_map->variable(var_num).type().order, 												el, dof_map->variable(var_num).type().order);
...
// 		auto local_fun_spaces = std::make_shared<FESpacesRAdapter>(master, slave, dof_master, dof_slave,
// dof_reverse_master, dof_reverse_slave, from_var_num, to_var_num, from_var_num_r,to_var_num_r);
...
// 			const int order = order_for_l2_integral(dim, master_el, dof_master->variable(0).type().order ,
// slave_el,dof_slave->variable(0).type().order); 			libMesh::Real weight_reverse = 0;
...
// 				if(intersect_2D(master_pts, slave_pts, intersection2)) {
//                     total_intersection_volume += fabs(Intersector::polygon_area_2(intersection2.m(),
//                     &intersection2.get_values()[0]));

//                     const libMesh::Real weight = Intersector::polygon_area_2(slave_pts.m(),
//                     &slave_pts.get_values()[0]); weight_reverse = Intersector::polygon_area_2(master_pts.m(),
//                     &master_pts.get_values()[0])/weight;
...
// 					mortar_assemble_weighted_biorth(*slave_fe, *master_fe, biorth_weights_r,
// elemmat_reverse);
...
// 				local_element_matrices_sum +=         std::accumulate(elemmat.get_values().begin(),
// elemmat.get_values().end(), libMesh::Real(0.0)); 				local_element_matrices_sum_reverse +=
// std::accumulate(elemmat_reverse.get_values().begin(), elemmat_reverse.get_values().end(), libMesh::Real(0.0));;
...
// 		if(!Assemble<Dimensions>(comm, master, slave, dof_master, dof_slave, dof_reverse_master, dof_reverse_slave,
// from_var_num, to_var_num, from_var_num_r, to_var_num_r, fun, settings, use_biorth_, n_var, n_var_r,tags)) { 			return
// false;
// 		}

// 		double volumes[3] = { local_element_matrices_sum,  total_intersection_volume,
// local_element_matrices_sum_reverse };
...
// ownershipRangesMaster.begin()); 		std::partial_sum(ownershipRangesSlave.begin(), ownershipRangesSlave.end(),
...
// 		std::partial_sum(ownershipRangesMaster_r.begin(), ownershipRangesMaster_r.end(),
// ownershipRangesMaster_r.begin()); 		std::partial_sum(ownershipRangesSlave_r.begin(), ownershipRangesSlave_r.end(),
// ownershipRangesSlave_r.begin());
...
// 		assert(ownershipRangesSlave.empty() == ownershipRangesMaster.empty() || ownershipRangesMaster.empty());
// 		assert(ownershipRangesMaster_r.empty() == ownershipRangesSlave_r.empty() ||
// ownershipRangesSlave_r.empty());
...
// 		const SizeType local_range_slave  = ownershipRangesSlave [comm.rank()+1] - ownershipRangesSlave
// [comm.rank()]; 		const SizeType local_range_master = ownershipRangesMaster[comm.rank()+1] -
// ownershipRangesMaster[comm.rank()];

// 		const SizeType local_range_slave_r  = ownershipRangesSlave_r [comm.rank()+1] - ownershipRangesSlave_r
// [comm.rank()]; 		const SizeType local_range_master_r = ownershipRangesMaster_r[comm.rank()+1] -
// ownershipRangesMaster_r[comm.rank()];

// 		USparseMatrix B_x = utopia::local_sparse(local_range_slave, local_range_master, mMaxRowEntries);
// 		USparseMatrix B_x_reverse = utopia::local_sparse(local_range_master_r, local_range_slave_r,
// mMaxRowEntries_reverse);
...
// 		utopia::each_read(B_x_reverse, [&](const utopia::SizeType i, const utopia::SizeType j, const double value)
// { 			for(utopia::SizeType d = 0; d < n_var_r ; ++d) { 				B_reverse.set(i + d, j + d, value);
// 			}
// 		});
...
// 									const std::shared_ptr<DofMap> &dof_slave,
// 									const std::shared_ptr<DofMap>
// &dof_reverse_master, 									const std::shared_ptr<DofMap> &dof_reverse_slave, 									const unsigned int & from_var_num, 									const
// unsigned int & to_var_num, 									const unsigned int & from_var_num_r, 									const unsigned int & to_var_num_r, 									bool  use_biorth_,
// 									int n_var,
// 									int n_var_r,
// 									USparseMatrix &B,
// 									USparseMatrix &B_reverse,

#include "utopia_assemble_volume_transfer_xfem.hpp"
...
static bool Assemble(const std::vector<double> &master_volfracs,
                     const std::vector<double> &slave_volfracs,
                     ...
                     const unsigned int &to_var_num, Fun process_fun,
                     const moonolith::SearchSettings &settings, bool use_biorth,
                     const std::vector<std::pair<int, int>> &tags, int n_var) {
  ...
  auto local_spaces = std::make_shared<FESpacesAdapter>(
      master, slave, dof_master, dof_slave, from_var_num, to_var_num);
  ...
  if (tags.empty()) {
    ...
    for (auto s : local_spaces->spaces()) {
      if (s) {
        ...
        for (auto it = s->active_local_elements_begin();
             it != s->active_local_elements_end(); ++it, ++local_element_id) {
          auto elem = *it;
          Adapter a(*s, elem->id(), offset + local_element_id, space_num);
          assert(!local_spaces->dof_map(space_num)[local_element_id].empty());
          a.set_dof_map(
              &local_spaces->dof_map(space_num)[local_element_id].global);
          tree->insert(a);
        }
      ...
      if (s) {
        ...
        for (auto it = s->active_local_elements_begin();
          ...
          if (predicate->select(elem->subdomain_id())) {
            Adapter a(*s, elem->id(), offset + local_element_id,
                      elem->subdomain_id());
            assert(!local_spaces->dof_map(space_num)[local_element_id].empty());
            a.set_dof_map(
                &local_spaces->dof_map(space_num)[local_element_id].global);
  ...
  auto read = [&spaces, &migrated_spaces, comm, &libmesh_comm_master,
               &libmesh_comm_slave, &tags](
      const long ownerrank, const long senderrank, bool is_forwarding,
      DataContainer &data, InputStream &in) {
    CHECK_STREAM_READ_BEGIN("vol_proj", in);

    std::shared_ptr<FESpacesAdapter> proc_space =
        std::make_shared<FESpacesAdapter>(comm);
    ...
    if (tags.empty()) {
      ...
      for (auto s : proc_space->spaces()) {
        if (s) {
          // ID_FIX this should be fine n_elem is actually local sence the mesh
          // is a SerialMesh
  ...
  auto write = [&local_spaces, &spaces, &comm](
      ...
      const std::vector<long>::const_iterator &end, const DataContainer &data,
      OutputStream &out) {
...
static void assemble_biorth_weights_from_space(
    const std::shared_ptr<MeshBase> &mesh,
    const std::shared_ptr<DofMap> &dof_map, const int var_num,
    libMesh::DenseMatrix<libMesh::Real> &weights) {
  const int dim = mesh->mesh_dimension();
  std::unique_ptr<libMesh::FEBase> biorth_elem =
      libMesh::FEBase::build(dim, dof_map->variable_type(var_num));

  auto &el = **mesh->active_local_elements_begin();

  const int order =
      order_for_l2_integral(dim, el, dof_map->variable(var_num).type().order,
...
bool Assemble(const std::vector<double> &master_volfracs,
              const std::vector<double> &slave_volfracs,
              moonolith::Communicator &comm,
              const std::shared_ptr<MeshBase> &master,
              const std::shared_ptr<MeshBase> &slave,
              const std::shared_ptr<DofMap> &dof_master,
              const std::shared_ptr<DofMap> &dof_slave,
              const unsigned int &from_var_num, const unsigned int &to_var_num,
              DSMatrixd &B, const moonolith::SearchSettings &settings,
              bool use_biorth, const std::vector<std::pair<int, int>> &tags,
              int n_var) {
  const int var_num_slave = to_var_num;

  std::shared_ptr<FESpacesAdapter> local_fun_spaces =
      std::make_shared<FESpacesAdapter>(master, slave, dof_master, dof_slave,
                                        from_var_num, to_var_num);
  ...
  if (use_biorth) {
    assemble_biorth_weights_from_space(slave_space, dof_slave, var_num_slave,
                                       biorth_weights);
  }
  //----------------------------------------------------------------------------//
  auto fun = [&](const VElementAdapter<Dimensions> &master,
    ...
    master_fe = libMesh::FEBase::build(src_mesh.mesh_dimension(),
                                       dof_master->variable_type(0));
    slave_fe = libMesh::FEBase::build(dest_mesh.mesh_dimension(),
                                      dof_slave->variable_type(0));
    ...
    const int order =
        order_for_l2_integral(dim, src_el, dof_master->variable(0).type().order,
                              dest_el, dof_slave->variable(0).type().order);
    ...
    if (dim == 2) {
      ...
      if (intersect_2D(src_pts, dest_pts, intersection2)) {
        total_intersection_volume += fabs(isector.polygon_area_2(
            intersection2.m(), &intersection2.get_values()[0]));
        const libMesh::Real weight =
            isector.polygon_area_2(dest_pts.m(), &dest_pts.get_values()[0]);
        const libMesh::Real updated_weights =
            weight * master_volfracs[src_index] * slave_volfracs[dest_index];
    ...
    if (pair_intersected) {
      ...
      transform_to_reference(*dest_trans, dest_el.type(), composite_ir,
                             dest_ir);
      ...
      assert(src_el.id() == src_index);
      assert(dest_el.id() == dest_index);

      if (master_volfracs[src_index] != 1 || slave_volfracs[dest_index] != 1)
        std::cout << "master id: " << src_index
                  << "\tfrac: " << master_volfracs[src_index]
                  << "\tslave id: " << dest_index
                  << "\tfrac: " << slave_volfracs[dest_index] << std::endl;

      if (use_biorth) {
        mortar_assemble_weighted_biorth(master_volfracs[src_index],
                                        slave_volfracs[dest_index], *master_fe,
                                        *slave_fe, biorth_weights, elemmat);
      } else {
        mortar_assemble(master_volfracs[src_index], slave_volfracs[dest_index],
                        *master_fe, *slave_fe, elemmat);
      }

      auto partial_sum =
          std::accumulate(elemmat.get_values().begin(),
  ...
  if (!Assemble<Dimensions>(master_volfracs, slave_volfracs, comm, master,
                            slave, dof_master, dof_slave, from_var_num,
                            to_var_num, fun, settings, use_biorth, tags,
                            n_var)) {
    return false;
  }
  ...
  if (comm.is_root()) {
    std::cout << "sum(B): " << volumes[0] << ", vol(I): " << volumes[1]
              << std::endl;
  }
  ...
  ownershipRangesMaster[comm.rank() + 1] +=
      static_cast<unsigned int>(n_dofs_on_proc_master);
  ownershipRangesSlave[comm.rank() + 1] +=
      static_cast<unsigned int>(n_dofs_on_proc_slave);

  comm.all_reduce(&ownershipRangesMaster[0], ownershipRangesMaster.size(),
                  moonolith::MPISum());
  comm.all_reduce(&ownershipRangesSlave[0], ownershipRangesSlave.size(),
                  moonolith::MPISum());
  ...
  moonolith::Redistribute<moonolith::SparseMatrix<double>> redist(
      comm.get_mpi_comm());
  redist.apply(ownershipRangesSlave, mat_buffer,
               moonolith::AddAssign<double>());
  ...
  const SizeType local_range_slave_range =
      ownershipRangesSlave[comm.rank() + 1] - ownershipRangesSlave[comm.rank()];
  const SizeType local_range_master_range =
      ownershipRangesMaster[comm.rank() + 1] -
      ownershipRangesMaster[comm.rank()];

  DSMatrixd B_x = utopia::local_sparse(
      local_range_slave_range, local_range_master_range, mMaxRowEntries);
  {
  ...
  utopia::each_read(B_x, [&](const utopia::SizeType i, const utopia::SizeType j,
                             const double value) {
...
bool assemble_volume_transfer(const std::vector<double> &master_volfracs,
                              const std::vector<double> &slave_volfracs,
                              moonolith::Communicator &comm,
                              const std::shared_ptr<MeshBase> &master,
                              const std::shared_ptr<MeshBase> &slave,
                              ...
                              const std::shared_ptr<DofMap> &dof_slave,
                              const unsigned int &from_var_num,
                              const unsigned int &to_var_num, bool use_biorth,
  ...
  if (master->mesh_dimension() == 2) {
    ok = utopia::Assemble<2>(master_volfracs, slave_volfracs, comm, master,
                             slave, dof_master, dof_slave, from_var_num,
                             to_var_num, B, settings, use_biorth, tags, n_var);
  } else if (master->mesh_dimension() == 3) {
    ok = utopia::Assemble<3>(master_volfracs, slave_volfracs, comm, master,
                             slave, dof_master, dof_slave, from_var_num,
                             to_var_num, B, settings, use_biorth, tags, n_var);
  } else {

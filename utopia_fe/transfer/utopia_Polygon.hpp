...
namespace utopia {
	...
	class Polygon {
	public:
		Polygon(const int n_vertices)
		: coordinates_(n_vertices * Dimensions)
		{}
		...
		inline T * point(const int index)
		{
			return &coordinates_[index * Dimensions];
		}

		inline const T * point(const int index) const
		{
			return &coordinates_[index * Dimensions];
		}
	...
	bool split_curve(Fun fun, const T *from, const T *to, 
		const int polynomial_order, 
		std::vector<T> &result, const T tol)
	{
		T f_from[Dimensions];
		T f_to  [Dimensions];	 
		T f_q   [Dimensions]; 
	...
	template<int Dimensions, typename T, class Fun>
	void discretize_curve(Fun fun, const T *from, const T *to, 
		const int polynomial_order, 

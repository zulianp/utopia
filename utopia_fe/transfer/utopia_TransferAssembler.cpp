...
namespace utopia {
    ...
    public:
        ...
        FESpaceSerializerDeserializer(const libMesh::Parallel::Communicator &comm,
                                      const TransferOptions &opts,
                                      const std::shared_ptr<FESpacesAdapter> &local_spaces)
            : comm(comm), m_comm(comm.get()), opts(opts), local_spaces(local_spaces) {}
        ...
        void read(const long ownerrank,
        ...
        void write(const long ownerrank,
                   const long recvrank,
    ...
    public:
        ...
        DefaultAlgorithm(const std::shared_ptr<MeshBase> &from_mesh,
        ...
        void init(const std::shared_ptr<MeshBase> &from_mesh,
                  const std::shared_ptr<DofMap> &from_dofs,
                  const std::shared_ptr<MeshBase> &to_mesh,
                  const std::shared_ptr<DofMap> &to_dofs,
                  const TransferOptions &opts,
            ...
            this->local_spaces = std::make_shared<FESpacesAdapter>(
                from_mesh, to_mesh, from_dofs, to_dofs, opts.from_var_num, opts.to_var_num);
        ...
        inline bool assemble(Adapter &master, Adapter &slave) {
        ...
        bool assemble_aux() {
            ...
            FESpaceSerializerDeserializer<Dimensions> serializer(from_mesh->comm(), opts, local_spaces);

            auto read = [&serializer](const long ownerrank,
                                      const long senderrank,
                                      bool is_forwarding,
                                      DataContainer &data,
                                      InputStream &in) {
            ...
            auto write = [&serializer](const long ownerrank,
                                       const long recvrank,
                                       const std::vector<long>::const_iterator &begin,
                                       const std::vector<long>::const_iterator &end,
                                       const DataContainer &data,
            ...
            auto fun = [&](Adapter &master, Adapter &slave) -> bool { return this->assemble(master, slave); };
    ...
    TransferAssembler::TransferAssembler(const std::shared_ptr<LocalAssembler> &assembler,
                                         const std::shared_ptr<Local2Global> &local2global)
        : assembler_(assembler), local2global_(local2global) {}

    TransferAssembler::~TransferAssembler() {}

    bool TransferAssembler::assemble(const std::shared_ptr<MeshBase> &from_mesh,
                                     const std::shared_ptr<DofMap> &from_dofs,
                                     const std::shared_ptr<MeshBase> &to_mesh,
                                     const std::shared_ptr<DofMap> &to_dofs,
        ...
        if (spatial_dimension == 1) {
            algorithm_ = std::make_shared<DefaultAlgorithm<1>>(
                from_mesh, from_dofs, to_mesh, to_dofs, opts, assembler_, local2global_);
        } else if (spatial_dimension == 2) {
            algorithm_ = std::make_shared<DefaultAlgorithm<2>>(
                from_mesh, from_dofs, to_mesh, to_dofs, opts, assembler_, local2global_);
        } else if (spatial_dimension == 3) {
            algorithm_ = std::make_shared<DefaultAlgorithm<3>>(
                from_mesh, from_dofs, to_mesh, to_dofs, opts, assembler_, local2global_);
        } else {

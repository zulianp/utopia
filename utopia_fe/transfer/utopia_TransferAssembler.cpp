...
namespace utopia {
	...
	public:
		...
		void post_assemble(std::size_t buffer_num)
		{
			...
			USMatrix mat_x = utopia::local_sparse(n_dofs_on_proc_test, n_dofs_on_proc_trial, m_max_row_entries);

			{
				utopia::Write<utopia::USMatrix> write(mat_x);
			...
			utopia::Write<USMatrix> w_mat(mat);

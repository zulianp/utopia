		...
		for (auto it = master_slave->active_local_elements_begin();
			...
			for(uint side_elem = 0; side_elem < elem->n_sides(); ++side_elem){
				if ((predicate->select(master_slave->get_boundary_info().boundary_id(elem, side_elem)))) {
					//ID_FIX
					SurfaceAdapter a(*master_slave, elem->id(), element_id, master_slave->get_boundary_info().boundary_id(elem, side_elem), search_radius);
...
// #endif //HACK_OVERRIDE_N_SLAVE_Z
		...
		USMatrix normal_tilde = utopia::local_sparse(n_local_side_node_dofs, dim, dim);
		{
			utopia::Write<utopia::USMatrix> write(normal_tilde);
		...
		USMatrix B_tilde = utopia::local_sparse(n_local_side_node_dofs, n_local_side_node_dofs, n_max_row_entries_b);
		{
		...
		USMatrix P = utopia::local_sparse(n_local_dofs_slave, n_local_side_node_dofs, n_max_row_entries_p);
		{
			utopia::Write<utopia::USMatrix> write(P);
			for (auto it = P_buffer.iter(); it; ++it) {
				P.set(it.row(), it.col(), *it);
			}
		}

		USMatrix Q = utopia::local_sparse(n_local_dofs_master, n_local_side_node_dofs, n_max_row_entries_q);
		{
			utopia::Write<utopia::USMatrix> write(Q);
			for (auto it = Q_buffer.iter(); it; ++it) {
				Q.set(it.row(), it.col(), *it);
			}
		}

		USMatrix Q_t = transpose(Q);
		USMatrix B_x = P * B_tilde * Q_t;
						  ...
						  const bool use_volume_differential)
	{
		...
		if(mesh->mesh_dimension() == 2) {
			ok = utopia::SurfaceAssemble<2>(comm, mesh, dof_map, var_num, B,  orthogonal_trafos, gap, normals, is_contact_node, settings, search_radius, tags, use_biorth, use_volume_differential);
		} else if(mesh->mesh_dimension() == 3) {
			ok = utopia::SurfaceAssemble<3>(comm, mesh, dof_map, var_num, B,  orthogonal_trafos, gap, normals,  is_contact_node, settings, search_radius, tags, use_biorth, use_volume_differential);
		} else {

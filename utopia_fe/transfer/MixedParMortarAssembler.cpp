...
namespace utopia {
	...
	MixedParMortarAssembler::MixedParMortarAssembler(libMesh::Parallel::Communicator &libmesh_comm,
		const std::shared_ptr<LibMeshFESpaceBase> &master,
		const std::shared_ptr<LibMeshFESpaceBase> &slave)
	:libmesh_comm_(libmesh_comm), master_(master), slave_(slave),use_biorth_(false)
	{ }
	...
	public:
		...
		Spaces(const std::shared_ptr<LibMeshFESpaceBase> &master, 
			const std::shared_ptr<LibMeshFESpaceBase> &slave)
		{
			...
			const int n_elements_master = master->mesh().n_elem();
			const int n_elements_slave  = slave->mesh().n_elem();
			...
			copy_var_order(*master, var_order_[0]);
			copy_var_order(*slave,  var_order_[1]);

		}


		inline std::vector< std::shared_ptr<LibMeshFESpaceBase> > &spaces()
		{
			return spaces_;

		}

		inline const std::vector< std::shared_ptr<LibMeshFESpaceBase> > &spaces() const
		{
			return spaces_;
	...
	private:
		...
		std::vector<std::shared_ptr< LibMeshFESpaceBase>> spaces_;
		...
		inline static void copy_global_dofs(LibMeshFESpaceBase &space, std::vector<ElementDofMap> &dof_map, std::vector<ElementDofMap> &variable_type, const int n_elements)
		{
 ...
 //       std::cout<<"------------------------------------WRITE_ELEM_SELEC-MID---------------------------------------"<<std::endl;
  ...
  //       std::cout<<"------------------------------------READ-SPACES-OUT------------------------------------------"<<std::endl;
	...
	static bool Assemble(express::Communicator &comm, 
		std::shared_ptr<LibMeshFESpaceBase> &master,
		std::shared_ptr<LibMeshFESpaceBase> &slave,
		Fun process_fun, 
		const cutk::Settings &settings, bool use_biorth_)
	{
		...
		const auto &master_mesh = master->mesh();
		const auto &slave_mesh  = slave->mesh();
		const int n_elements_master = master_mesh.n_elem();
		const int n_elements_slave  = slave_mesh.n_elem();
		const int n_elements 		= n_elements_master + n_elements_slave;


        const Parallel::Communicator &libmesh_comm_master = master->mesh().comm();
        const Parallel::Communicator &libmesh_comm_slave = slave->mesh().comm();
        ...
        for(auto s : local_spaces->spaces()) {
			if(s) {
                ...
                bool first = true;
                   for (auto it = s->mesh().active_local_elements_begin(); it != s->mesh().active_local_elements_end(); ++it) {
                    auto elem=*it;
				...
				offset += s->mesh().n_elem(); //s->mesh().n_active_local_elem();//(*s->mesh().active_local_elements_end())->id();
...
//        custom_settings.set("disable_redistribution", cutk::Boolean(true));
	...
	bool Assemble(
		...
		const cutk::Settings &settings, bool use_biorth_)
	{
		std::shared_ptr<Spaces> local_fun_spaces = cutk::make_shared<Spaces>(master, slave);
		...
		std::shared_ptr<LibMeshFESpaceBase> master_space = master;
		std::shared_ptr<LibMeshFESpaceBase> slave_space  = slave;
		...
		auto fun = [&](const ElementAdapter<Dimensions> &master, 
			...
			const int order = order_for_l2_integral(dim, src_el, master_space->order() , dest_el, slave_space->order());
    ...
    //            dest.dof_map().dof_indices(&dest_el, slave_dofs);
                ...
                if(use_biorth_) {
                    ...
                    mortar_assemble_biorth(*slave_fe,  *slave_fe, dest_el.type(), other_mat);
		...
		// utopia::Chrono c2;
		// c2.start();
		if(!Assemble<Dimensions>(comm, master, slave, fun, settings, use_biorth_)) {
        ...
        const dof_id_type n_dofs_on_proc_master = master->dof_map().n_local_dofs();

		const processor_id_type slave_proc_id   = slave->mesh().processor_id();

		const dof_id_type n_dofs_on_proc_slave  =slave->dof_map().n_local_dofs();
        
        const int n_dofs_on_proc_print  = master->dof_map().n_local_dofs();
...
//        std::cout<<"prova"<<n_dofs_on_proc_master<<std::end;
...
//         MatGetOwnershipRange(utopia::raw_type(B),m_1,n_1);
//         std::cout<<"prova"<<*m_1<<*n_1<<std::endl;
	...
	bool MixedParMortarAssembler::Assemble(DSMatrixd &B)
	{
	...
	bool MixedParMortarAssembler::Transfer(DSMatrixd &B, DSMatrixd &T)
	{
		DVectord               diag_elem;
		PetscInt               mG;
		PetscInt               nG;

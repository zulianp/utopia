...
namespace utopia {
    ...
    public:
        ...
        static void check_for_constraint_loops(libMesh::DofMap &dof_map, libMesh::DofConstraints &_dof_constraints);
        ...
        static void construct_constraint_matrix(const libMesh::Elem *elem,
            ...
            {
                const unsigned int old_size = static_cast<unsigned int>(dof_indices.size());

                // Add new dependency dofs to the end of the current dof set
                dof_indices.insert(dof_indices.end(), dof_set.begin(), dof_set.end());
        ...
        static void constrain_matrix(const libMesh::Elem *elem,
            ...
            construct_constraint_matrix(elem,
                                        dof_map,
                                        dof_constraints,
                                        C,
                                        dof_indices,
        ...
        static void constrain_matrix_and_vector(const libMesh::Elem *elem,
                                                const libMesh::DofMap &dof_map,
                                                const libMesh::DofConstraints dof_constraints,
                                                ElementMatrix &u_mat,
                                                ElementVector &u_vec,
            ...
            for (uint i = 0; i < ndofs; ++i) {
                if (dof_map.is_constrained_dof(dof_indices[i])) {
                    ...
                    for (const auto &item : c_row) {
                        for (uint j = 0; j < ndofs; j++) {
                            if (dof_indices[j] == item.first) {
                                // std::cout<<"dof_indices[i]:"<<dof_indices[i]<<" and hanging
                                // entry:"<<item.second<<std::endl;
                                mat(i, j) = -item.second;
                            }
                        }
                    }

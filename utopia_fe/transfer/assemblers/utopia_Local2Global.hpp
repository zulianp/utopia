...
namespace utopia {
    ...
    public:
        ...
        Local2Global(const bool use_set_instead_of_add) : use_set_instead_of_add(use_set_instead_of_add) {}

        void apply(const std::vector<long> &trial,
                   const std::vector<long> &test,
                   const LocalMatrix &local_mat,
                   moonolith::SparseMatrix<double> &global_mat) const;

        void add(const std::vector<long> &trial,
                 const std::vector<long> &test,
                 const LocalMatrix &local_mat,
                 moonolith::SparseMatrix<double> &global_mat) const;

        void set_non_zero(const std::vector<long> &trial,
                          const std::vector<long> &test,
                          const LocalMatrix &local_mat,
                          moonolith::SparseMatrix<double> &global_mat) const;

        void redistribute(moonolith::Communicator &comm,

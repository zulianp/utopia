...
namespace utopia {
    ...
    L2LocalAssembler::L2LocalAssembler(const int dim,
    ...
    bool L2LocalAssembler::assemble(const Elem &trial,
                                    FEType trial_type,
                                    const Elem &test,
                                    FEType test_type,
                                    Matrix &mat) {
    ...
    bool L2LocalAssembler::assemble(const Elem &trial,
        ...
        const int order = std::max(order_for_l2_integral(dim, trial, trial_type.order, test, test_type.order),
    ...
    void L2LocalAssembler::init_fe(const Elem &trial, FEType trial_type, const Elem &test, FEType test_type) {
        if (trial_fe) return;
    ...
    void L2LocalAssembler::init_biorth(const Elem &test, FEType test_type) {
        ...
        assemble_biorth_weights(test, test.dim(), test_type, test_type.order, biorth_weights);

        must_compute_biorth = false;
    }

    void L2LocalAssembler::assemble_biorth_weights(const libMesh::Elem &el,
                                                   const int dim,

...
namespace utopia {
    void Local2Global::apply(const std::vector<long> &trial,
    ...
    void Local2Global::add(const std::vector<long> &trial,
                           const std::vector<long> &test,
                           const LocalMatrix &local_mat,
                           moonolith::SparseMatrix<double> &global_mat) const {
    ...
    void Local2Global::set_non_zero(const std::vector<long> &trial,
                                    const std::vector<long> &test,
                                    const LocalMatrix &local_mat,
                                    moonolith::SparseMatrix<double> &global_mat) const {
        for (std::size_t i = 0; i < test.size(); ++i) {
    ...
    void Local2Global::redistribute(moonolith::Communicator &comm,

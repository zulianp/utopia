...
namespace utopia {
    ...
    //   std::cout<<"Adaptivity::process_constraints::BEGIN "<<std::endl;
       ...
       //  auto a = const_cast<std::numeric_limits<unsigned int>*>(&std::numeric_limits<unsigned int>::max());

       //  auto b = const_cast<std::numeric_limits<double>*>(&std::numeric_limits<double>::max());

       // const auto invalid_pair = std::make_pair(a,b);

       compute_boundary_nodes_to_skip(mesh, dof_map, 0,0, index_2);

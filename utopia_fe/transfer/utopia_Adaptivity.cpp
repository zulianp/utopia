...
void Adaptivity::compute_all_constraints(const libMesh::MeshBase &mesh,
  ...
  for (; el != end_el; ++el) {
    ...
    if (fe_type.order > 0) {
      ...
      compute_constraints(constraints, dof_map, var_num, elem,
                          mesh.mesh_dimension());
    }
    // }
  }
  ...
  std::cout << "--------------------------------------------------\n";
  std::cout << "[Adaptivity::compute_all_constraints] n_constraints: "
            << constraints.size() << std::endl;
  std::cout << "--------------------------------------------------\n";
}

void Adaptivity::assemble_constraint(const libMesh::MeshBase &mesh,
                                     const libMesh::DofMap &dof_map) {
  ...
  if (fe_type.order > 0) {
    ...
    for (; el != end_el; ++el) {
      ...
      //      ++var_num) {
      compute_constraints(dof_constraints_, dof_map, var_num, elem,
                          mesh.mesh_dimension());
      //ß}
    }
  ...
  std::cout << "--------------------------------------------------\n";
  std::cout << "[Adaptivity::assemble_constraint] n_constraints: "
            << dof_constraints_.size() << std::endl;
  std::cout << "--------------------------------------------------\n";
}

void Adaptivity::constraint_matrix(const LibMeshFunctionSpace &V,
...
void Adaptivity::constraint_matrix(const libMesh::MeshBase &mesh,
  ...
  Write<USparseMatrix> w(M, utopia::GLOBAL_INSERT),
      w_2(S, utopia::GLOBAL_INSERT);
    ...
    if (!dof_set.empty() || !called_recursively) {
      ...
      for (unsigned int i = 0; i != old_size; i++)
        if (dof_map.is_constrained_dof(elem_dofs[i])) {
          ...
          for (const auto &item : constraint_row) {
            for (unsigned int j = 0, n_elem_dofs = static_cast<unsigned int>(
                                         elem_dofs.size());
                 j != n_elem_dofs; j++) {
  ...
  for (auto s : elem->side_index_range()) {
    if (elem->neighbor_ptr(s) != nullptr &&
...

      {
        ...
        const unsigned int n_side_dofs = libMesh::FEInterface::n_dofs(
            mesh_dim - 1, fe_type, my_side->type());

        const unsigned int n_parent_side_dofs = libMesh::FEInterface::n_dofs(
            mesh_dim - 1, fe_type, parent_side->type());
        ...
        for (unsigned int my_dof = 0; my_dof != n_side_dofs; my_dof++) {
          ...
          for (unsigned int their_dof = 0; their_dof != n_parent_side_dofs;
               their_dof++) {
          ...
          const libMesh::Point mapped_point = libMesh::FEInterface::inverse_map(
              mesh_dim - 1, fe_type, parent_side.get(), support_point);
          ...
          for (unsigned int their_dof = 0; their_dof != n_parent_side_dofs;
            ...
            const double their_dof_value = libMesh::FEInterface::shape(
                mesh_dim - 1, fe_type, parent_side->type(), their_dof,
                mapped_point);
            if ((std::abs(their_dof_value) > 1.e-5) &&
              ...
              // if(!check)
              constraint_row->insert(
                  std::make_pair(their_dof_g, their_dof_value));
            }
          }
        }
      }
  ...
  while (!unexpanded_set.empty())
    for (RCSet::iterator i = unexpanded_set.begin(); i != unexpanded_set.end();
      ...
      for (const auto &expandable : constraints_to_expand) {
        ...
        DofConstraints::const_iterator subpos =
            _dof_constraints.find(expandable);

        libmesh_assert(subpos != _dof_constraints.end());

        if (subpos == _dof_constraints.end())
          return;
...
void Adaptivity::compute_boundary_nodes(const libMesh::MeshBase &mesh,
  ...
  if (mesh.mesh_dimension() < 3) {
    ...
    const libMesh::MeshBase::const_node_iterator end_it =
  ...
  else {
    {
      ...
      const libMesh::MeshBase::const_element_iterator end_it =
      ...
      for (; it != end_it; ++it) {
        ...
        for (int jj = 0; jj < ele_parent_0->n_sides(); jj++) {
          ...
          for (int ll = 0; ll < parent_side_0->n_nodes(); ll++) {
            ...
            const libMesh::dof_id_type node_dof_0 =
                node_0->dof_number(sys_number, var_number, 0);
          ...
          if (index_local.size() == parent_side_0->n_nodes()) {
            ...
            auto check = (std::find(dirichlet_id.begin(), dirichlet_id.end(),
                                    bc_id) != dirichlet_id.end());

            if (!check)
              dirichlet_id.push_back(bc_id);
          }
        }
      }
    }
...
void Adaptivity::compute_boundary_nodes_to_skip(const libMesh::MeshBase &mesh,
  ...
  std::cout << "Adaptivity::compute_boundary_nodes_to_skip::BEGIN "
            << std::endl;
  ...
  const libMesh::MeshBase::const_element_iterator end_it_0 =
      mesh.active_elements_end();
  ...
  for (; it_0 != end_it_0; ++it_0) {
    ...
    libMesh::FEType fe_type = dof_map.variable_type(var_num);
    fe_type.order =
        static_cast<libMesh::Order>(fe_type.order + elem->p_level());
          ...
          for (unsigned int my_dof = 0; my_dof != n_side_dofs; my_dof++) {
            ...
            for (unsigned int their_dof = 0; their_dof != n_parent_side_dofs;
              ...
              if (their_dof_g == my_dof_g) {
                ...
                auto check_2 = (std::find(index_self.begin(), index_self.end(),
                                          their_dof_g) != index_self.end());
  ...
  const libMesh::MeshBase::const_element_iterator end_it_1 =
      mesh.active_elements_end();
  ...
  const libMesh::MeshBase::const_element_iterator end_it_2 =
      mesh.active_elements_end();
  ...
  for (; it_2 != end_it_2; ++it_2) {
    ...
    { // if the element is subactive (i.e. has no active descendants)
      ...
      for (int kk = 0; kk < ele->n_sides(); kk++) {
        ...
        auto neigh = ele->neighbor_ptr(kk);
        if (ele->neighbor_ptr(kk) == nullptr &&
            ele->neighbor_ptr(kk) != libMesh::remote_elem) {
  ...
  for (auto it = tmp.begin(); it != tmp.end(); ++it) {
    ...
    auto check = (std::find(index_self.begin(), index_self.end(), *it) !=
                  index_self.end());

    if (!check) {

      auto check_2 =
          (std::find(index.begin(), index.end(), *it) != index.end());
  ...
  std::cout << "Adaptivity::compute_boundary_nodes_to_skip::END " << tmp.size()
            << " and " << index.size() << std::endl;
}

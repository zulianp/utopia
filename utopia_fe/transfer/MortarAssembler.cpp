...
namespace utopia {
    ...
    // 		D = sparse(slave_->dof_map().n_local_dofs(), slave_->dof_map().n_local_dofs(), std::max(1,
    // int(master_->dof_map().n_local_dofs() * 0.2)));
    ...
    // 			int i = 0;
    // 			for(auto e_it = slave_mesh.active_local_elements_begin(); e_it != slave_mesh.active_local_elements_end();
    // ++e_it, ++i) { 				const auto &elem = **e_it; 				if(dim == 2) { 					make_polygon(elem, slave_pts); 					volumes[i] =
    // Intersector::polygon_area_2(slave_pts.m(), &slave_pts.get_values()[0]); 				} else if(dim == 3) {
    ...
    // 				const int order = order_for_l2_integral(dim, master_el, master_order, slave_el,
    // slave_order);
    ...
    // 							const double area = Intersector::polygon_area_2(intersection2.m(),
    // &intersection2.get_values()[0]); 							total_intersection_volume += fabs(area);
    ...
    // 						triangulate_polygon(slave_pts.m(), &slave_pts.get_values()[0],
    // slave_tri); 						triangulate_polygon(master_pts.m(), &master_pts.get_values()[0],  master_tri);
    ...
    // 							s_tri(0, 0) = slave_pts(v1_s, 0); s_tri(0, 1) = slave_pts(v1_s,
    // 1); 							s_tri(1, 0) = slave_pts(v2_s, 0); s_tri(1, 1) = slave_pts(v2_s, 1); 							s_tri(2, 0) = slave_pts(v3_s, 0);
    // s_tri(2, 1) = slave_pts(v3_s, 1);

    // 							for(size_t i_master = 0; i_master < master_tri.size(); i_master += 3)
    // { 								const int v1_m = master_tri[i_master]; 								const int v2_m = master_tri[i_master + 1]; 								const int v3_m =
    // master_tri[i_master + 2];

    // 								m_tri(0, 0) = master_pts(v1_m, 0); m_tri(0, 1) = master_pts(v1_m,
    // 1); 								m_tri(1, 0) = master_pts(v2_m, 0); m_tri(1, 1) = master_pts(v2_m, 1); 								m_tri(2, 0) = master_pts(v3_m, 0);
    // m_tri(2, 1) = master_pts(v3_m, 1);

    // 								assert(Intersector::polygon_area_2(m_tri.m(), &m_tri.get_values()[0]) >
    // 0); 								assert(Intersector::polygon_area_2(s_tri.m(), &s_tri.get_values()[0]) > 0);

    // 								if(intersect_2D(m_tri, s_tri, intersection2)) {
    // 									const double area = Intersector::polygon_area_2(intersection2.m(),
    // &intersection2.get_values()[0]); 									relative_intersection_volume += fabs(area);

    // 									triangulate_polygon(intersection2.m(), &intersection2.get_values()[0],
    // temp_tri);

    // 									const int offset = intersection_points.size()/2;
    // 									std::transform(temp_tri.begin(), temp_tri.end(),
    // temp_tri.begin(), 										[offset](const int val) -> int { return offset + val; });

    // 									intersection_tri.insert(intersection_tri.end(), temp_tri.begin(),
    // temp_tri.end()); 									intersection_points.insert(intersection_points.end(), intersection2.get_values().begin(),
    // intersection2.get_values().end()); 									pair_intersected = true;
    // 								}
    // 							}
    // 						}
    ...
    // 							libMesh::DenseMatrix<libMesh::Real> isect_mesh_pts(intersection_points.size()/2,
    // 2); 							isect_mesh_pts.get_values() = std::move(intersection_points);
    // 							make_composite_quadrature_2D_from_tri_mesh(intersection_tri, isect_mesh_pts, weight, order,
    // composite_ir);
    ...
    // 	 				// 				  slave_order  * ( is_hex(slave_el.type())? 3  : 1 ) * (slave_el.has_affine_map()? 1 :
    // 2);

    // 						make_composite_quadrature_3D(intersection3, weight, order,
    // composite_ir); 						pair_intersected = true;
    ...
    // 					transform_to_reference(*master_trans, master_el.type(), composite_ir,
    // master_ir); 					transform_to_reference(*slave_trans,  slave_el.type(),  composite_ir,  slave_ir);
    ...
    // 						mortar_assemble_biorth(*slave_fe,  *slave_fe, slave_el.type(),
    // other_mat);
    ...
    // 					local_element_matrices_sum += std::accumulate(elemmat.get_values().begin(), elemmat.get_values().end(),
    // libMesh::Real(0.0)); 					other_sum += std::accumulate(other_mat.get_values().begin(), other_mat.get_values().end(),
    // libMesh::Real(0.0)); 					intersected = true;
    // 					++n_intersections;
    // 				}
    // 			}
    ...
    // 				std::cout << "B in R^(" << size(B).get(0) <<  " x " << size(B).get(1) << ")" <<
    // std::endl;
    ...
    void ContactAssembly::finalize() {
        ...
        avg_gap = (std::accumulate(v.begin(), v.end(), RealT(0.)) / std::accumulate(m.begin(), m.end(), RealT(0.))) *
                  normals.n();
    }
    ...
    // 	void build_dag(std::vector< std::shared_ptr<ContactAssembly> > &contacts, std::vector< std::vector<long> > &dag,
    ...
    // 		std::sort(contacts.begin(), contacts.end(), [](const std::shared_ptr<ContactAssembly> &left, const
    // std::shared_ptr<ContactAssembly> &right) -> bool { 			return *left < *right;
    // 		});
    ...
    // 	void assign_master_and_slave_roles(const std::vector< std::vector<long> > &dag, const std::vector<long>
    // &ordering, const std::vector< std::vector<long> > &adj_list, std::vector<ushort> &role)
    // 	{
    // 		size_t source = 0;
    ...
    // skip 		if(cos_angle >= -0.5) { 			continue;
    // 		}
    ...
    // &side_polygon_2.get_values()[0]); 			const Scalar area   	= Intersector::polygon_area_3(isect_polygon_2.m(),
    // &isect_polygon_2.get_values()[0]); 			const Scalar relative_area 	= area/area_slave; 			const Scalar weight
    // = 1./area_slave;
    ...
    // 						libMesh::FEVectorBase::build(dim,
    // space.dof_map().variable_type(space.var_num()));

    // 						QMortar q_biorth(dim);

    // 						const int order = order_for_l2_integral(dim, el_2, approx_order, el_2,
    // approx_order);

    // 						if(dim == 3) {
    // 							make_composite_quadrature_on_surf_3D(side_polygon_2, 1., order,
    // q_biorth); 						} else { 							make_composite_quadrature_on_surf_2D(side_polygon_2, 1., order, q_biorth);
    // 						}

    // 						QMortar q_biorth_ref(dim);
    // 						transform_to_reference_surf(*transform_2, el_2.type(), q_biorth,
    // q_biorth_ref);
    ...
    // 					// plot_polygon(dim, side_polygon_2.m(),  &side_polygon_2.get_values()[0],  "s/poly/"  +
    // std::to_string(index_2));
    // 					// plot_polygon(dim, isect_polygon_2.m(), &isect_polygon_2.get_values()[0], "s/isect/" +
    // std::to_string(index_2));
    // 					// plot_polygon(dim, side_polygon_1.m(),  &side_polygon_1.get_values()[0],  "m/poly/p" +
    // std::to_string(index_1));
    // 					// plot_polygon(dim, isect_polygon_1.m(), &isect_polygon_1.get_values()[0], "m/isect/" +
    // std::to_string(index_1));
    ...
    // 					const Scalar local_mat_sum = std::accumulate(current_contact->coupling.get_values().begin(),
    // current_contact->coupling.get_values().end(), libMesh::Real(0.0)); 					local_element_matrices_sum += local_mat_sum;
    // 					intersected = true;
    ...
    //                 pow(current_contact->isect_area, dim/(dim-1.)) * dim  <<std::endl;

    //                 // std::cout<<" partial sum = " << local_mat_sum <<std::endl;

    //                 // std::cout<<" local_element_matrices_sum = " << local_element_matrices_sum <<std::endl;

    //                 // assert(fabs(local_mat_sum - pow(current_contact->isect_area, dim/(dim-1.)) * dim) < 1e-8 ||
    //                 (!is_quad(el_2.type()) && !is_hex(el_2.type())));
    ...
    // 	std::vector< std::shared_ptr<ContactAssembly> > contacts;
    // 	if(!find_contacts(space, master_fe, slave_fe, contacts, search_radius, strict_gap_policy, predicate)) return
    // false;
    ...
    // 			// 	plot_polygon(dim, side_polygon_2.m(), &side_polygon_2.get_values()[0], "s/el/" +
    // std::to_string(c_ptr->parent_element_slave));
    // 			// }
    ...
    // 		each_read(coupling, [&sum_c, &coupling_2, &is_contact_node](const SizeType i, const SizeType j, const Scalar
    // value) { 			if(is_contact_node[i]) { 				coupling_2.set(i, j, value/sum_c.get(i));
    // 			}
    // 		});
    // 	}
    ...
    // bool MortarContactAssembler::assemble(USparseMatrix &coupling, UVector &gap, UVector &normals, USparseMatrix
    // &orthogonal_trafos, std::vector<bool> &is_contact_node, const libMesh::Real search_radius, const
    // std::shared_ptr<moonolith::Predicate> &predicate) {
    ...
    // 			space_->dof_map().variable_type(space_->var_num()));
    // 		return assemble_aux(*space_, master_fe, slave_fe, coupling, gap, normals, orthogonal_trafos, is_contact_node,
    // search_radius, strict_gap_policy, predicate);

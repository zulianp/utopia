...
namespace utopia {
    ...
    public:
        ...
        Socket(const std::string &_host, const std::string &_service) : _host(_host), _service(_service) {}
    ...
    void plot_polygon_mesh(const libMesh::MeshBase &mesh, const std::string &name) {
        ...
        for (auto e_it = mesh.active_local_elements_begin(); e_it != mesh.active_local_elements_end(); ++e_it) {
            ...
            plot_polygon(
                polygon.n(), polygon.m(), &polygon.get_values()[0], name + "/" + std::to_string((*e_it)->id()));
        }
    }

    void plot_mesh_aux(const libMesh::MeshBase &mesh,
        ...
        int index = 0;
        for (auto e_it = mesh.active_local_elements_begin(); e_it != mesh.active_local_elements_end();
             ++e_it, ++index) {
            auto &el = **e_it;
    ...
    void plot_scaled_normal_field(const libMesh::MeshBase &mesh,
        ...
        for (auto n_it = mesh.active_nodes_begin(); n_it != mesh.active_nodes_end(); ++n_it) {
            ...
            all_points.insert(all_points.end(), point.begin(), point.end());

            all_normals.insert(all_normals.end(), local_normal.get_values().begin(), local_normal.get_values().end());
        }
        ...
        quiver(mesh_dim, all_points.size() / mesh_dim, &all_points[0], &all_normals[0], name);
    }
}  // namespace utopia
...
namespace utopia {
    ...
    void plot_polygon(const int, const int, const double *, const std::string &) {
        std::cerr << "[Warning] plot function not implemented, make sure to have a proper boost installation"
                  << std::endl;
    }
    ...
    void plot_scaled_normal_field(const libMesh::MeshBase &, const UVector &, const UVector &, const std::string &) {
        std::cerr << "[Warning] plot function not implemented, make sure to have a proper boost installation"
                  << std::endl;
    }
}  // namespace utopia

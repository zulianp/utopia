...
namespace utopia {
    ...
    void apply_displacement(const UVector &displacement_increment,
        ...
        if (!comm.is_alone()) {
            ...
            UVector out;
            displacement_increment.select(idx, out);
    ...
    Wear::Wear() : wear_coefficient(7e-3), extrapolation_factor(10.) {}

    void Wear::compute_displacement(ProductFunctionSpace<LibMeshFunctionSpace> &V,
            ...
            for (auto i = r.begin(); i < r.end(); i += dim) {
                if (is_normal_component.get(i) > 0) {
                    for (unsigned int d = 0; d < dim; ++d) {
                        ...
                        wear_induced_displacement.set(i + d,
                                                      normals.get(i + d) * (-extrapolation_factor) * wear.get(i));
                    }
                }
            }
        }
    ...
    void Wear::mesh_displacement(ProductFunctionSpace<LibMeshFunctionSpace> &V,
                                 const std::vector<int> &boundary_tags,
    ...
    void Wear::init_aux_system(libMesh::EquationSystems &es, libMesh::Order order) {
        ...
        if (dim > 2) var_num_aux.push_back(aux.add_variable("inc_z", libMesh::Order(order), libMesh::LAGRANGE));
        ...
        if (dim > 2) var_num_aux.push_back(aux.add_variable("vel_z", libMesh::Order(order), libMesh::LAGRANGE));
        ...
        if (dim > 2) var_num_aux.push_back(aux.add_variable("f_z", libMesh::Order(order), libMesh::LAGRANGE));
        ...
        if (dim > 2) var_num_aux.push_back(aux.add_variable("fext_z", libMesh::Order(order), libMesh::LAGRANGE));
        ...
        if (dim > 2) var_num_aux.push_back(aux.add_variable("n_z", libMesh::Order(order), libMesh::LAGRANGE));
        ...
        if (dim > 2) var_num_aux.push_back(aux.add_variable("wear_disp_z", libMesh::Order(order), libMesh::LAGRANGE));
    ...
    void Wear::update_aux_system(const int main_system_number,

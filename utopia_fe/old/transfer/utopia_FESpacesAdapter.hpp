...
namespace utopia {
    ...
    public:
        inline FESpacesAdapter(const moonolith::Communicator &comm) : comm(comm) {}

        FESpacesAdapter(const std::shared_ptr<libMesh::MeshBase> &master,
        ...
        inline std::vector<std::shared_ptr<libMesh::MeshBase>> &spaces() { return spaces_; }

        inline const std::vector<std::shared_ptr<libMesh::MeshBase>> &spaces() const { return spaces_; }

        inline std::vector<std::shared_ptr<libMesh::MeshBase>> &meshes() { return spaces_; }

        inline const std::vector<std::shared_ptr<libMesh::MeshBase>> &meshes() const { return spaces_; }
    ...
    static void write_element_selection(const Iterator &begin,
        ...
        if (has_master) {
            write_space(master_selection.begin(),
        ...
        if (has_slave) {
            write_space(slave_selection.begin(),
                        slave_selection.end(),
                        *s,
                        spaces.dof_map(1),
...
#endif  // UTOPIA_FE_SPACES_ADAPTER_HPP

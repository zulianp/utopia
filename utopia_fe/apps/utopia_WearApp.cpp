...
namespace utopia {
    ...
    public:
        ...
        virtual bool init_sim(libMesh::Parallel::Communicator &comm, Input &is) {
            ...
            is.get("simulation", [this, &ok, &comm](Input &is) { ok = init(comm, is); });
        ...
        virtual bool init(libMesh::Parallel::Communicator &comm, Input &is) {
            ...
            if (desc_.stabilization != "none") {
                std::cout << "using stabilization: " << desc_.stabilization << " mag: " << desc_.stabilization_mag
                          << std::endl;
                material = std::make_shared<StabilizedMaterial<decltype(V), USparseMatrix, UVector>>(
                    V, desc_.stabilization_mag, material, desc_.stabilization);
            }
            ...
            material = std::make_shared<ForcedMaterial<USparseMatrix, UVector>>(material, forcing_function);

            material->rescaling(desc_.rescaling);
            /////////////////////////////////////////////////////////////////////////////
            return true;
        }
        ...
        inline Path output_path() const { return desc_.output_path; }
    ...
    class WearApp::SimulationInput : public ContactSimulation {
    public:
    ...
    void WearApp::run(Input &is) {
        ...
        auto configuration_forced_material =
            std::make_shared<ForcedMaterial<USparseMatrix, UVector>>(in.material, configuration_forces);
        ...
        if (in.is_steady) {
            ...
            transient_solver = std::make_shared<TransientContactSolverT>(
                make_ref(in.V), configuration_forced_material, dt, in.contact_params);
        ...
        wear.init_aux_system(*in.equation_systems);
        ...
        for (int i = 1; i <= in.n_cycles; ++i) {
            ...
            for (int t = 0; t < in.gc.conf().n_steps(); ++t) {
                ...
                wear.update_aux_system(0, state, solver->contact(), in.gc.conf().dt(), *in.equation_systems);

        ...
        if (!is_time_series) {
            ...
            for (uint i = from; i < to; i += every) {
                space.space().each([&](const int subspace_id, LibMeshFunctionSpace &s) {
                    ...
                    io.copy_nodal_solution(s.equation_system(), name, name, i + 1);
                });
                ...
                if (apply_disp) {
                    ...
                    deform_mesh(mesh.mesh(), space.space()[0].dof_map(), data);
                    ...
                    deform_mesh(mesh.mesh(), space.space()[0].dof_map(), aux);
                } else {
                    space.space().each([&](const int subspace_id, LibMeshFunctionSpace &s) {
                        const auto &name = s.var_name();

                        DiegoMeshWriter().write_field(
                            folder, name + "_t" + std::to_string(i + 1), mesh.mesh(), data, subspace_id);
                    });
                }
            }
        }

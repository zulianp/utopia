...
namespace utopia {
    ...
    public:
        OwnershipRanges(const int rank, const int comm_size) : rank_(rank) {
        ...
        inline long local_nodes_begin() const { return ownership_ranges_[rank_]; }

        inline long local_nodes_end() const { return ownership_ranges_[rank_ + 1]; }

        inline long n_local_dofs() const { return local_nodes_end() - local_nodes_begin(); }

        const std::vector<long> &get() { return ownership_ranges_; }
    ...
    public:
        Grid2(const int rank, const int comm_size) : ownership_ranges(rank, comm_size) {}
        ...
        inline long local_nodes_begin() const { return ownership_ranges.local_nodes_begin(); }

        inline long local_nodes_end() const { return ownership_ranges.local_nodes_end(); }

        inline long n_local_dofs() const { return ownership_ranges.n_local_dofs(); }

        inline bool empty() const { return this->n_nodes() == 0; }
    ...
    public:
        Grid3(const int rank, const int comm_size) : ownership_ranges(rank, comm_size) {}
    ...
    public:
        InputGrid(libMesh::Parallel::Communicator &comm)
            : comm_(comm), grid2(comm.rank(), comm.size()), grid3(comm.rank(), comm.size()) {}
    ...
    void Grid2MeshTransferApp::run(Input &in) {
        ...
        in.get("transfer", [&](Input &is) {
            ...
            if (type == "l2-projection") {
                ...
                local_assembler_ =
                    std::make_shared<L2LocalAssembler>(master_mesh_dim,
                                                       biorth_basis,
        ...
        if (input_master.is_3D) {
            ...
            if (volume_to_surface) {
                ...
                ok = transfer_assembler.assemble(grid3,
            ...
            if (volume_to_surface) {
                Grid2MeshSurfaceTransferAssembler transfer_assembler(local_assembler_, local2global_);
                ok = transfer_assembler.assemble(grid2,
        ...
        if (type == "l2-projection" || type == "approx-l2-projection") {
            if (biorth_basis && type != "approx-l2-projection") {
                ...
                if (mats.size() == 2) {
                    auto l2op = std::make_shared<L2TransferOperator>(
                        mats[0], mats[1], std::make_shared<Factorization<USparseMatrix, UVector>>());
                    ...
                    auto l2op = std::make_shared<L2TransferOperator>(
                        mats[0], D, std::make_shared<Factorization<USparseMatrix, UVector>>());
                    l2op->init();
                    transfer_op_ = l2op;
                }
            }

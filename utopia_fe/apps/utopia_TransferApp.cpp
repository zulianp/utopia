	...
	void TransferApp::run(const std::string &conf_file_path)
	{
		...
		if(type == "l2-projection" || type == "approx-l2-projection") {
			if(biorth_basis && type != "approx-l2-projection") {
				...
				if(mats.size() == 2) {
					auto l2op = std::make_shared<L2TransferOperator>(mats[0], mats[1], std::make_shared<Factorization<USMatrix, UVector>>());
					...
					auto D = std::make_shared<USMatrix>();

					assemble(inner(u, v) * dX, *D);
					transfer_op_ = std::make_shared<L2TransferOperator>(mats[0], D, std::make_shared<Factorization<USMatrix, UVector>>());
				}
			}
		...
		if(!fun_is_constant) {
			...
			BiCGStab<USMatrix, UVector> solver;

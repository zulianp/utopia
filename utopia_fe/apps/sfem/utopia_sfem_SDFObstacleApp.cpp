...
namespace utopia {
    ...
    public:
        ...
        void run(Input &in) {
            ...
            Vector_t gap, normal;
            sdf.to_mesh(space.mesh(), gap, normal);
            ...
            //         space.comm().root_print("SDFObstacleApp[Warning] solver failed to converge!");
            //     }
...
void sfem_sdf_obstacle(utopia::Input &in) {
    utopia::SDFObstacleApp<utopia::sfem::FunctionSpace> app;
    app.run(in);
}

UTOPIA_REGISTER_APP(sfem_sdf_obstacle);

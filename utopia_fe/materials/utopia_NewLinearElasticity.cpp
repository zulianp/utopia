...
namespace utopia {
        ...
        void assemble(FE &element, USerialMatrix &mat) override {
            ...
            loop(dx.size(), [&](const SizeType &q) {
                ...
                for (SizeType i = 0; i < n_funs; ++i) {
                    ...
                    // exploit symmetry
                    const Scalar v_ii = r_mu_dx * inner(strain[i], strain[i]) + r_lambda_dx * inner(div_i, div_i);

                    mat.add(i, i, v_ii);

                    for (SizeType j = i + 1; j < n_funs; ++j) {
                        const Scalar v_ij =
                            r_mu_dx * inner(strain[i], strain[j]) + r_lambda_dx * inner(div_i, div_u[j][q]);
    ...
    NewLinearElasticity<FunctionSpaceT, Matrix, Vector>::NewLinearElasticity(FunctionSpaceT &V,
                                                                             const LameeParameters &params)
    ...
    template <class FunctionSpaceT, class Matrix, class Vector>
    bool NewLinearElasticity<FunctionSpaceT, Matrix, Vector>::normal_stress(const UVector &x,
                                                                            UVector &out,
                                                                            const int subspace) {
        auto u = trial(V_);
    ...
    template <class FunctionSpaceT, class Matrix, class Vector>
    bool NewLinearElasticity<FunctionSpaceT, Matrix, Vector>::von_mises_stress(const UVector &x,
                                                                               UVector &out,
                                                                               const int subspace) {
        auto u = trial(V_);
        auto vx = test(V_[subspace]);

...
namespace utopia {
    ...
    UFlow<FunctionSpace, Matrix, Vector>::UFlow(FunctionSpace &space) : space_(space), rescale_(1.0) {}
    ...
    template <class FunctionSpace, class Matrix, class Vector>
    bool UFlow<FunctionSpace, Matrix, Vector>::assemble_hessian_and_gradient(const Vector &x,
                                                                             Matrix &hessian,
                                                                             Vector &gradient) {
        assert(is_valid());

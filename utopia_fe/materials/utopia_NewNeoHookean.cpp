...
namespace utopia {
    ...
    bool NewNeoHookean<FunctionSpace, Matrix, Vector>::assemble_hessian_and_gradient(const Vector &x,
                                                                                     Matrix &hessian,
                                                                                     Vector &gradient) {
    ...
    bool NewNeoHookean<FunctionSpace, Matrix, Vector>::stress(const Vector &x, Vector &result) {
        ...
        assemble(V_, inner(grad(trial(V_)), grad(trial(V_))) * dX, result, [&](FE &element, USerialVector &vec) {
            assembler.init(element);

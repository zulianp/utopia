...
namespace utopia {
    ...
    public:
        ...
        public:
            ...
            inline auto get_cubature(CellTopology &cell, const int order)
                -> decltype(this->cub_factory_.create<Device, Scalar, Scalar>(cell, order)) {
        ...
        public:
            void init(const LibMeshFunctionSpace &space) {
                ...
                cell_ptr_ = std::make_shared<CellTopology>(
                    shards::getCellTopologyData<shards::Triangle<3>>());  // TODO fixed to triangle in 3D
                ...
                int index = 0;
                for (auto e_it = m.active_local_elements_begin(); e_it != m.active_local_elements_end();
                     ++e_it, ++index) {
                    const auto &e = **e_it;
        ...
        Intrepid2Assembler() : verbose_(Utopia::instance().verbose()) {}

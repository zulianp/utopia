...
namespace utopia {
    ...
    void libmesh_assemble_aux(FunctionSpace<Derived> &V_w,
                              FE &element,
        ...
        if (!disable_adaptivity) {
            Adaptivity::compute_all_constraints(mesh, dof_map, constraints);
        }
    ...
    void assemble(FunctionSpace<LibMeshFunctionSpace> &V_w,
                  const Expression<InitExpr> &init_expr,
                  USparseMatrix &matrix,
                  LocalAssembler assembler) {
    ...
    template <class LocalAssembler, class InitExpr>
    void assemble(ProductFunctionSpace<LibMeshFunctionSpace> &V_w,
                  const Expression<InitExpr> &init_expr,
                  USparseMatrix &matrix,
                  LocalAssembler assembler) {

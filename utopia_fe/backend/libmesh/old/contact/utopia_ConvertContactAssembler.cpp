...
namespace utopia {
    ...
    public:
        ...
        static void normalize_and_build_orthgonal_trafo(const SpaceT<Dim> &node_wise_space,
            ...
            node_wise.orthogonal_trafo =
                local_sparse(node_wise_space.dof_map().n_local_dofs(), node_wise_space.dof_map().n_local_dofs(), Dim);
        ...
        static void convert_to_node_wise(const SpaceT<Dim> &elem_wise_space,
...

            {
                ...
                each_transform(
                    node_wise.is_glue, node_wise.is_glue, [&node_wise](const SizeType i, const double value) -> double {
                        if (value > 0.0) {
                            return 1.0;
                        } else {
                            return 0.0;
    ...
    bool ConvertContactAssembler::assemble(libMesh::MeshBase &mesh,
                                           libMesh::DofMap &dof_map,
    ...
    bool ConvertContactAssembler::init_no_contact(const libMesh::MeshBase &mesh, const libMesh::DofMap &dof_map) {
        if (!contact_tensors_) {

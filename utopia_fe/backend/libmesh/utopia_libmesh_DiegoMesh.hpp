...
namespace utopia {
    ...
    public:
        ...
        bool write_field(const Path &folder,
        ...
        void build_index(const libMesh::MeshBase &mesh, const std::set<int> &blocks, Selector &select) {
            select.blocks = blocks;
            build_index(mesh, blocks, select.n_elements, select.node_index, select.n_nodes);
        }

        bool write_coords(const Path &folder,
                          const libMesh::MeshBase &mesh,
                          const Selector &selector,
                          const std::string &postfix = "");

        void build_index(const libMesh::MeshBase &mesh,

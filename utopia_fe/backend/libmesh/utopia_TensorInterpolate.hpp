...
namespace utopia {
    ...
    public:
        ...
        TensorInterpolate(const FunctionSpace &space, const Vector &vec, const SizeType rows, const SizeType cols)
            : space_(space), vec_(vec), rows_(rows), cols_(cols) {}

        const FunctionSpace &space() const { return space_; }
    ...
    template <class FunctionSpace, class Vector>
    inline TensorInterpolate<FunctionSpace, Vector> tensor_interpolate(const FunctionSpace &space,
                                                                       const Vector &vec,
                                                                       const int rows,
                                                                       const int cols) {
    ...
    inline void build_tensor_ghosted(const Traits<UVector>::SizeType &n_local_dofs,
    ...
    inline bool read_tensor_data(const utopia::Path &path,

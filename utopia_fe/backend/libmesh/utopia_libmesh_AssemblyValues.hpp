...
namespace utopia {
    ...
    public:
        ...
        public:
            ...
            void init(const int spatial_dim, const std::vector<std::vector<libMesh::VectorValue<double>>> &v) {
        ...
        inline std::vector<std::unique_ptr<FE>> &fe() { return fe_; }

        inline const std::vector<std::unique_ptr<FE>> &fe() const { return fe_; }

        inline const std::vector<std::shared_ptr<VectorElement>> &vector_fe() const { return vector_fe_; }

        inline std::vector<std::shared_ptr<VectorElement>> &vector_fe() { return vector_fe_; }

        inline std::vector<std::unique_ptr<FE>> &test() { return fe_; }

        inline std::vector<std::unique_ptr<FE>> &trial() { return fe_; }

        inline const std::vector<std::unique_ptr<FE>> &test() const { return fe_; }
        ...
        inline const std::vector<std::unique_ptr<FE>> &trial() const { return fe_; }

        inline std::shared_ptr<libMesh::QBase> quad_test() const { return quad_test_; }
        ...
        inline bool on_boundary() const { return on_boundary_; }
        ...
        inline int block_id() const { return block_id_; }

        inline void set_current_element(const long current_element) { current_element_ = current_element; }

        inline long current_element() const { return current_element_; }
        ...
        void set_has_dual(const bool val) { has_dual_ = val; }
    ...
    private:
        ...
        class FEInitializer {
        public:
            FEInitializer(LibMeshAssemblyValues &ctx) : ctx(ctx) {}
            ...
            void init_phi(const LibMeshFunctionSpace &s) { ctx.fe()[s.subspace_id()]->get_phi(); }

            void init_phi(const ProductFunctionSpace<LibMeshFunctionSpace> &s) {
                s.each([&](const int, const LibMeshFunctionSpace &space) { ctx.fe()[space.subspace_id()]->get_phi(); });
            }
            ...
            void init_JxW(const ProductFunctionSpace<LibMeshFunctionSpace> &s) {
                s.each([&](const int, const LibMeshFunctionSpace &space) { ctx.fe()[space.subspace_id()]->get_JxW(); });
            }
            ...
            void init_normals() { ctx.fe()[0]->get_normals(); }

            void init_dphi(const ProductFunctionSpace<LibMeshFunctionSpace> &s) {
                s.each(
                    [&](const int, const LibMeshFunctionSpace &space) { ctx.fe()[space.subspace_id()]->get_dphi(); });
            ...
            template <class C, template <class> class Function>
            inline int visit(
                const Gradient<Interpolate<C, Function<ProductFunctionSpace<LibMeshFunctionSpace>>>> &expr) {
                init_dphi(*expr.expr().fun().space_ptr());
                return TRAVERSE_SKIP_SUBTREE;
            }
            ...
            template <class C, template <class> class Function>
            inline int visit(
                const Divergence<Interpolate<C, Function<ProductFunctionSpace<LibMeshFunctionSpace>>>> &expr) {
                init_dphi(*expr.expr().fun().space_ptr());
                return TRAVERSE_SKIP_SUBTREE;
            }

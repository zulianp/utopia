    ...
    namespace mars {
        ...
        public:
            ...
            bool add_offsetted_scalar_op_to_vector_owned(const int s_offset_i,
                ...
                auto el_max_size = node_to_element.extent(1);

                Kokkos::parallel_for(
                    "Count_nodes", owned_size, MARS_LAMBDA(const ::mars::Integer i) {
                        /*auto owned_dof = handler.get_owned_dof(i);*/
                        /*auto label = handler.get_label(owned_dof);*/
                        /* const auto local_dof_i = handler.get_owned_dof(i); */
                        Scalar val = 0;
            ...
            void collect_ghost_layer_with_callback(const Vector &in,
                ...
                ::mars::ViewVectorType<Scalar> x_view_rank1(::Kokkos::subview(x_view, ::Kokkos::ALL, 0));
                ::mars::set_locally_owned_data(dof_handler, out, x_view_rank1);
                UTOPIA_TRACE_REGION_BEGIN("mars::ConcreteFEAssember::gather_ghost_data");
                ::mars::gather_ghost_data(dof_handler, out, callback);
                UTOPIA_TRACE_REGION_END("mars::ConcreteFEAssember::gather_ghost_data");
                Kokkos::fence();
            ...
            bool block_op_apply(Op op, const Vector &x, Vector &y) {
                ...
                if (compute_communicate_overlap_) {
                    ...
                    collect_ghost_layer(x, x_current_local_view);
                    UTOPIA_TRACE_REGION_BEGIN("mars::ConcreteFEAssember::add_offset()");
                    ok = add_offsetted_block_op_to_vector(0, 0, op, x_current_local_view, y_view);
                    UTOPIA_TRACE_REGION_END("mars::ConcreteFEAssember::add_offset()");
                }

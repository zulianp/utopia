...
namespace utopia {
	...
	//P0/P1 only
	template<mars::Integer Dim, mars::Integer ManifoldDim>
	class MarsFunctionSpace : public FunctionSpace<MarsFunctionSpace<Dim, ManifoldDim>> {
	public:
		using Integer = mars::Integer;

		class Variable {
		public:
			Variable(const std::string &name = "u", const Integer &order = 1)
		...
		MarsFunctionSpace(
			const std::shared_ptr<mars::Mesh<Dim, ManifoldDim>> &mesh,

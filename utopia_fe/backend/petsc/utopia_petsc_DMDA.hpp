...
namespace utopia {
    ...
    namespace petsc {
        ...
        template <class Point, class IntArray>
        class DMDA : public StructuredGrid<Point, IntArray>, public PetscDMBase {
        public:
            ...
            DMDA(const PetscCommunicator &comm, const DMDAElementType &type_override = DMDA_ELEMENT_Q1)
                : PetscDMBase(comm), type_override_(type_override) {
                this->set_n_components(1);
            }

            DMDA(DM &dm, const bool delegate_ownership) {
                this->set_n_components(1);
            ...
            std::unique_ptr<DMDA> uniform_refine() const {
                auto fine = utopia::make_unique<DMDA>(comm(), type_override_);
            ...
            std::unique_ptr<DMDA> clone(const SizeType &n_components) const {
                auto cloned = utopia::make_unique<DMDA>(comm(), type_override_);
                cloned->copy(*this);
            ...
            std::unique_ptr<DMDA> clone() const { return clone(this->n_components()); }

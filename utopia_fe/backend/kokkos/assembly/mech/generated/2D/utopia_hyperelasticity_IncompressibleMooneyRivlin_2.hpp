	...
	namespace kernels {
		...
		public:
				...
				T *UTOPIA_RESTRICT bf) const
			{
				...
				bf[4] += -x2*(grad_trial[0]*(C2*f[1]*x10 - grad_test[0]*(C1 + C2*pow(f[1], 2))) + grad_trial[1]*(C2*f[0]*x11 - grad_test[1]*(C1 + C2*pow(f[0], 2))));
				bf[8] += 0;
				...
				T *UTOPIA_RESTRICT lf) const
			{
				...
				T x8 = f[1]*f[2];
				T x9 = grad_test[0]*x6;
				lf[0] += dx*(f[0]*pow(f[3], 2)*x9 + f[0]*x2 + f[1]*pow(f[2], 2)*x7 + f[1]*x3 - f[2]*x4 - f[2]*x5*x7 + f[3]*x0 - f[3]*x8*x9);
				lf[1] += dx*(pow(f[0], 2)*f[3]*x7 + f[0]*x4 - f[0]*x7*x8 + pow(f[1], 2)*f[2]*x9 - f[1]*x0 - f[1]*x5*x9 + f[2]*x2 + f[3]*x3);
				...
				) const
			{
				...
				T x2 = x0 + x1;
				e += dx*(C1*(x2 - 2) + C2*(-1.0/2.0*pow(x0, 2) - 1.0/2.0*pow(x1, 2) + (1.0/2.0)*pow(x2, 2) - pow(f[0]*f[1] + f[2]*f[3], 2) - 2) + p*(f[0]*f[3] - f[1]*f[2] - 1));
			}
				...
				T *UTOPIA_RESTRICT bf) const
			{
				...
				lf[0] += dx*(f[0]*x15 + f[0]*x22*x23 + f[1]*x16 + f[1]*x24*x25 + p*x13 - p*x17 - x18*x20 - x20*x21);
				lf[1] += dx*(f[2]*x15 + f[2]*x23*x29 + f[3]*x16 + f[3]*x25*x28 + p*x18 - p*x21 - x11*x26 - x11*x27);
				lf[2] += x3*x30;
				bf[0] += -x32*(grad_trial[0]*(-grad_test[0]*(C1 + x22) + grad_test[1]*x31) + grad_trial[1]*(grad_test[0]*x31 - grad_test[1]*(C1 + x24)));
				bf[1] += -dx*(grad_trial[0]*(grad_test[1]*x34 + x33) + grad_trial[1]*(-grad_test[0]*x36 + x35));
				bf[3] += -dx*(grad_trial[0]*(-grad_test[1]*x36 + x33) + grad_trial[1]*(grad_test[0]*x34 + x35));
				bf[4] += -x32*(grad_trial[0]*(-grad_test[0]*(C1 + x29) + grad_test[1]*x37) + grad_trial[1]*(grad_test[0]*x37 - grad_test[1]*(C1 + x28)));
				bf[8] += 0;

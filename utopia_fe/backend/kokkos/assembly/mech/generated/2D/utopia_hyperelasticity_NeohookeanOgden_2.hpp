	...
	namespace kernels {
		...
		public:
				...
				T *UTOPIA_RESTRICT bf)
			{
				...
				T x6 = log(x2);
				T x7 = lmbda*x6;
				T x8 = -lmbda*x5 - mu*x5 + x5*x7;
				T x9 = pow(f[3], 2)*x3;
				T x10 = lmbda*x9;
				T x11 = pow(f[2], 2)*x3;
				T x12 = lmbda*x11;
				T x13 = f[1]*x4;
				T x14 = grad_test[0]*(-lmbda*x13 - mu*x13 + x13*x7);
				T x15 = 1.0/x2;
				T x16 = mu*x15;
				T x17 = x1*x3;
				T x18 = x15*x7;
				T x19 = lmbda*x17 + mu*x17 + x16 - x17*x7 - x18;
				T x20 = f[0]*x3;
				T x21 = f[2]*x20;
				T x22 = grad_test[1]*(-lmbda*x21 - mu*x21 + x21*x7);
				T x23 = x0*x3;
				T x24 = lmbda*x23 + mu*x23 - x16 + x18 - x23*x7;
				T x25 = f[1]*x20;
				T x26 = -lmbda*x25 - mu*x25 + x25*x7;
				T x27 = pow(f[1], 2)*x3;
				T x28 = pow(f[0], 2)*x3;
				...
				bf[3] += dx*(grad_trial[0]*(grad_test[0]*(lmbda*x27 + mu*x27 + mu - x27*x7) + grad_test[1]*x26) + grad_trial[1]*(grad_test[0]*x26 + grad_test[1]*(lmbda*x28 + mu*x28 + mu - x28*x7)));
			}
				...
				T *UTOPIA_RESTRICT bf)
			{
				...
				T x25 = mu*x9;
				T x26 = x1*x15;
				T x27 = lmbda*x26 + mu*x26 - x11*x26 - x12 + x25;
				T x28 = f[2]*x15;
				T x29 = f[0]*lmbda;
				T x30 = f[0]*x11;
				T x31 = grad_test[1]*(-x28*x29 + x28*x30 - x28*x8);
				T x32 = x0*x15;
				T x33 = lmbda*x32 + mu*x32 - x11*x32 + x12 - x25;
				T x34 = f[1]*x15;
				T x35 = -x29*x34 + x30*x34 - x34*x8;
				T x36 = x15*x5;
				T x37 = x15*x4;
				e += dx*((1.0/2.0)*lmbda*pow(x3, 2) - mu*x3 + (1.0/2.0)*mu*(x4 + x5 + x6 + x7 - 2));
				...
				bf[3] += dx*(grad_trial[0]*(grad_test[0]*(lmbda*x36 + mu*x36 + mu - x11*x36) + grad_test[1]*x35) + grad_trial[1]*(grad_test[0]*x35 + grad_test[1]*(lmbda*x37 + mu*x37 + mu - x11*x37)));
			}

...
//	- Subexpressions: 2*ADD + 12*MUL + 5*SUB
T x0 = -x - y + 1;
T x1 = weight*(px[0]*py[1] - px[0]*py[2] - px[1]*py[0] + px[1]*py[2] + px[2]*py[0] - px[2]*py[1]);
T x2 = x0*x1;
T x3 = x*x2;
T x4 = x2*y;
T x5 = x*x1*y;
...
//	- Subexpressions: 4*ADD + 11*MUL + 5*SUB
T x0 = -x - y + 1;
T x1 = weight*(u[0]*x0 + u[1]*x + u[2]*y)*(px[0]*py[1] - px[0]*py[2] - px[1]*py[0] + px[1]*py[2] + px[2]*py[0] - px[2]*py[1]);
Hx[0] += x0*x1;
Hx[1] += x*x1;
Hx[2] += x1*y;
			}
...
g[2] += x1*y;
				...
				T *UTOPIA_RESTRICT H) const
			{
			    ...
			    // Automatically generated
				//FLOATING POINT OPS!
//	- Result: 12*ADDAUGMENTEDASSIGNMENT + 6*MUL + 3*POW
//	- Subexpressions: 4*ADD + 15*MUL + 5*SUB
T x0 = -x - y + 1;
T x1 = weight*(px[0]*py[1] - px[0]*py[2] - px[1]*py[0] + px[1]*py[2] + px[2]*py[0] - px[2]*py[1]);
T x2 = x0*x1;
T x3 = x*x2;
...
g[2] += x1*x7*y;
	...
	namespace kokkos {
		template<class FE>
		using MassTri3 = utopia::kokkos::AutoKernel<FE, utopia::kernels::Mass<utopia::kernels::Tri3<typename FE::Scalar, typename FE::Scalar>>, 2>;
	}
}

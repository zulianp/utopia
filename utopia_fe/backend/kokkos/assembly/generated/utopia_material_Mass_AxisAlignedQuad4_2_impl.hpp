...
//	- Subexpressions: ADD + 20*MUL + 2*POW + 5*SUB
T x0 = 1 - x;
T x1 = 1 - y;
T x2 = weight*(px[0]*py[0] - px[0]*py[2] - px[2]*py[0] + px[2]*py[2]);
T x3 = pow(x1, 2)*x2;
T x4 = x*x0*x3;
T x5 = x*y;
T x6 = x0*x1*x2*x5;
...
//	- Subexpressions: 4*ADD + 13*MUL + 5*SUB
T x0 = 1 - y;
T x1 = x0*(1 - x);
T x2 = x*y;
T x3 = 1 - x2;
T x4 = x*x0;
T x5 = weight*(px[0]*py[0] - px[0]*py[2] - px[2]*py[0] + px[2]*py[2])*(u[0]*x1 + u[1]*x4 + u[2]*x2 + u[3]*x3);
Hx[0] += x1*x5;
Hx[1] += x4*x5;
Hx[2] += x2*x5;
Hx[3] += x3*x5;
...
//	- Subexpressions: 4*ADD + 25*MUL + 2*POW + 5*SUB
T x0 = 1 - x;
T x1 = 1 - y;
T x2 = weight*(px[0]*py[0] - px[0]*py[2] - px[2]*py[0] + px[2]*py[2]);
T x3 = pow(x1, 2)*x2;
...
g[3] += x17*x9;
	...
	namespace kokkos {
		template<class FE>
		using MassAxisAlignedQuad4 = utopia::kokkos::AutoKernel<FE, utopia::kernels::Mass<utopia::kernels::AxisAlignedQuad4<typename FE::Scalar, typename FE::Scalar>>, 2>;
	}
}

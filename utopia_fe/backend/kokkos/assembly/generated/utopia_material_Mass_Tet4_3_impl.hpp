...
T x8 = -py[0] + py[1];
T x9 = -pz[0] + pz[2];
T x10 = weight*(x1*x2*x3 - x1*x5*x9 - x2*x6*x7 - x3*x4*x8 + x4*x5*x6 + x7*x8*x9);
T x11 = x0*x10;
T x12 = x*x11;
T x13 = x11*y;
T x14 = x11*z;
...
T x8 = -py[0] + py[1];
T x9 = -pz[0] + pz[2];
T x10 = weight*(u[0]*x0 + u[1]*x + u[2]*y + u[3]*z)*(x1*x2*x3 - x1*x5*x9 - x2*x6*x7 - x3*x4*x8 + x4*x5*x6 + x7*x8*x9);
Hx[0] += x0*x10;
Hx[1] += x*x10;
Hx[2] += x10*y;
Hx[3] += x10*z;
...
g[3] += x10*x20*z;
	...
	namespace kokkos {
		template<class FE>
		using MassTet4 = utopia::kokkos::AutoKernel<FE, utopia::kernels::Mass<utopia::kernels::Tet4<typename FE::Scalar, typename FE::Scalar>>, 3>;
	}
}

...
namespace utopia {
    ...
    inline double surface_area(LibMeshFunctionSpace &V, int side) {
        ...
        utopia::assemble(surface_integral(interpolate(x, u), side), area);
        ...
        void read(Input &is) override {
            is.get_all([this](Input &is) {
                ...
                if (clazz == "AdaptiveFunction") {
                    auto ff = std::make_shared<AdaptiveFunction<FunctionSpace, Vector>>(V_);
    ...
    template <class FunctionSpace, class Vector>
    class UIForcingFunction<ProductFunctionSpace<FunctionSpace>, Vector> final
        : public CompositeForcingFunction<Vector>,
          public Configurable {
    public:

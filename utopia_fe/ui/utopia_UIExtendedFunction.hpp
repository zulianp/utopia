...
namespace utopia {
    ...
    public:
        ...
        inline bool gradient(const Vector &x, Vector &gradient) const override { return fun_->gradient(x, gradient); }

        inline bool hessian(const Vector &x, Matrix &hessian) const override { return fun_->hessian(x, hessian); }

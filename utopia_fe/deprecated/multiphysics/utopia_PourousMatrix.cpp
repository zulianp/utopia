...
namespace utopia {
    ...
    inline static auto apply_gradient_diff(
        ...
        -> decltype(FEBackend<LIBMESH_TAG>::grad(right.expr().coefficient(), right.expr().fun(), ctx)) {
    ...
    using GradientDiffExpr = Binary<Interpolate<UVector, TrialFunction<ProductFunctionSpace<LibMeshFunctionSpace>>>,
    ...
    public:
        inline static auto apply(const GradientDiffExpr &expr, AssemblyContext<Backend> &ctx)
            -> decltype(apply_gradient_diff(expr.left(), expr.right(), expr.operation(), ctx)) {
    ...
    static void make_permutation(const int dim,
    ...
    static void make_permutation(const libMesh::MeshBase &mesh,
        ...
        a_from.init(mesh, dof_map_from, var_num_from);

        a_to.init(mesh, dof_map_to, var_num_to);
    ...
    void GradientRecovery<Matrix, Vector>::init(FunctionSpaceT &V) {
        ...
        if (grad_space_.empty()) {
            ...
            grad_space_ *=
                LibMeshFunctionSpace(aux, aux.add_variable("fun", libMesh::Order(V.order(0)), libMesh::LAGRANGE));

            for (int i = 0; i < dim; ++i) {
                grad_space_ *= LibMeshFunctionSpace(
                    aux, aux.add_variable("grad_" + std::to_string(i), libMesh::Order(V.order(0)), libMesh::LAGRANGE));
            }
    ...
    template <class Matrix, class Vector>
    bool GradientRecovery<Matrix, Vector>::refine(const Mortar<Matrix, Vector> &mortar,
                                                  FunctionSpaceT &V,
                                                  const Vector &sol) {
    ...
    template <class Matrix, class Vector>
    void GradientRecovery<Matrix, Vector>::estimate_error(const Mortar<Matrix, Vector> &mortar,
                                                          FunctionSpaceT &V,
                                                          const Vector &sol) {
        Chrono chrono;
        chrono.start();
        ...
        if (!mortar.empty()) {
            ...
            mortar_matrix =
                USparseMatrix(permutation * *mortar.mortar_matrix_without_slave_dofs() * transpose(permutation));

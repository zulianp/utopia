...
namespace utopia {
    ...
    std::shared_ptr<libMesh::DistributedMesh> square(libMesh::Parallel::Communicator &comm,
        ...
        libMesh::MeshTools::Generation::build_square(*mesh, nx, ny, x_left, x_right, y_bottom, y_top, elem_type);
    ...
    void NonLinearElasticityTest::run(Input &in) {
        ...
        if (dim == 3) {
            ...
            nl_solve(equations(eq),
                     constraints(boundary_conditions(ux == coeff(0.), {1, 3}),
            ...
            nl_solve(equations(eq),
                     constraints(boundary_conditions(ux == coeff(0.), {0, 2}),

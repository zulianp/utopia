...
namespace utopia {
    ...
    static void apply_displacement(moonolith::Communicator &comm,
    ...
    void FSITest::run(Input &in) {
        ...
        libMesh::MeshTools::Generation::build_square(*fluid_mesh, nx_fluid, ny_fluid, -1.5, 1.5, 0, 1., libMesh::QUAD8);
        ...
        libMesh::MeshTools::Generation::build_square(
            *solid_mesh, nx_solid, ny_solid, -0.05, 0.05, 0, .4, libMesh::QUAD4);
        ...
        auto b_form_f = integral(inner(u_f, v_f)) +
                        ...
                        dt * integral(inner(p_f, div(v_f))) + integral(inner(div(u_f), q_f));

        auto l_form_f =  // integral(inner(coeff(0.), q_f)) +
            integral(inner(uk_fold, v_f)) + integral(dt * inner(fk_f, v_f));
        ...
        // force response
        auto eq_solid = integral((2. * mu_s) * inner(e_u, e_v) + lambda_s * inner(div(u_s), div(v_s))) ==
                        integral(inner(coeff(z), v_s));

        // constraints fluid
        auto constr_f = constraints(boundary_conditions(u_fy == coeff(0.), {0, 1, 2, 3}),
        ...
        // constraints solid
        auto constr_s =
            constraints(boundary_conditions(u_sx == coeff(0.), {0}), boundary_conditions(u_sy == coeff(0.), {0}));

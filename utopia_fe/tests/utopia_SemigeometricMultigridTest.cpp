...
namespace utopia {
	...
	void init_mg_test_problem(libMesh::LibMeshInit &init, MGTestProblem &p)
	{
		...
		p.fine_space = make_shared<MGTestProblem::FESpaceT>( fe_space(LAGRANGE, order_elem, *p.fine_context) );
		p.coarse_space = make_shared<MGTestProblem::FESpaceT>( fe_space(LAGRANGE, order_elem, *p.coarse_context) );
		...
		u.set_quad_rule(std::make_shared<libMesh::QGauss>(dim, SECOND));
		...
		//compute reference solution using libmesh solver

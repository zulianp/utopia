...
namespace utopia {
    ...
    public:
        ...
        static void eval_equation(const Equality<Left, Right> &expr,
                                  Tensor<Matrix, 2> &mat,
                                  Tensor<Vector, 1> &vec,
        ...
        template <class Form, class Derived, int Order>
        static void eval(const Form &expr,
                         Tensor<Derived, Order> &tensor,
                         AssemblyContext<BAKEND_FLAG> &ctx,
                         const bool reset_tensor) {
            ctx.init_tensor(tensor.derived(), reset_tensor);
            FormEval<Form, BAKEND_FLAG>::apply(expr, tensor.derived(), ctx);
        }

        template <class Form, class Derived, int Order>
        static void eval(Form &expr,
                         Tensor<Derived, Order> &tensor,
                         AssemblyContext<BAKEND_FLAG> &ctx,
                         const bool reset_tensor) {
            ctx.init_tensor(tensor.derived(), reset_tensor);
        ...
        static void eval(const Expr &expr, Number<T> &value, AssemblyContext<BAKEND_FLAG> &ctx) {

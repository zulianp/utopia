...
namespace utopia {
    ...
    Binary<Number<typename FormTensor<T2, Order>::Scalar>, FormTensor<T2, Order>, Multiplies> operator
        *(const typename FormTensor<T2, Order>::Scalar &left, const FormTensor<T2, Order> &right) {
    ...
    public:
        ...
        inline std::string get_class() const override { return "Form" + Order2String<Order>::Value(); }

        FormTensor() {}

        FormTensor(FormTensor &&other) : values_(std::move(other.values_)) {}

        FormTensor(const FormTensor &other) : values_(other.values_) {}
        ...
        void assign(const FormTensor &other) override { values_ = other.values_; }

        void assign(FormTensor &&other) override { values_ = std::move(other.values_); }

        void resize(const SizeType n) { values_.resize(n); }
        ...
        inline bool is_alias(const FormTensor &other) const { return this == &other; }

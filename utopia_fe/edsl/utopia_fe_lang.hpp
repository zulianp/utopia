...
namespace utopia {
    ...
    inline MatrixFE<FEFun, 2, 2> tensor_product_fe_2x2(FEFun &fe_1, FEFun &fe_2, FEFun &fe_3, FEFun &fe_4) {
        return MatrixFE<FEFun, 2, 2>({make_ref(fe_1), make_ref(fe_2), make_ref(fe_3), make_ref(fe_4)});
    }

    template <class FEFun>
    inline MatrixFE<FEFun, 3, 3> tensor_product_fe_3x3(FEFun &fe_1,
                                                       FEFun &fe_2,
                                                       FEFun &fe_3,
                                                       FEFun &fe_4,
                                                       FEFun &fe_5,
                                                       ...
                                                       FEFun &fe_8,
                                                       FEFun &fe_9) {
        return MatrixFE<FEFun, 3, 3>({make_ref(fe_1),
                                      make_ref(fe_2),
                                      make_ref(fe_3),
                                      make_ref(fe_4),
                                      make_ref(fe_5),
    ...
    inline auto tensor_product_fe_2x2(FESpace<Traits> &fe_1,
        ...
        return MatrixFE<FEFun, 2, 2>(
            {make_shared<FEFun>(fe_1), make_shared<FEFun>(fe_2), make_shared<FEFun>(fe_3), make_shared<FEFun>(fe_4)});
    }

    template <class FESpace, int ROWS, int COLS>
    MatrixFE<typename FESpace::Function, ROWS, COLS> fe_function(
        const TensorProductFESpace<FESpace, ROWS, COLS> &space) {
    ...
    public:
        DirichletBoundaryCondition(const Expr &expr, std::initializer_list<int> boundary_tags) : expr_(expr) {
            boundary_tags_.insert(boundary_tags.begin(), boundary_tags.end());
        }

        inline Expr &expr() { return expr_; }

        inline const Expr &expr() const { return expr_; }

        const std::set<int> &boundary_tags() const { return boundary_tags_; }

        inline std::string get_class() const { return "DirichletBoundaryCondition<" + expr().get_class() + ">"; }
    ...
    DirichletBoundaryCondition<Equality<Left, Right> > boundary_conditions(const Equality<Left, Right> &expr,

...
namespace utopia {
    ...
    public:
        ...
        inline static auto apply(const Expr &expr, AssemblyContext<Backend> &ctx) -> Out { return expr.eval(ctx); }
    };

    template <class Out, class Fun, class AssemblyContext>
    class FunctionalTraits<ContextFunction<Out, Fun>, AssemblyContext> {
    public:
        inline static int type(const ContextFunction<Out, Fun> &expr, const AssemblyContext &ctx) { return 0; }

        inline static int order(const ContextFunction<Out, Fun> &expr, const AssemblyContext &ctx) { return 0; }
    };
}  // namespace utopia

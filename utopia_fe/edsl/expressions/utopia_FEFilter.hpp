...
namespace utopia {
    ...
    public:
        ...
        Filter(const Expr &expr, Fun fun) : expr_(expr), fun_(fun) {}
    ...
    Filter<Expr, Out, Order, std::function<Out(In &&)>> filter(const Expr &expr, std::function<Out(In &&)> fun) {
        return Filter<Expr, Out, Order, std::function<Out(In &&)>>(expr, fun);
    }

    template <class Expr, class In, class Out, int Order = Traits<Out>::Order>
    Filter<Expr, Out, Order, std::function<Out(const In &)>> filter(const Expr &expr,
                                                                    std::function<Out(const In &)> fun) {
        return Filter<Expr, Out, Order, std::function<Out(const In &)>>(expr, fun);
    }

    template <class Expr, class In, class Out, int Order = Traits<Out>::Order>
    Filter<Expr, Out, Order, Out (*)(const In &)> filter(const Expr &expr, Out (*fun)(const In &)) {

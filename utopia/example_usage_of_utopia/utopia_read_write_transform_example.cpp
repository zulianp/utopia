...
void run() {
...

    {
...

        {
            ...
            each_transform(
                a, b, [&c](const SizeType i, const double entry) -> double { return exp(-entry) * c.get(i); });
        }
...
// if it has compiled with blas or petsc WITH_BLAS or WITH_PETSC macros are available (if you want to make it compile no
// matter the utopia installation)
#ifdef WITH_PETSC

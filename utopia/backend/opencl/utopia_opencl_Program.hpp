...
namespace utopia {
    namespace opencl {
        ...
        public:
            static const Multiply<Left, Right> &transform(const Multiply<Left, Right> &expr) { return expr; }
        };
        ...
        public:
            ...
            static const Type &transform(const Multiply<Left, Right> &expr) {
                return Reduce<Binary<Left, Right, EMultiplies>, Plus>(
                    Binary<Left, Right, EMultiplies>(expr.left().expr(), expr.right()));
            }
        };
        ...
        public:
            ...
            static const _Expr &transform(const _Expr &expr) { return expr; }

            static _Expr &transform(_Expr &expr) { return expr; }
        };
        ...
        public:
            ...
            static Type transform(const Multiply<Left, Right> &expr) {
                return
                    // make_evaluate(
                    make_evaluate(mk_transform(expr.left())) * make_evaluate(mk_transform(expr.right()))
                    // )
                    ;
            }
        };
        ...
        public:
            typedef utopia::opencl::MultiKernelTransform<typename MultiplyTransform<Transposed<Left>, Right>::Type>
                Transform;
            typedef typename Transform::Type Type;

            static Type transform(const Multiply<Transposed<Left>, Right> &expr) {
                return Type(dot(mk_transform(expr.left().expr()), mk_transform(expr.right())));
            }
        };
        ...
        public:
            ...
            // transform it to the evaluate expression
            typedef utopia::Binary<TLeft, TRight, Operation> Type;
        ...
        public:
            ...
            // transform it to the evaluate expression
            typedef utopia::Construct<Left, TRight> Type;

            static Type transform(const Expr &expr) { return Type(expr.left(), mk_transform(expr.right())); }
        };
        ...
        public:
            ...
            static Type transform(const Unary<InnerExpr, Operation> &expr) { return Type(mk_transform(expr.expr())); }
        };
        ...
        public:
            ...
            static Type transform(const Reduce<InnerExpr, Operation> &expr) { return Type(mk_transform(expr.expr())); }
        };
        ...
        public:
            ...
            static Type transform(const Transposed<InnerExpr> &expr) { return Type(mk_transform(expr.expr())); }
        };
        ...
        public:
            ...
            public:
                ...
                // void post_order_visit(const Evaluate< Construct<Number<T>, Evaluate< Reduce<InnerExpr, Operation> > >
                // > &expr)
                // {
                // 	typedef utopia::Reduce<InnerExpr, Operation> ExprT;
                // 	auto k = static_cast< Kernel<ExprT> * >( kernels_[next_kernel_] );
            ...
            void initialize(const Expr &expr) {
                ...
                for (auto k_ptr : kernels_) {
                    if (!k_ptr->is_callable()) {
                        ...
                        write(k_ptr->get_name() + ".cl", k_ptr->get_code_string());
#endif  // UTOPIA_DEBUG_CL_KERNELS

                        sources.push_back({k_ptr->get_code_string().c_str(), k_ptr->get_code_string().size()});
                    }
                }

...
namespace utopia {
    namespace opencl {
        ...
        private:
            ...
            void pre_visit() { depth_++; }

            void post_visit() { depth_--; }
        };

        class KernelNameGenerator {
        public:
            constexpr static const char *base_name() { return "utopia_auto_generated_kernel_"; }
        ...
        private:
            KernelNameGenerator() : n_generated_kernels(0) {}
        ...
        public:
            ...
            public:
                ...
                RemapVariables(Env &env) : env_(&env) { env_->clear(); }
            ...
            void initialize(const Construct<Number<T>, Evaluate<InnerExpr> > &) {
            ...
            void initialize(const Expr &expr) {
                ...
                bool ok = read(utopia::Utopia::instance().get("opencl_templates_path") + get_template_path(expr),
                               code_template);
                assert(ok);
            ...
            inline std::pair<cl_ulong, cl_ulong> optimal_group_setup(const cl_ulong n) {
                ...
                kernel_.getWorkGroupInfo(CLContext::instance().current_device(),
                                         CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE,
                                         &preferred_work_group_size);
                // clGetKernelWorkGroupInfo(kernel_, CLContext::instance().current_device(),
                // CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE, sizeof(cl_ulong), &preferred_work_group_size, NULL);
            ...
            bool call(const Evaluate<Construct<Number<T>, Evaluate<Reduce<InnerExpr, Operation> > > > &eval) {
            ...
            bool call(const Evaluate<Reduce<InnerExpr, Operation> > &eval) {
                ...
                err = kernel_.setArg(0, n_entries_to_reduce);
                assert(check_cl_error(err));
                err = kernel_.setArg(1, result.buffer);
                assert(check_cl_error(err));
                ...
                err = queue.enqueueNDRangeKernel(
                    kernel_, cl::NullRange, cl::NDRange(n_entries_to_reduce), cl::NDRange(group_size));
                assert(check_cl_error(err));
                err = queue.finish();
                assert(check_cl_error(err));
            ...
            bool call(const Expression<Derived> &all_expr) {
                ...
                err = kernel_.setArg(0, n_entries);
                assert(check_cl_error(err));
                ...
                assert(check_cl_error(err));
                err = CLContext::instance().current_queue().finish();
                assert(check_cl_error(err));
            ...
            bool execute(const Expr &expr) {
                ...
                // //run the kernel
                // cl::KernelFunctor callback(cl::Kernel(program, get_name().c_str()), queue, cl::NullRange,
                // cl::NDRange(10), cl::NullRange); callback(buffer_A,buffer_B,buffer_C);
            ...
            const std::string &get_code_string() const override { return code_; }

            const std::string &get_name() const override { return name_; }

            bool is_callable() const override { return is_callable_; }
        ...
        public:
            ...
            public:
                ...
                Scalar get_value() const { return value_; }
            ...
            bool call(const Evaluate<Construct<Number<Scalar>, Evaluate<EvaluatedExpr> > > &expr) {
            ...
            const std::string &get_code_string() const { return null_; }

            const std::string &get_name() const { return null_; }

            bool is_callable() const { return false; }

            bool make_callable(cl::Program &) { return false; }

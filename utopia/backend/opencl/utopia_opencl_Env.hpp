...
namespace utopia {
    namespace opencl {
        ...
        class Options {
        public:
            Options() : _transposed(false), is_permutation_(false), is_const_(false) {}

            inline bool transposed() const { return _transposed; }

            inline bool is_permutation() const { return is_permutation_; }

            inline bool is_const() const { return is_const_; }
        ...
        private:
            ...
            inline void toggleTransposed() { _transposed = !_transposed; }

            inline void setTransposed(const bool transposed) { _transposed = transposed; }
        };
        ...
        public:
            ...
            inline long get_id() const { return id_; }

            inline void set_id(const long id) { id_ = id; }

            inline bool has_null_id() const { return id_ == -1; }

            virtual ~Var() {}

            Var() : arg_num_(-1), id_(-1), aliased_arg_num_(-1) {}

            inline bool is_alias() const { return get_aliased_arg_num() != -1; }

            inline int get_aliased_arg_num() const { return aliased_arg_num_; }

            inline void set_aliased_arg_num(const int aliased_arg_num) { aliased_arg_num_ = aliased_arg_num; }
        ...
        public:
            VarRef(const Expr &expr) : expr_(expr) {}

            const cl::Buffer &buffer() const { return get_buffer(expr_); }

            virtual bool set_as_arg_at(const int arg_num, cl::Kernel &Kernel) {
                const cl_int err = Kernel.setArg(arg_num, buffer());
                assert(check_cl_error(err));
                return CL_SUCCESS == err;
            }

            virtual void describe(std::ostream &os) const { os << expr_.get_class() << "\n"; }
        ...
        public:
            VarCopy(const Expr &expr) : expr_(expr) {}
        ...
        public:
            ...
            VarCopy(const Evaluate<Expr, 0> &expr) : expr_(expr) {}

            virtual bool set_as_arg_at(const int arg_num, cl::Kernel &Kernel) {
                const cl_int err = Kernel.setArg(arg_num, expr_.get_value());
                assert(check_cl_error(err));
                return CL_SUCCESS == err;
            }
        ...
        public:
            VarCopy(const utopia::Number<T> &expr) : value_(expr) {}

            virtual bool set_as_arg_at(const int arg_num, cl::Kernel &Kernel) {
                const cl_int err = Kernel.setArg(arg_num, value_);
                assert(check_cl_error(err));
                return CL_SUCCESS == err;
            }

            virtual void describe(std::ostream &os) const { os << value_ << "\n"; }
        ...
        public:
            ...
            std::vector<std::shared_ptr<Var> > &args() { return args_; }

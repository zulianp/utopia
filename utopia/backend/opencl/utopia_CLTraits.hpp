...
namespace utopia {
	...
	public:
		...
		inline cl::Device &current_device()
		{
			return devices_[current_device_];
		}

		inline cl::Platform &current_platform()
		{
			return platforms_[current_platform_];
		}

		inline const cl::Device &current_device() const
		{
			return devices_[current_device_];
		}

		inline const cl::Platform &current_platform() const
		{
			return platforms_[current_platform_];
		}
	...
	private:
		...
		CLContext()
		: current_platform_(0), current_device_(0)
		{
			init();	
		}
	...
	inline bool compile_opencl_programs(const cl::Program::Sources &sources, cl::Program &program, const std::string &flags)
	{
		program = cl::Program(CLContext::instance().current(), sources);
	...
	public:
		...
		bool resize(const SizeType rows, const SizeType cols)
		{
			...
			if(n_entries != entries.size()) {
				...
				cl_int err;
				buffer = cl::Buffer(CLContext::instance().current(), 
									CL_MEM_USE_HOST_PTR, sizeof(T) * n_entries,
									&entries[0],
									&err
									);
		...
		{
			cl_int err = queue.enqueueReadBuffer(buffer, CL_FALSE, 0, sizeof(T) * entries.size(), &entries[0], nullptr, &event);
			assert(check_cl_error(err));
			return CL_SUCCESS == err;
		}
		...
		{
			cl_int err = queue.enqueueWriteBuffer(buffer, CL_FALSE, 0, sizeof(T) * entries.size(), &entries[0], nullptr, &event);
			assert(check_cl_error(err));
			return CL_SUCCESS == err;
		}
	};
	...
	public:
		...
		CLVector(const SizeType n_entries)
		{
			resize(n_entries);
		}
	...
	private:
		...
		inline CLStats()
		: n_kernel_calls(0), n_code_gen_calls(0), code_generation_time(0), kernel_execution_time(0)
		{}
	};

...
namespace utopia {
    ...
    public:
        ...
        inline const std::string &ksp(const SizeType i) { return ksp_.at(i); }

        inline const std::string &pc(const SizeType i) { return pc_.at(i); }

        inline const std::string &package(const SizeType i) { return package_.at(i); }

        inline bool is_ksp_valid(const std::string &type) const { return in_array(type, ksp_); }

        inline bool is_pc_valid(const std::string &type) const { return in_array(type, pc_); }

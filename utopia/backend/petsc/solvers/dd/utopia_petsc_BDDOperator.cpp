...
namespace utopia {
    ...
    bool BDDOperator<Matrix, Vector>::initialize(const std::shared_ptr<const Vector> &rhs) {
        ...
        auto G_layout = layout(comm, impl_->n_interface, Traits::determine());
    ...
    bool BDDOperator<Matrix, Vector>::initialize(const std::shared_ptr<const Matrix> &matrix) {
        ...
        const IndexSet &local_interface_idx = impl_->local_interface_idx;
        const IndexSet &global_to_local = impl_->global_to_local;
        const SizeType interface_offset = impl_->interface_offset;
        const SizeType n_interface = impl_->n_interface;

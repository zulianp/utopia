...
namespace utopia {
    ...
    SlepcSolver<Matrix, Vector, PETSC_EXPERIMENTAL>::~SlepcSolver() {
        if (initialized_) EPSDestroy(&eps_);
    }
    ...
    bool SlepcSolver<Matrix, Vector, PETSC_EXPERIMENTAL>::print_eigenpairs() {
        ...
        if (!solved_) {
            std::cerr
                << "[Warning] You need to solve (or let the solver converge) before querying the eigenpairs.... \n";
            return false;
        }
    ...
    template <typename Matrix, typename Vector>
    void SlepcSolver<Matrix, Vector, PETSC_EXPERIMENTAL>::get_eigenpairs(const SizeType& i,
                                                                         Scalar& iegr,
                                                                         Scalar& eigi,
                                                                         Vector& vr,
                                                                         Vector& vi) {
    ...
    template <typename Matrix, typename Vector>
    void SlepcSolver<Matrix, Vector, PETSC_EXPERIMENTAL>::get_real_eigenpair(const SizeType& i,
                                                                             Scalar& iegr,
                                                                             Vector& vr) {
        ...
        if (!empty(vr)) VecDestroy(&raw_type(vr));

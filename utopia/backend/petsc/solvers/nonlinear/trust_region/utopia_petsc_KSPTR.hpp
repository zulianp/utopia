...
namespace utopia {
     ...
     *				1. gltr, stcg, nash, qcg: \n
                                             Uses preconditioned conjugate gradient to compute an approximate minimizer
     of the quadratic function.  \n \f$ q(s) = g^T * s + .5 * s^T * H * s   \f$  \n subject to  \f$ || s || <= delta \f$
     \n
                    2. cgne				: \n
                                            Applies the preconditioned conjugate gradient method to the normal equations
     without explicitly forming A^t*A. \n Therefore, it's very suitable to use in combination with TR_NormalEquation,
     LeastSquaresFunction and solution method: dogleg \n Note, that CGNE is a general-purpose non-symmetric method. \n
     */
    ...
    public:
        KSP_TR(const bool redundant_flg = false)
            : TRSubproblem(),
        ...
        KSP_TR(const std::string &t, const std::string &pc_t = "jacobi", const bool redundant_flg = false)
            : TRSubproblem(), redundant_solve_flg_(redundant_flg) {
            this->pc_type(pc_t);
            this->ksp_type(t);
        }
        ...
        void number_of_parallel_solves(const SizeType &number) {
            if (redundant_solve_flg_) {
                ...
                utopia_error(
                    "KSP_TR::number_of_parallel_solves:: If you want to specify number of solves, use redundant solver "
                    "first.");
            }
        }

        bool redundant() const { return redundant_solve_flg_; }
...
#endif
                ierr = KSPSetTolerances(ksp_.implementation(),
                                        TRSubproblem::rtol(),
                                        TRSubproblem::atol(),
                                        PETSC_DEFAULT,
                                        TRSubproblem::max_it());

...
namespace utopia {
    ...
    private:
        ...
        TRStrategyFactory() { init(); }

        void init() {
            strategies_[Solver::cauchypoint()] =
                std::make_shared<TRFactoryMethod<utopia::CauchyPoint<Matrix, Vector>>>();
            strategies_[Solver::dogleg()] = std::make_shared<TRFactoryMethod<utopia::Dogleg<Matrix, Vector>>>();
            strategies_[Solver::steihaug_toint()] =
                std::make_shared<TRFactoryMethod<utopia::SteihaugToint<Matrix, Vector>>>();

            strategies_[Solver::automatic()] =
                std::make_shared<TRFactoryMethod<utopia::SteihaugToint<Matrix, Vector>>>();

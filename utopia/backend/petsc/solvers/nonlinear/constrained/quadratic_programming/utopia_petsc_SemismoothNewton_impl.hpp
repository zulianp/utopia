...
namespace utopia {
    ...
    template <class Matrix, class Vector>
    SemismoothNewton<Matrix, Vector, PETSC_EXPERIMENTAL> *SemismoothNewton<Matrix, Vector, PETSC_EXPERIMENTAL>::clone()
        const {
    ...
    bool SemismoothNewton<Matrix, Vector, PETSC_EXPERIMENTAL>::apply(const Vector &rhs, Vector &sol) {
        ...
        QuadraticFunction<Matrix, Vector> fun(std::make_shared<Matrix>(*this->get_operator()),

...
class Multigrid<Matrix, Vector, PETSC_EXPERIMENTAL>
    : public LinearMultiLevel<Matrix, Vector> {
 ...
 public:
  void update(const std::shared_ptr<const Matrix> &op) override {
    ...
    for (std::size_t i = 0; i < n_smoothers; i++) {
      ...
      if (block_size_ > 1) {
        const_cast<Matrix &>(this->level(i).A())
            .convert_to_mat_baij(block_size_);
      }

      KSPSetOperators(smoother, raw_type(this->level(i).A()),
                      raw_type(this->level(i).A()));
    }
  }
  ...
  Multigrid(std::shared_ptr<Smoother> smoother = nullptr,
            std::shared_ptr<Solver> linear_solver = nullptr)
  ...
  void update_transfer(const SizeType level,
                       const std::shared_ptr<Transfer> &t) override {
  ...
  void print_usage(std::ostream &os) const override {
    ...
    this->print_param_usage(os, "block_size", "int", "Block size for systems.",
                            "1");
    this->print_param_usage(os, "smoother", "Smoother",
                            "Input parameters for all smoothers.", "-");
 ...
 private:
  ...
  void aux_update_transfer(const SizeType level) {
    auto mat_transfer = dynamic_cast<IPRTransfer *>(&this->transfer(level));
  ...
  bool set_solver(Solver &solver, KSP &ksp) {
    ...
    if (casted) {
      ...
      if (casted->ksp_type() == KSPGMRES || casted->ksp_type() == KSPFGMRES) {
        ...
        if (casted->ksp_type() != default_ksp_type_) {
          std::cerr << "[Warning] " << casted->ksp_type()
                    << " not supported by petsc PCMG using fallback option "
                       "KSPRICHARDSON"
                    << std::endl;
        }
      ...
      auto *factor =
          dynamic_cast<Factorization<Matrix, Vector, PETSC> *>(&solver);
  ...
  void init_ksp(MPI_Comm comm) {
    ...
    for (std::size_t i = 0; i < n_smoothers; i++) {
      ...
      auto mat_transfer = dynamic_cast<IPRTransfer *>(&this->transfer(i));
    ...
    m_utopia_warning_once(
        "PCMGSetNumberSmooth{Up,Down} not available in petsc 3.9.0 find "
        "equivalent");
#endif

    KSPSetTolerances(*ksp_, this->rtol(), this->atol(), PETSC_DEFAULT,
                     this->max_it());

...
#else
// #if UTOPIA_PETSC_VERSION_GREATER_EQUAL_THAN(3,11,0) || (UTOPIA_PETSC_VERSION_GREATER_EQUAL_THAN(3,10,0) &&
// PETSC_VERSION_RELEASE == 0)
#if (UTOPIA_PETSC_VERSION_GREATER_EQUAL_THAN(3, 10, 0) && PETSC_VERSION_RELEASE == 0)
    // for some period PETSc had in its master branch a different API, however it was never released (hence
    // PETSC_VERSION_RELEASE==0)

...
namespace utopia {
    ...
    public:
        ...
        virtual ~Wrapper() { destroy(); }

        inline VecScatter &raw_type() { return scatter; }
    ...
    void PetscVecScatter::create(const PetscVector &from,
                                 const PetscIS &from_is,
                                 const PetscVector &to,
                                 const PetscIS &to_is) {
        if (!wrapper_) {
    ...
    void Redundant<PetscMatrix, PetscVector>::create_sub_matrix(const PetscMatrix &mat, PetscMatrix &mat_sub) {
        if (empty()) {
            ...
            MatCreateRedundantMatrix(mat.raw_type(), psubcomm->n, subcomm, MAT_INITIAL_MATRIX, &mat_sub.raw_type());
        }
    }
    ...
    void Redundant<PetscMatrix, PetscVector>::super_to_sub(const PetscMatrix &mat, PetscMatrix &mat_sub) {
        if (empty()) {
            ...
            MatCreateRedundantMatrix(
                mat.raw_type(), psubcomm->n, PetscSubcommChild(psubcomm), MAT_REUSE_MATRIX, &mat_sub.raw_type());
        }
    }
}  // namespace utopia

...
//
    ...
    void PetscBackend::build_local_redistribute(PetscVector &result, const PetscVector &x_from, const PetscVector &shape_vec)
    {
        ...
        else
        {
            ...
            UtopiaVecScatterCreate(x_from.implementation(), is, x_to.implementation(), is, &newctx);
    ...
    void PetscBackend::build_from_structure(PetscVector &lhs, const PetscVector &rhs)
    {
        lhs.zeros(rhs.communicator(), lhs.type_override(), rhs.local_size(), rhs.size());
    }

    // void PetscBackend::build_from_structure(PetscMatrix &lhs, const PetscMatrix &rhs)
    // {
    // 	//TODO

...
//   PetscBool                                                                                                     \
//       free_ij;      /* free the column indices j and row offsets i when the matrix is destroyed */              \
...
//   Mat parent                /* set if this matrix was formed with                                               \
//                               MatDuplicate(...,MAT_SHARE_NONZERO_PATTERN,....);                                 \
//                               means that this shares some data structures with the parent including diag, ilen, \
//                               imax, i, j */
...
// typedef struct
// {             /* for MatTransposeMatMult_SeqAIJ_SeqDense() */
...
//   MatScalar * work; /* work array to store columns of A*R^T used in
...
// typedef struct
// {
//   MatScalar *bdiag, *ibdiag,
//       *ssor_work;        /* diagonal blocks of matrix used for MatSOR_SeqAIJ_Inode() */
...
// PETSC_INTERN PetscErrorCode MatLUFactorNumeric_SeqAIJ_Inode_inplace(Mat,
//                                                                     Mat,
//                                                                     const MatFactorInfo *);
...
//   PetscScalar *idiag, *mdiag, *ssor_work; /* inverse of diagonal entries, diagonal values and
//                                              workspace for Eisenstat trick */
...
// MatSeqXAIJFreeAIJ(Mat AA, MatScalar ** a, PetscInt ** j, PetscInt ** i)
// {
//   PetscErrorCode ierr;
//   Mat_SeqAIJ * A = (Mat_SeqAIJ *)AA->data;
//   if (A->singlemalloc)
//   {
//     ierr = PetscFree3(*a, *j, *i);
//     CHKERRQ(ierr);
//   }
//   else
//   {
...
//     This is a macro because it takes the datatype as an argument which can be either a Mat or a
//    MatScalar
// */
// #define MatSeqXAIJReallocateAIJ(                                                                   \
//     Amat, AM, BS2, NROW, ROW, COL, RMAX, AA, AI, AJ, RP, AP, AIMAX, NONEW, datatype)               \
//   if (NROW >= RMAX)                                                                                \
//   {                                                                                                \
...
//                "New nonzero at (%D,%D) caused a malloc\nUse MatSetOption(A, "                      \
//                "MAT_NEW_NONZERO_ALLOCATION_ERR, PETSC_FALSE) to turn off this check",              \
//                ROW,                                                                                \
//                COL);                                                                               \
//     /* malloc new storage space */                                                                 \
//     ierr = PetscMalloc3(BS2 * new_nz, &new_a, new_nz, &new_j, AM + 1, &new_i);                     \
//     CHKERRQ(ierr);                                                                                 \
//                                                                                                    \
...
//       new_i[ii] = AI[ii];                                                                          \
//     }                                                                                              \
//     for (ii = ROW + 1; ii < AM + 1; ii++)                                                          \
//     {                                                                                              \
//       new_i[ii] = AI[ii] + CHUNKSIZE;                                                              \
//     }                                                                                              \
//     ierr = PetscMemcpy(new_j, AJ, (AI[ROW] + NROW) * sizeof(PetscInt));                            \
//     CHKERRQ(ierr);                                                                                 \
//     len = (new_nz - CHUNKSIZE - AI[ROW] - NROW);                                                   \
//     ierr = PetscMemcpy(                                                                            \
//         new_j + AI[ROW] + NROW + CHUNKSIZE, AJ + AI[ROW] + NROW, len * sizeof(PetscInt));          \
//     CHKERRQ(ierr);                                                                                 \
//     ierr = PetscMemcpy(new_a, AA, BS2 * (AI[ROW] + NROW) * sizeof(datatype));                      \
//     CHKERRQ(ierr);                                                                                 \
//     ierr = PetscMemzero(new_a + BS2 * (AI[ROW] + NROW), BS2 * CHUNKSIZE * sizeof(datatype));       \
//     CHKERRQ(ierr);                                                                                 \
//     ierr = PetscMemcpy(new_a + BS2 * (AI[ROW] + NROW + CHUNKSIZE),                                 \
//                        AA + BS2 * (AI[ROW] + NROW),                                                \
//                        BS2 * len * sizeof(datatype));                                              \
//     CHKERRQ(ierr);                                                                                 \
//     /* free up old matrix storage */                                                               \
//     ierr = MatSeqXAIJFreeAIJ(A, &Ain->a, &Ain->j, &Ain->i);                                        \
//     CHKERRQ(ierr);                                                                                 \
//     AA = new_a;                                                                                    \
...
// MatILUFactorSymbolic_SeqAIJ_inplace(Mat, Mat, IS, IS, const MatFactorInfo *);
// PETSC_INTERN PetscErrorCode MatILUFactorSymbolic_SeqAIJ(Mat, Mat, IS, IS, const MatFactorInfo *);
// PETSC_INTERN PetscErrorCode
// MatILUFactorSymbolic_SeqAIJ_ilu0(Mat, Mat, IS, IS, const MatFactorInfo *);

// PETSC_INTERN PetscErrorCode MatICCFactorSymbolic_SeqAIJ_inplace(Mat,
//                                                                 Mat,
//                                                                 IS,
//                                                                 const MatFactorInfo *);
...
// PETSC_INTERN PetscErrorCode MatCholeskyFactorNumeric_SeqAIJ_inplace(Mat,
//                                                                     Mat,
//                                                                     const MatFactorInfo *);
...
// PETSC_INTERN PetscErrorCode
//     MatSOR_SeqAIJ(Mat, Vec, PetscReal, MatSORType, PetscReal, PetscInt, PetscInt, Vec);
...
// PETSC_INTERN PetscErrorCode
// MatGetSymbolicTransposeReduced_SeqAIJ(Mat, PetscInt, PetscInt, PetscInt * [], PetscInt * []);
...
// PETSC_INTERN PetscErrorCode MatToSymmetricIJ_SeqAIJ(
//     PetscInt, PetscInt *, PetscInt *, PetscInt, PetscInt, PetscInt **, PetscInt **);
// PETSC_INTERN PetscErrorCode
// MatLUFactorSymbolic_SeqAIJ_inplace(Mat, Mat, IS, IS, const MatFactorInfo *);
...
// PETSC_INTERN PetscErrorCode MatLUFactorNumeric_SeqAIJ_InplaceWithPerm(Mat,
//                                                                       Mat,
//                                                                       const MatFactorInfo *);
...
// PETSC_INTERN PetscErrorCode MatMatMultSymbolic_SeqAIJ_SeqAIJ_Scalable_fast(Mat,
//                                                                            Mat,
//                                                                            PetscReal,
//                                                                            Mat *);
...
// PETSC_INTERN PetscErrorCode MatRARtSymbolic_SeqAIJ_SeqAIJ_matmattransposemult(Mat,
//                                                                               Mat,
//                                                                               PetscReal,
//                                                                               Mat *);
...
// PETSC_INTERN PetscErrorCode
// MatTransposeMatMult_SeqAIJ_SeqDense(Mat, Mat, MatReuse, PetscReal, Mat *);
...
// PETSC_INTERN
//     PetscErrorCode MatTransposeColoringCreate_SeqAIJ(Mat, ISColoring, MatTransposeColoring);
...
// PETSC_INTERN PetscErrorCode
// MatMatMatMult_SeqAIJ_SeqAIJ_SeqAIJ(Mat, Mat, Mat, MatReuse, PetscReal, Mat *);
// PETSC_INTERN PetscErrorCode
// MatMatMatMultSymbolic_SeqAIJ_SeqAIJ_SeqAIJ(Mat, Mat, Mat, PetscReal, Mat *);
// PETSC_INTERN PetscErrorCode MatMatMatMultNumeric_SeqAIJ_SeqAIJ_SeqAIJ(Mat, Mat, Mat, Mat);

// PETSC_INTERN PetscErrorCode MatSetValues_SeqAIJ(
//     Mat, PetscInt, const PetscInt[], PetscInt, const PetscInt[], const PetscScalar[], InsertMode);
// PETSC_INTERN PetscErrorCode
// MatGetRow_SeqAIJ(Mat, PetscInt, PetscInt *, PetscInt **, PetscScalar **);
// PETSC_INTERN PetscErrorCode
// MatRestoreRow_SeqAIJ(Mat, PetscInt, PetscInt *, PetscInt **, PetscScalar **);
// PETSC_INTERN PetscErrorCode MatAXPY_SeqAIJ(Mat, PetscScalar, Mat, MatStructure);
// PETSC_INTERN PetscErrorCode MatGetRowIJ_SeqAIJ(Mat,
//                                                PetscInt,
//                                                PetscBool,
//                                                PetscBool,
//                                                PetscInt *,
...
//                                                const PetscInt * [],
//                                                PetscBool *);
// PETSC_INTERN PetscErrorCode MatRestoreRowIJ_SeqAIJ(Mat,
//                                                    PetscInt,
//                                                    PetscBool,
//                                                    PetscBool,
//                                                    PetscInt *,
...
//                                                    const PetscInt * [],
//                                                    PetscBool *);
// PETSC_INTERN PetscErrorCode MatGetColumnIJ_SeqAIJ(Mat,
//                                                   PetscInt,
//                                                   PetscBool,
//                                                   PetscBool,
//                                                   PetscInt *,
...
//                                                   const PetscInt * [],
//                                                   PetscBool *);
// PETSC_INTERN PetscErrorCode MatRestoreColumnIJ_SeqAIJ(Mat,
//                                                       PetscInt,
//                                                       PetscBool,
//                                                       PetscBool,
//                                                       PetscInt *,
...
//                                                       const PetscInt * [],
//                                                       PetscBool *);
// PETSC_INTERN PetscErrorCode MatGetColumnIJ_SeqAIJ_Color(Mat,
//                                                         PetscInt,
//                                                         PetscBool,
//                                                         PetscBool,
//                                                         PetscInt *,
...
//                                                         PetscInt * [],
//                                                         PetscBool *);
// PETSC_INTERN PetscErrorCode MatRestoreColumnIJ_SeqAIJ_Color(Mat,
//                                                             PetscInt,
//                                                             PetscBool,
//                                                             PetscBool,
//                                                             PetscInt *,
...
// PETSC_INTERN PetscErrorCode MatAXPYGetPreallocation_SeqX_private(
//     PetscInt, const PetscInt *, const PetscInt *, const PetscInt *, const PetscInt *, PetscInt *);
// PETSC_INTERN PetscErrorCode
// MatCreateMPIMatConcatenateSeqMat_SeqAIJ(MPI_Comm, Mat, PetscInt, MatReuse, Mat *);
// PETSC_INTERN PetscErrorCode
// MatCreateMPIMatConcatenateSeqMat_MPIAIJ(MPI_Comm, Mat, PetscInt, MatReuse, Mat *);

// PETSC_INTERN PetscErrorCode MatSetSeqMat_SeqAIJ(Mat, IS, IS, MatStructure, Mat);

// /*
//     PetscSparseDenseMinusDot - The inner kernel of triangular solves and Gauss-Siedel smoothing.
// \sum_i xv[i] * r[xi[i]] for CSR storage
...
// /*
//     PetscSparseDensePlusDot - The inner kernel of matrix-vector product \sum_i xv[i] * r[xi[i]] for
// CSR storage
...
// /*
//     PetscSparseDenseMaxDot - The inner kernel of a modified matrix-vector product \max_i xv[i] *
// r[xi[i]] for CSR storage
...
//     PetscInt __i;                                                                                  \
//     for (__i = 0; __i < nnz; __i++)                                                                \
//       max = PetscMax(PetscRealPart(max), PetscRealPart(xv[__i] * r[xi[__i]]));                     \
//   }

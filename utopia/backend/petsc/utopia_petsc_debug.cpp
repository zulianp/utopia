...
namespace utopia {
    ...
    PetscDebugger::PetscDebugger() : impl_(utopia::make_unique<Impl>()) {}
    ...
    void PetscDebugger::memory_check_begin() {
        ...
        PetscErrorCode ierr = PetscMallocGetCurrentUsage(&space);
        assert(ierr == 0);
        (void)ierr;
    ...
    void PetscDebugger::memory_check_end() {
        ...
        if (space > previous_space) {
            std::cerr << "[Memory check] expected " << (previous_space / 1024) << "KB have " << (space / 1024)
                      << "KB diff " << ((space - previous_space) / 1024) << "KB" << std::endl;
    ...
    void PetscDebugger::print_current_usage(std::ostream &os) const {
        PetscErrorCode ierr;
        PetscLogDouble space;
        ierr = PetscMemoryGetCurrentUsage(&space);
        assert(ierr == 0);
        (void)ierr;

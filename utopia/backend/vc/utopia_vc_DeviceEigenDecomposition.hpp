#ifndef UTOPIA_VC_DEVICE_EIGEN_DECOMPOSITION_NEW_HPP
#define UTOPIA_VC_DEVICE_EIGEN_DECOMPOSITION_NEW_HPP
...
namespace utopia {
    ...
    // FIXME Move me to different files
    ...
    template <typename T>
    class DeviceIdentity<Vc::Vector<T>> : public DeviceExpression<DeviceIdentity<Vc::Vector<T>>> {
    public:
        using Scalar = Vc::Vector<T>;

        template <typename SizeType>
        UTOPIA_INLINE_FUNCTION Vc::Vector<T> operator()(const SizeType &i, const SizeType &j) const {
            return static_cast<Vc::Vector<T>>((i == j) ? Scalar(1) : Scalar(0));
        }
    ...
    class DeviceOp<Vc::Vector<Scalar>, Max> {
    public:
        inline static constexpr Vc::Vector<Scalar> apply(const Vc::Vector<Scalar> &a, const Vc::Vector<Scalar> &b) {
            return std::max(a, b);
        }
    };

    template <class Scalar>
    class DeviceOp<Vc::Vector<Scalar>, Abs> {
    public:
        UTOPIA_INLINE_FUNCTION static constexpr Vc::Vector<Scalar> apply(const Vc::Vector<Scalar> &a) {
            return std::abs(a);
        }
    };

    template <class Expr>
    class DeviceEigenDecompositionVc : public DeviceExpression<DeviceEigenDecompositionVc<Expr>> {
    public:
    ...
    private:
        ...
        UTOPIA_INLINE_FUNCTION static void apply_2(const Expr &mat,
            ...
            auto cond_temp =
                (eigen_values(1) - eigen_values(0)) <= device::abs(eigen_values(1)) * device::epsilon<Scalar>();

            Scalar cond = 1.0, not_cond = 1.0;
            cond.setZeroInverted(cond_temp);
            not_cond.setZero(cond_temp);
            ...
            Scalar a2_greater_than_c2 = 1.0;
            a2_greater_than_c2.setZeroInverted(cond_temp);

            Scalar a2_not_greater_than_c2 = 1.0;
            a2_not_greater_than_c2.setZero(cond_temp);

            const Scalar scale1 = a2_greater_than_c2 / device::sqrt(a2 + b2);
            const Scalar scale2 = a2_not_greater_than_c2 / device::sqrt(c2 + b2);

            eigen_vectors(0, 1) = scale1 * (-scaled_mat(1, 0)) + scale2 * (-scaled_mat(1, 1));
            eigen_vectors(1, 1) = scale1 * (scaled_mat(0, 0)) + scale2 * (-scaled_mat(1, 0));

            //((-y, x))
            eigen_vectors(0, 0) = -eigen_vectors(1, 1);
            eigen_vectors(1, 0) = eigen_vectors(0, 1);

            eigen_vectors(0, 0) = cond + not_cond * eigen_vectors(0, 0);
            eigen_vectors(0, 1) = not_cond * eigen_vectors(0, 1);
            eigen_vectors(1, 0) = not_cond * eigen_vectors(1, 0);
            eigen_vectors(1, 1) = cond + not_cond * eigen_vectors(1, 1);
...
#endif  // UTOPIA_VC_DEVICE_EIGEN_DECOMPOSITION_NEW_HPP

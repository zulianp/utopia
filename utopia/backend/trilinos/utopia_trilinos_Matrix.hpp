...
namespace utopia {
    ...
    public:
        ...
        TpetraMatrix (Tpetra::global_size_t numGlobalElements, GlobalOrdinal indexBase, const rcp_comm_type & comm, Tpetra::LocalGlobal lOrG, const Teuchos::RCP<Node> &node)
                : _comm(comm), _owner(true) {
        rcp_map_type _map (new Tpetra::Map<LocalOrdinal,GlobalOrdinal,Node> (numGlobalElements, indexBase, &comm, lOrG, &node));
        rcp_crs_matrix_type _mat (new crs_matrix_type (_map, 0)); 
        }

        template <class LocalOrdinal, class GlobalOrdinal, class Node>
        TpetraMatrix (Tpetra::global_size_t numGlobalElements, size_t numLocalElements, GlobalOrdinal indexBase, const rcp_comm_type &comm, const Teuchos::RCP<Node> &node)
                : _comm(comm), _owner(true) {
        rcp_map_type _map (new Tpetra::Map<LocalOrdinal,GlobalOrdinal,Node> (numGlobalElements, numLocalElements, indexBase, &comm, &node));
        rcp_crs_matrix_type _mat (new crs_matrix_type (_map, 0)); 
        }

        template <class LocalOrdinal, class GlobalOrdinal, class Node>
        TpetraMatrix (Tpetra::global_size_t numGlobalElements, const Teuchos::ArrayView<const GlobalOrdinal> &entryList, GlobalOrdinal indexBase, const rcp_comm_type &comm, const Teuchos::RCP<Node> &node)
                : _comm(comm), _owner(true) {
        rcp_map_type _map (new Tpetra::Map<LocalOrdinal,GlobalOrdinal,Node> (numGlobalElements, &entryList, indexBase, &comm, &node));
        rcp_crs_matrix_type _mat (new crs_matrix_type (_map, 0)); 
        }

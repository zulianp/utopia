...
// void ssytrd_(char *uplo, int * n, float *A, int * lda, float * D, float * E, float * TAU, float * work, int * lwork,
// int * info); void sorgtr_(char *uplo, int * n, float *A, int * lda, float * tau, float * work, int *lwork, int
// *info);
}

namespace utopia {

    bool LapackEigenSolver::spd_geig(const BlasMatrixd &A,
                                     const BlasMatrixd &B,
                                     BlasVectord &evalues,
                                     BlasMatrixd &evectors) const {
    ...
    bool LapackEigenSolver::spd_geig_small(const BlasMatrixd &A,
                                           const BlasMatrixd &B,
    ...
    static bool aux_tridiagonalize(int n,
        ...
        // guess work size
        dsytrd_(&uplo, &n, &reflectors[0], &lda, &diagonal[0], &off_diagonal[0], &tau[0], &work[0], &lwork, &info);
        ...
        lwork = -1;
        dorgtr_(&uplo, &n, &reflectors[0], &lda, &tau[0], &work[0], &lwork, &info);
    ...
    static bool aux_eig(int n,
        ...
        dpteqr_(&compz, &n, &eigen_values[0], &off_diag[0], &eigen_vectors[0], &ldz, &work[0], &info);

...
namespace utopia {
    ...
    public:
        ...
        void assign(const BlasVector &other) override { copy(other); }

        void assign(BlasVector &&other) override { entries_ = std::move(other.entries_); }

        ////////////////////////////////////////////////////////////////////

        BlasVector() = default;

        explicit BlasVector(const Layout &layout, const Scalar &val = 0.0) { values(layout, val); }

        BlasVector(std::initializer_list<T> args) : entries_(args) {
            UTOPIA_REPORT_ALLOC("BlasVector::BlasVector(std::initializer_list<T> args)");
        }

        BlasVector(const BlasVector &other) : entries_(other.entries_) {
            UTOPIA_REPORT_ALLOC("BlasVector::BlasVector(const BlasVector &other)");
        }

        BlasVector(BlasVector &&other) : entries_(std::move(other.entries_)) {}
        ...
        inline T *ptr() { return &entries_[0]; }

        inline const T *ptr() const { return &entries_[0]; }
        ...
        ///< T>SWAP - swap x and y
        inline void swap(BlasVector &x) override { std::swap(entries_, x.entries_); }

        ///< T>SCAL - x = a*x
        inline void scale(const T &a) override { BLASAlgorithms<T>::scal(size(), a, ptr(), 1); }
        ...
        inline T norm2() const override { return BLASAlgorithms<T>::nrm2(size(), ptr(), 1); }

        ///< T>ASUM - sum of absolute values
        inline T norm1() const override { return BLASAlgorithms<T>::asum(size(), ptr(), 1); }
        ///////////////////////////////////////////////////////////////////////////
        ...
        template <class Op>
        inline void transform_values(const Op &op) {
            for (auto &e : entries_) {
                e = op(e);
            }
        }
        ...
        inline std::string get_class() const override { return "BlasVector"; }
        ...
        inline bool is_alias(const BlasVector &other) const { return this == &other; }

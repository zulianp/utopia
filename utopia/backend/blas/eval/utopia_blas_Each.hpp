...
namespace utopia {
    ...
    public:
        ...
        inline static void apply_read(const BlasVector &v, Fun fun) {
            For<>::apply(0, v.size(), [&v, &fun](const std::size_t i) { fun(i, v.get(i)); });
        }

        template <class Fun>
        inline static void apply_write(BlasVector &v, Fun fun) {
            For<>::apply(0, v.size(), [&v, &fun](const std::size_t i) { v.set(i, fun(i)); });
        }
        ...
        inline static void apply_transform(const BlasVector &in, BlasVector &out, Fun fun) {
            ...
            For<>::apply(0, s, [&in, &out, &fun](const std::size_t i) { out.set(i, fun(i, in.get(i))); });
        }
    };
}  // namespace utopia

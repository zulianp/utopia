...
* In fact, the EDSL exclusively generates a statically typed expression tree. This expression tree can be used or transformed for different purposes. 
* Its most obvious usage is evaluation of the represented expression (e.g., @code b = A*x; @endcode), however it can be transformed to fit other purposes such as expression optimization (e.g.,  @em (A * B) * x, where @em A and @em B are matrices and @em x a vector, becomes A * ( B * x)), simplification or matrix differentiation. 
An example of matrix differentiation is the computation of the derivative of 
...
* The manipulation of the expression tree is still an application layer activity and no actual computation is yet performed.
* 
* With Utopia the evaluation of an expression can be performed in different ways. We identify three main ways: @em expression to function mapping, @em code @em generation, and @em expression @em templates. 
* 
* Expression to function mapping
* This type of evaluation is performed by mapping expressions, which are the nodes of the expression tree, to functions of specific back-ends. 
* For instance, the expression @code Vector v = values(n, 0.1); @endcode in our blas based back-end is simply mapped to a function such as 
* @code
...
* }
* @endcode
* which constructs a vector of length @em n with entries equal to 0.1. The same can be said for all single expressions. However, more complex composite expression can be pattern-matched and mapped to specific back-end calls; for instance the expression @em y= alpha * x + y, where @em x, @em y are vectors and @em alpha is a scalar, is mapped to the blas function @em axpy.
* 
...
* The utopia::Range class provides the methods @em begin and @em end which are not iterators but just begin and end of the range.
...
* For instance, the memory region we want to access might not be directly accessible because it is GPU memory hence for performing CPU operations we need to copy it to the CPU memory.
* When writing in a distributed data-structure such as a distributed matrix the entries we are writing might not be directly physically accessible, hence a write lock allows communicating all the data at once at the end of the writing procedure instead of doing each time, which would result in a very slow program.
* 
* @code
* Matrix m = zeros(n, n);
...
*		- utopia::sparse()
*		- utopia::dense()
* 	- Local factory (usefull if your program needs to interface with external software which provides its own data distribution)
*		- utopia::local_identity()
*		- utopia::local_zeros()
*		- utopia::local_identity()
*		- utopia::local_values()
*		- utopia::local_sparse()
...
* Utopia allows for modular extensions for evaluation optimization which is then applied transparently to the whole code.
...
* If you encounter a bug please provide your code snippet responsible for the bug. If you really like us send us a minimal compilable and runnable example.
* contacts: 

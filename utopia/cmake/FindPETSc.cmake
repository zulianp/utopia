# * Try to find PETSc Once done this will define
#
# PETSC_FOUND        - system has PETSc PETSC_INCLUDES     - the PETSc include
# directories PETSC_LIBRARIES    - Link these to use PETSc PETSC_COMPILER     -
# Compiler used by PETSc, helpful to find a compatible MPI PETSC_DEFINITIONS  -
# Compiler switches for using PETSc PETSC_MPIEXEC      - Executable for running
# MPI programs PETSC_VERSION      - Version string (MAJOR.MINOR.SUBMINOR)
#
# Usage: find_package(PETSc COMPONENTS CXX)  - required if build
# --with-clanguage=C++ --with-c-support=0 find_package(PETSc COMPONENTS C)    -
# standard behavior of checking build using a C compiler find_package(PETSc) -
# same as above
#
# Setting these changes the behavior of the search PETSC_DIR - directory in
# which PETSc resides PETSC_ARCH - build architecture
#
...
if(NOT PETSc_FIND_COMPONENTS)
  ...
  # Right now, this is designed for compatability with the --with-clanguage
  # option, so only allow one item in the components list.
  list(LENGTH ${PETSc_FIND_COMPONENTS} components_length)
  if(${components_length} GREATER 1)
    message(
      FATAL_ERROR "Only one component for PETSc is allowed to be specified")
  endif()
  # This is a stub for allowing multiple components should that time ever come.
  # Perhaps to also test Fortran bindings?
...
function(petsc_get_version)
  if(EXISTS "${PETSC_DIR}/include/petscversion.h")
    file(STRINGS "${PETSC_DIR}/include/petscversion.h" vstrings
         REGEX "#define PETSC_VERSION_(RELEASE|MAJOR|MINOR|SUBMINOR|PATCH) ")
    foreach(line ${vstrings})
      string(REGEX REPLACE " +" ";" fields ${line}) # break line into three
                                                    # fields (the first is
                                                    # always "#define")
    ...
    if(PETSC_VERSION_RELEASE)
      if($(PETSC_VERSION_PATCH) GREATER 0)
        set(PETSC_VERSION
            "${PETSC_VERSION_MAJOR}.${PETSC_VERSION_MINOR}.${PETSC_VERSION_SUBMINOR}p${PETSC_VERSION_PATCH}"
            CACHE INTERNAL "PETSc version")
      else()
        set(PETSC_VERSION
      ...
      # make dev version compare higher than any patch level of a released
      # version
      set(PETSC_VERSION
          "${PETSC_VERSION_MAJOR}.${PETSC_VERSION_MINOR}.${PETSC_VERSION_SUBMINOR}.99"
          CACHE INTERNAL "PETSc version")
    endif()
  else()
    message(
      SEND_ERROR
        "PETSC_DIR can not be used, ${PETSC_DIR}/include/petscversion.h does not exist"
    )
  endif()
endfunction()
...
set(Petsc_SEARCH_PATHS
    "/usr/lib/petsc ${DEB_PATHS};/opt/petsc/linux-c-opt;/opt/local/lib/petsc")
  ...
  foreach(arch ${_petsc_arches})
    if(NOT PETSC_ARCH)
      ...
      if(petscconf)
        set(PETSC_ARCH
            "${arch}"
            CACHE STRING "PETSc build architecture")
      endif(petscconf)
    endif(NOT PETSC_ARCH)
  endforeach(arch)
  set(petscconf
      "NOTFOUND"
      CACHE INTERNAL "Scratch variable" FORCE)
...
# Determine whether the PETSc layout is old-style (through 2.3.3) or new-style
# (>= 3.0.0)
...
")
  ...
  macro(PETSC_GET_VARIABLE name var)
    set(${var}
        "NOTFOUND"
        CACHE INTERNAL "Cleared" FORCE)
    execute_process(
      COMMAND ${MAKE_EXECUTABLE} --no-print-directory -f
              ${petsc_config_makefile} show VARIABLE=${name}
      OUTPUT_VARIABLE ${var}
  ...
  # on windows we need to make sure we're linking against the right runtime
  # library
  if(WIN32)
    if(petsc_cc_flags MATCHES "-MT")
      ...
      if(${using_md} MATCHES "True")
        message(
          WARNING
            "PETSc was built with /MT, but /MD is currently set.
 See http://www.cmake.org/Wiki/CMake_FAQ#How_can_I_build_my_MSVC_application_with_a_static_runtime.3F"
  ...
  macro(PETSC_FIND_LIBRARY suffix name)
    set(PETSC_LIBRARY_${suffix}
        "NOTFOUND"
        CACHE INTERNAL "Cleared" FORCE) # Clear any stale value, if we got here,
  ...
  # Look for petscvec first, if it doesn't exist, we must be using
  # single-library
  petsc_find_library(VEC petscvec)
  if(PETSC_LIBRARY_VEC)
    petsc_find_library(
      SYS "petscsys;petsc") # libpetscsys is called libpetsc prior to 3.1 (when
                            # single-library was introduced)
    ...
    set(PETSC_LIBRARY_VEC
        "NOTFOUND"
        CACHE INTERNAL "Cleared" FORCE) # There is no libpetscvec
    petsc_find_library(SINGLE petsc)
    # Debian 9/Ubuntu 16.04 uses _real and _complex extensions when using
    # libraries in /usr/lib/petsc.
  ...
  if(PETSC_LIBRARY_TS)
    message(
      STATUS "Recognized PETSc install with separate libraries for each package"
    )
  else()
    message(
      STATUS "Recognized PETSc install with single library for all packages")
  endif()
...
")
    multipass_source_runs("${includes}" "${libraries}" "${_PETSC_TEST_SOURCE}"
                          ${runs} "${PETSC_LANGUAGE_BINDINGS}")
    if(${${runs}})
      set(PETSC_EXECUTABLE_RUNS
          "YES"
          CACHE
            BOOL
            "Can the system successfully run a PETSc executable?  This variable can be manually set to \"YES\" to force CMake to accept a given PETSc configuration, but this will almost always result in a broken build.  If you change PETSC_DIR, PETSC_ARCH, or PETSC_CURRENT you would have to reset this variable."
            FORCE)
    endif(${${runs}})
  endmacro(PETSC_TEST_RUNS)

  find_path(
    PETSC_INCLUDE_DIR petscts.h
    HINTS "${PETSC_DIR}"
    PATH_SUFFIXES include
    NO_DEFAULT_PATH)
  find_path(
    PETSC_INCLUDE_CONF petscconf.h
    HINTS "${PETSC_DIR}"
  ...
  petsc_test_runs("${petsc_includes_minimal}" "${PETSC_LIBRARIES_TS}"
                  petsc_works_minimal)
  if(petsc_works_minimal)
  ...
  else(petsc_works_minimal) # Minimal includes fail, see if just adding full
                            # includes fixes it
    petsc_test_runs("${petsc_includes_all}" "${PETSC_LIBRARIES_TS}"
    ...
    if(petsc_works_allincludes) # It does, we just need all the includes (
    ...
    else(petsc_works_allincludes) # We are going to need to link the external
                                  # libs explicitly
      ...
      if(petsc_works_alllibraries)
        ...
        if(petsc_works_all) # We fail anyways
          ...
          message(
            STATUS "PETSc could not be used, maybe the install is broken.")
        endif(petsc_works_all)
      endif(petsc_works_alllibraries)
    endif(petsc_works_allincludes)
  endif(petsc_works_minimal)

  # We do an out-of-source build so __FILE__ will be an absolute path, hence
  # __INSDIR__ is superfluous
  if(${PETSC_VERSION} VERSION_LESS 3.1)
    set(PETSC_DEFINITIONS
        "-D__SDIR__=\"\""
        CACHE STRING "PETSc definitions" FORCE)
  else()
    set(PETSC_DEFINITIONS
        "-D__INSDIR__="
        CACHE STRING "PETSc definitions" FORCE)
  ...
  set(PETSC_MPIEXEC
      ${petsc_mpiexec}
      CACHE FILEPATH "Executable for running PETSc MPI programs" FORCE)
  set(PETSC_INCLUDES
      ${petsc_includes_needed}
      CACHE STRING "PETSc include path" FORCE)
  set(PETSC_LIBRARIES
      ${PETSC_LIBRARIES_ALL}
      CACHE STRING "PETSc libraries" FORCE)
  set(PETSC_COMPILER
      ${petsc_cc}
      CACHE FILEPATH "PETSc compiler" FORCE)
  # Note that we have forced values for all these choices.  If you change these,
  # you are telling the system to trust you that they work.  It is likely that
  # you will end up with a broken build.
  mark_as_advanced(PETSC_INCLUDES PETSC_LIBRARIES PETSC_COMPILER
                   PETSC_DEFINITIONS PETSC_MPIEXEC PETSC_EXECUTABLE_RUNS)
endif()
...
find_package_handle_standard_args(
  ...
  FAIL_MESSAGE
    "PETSc could not be found.  Be sure to set PETSC_DIR and PETSC_ARCHPETSC_ARC."
)

  ...
  foreach(_library ${_list})
    ...
    if(_libraries_found)
      ...
      if ( WIN32 )
        find_library(${_prefix}_${_library}_LIBRARY
                    NAMES ${_library}
                    PATHS ENV LIB $ENV{LAPACK_ROOT}
                    )
      elseif ( APPLE )
        find_library(${_prefix}_${_library}_LIBRARY
                    NAMES ${_library}
                    PATHS /usr/local/lib /usr/lib /usr/local/lib64 /usr/lib64 ENV DYLD_LIBRARY_PATH $ENV{LAPACK_ROOT}
                    )
      else ()
        find_library(${_prefix}_${_library}_LIBRARY
                    NAMES ${_library}
                    PATHS /usr/local/lib /usr/lib /usr/local/lib64 /usr/lib64 ENV LD_LIBRARY_PATH $ENV{LAPACK_ROOT}
                    )
      endif()
...
IF(LAPACK_FOUND)
  ...
  ELSE()
    MESSAGE(STATUS "Lapack not found, try to set LAPACK_ROOT env var")
  ENDIF()

...
namespace utopia {
    ...
    public:
        ...
        PrincipalShapeStress(const FunctionSpace &space, const Quadrature &q, const Scalar &mu, const Scalar &lambda)
        ...
        void compute_aggregate_stress(const FunctionSpace &space,
            ...
            for (SizeType i = 0; i < e.n_functions(); ++i) {
                for (SizeType k = 0; k < Quadrature::NPoints; ++k) {
                    ...
                    view_device_.stress(i, k) =
                        2.0 * mu * strain + lambda * trace(strain) * (device::identity<Scalar>());
                }
            }
        }
    ...
    template <class Mesh, int NComponents, class Quadrature, typename... Args>
    class ShapeStress<FunctionSpace<Mesh, NComponents, Args...>, Quadrature> {
    public:
        using FunctionSpace = utopia::FunctionSpace<Mesh, NComponents, Args...>;
        using Vector = typename FunctionSpace::Vector;
        using Scalar = typename FunctionSpace::Scalar;

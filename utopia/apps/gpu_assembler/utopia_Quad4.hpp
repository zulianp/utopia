...
namespace utopia {
    ...
    public:
        ...
        UTOPIA_INLINE_FUNCTION const Point &node(const std::size_t &i) const { return nodes_[i]; }

        UTOPIA_INLINE_FUNCTION Point &node(const std::size_t &i) { return nodes_[i]; }
        ...
        UTOPIA_INLINE_FUNCTION bool is_affine() const { return is_affine_; }
        ...
        // template<typename PhysicalPoint>
        // UTOPIA_INLINE_FUNCTION bool contains(const PhysicalPoint &p, const Scalar &tol = device::epsilon<Scalar>())
        // const
        // {
        ...
        UTOPIA_INLINE_FUNCTION void point(const RefPointT &in, PhysicalPointT &out) const {
            out = RefQuad4::fun(0, in) * nodes_[0] + RefQuad4::fun(1, in) * nodes_[1] +
                  RefQuad4::fun(2, in) * nodes_[2] + RefQuad4::fun(3, in) * nodes_[3];
        }
        ...
        UTOPIA_INLINE_FUNCTION constexpr static int n_functions() { return NFunctions; }

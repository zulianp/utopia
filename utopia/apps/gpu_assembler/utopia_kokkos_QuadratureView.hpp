...
namespace utopia {
    ...
    public:
        ...
        Quadrature() : points_("Quadrature::points", NPoints, Dim), weights_("Quadrature::weights", NPoints) { init(); }

        inline ViewDevice view_device() const { return ViewDevice(points_.view_device(), weights_.view_device()); }

        inline ViewHost view_host() const { return ViewHost(points_.view_host(), weights_.view_host()); }

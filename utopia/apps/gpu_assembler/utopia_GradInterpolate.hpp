...
namespace utopia {
    ...
    public:
        ...
        using ViewDevice = utopia::GradInterpolateView<FunctionSpaceViewDevice,
        ...
        GradInterpolate(const std::shared_ptr<Coefficient> &coeff, const Quadrature &q)
            : coeff_(coeff), grad_(coeff->space(), q) {}

        ViewDevice view_device() const { return ViewDevice(coeff_->view_device(), grad_.view_device()); }

...
public:
  ...
  void read(Input &in) override {
    ...
    if (nu != 0.0 && E != 0.0) {
      ...
      if (mpi_world_rank() == 0) {
        utopia::out() << "mu: " << mu << "  lambda: " << lambda
                      << "  Gc: " << fracture_toughness << "  \n";
      }
    }
  }

  PFFracParameters()
      : a(1.0), b(1.0), d(1.0), f(1.0), length_scale(0.0),
        fracture_toughness(1e-3), mu(80.0), lambda(120.0),
        // mu(100.0),
        // lambda(100.0),
        nu(0.0), E(0.0), l_0(1.0), pressure0(1e-3), regularization(1e-10),
        pressure(0.0), penalty_param(0.0), crack_set_tol(0.93),
        // mobility(1e-5)
        mobility(1e-6)
      ...
      for (SizeType j = 0; j < Dim; ++j) {
        for (SizeType k = 0; k < Dim; ++k) {
          for (SizeType l = 0; l < Dim; ++l) {
            Scalar val =
                this->lambda * kroneckerDelta(i, j) * kroneckerDelta(k, l);
  ...
  bool use_penalty_irreversibility{false},
      use_crack_set_irreversibiblity{false}, use_pressure{false};
...
template <class FunctionSpace, int Dim = FunctionSpace::Dim>
class PhaseFieldFracBase
    : public ExtendedFunction<typename FunctionSpace::Matrix,
...
public:
  ...
  void use_crack_set_irreversibiblity(const bool &flg) {
    params_.use_crack_set_irreversibiblity = flg;
  }
  void use_penalty_irreversibility(const bool &flg) {
    params_.use_penalty_irreversibility = flg;
  }

  void turn_off_uc_coupling(const bool &flg) {
    params_.turn_off_uc_coupling = flg;
  }
  void turn_off_cu_coupling(const bool &flg) {
    params_.turn_off_cu_coupling = flg;
  }
  ...
  PhaseFieldFracBase(FunctionSpace &space) : space_(space) {
    ...
    if (params_.use_penalty_irreversibility) {
      Scalar tol = 1e-3;
      Scalar tol2 = tol * tol;
      params_.penalty_param = params_.fracture_toughness /
                              params_.length_scale * (1.0 / tol2 - 1.0);
    }
  ...
  PhaseFieldFracBase(FunctionSpace &space, const PFFracParameters &params)
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  virtual bool fracture_energy(const Vector & /*x_const*/,
                               Scalar & /*val*/) const = 0;
  virtual bool elastic_energy(const Vector & /*x_const*/,
                              Scalar & /*val*/) const = 0;
  ...
  virtual bool compute_tcv(const Vector &x_const, Scalar &error) const {
      ...
      Device::parallel_reduce(
          ...
          UTOPIA_LAMBDA(const SizeType &i) {
            ...
            for (SizeType qp = 0; qp < NQuadPoints; ++qp) {
              if (Dim == 2) {
                tcv_mine += ((u1[qp] * c_grad_el[qp](0)) +
                             (u2[qp] * c_grad_el[qp](1))) *
                            dx(qp);
              } else {
                tcv_mine +=
                    ((u1[qp] * c_grad_el[qp](0)) + (u2[qp] * c_grad_el[qp](1)) +
    ...
    if (Dim == 2) {
      tcv_exact = 2.0 * this->params_.pressure0 * this->params_.l_0 *
                  this->params_.l_0 *
                  (1.0 - this->params_.nu * this->params_.nu) * PI /
                  this->params_.E;
    } else {
      tcv_exact = 16.0 * this->params_.pressure0 * this->params_.l_0 *
                  this->params_.l_0 * this->params_.l_0 *
                  (1.0 - this->params_.nu * this->params_.nu) /
                  this->params_.E / 3.0;
    }

    error = device::abs(computed_tcv - tcv_exact);
    if (mpi_world_rank() == 0) {
      std::cout << "computed_tcv: " << computed_tcv << "  exact: " << tcv_exact
                << "  error: " << error << "\n ";
    }
  ...
  virtual void compute_cod(const Vector &x_const, Scalar &error) const {
    ...
    Scalar cod_exact = 4.0 / PI * this->params_.pressure0 * this->params_.l_0 *
                       (1. - (this->params_.nu * this->params_.nu)) /
                       this->params_.E;
    ...
    if (mpi_world_rank() == 0) {
      std::cout << "cod_exact: " << cod_exact
                << "  cod_computed: " << cod_computed << "  error: " << error
                << "\n ";
    }
  }
  ...
  UTOPIA_INLINE_FUNCTION static Scalar
  bilinear_uu(const PFFracParameters &params, const Scalar &phase_field_value,
              const StressShape &stress, const Grad &strain_test) {
    const Scalar gc = ((1.0 - params.regularization) *
                           quadratic_degradation(params, phase_field_value) +
  ...
  template <class Grad>
  UTOPIA_INLINE_FUNCTION static Scalar
  diffusion_c(const PFFracParameters &params, const Grad &g_trial,
              const Grad &g_test) {
    return params.fracture_toughness * params.length_scale *
           inner(g_trial, g_test);
  }

  UTOPIA_INLINE_FUNCTION static Scalar
  reaction_c(const PFFracParameters &params, const Scalar &trial,
             const Scalar &test) {
    return (params.fracture_toughness / params.length_scale) * trial * test;
  }

  UTOPIA_INLINE_FUNCTION static Scalar
  elastic_deriv_cc(const PFFracParameters &params,
                   const Scalar &phase_field_value,
                   const Scalar &elastic_energy_positive, const Scalar &trial,
                   const Scalar &test) {
    const Scalar dcc = (1.0 - params.regularization) *
                       quadratic_degradation_deriv2(params, phase_field_value);
  ...
  UTOPIA_INLINE_FUNCTION static Scalar grad_fracture_energy_wrt_c(
      const PFFracParameters &params, const Scalar &phase_field_value,
      const Grad &phase_field_grad, const Scalar &test_function,
      const GradTest &grad_test_function) {
    return params.fracture_toughness *
           ((1. / params.length_scale * phase_field_value * test_function) +
            (params.length_scale *
             inner(phase_field_grad, grad_test_function)));
  }

  template <class Stress, class FullStrain>
  UTOPIA_INLINE_FUNCTION static Scalar
  bilinear_uc(const PFFracParameters &params, const Scalar &phase_field_value,
              const Stress &stress_p, const FullStrain &full_strain,
              const Scalar &c_trial_fun) {
    return c_trial_fun *
           inner(quadratic_degradation_deriv(params, phase_field_value) *
                     stress_p,
                 full_strain);
  }
  ...
  template <class Grad>
  UTOPIA_INLINE_FUNCTION static Scalar
  fracture_energy(const PFFracParameters &params,
                  const Scalar &phase_field_value,
                  const Grad &phase_field_grad) {
    return params.fracture_toughness *
           (1. / (2.0 * params.length_scale) * phase_field_value *
                phase_field_value +
            params.length_scale / 2.0 *
                inner(phase_field_grad, phase_field_grad));
  }

  template <class GradShape>
  UTOPIA_INLINE_FUNCTION static Scalar
  bilinear_cc(const PFFracParameters &params, const Scalar &phase_field_value,
              const Scalar &elastic_energy_p, const Scalar &shape_trial,
              const Scalar &shape_test, const GradShape &grad_trial,
              const GradShape &grad_test) {
    return diffusion_c(params, grad_trial, grad_test) +
           reaction_c(params, shape_trial, shape_test) +
           elastic_deriv_cc(params, phase_field_value, elastic_energy_p,
                            shape_trial, shape_test);
  }

  UTOPIA_INLINE_FUNCTION static Scalar
  quadratic_degradation(const PFFracParameters &, const Scalar &c) {
    Scalar imc = 1.0 - c;
    return imc * imc;
  }

  UTOPIA_INLINE_FUNCTION static Scalar
  quadratic_degradation_deriv(const PFFracParameters &, const Scalar &c) {
    Scalar imc = 1.0 - c;
    return -2.0 * imc;
  }

  UTOPIA_INLINE_FUNCTION static Scalar
  quadratic_degradation_deriv2(const PFFracParameters &, const Scalar &) {
    return 2.0;
  }
  ...
  // this 2 functions need to be moved to BC conditions
  virtual void apply_zero_constraints_irreversibiblity(Vector &g,
  ...
  // this 2 functions need to be moved to BC conditions
  void add_irr_values_markers(Vector &val, Vector &flg,
                              const Vector &x_current) const {
    {
  ...
  virtual void apply_zero_constraints_irreversibiblity(Matrix &H,
                                                       const Vector &x) const {
    std::vector<SizeType> indices;
    {
      Read<Vector> r(x_old_);

  ...
  void init(PetscVector &x) override {
    ...
    auto sampler = utopia::sampler(
        C, UTOPIA_LAMBDA(const Point &x)->Scalar {
          ...
          Scalar f = 0.0;
          if ((r_squared <= l_0_ * l_0_) &&
              ((device::abs(2.0 * x[1]) <= thickness))) {
            f = 1.0;
          } else {
            f = 0.0;
          }
  ...
  void init(PetscVector &x) override {
    ...
    auto sampler = utopia::sampler(
        C, UTOPIA_LAMBDA(const Point &x)->Scalar {
          Scalar f = 0.0;
          if (x[0] > (0.5 - this->space_.mesh().min_spacing()) &&
              x[0] < (0.5 + this->space_.mesh().min_spacing()) && x[1] > 0.3 &&
              x[1] < 0.5) {
            f = 1.0;
...
template <class FunctionSpace>
class InitialCondidtionPFParallelFrac3D
...
public:
  ...
  InitialCondidtionPFParallelFrac3D(FunctionSpace &space,
  ...
  void init(PetscVector &x) override {
    ...
    auto sampler = utopia::sampler(
        C, UTOPIA_LAMBDA(const Point &x)->Scalar {
          ...
          if (x[0] > 0.55 && x[0] < 0.70 && x[1] > (0.40 - h) &&
              x[1] < (0.40 + h) && x[2] < (0.6 + h) && x[2] > (0.6 - h)) {
            f = 1.0;
          } else if (x[0] > (0.26 - h) && x[0] < (0.26 + h) && x[1] > 0.38 &&
                     x[1] < 0.55 && x[2] < (0.40 + h) && x[2] > (0.40 - h)) {
            f = 1.0;
          } else {
            f = 0.0;
          }
...
public:
  ...
  void init(PetscVector &x) override {
    ...
    rectangles.push_back(Rectangle<Scalar>(Coord(0.308514, 1.531184),
                                           Coord(0.488788, 1.711458), width));
    rectangles.push_back(Rectangle<Scalar>(Coord(0.605291, 1.511563),
                                           Coord(0.713210, 1.332516), width));
    rectangles.push_back(Rectangle<Scalar>(Coord(1.128942, 1.518921),
                                           Coord(1.359495, 1.694289), width));
    rectangles.push_back(Rectangle<Scalar>(Coord(1.517694, 1.412228),
                                           Coord(1.673441, 1.229502), width));
    rectangles.push_back(Rectangle<Scalar>(Coord(0.268045, 0.819902),
                                           Coord(0.411528, 0.991591), width));
    rectangles.push_back(Rectangle<Scalar>(Coord(0.829713, 0.903294),
                                           Coord(1.087246, 0.957253), width));
    rectangles.push_back(Rectangle<Scalar>(Coord(1.456377, 0.994043),
                                           Coord(1.592502, 0.819902), width));
    rectangles.push_back(Rectangle<Scalar>(Coord(0.326910, 0.368605),
                                           Coord(0.482656, 0.520673), width));
    rectangles.push_back(Rectangle<Scalar>(Coord(0.908199, 0.465487),
                                           Coord(1.090925, 0.346531), width));
    rectangles.push_back(Rectangle<Scalar>(Coord(1.436755, 0.364926),
                                           Coord(1.624387, 0.493693), width));

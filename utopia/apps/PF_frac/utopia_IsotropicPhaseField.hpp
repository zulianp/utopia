...
class IsotropicPhaseFieldForBrittleFractures final
 ...
 public:
  ...
  IsotropicPhaseFieldForBrittleFractures(FunctionSpace &space,
      ...
      : space_(space),
        params_(params),
        use_dense_hessian_(false),
        check_derivatives_(false) {
    this->local_x_ = std::make_shared<Vector>();
    space_.create_local_vector(*this->local_x_);
          ...
          UTOPIA_LAMBDA(const SizeType &i) {
            ...
            for (SizeType qp = 0; qp < NQuadPoints; ++qp) {
              ...
              if (params_.use_pressure) {
                el_energy +=
                    quadratic_degradation(params_, c[qp]) * p[qp] * tr * dx(qp);
              }

              el_energy += energy(params_, c[qp], c_grad_el[qp], tr,
                                  el_strain.strain[qp]) *
                           dx(qp);
              ...
              if (params_.use_penalty_irreversibility) {
                ...
                el_energy += params_.penalty_param / 2.0 * c_cold_bracket *
  ...
  bool elastic_energy(const Vector &x_const, Scalar &val) const {
  ...
  bool gradient(const Vector &x_const, Vector &g) const override {
    UTOPIA_TRACE_REGION_BEGIN(
        "IsotropicPhaseFieldForBrittleFractures::gradient");
  ...
  bool hessian(const Vector &x_const, Matrix &H) const override {
    UTOPIA_TRACE_REGION_BEGIN(
        "IsotropicPhaseFieldForBrittleFractures::hessian");
    ...
    // reference based
    PrincipalShapeStress<USpace, Quadrature> p_stress(U, q, params_.mu,
                                                      params_.lambda);
    Strain<USpace, Quadrature> ref_strain_u(U, q);
  ...
  UTOPIA_INLINE_FUNCTION static Scalar bilinear_cc(
      const Parameters &params, const Scalar &phase_field_value,
      const Scalar &elastic_energy, const Scalar &shape_trial,
    ...
    return diffusion_c(params, grad_trial, grad_test) +
           ...
           elastic_deriv_cc(params, phase_field_value, elastic_energy,
  ...
  UTOPIA_INLINE_FUNCTION static Scalar bilinear_cu(
    ...
    return ((1.0 - params.regularization) *
            quadratic_degradation_deriv(params, phase_field_value)) *
  ...
  UTOPIA_INLINE_FUNCTION static Scalar bilinear_uc(
      const Parameters &params, const Scalar &phase_field_value,
      const Stress &stress, const FullStrain &full_strain,
      const Scalar &c_trial_fun) {
  ...
  //     params.regularization); auto C_test  = 0.5 * (g_test  +
  //     transpose(g_test)); return inner(gc * stress, C_test);
  // }
  ...
  UTOPIA_INLINE_FUNCTION static Scalar elastic_deriv_cc(
      const Parameters &params, const Scalar &phase_field_value,
      const Scalar &elastic_energy, const Scalar &trial, const Scalar &test) {
  ...
  UTOPIA_INLINE_FUNCTION static Scalar grad_elastic_energy_wrt_c(
      const Parameters &params, const Scalar &phase_field_value,
      const Scalar &trace, const Strain &strain) {
    return (quadratic_degradation_deriv(params, phase_field_value) *
  ...
  UTOPIA_INLINE_FUNCTION static void compute_stress(const Parameters &params,
                                                    ...
                                                    const Strain &strain,
                                                    Stress &stress) {
    stress = (2.0 * params.mu * strain) +
             (params.lambda * tr * (device::identity<Scalar>()));
  }
  ...
  UTOPIA_INLINE_FUNCTION static Scalar strain_energy(const Parameters &params,
                                                     const Scalar trace,
                                                     const Strain &strain) {
    return 0.5 * params.lambda * trace * trace +
           params.mu * inner(strain, strain);
  }
  ...
  UTOPIA_INLINE_FUNCTION static Scalar degradation(const Parameters &params,
                                                   const Scalar &c) {
    Scalar imc = 1.0 - c;
  ...
  UTOPIA_INLINE_FUNCTION static Scalar quadratic_degradation(const Parameters &,
                                                             const Scalar &c) {
    Scalar imc = 1.0 - c;
    return imc * imc;
  }
  ...
  // this 2 functions need to be moved to BC conditions
  void apply_zero_constraints_irreversibiblity(Vector &g,
  ...
  void apply_zero_constraints_irreversibiblity(Matrix &H,
                                               const Vector &x) const {
    std::vector<SizeType> indices;
    {
      Read<Vector> r(x_old_);

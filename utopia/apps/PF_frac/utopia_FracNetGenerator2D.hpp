...
class Rectangle {
 public:
  Rectangle(const Point2D<T> &A, const Point2D<T> &B, const Point2D<T> &C,
            const Point2D<T> &D)
      : A_(A), B_(B), C_(C), D_(D) {}
  ...
  Rectangle(const Point2D<T> &A, const T &length, const T &width,
            const T &theta)
      : A_(A) {
  ...
  bool belongs_to_rectangle(Point2D<T> M) {
    ...
    return ((0.0 <= dotABAM) && (dotABAM <= dotABAB) && (0.0 <= dotBDBM) &&
            (dotBDBM <= dotBDBD));
  }
 ...
 private:
  void randomly_generate(const T &width,
                         const FracNetSamplerParams2D<T> &params) {
    // unsigned seed =
    ...
    std::uniform_real_distribution<> distr_point_xdir(params.uni_x_min,
                                                      params.uni_x_max);
    std::uniform_real_distribution<> distr_point_ydir(params.uni_y_min,
                                                      params.uni_y_max);

    std::uniform_int_distribution<> distr_angle(params.alpha_min,
                                                params.alpha_max);
    ...
    T length =
        x_min * std::pow((1. - r), (-1. / (params.pow_dist_coef_length - 1.)));
  ...
  void generate_rectangle(const T &a, const T &b, const T &theta) {
    ...
    D_.x = A_.x +
           ((a * std::cos(theta_rad)) + (b * std::cos(theta_rad + pi / 2.0)));
    D_.y = A_.y +
           ((a * std::sin(theta_rad)) + (b * std::sin(theta_rad + pi / 2.0)));
  }

  void build_vector(const Point2D<T> &A, const Point2D<T> &B,
                    Point2D<T> &result) {
    result.x = B.x - A.x;
    result.y = B.y - A.y;
  }
...
class InitialCondidtionPFFracNet2D : public InitialCondition<FunctionSpace> {
 public:
  ...
  InitialCondidtionPFFracNet2D(FunctionSpace &space,
    ...
    if (!csv_file_name_.empty()) {
      ...
      if (mpi_world_rank() == 0) {
        if (!writer.file_exists(csv_file_name_)) {
          ...
          writer.write_table_row<std::string>({"frac-id", "C1x", "C1y", "C2x",
                                               "C2y", "C3x", "C3y", "C4x",
                                               "C4y"});
        } else {
          writer.open_file(csv_file_name_);
        ...
        for (auto id = 0; id < rectangles.size(); id++) {
  ...
  void init(PetscVector &sol_vec, PetscVector &press_vec) override {
    ...
    auto press_sampler =
        utopia::sampler(C, [&rectangle, p](const Point &x) -> Scalar {
          if (rectangle.belongs_to_rectangle(x[0], x[1])) {
            return p;
          } else {
            return p / 10.0;

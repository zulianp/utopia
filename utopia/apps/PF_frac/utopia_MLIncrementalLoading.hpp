...
class MLIncrementalLoading final
    : public IncrementalLoadingBase<FunctionSpace> {
public:
  ...
  MLIncrementalLoading(FunctionSpace &space_coarse)
      : init_(false), n_levels_(2), n_coarse_sub_comm_(1),
        log_output_path_("rmtr_log_file.csv"), save_output_(true),
        mprgp_smoother_(false), hjsmn_smoother_(false) {
    spaces_.resize(2);
  ...
  bool init_ml_setup() {
    ...
    if (mprgp_smoother_) {
      ...
      auto qp = std::make_shared<SemismoothNewton<Matrix, Vector>>(
          std::make_shared<MPRGP<Matrix, Vector>>());
  ...
  void init_solution() override {
    ...
    for (auto l = n_levels_ - 1; l > 0; l--) {
      ...
      auto *fun_coarse =
          dynamic_cast<ProblemType *>(level_functions_[l - 1].get());
  ...
  void prepare_for_solve() override {
    ...
    auto press_ts =
        this->pressure0_ + (this->time_ * this->pressure_increase_factor_);
  ...
  void update_time_step(const SizeType &conv_reason) override {
    ...
    if (this->adjust_dt_on_failure_ && conv_reason < 0) {
      ...
      if (auto *fun_finest =
              dynamic_cast<ProblemType *>(level_functions_.back().get())) {
        fun_finest->make_iterate_feasible(this->lb_, this->ub_,
                                          this->solution_);
    ...
    if (!csv_file_name_.empty()) {
      ...
      if (mpi_world_rank() == 0) {
        if (!writer.file_exists(csv_file_name_)) {
          ...
          writer.write_table_row<std::string>({"time", "elastic_energy",
                                               "fracture_energy", "error_tcv",
                                               "error_cod"});
        } else {
          writer.open_file(csv_file_name_);
        }

        writer.write_table_row<Scalar>({this->time_, elastic_energy,
                                        fracture_energy, error_tcv, error_cod});
        writer.close_file();
      }
    }
  }
  ...
  void run() override {
    ...
    while (this->time_ < this->final_time_) {
      if (mpi_world_rank() == 0) {
        utopia::out() << "#####################################################"
                         "################# \n";
        utopia::out() << "Time-step: " << this->time_step_counter_

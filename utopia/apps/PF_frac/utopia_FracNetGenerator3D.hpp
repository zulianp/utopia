...
class Paralleloid {
 public:
  ...
  Paralleloid(const Point3D<T> &A, const T &length, const T &width,
              const T &theta, const T &gamma) {
    points_.resize(8);
    points_[0] = A;
    this->generate_paralleloid(length, width, theta, gamma);
  }

  bool belongs_to_paralleloid(const T &x_coord, const T &y_coord,
                              const T &z_coord) {
    Point3D<T> M{};
    M.x = x_coord;
    M.y = y_coord;
  ...
  bool belongs_to_paralleloid(Point3D<T> M) {
    ...
    bool flg1 =
        (vec_dot(u, points_[4]) <= dotMu) && (dotMu <= vec_dot(u, points_[0]));
    bool flg2 =
        (vec_dot(v, points_[1]) <= dotMv) && (dotMv <= vec_dot(v, points_[0]));
    bool flg3 =
        (vec_dot(w, points_[2]) <= dotMw) && (dotMw <= vec_dot(w, points_[0]));

    return flg1 && flg2 && flg3;
  }

  void describe() {
    utopia::out() << "A: " << points_[0].x << " " << points_[0].y << " "
                  << points_[0].z << " \n";
    utopia::out() << "B: " << points_[1].x << " " << points_[1].y << " "
                  << points_[1].z << " \n";
    utopia::out() << "C: " << points_[2].x << " " << points_[2].y << " "
                  << points_[2].z << " \n";
    utopia::out() << "D: " << points_[3].x << " " << points_[3].y << " "
                  << points_[3].z << " \n \n";

    utopia::out() << "E: " << points_[4].x << " " << points_[4].y << "  "
                  << points_[4].z << "  \n";
    utopia::out() << "F: " << points_[5].x << " " << points_[5].y << "  "
                  << points_[5].z << "  \n";
    utopia::out() << "G: " << points_[6].x << " " << points_[6].y << "  "
                  << points_[6].z << "  \n";
    utopia::out() << "H: " << points_[7].x << " " << points_[7].y << "  "
                  << points_[7].z << "  \n";
    utopia::out() << "------------------------------  \n";
  }
 ...
 private:
  void randomly_generate(const T &depth,
    ...
    std::uniform_real_distribution<> distr_point_x(params.uni_min,
                                                   params.uni_max);
    std::uniform_real_distribution<> distr_point_y(params.uni_min,
                                                   params.uni_max);
    std::uniform_real_distribution<> distr_point_z(params.uni_min,
                                                   params.uni_max);

    std::uniform_int_distribution<> distr_angle_alpha(params.alpha_min,
                                                      params.alpha_max);
    std::uniform_int_distribution<> distr_angle_beta(params.beta_min,
                                                     params.beta_max);
    ...
    if (params.uniform_length == 0) {
      const T x_min =
          (params.min_length == 0) ? 3.0 * depth : params.min_length;
      const T r = distr_length(generator);
      length = x_min *
               std::pow((1. - r), (-1. / (params.pow_dist_coef_length - 1.)));
    } else {
    ...
    if (params.uniform_width == 0 && params.linear_relation == 0) {
      ...
      const T r = distr_length(generator);
      width =
          x_min * std::pow((1. - r), (-1. / (params.pow_dist_coef_width - 1.)));
  ...
  void generate_paralleloid(const T &a, const T &b, const T &c, const T &theta,
                            const T &gamma,
                            const FracNetSamplerParams3D<T> &params) {
  ...
  void build_vector(const Point3D<T> &A, const Point3D<T> &B,
                    Point3D<T> &result) {
    result.x = A.x - B.x;
    result.y = A.y - B.y;
    result.z = A.z - B.z;
...
class InitialCondidtionPFFracNet3D : public InitialCondition<FunctionSpace> {
 public:
  ...
  InitialCondidtionPFFracNet3D(FunctionSpace &space,
                               const SizeType &PF_component,
                               const SizeType &num_fracs = 10)
      ...
      if (mpi_world_rank() == 0) {
        if (!writer.file_exists(csv_file_name_)) {
          ...
          writer.write_table_row<std::string>(
              {"frac-id", "C1x", "C1y", "C1x", "C2x", "C2y", "C2x",
        ...
        for (auto id = 0; id < paralleloids.size(); id++) {
  ...
  void init(PetscVector &x) override {
    ...
    auto sampler =
        utopia::sampler(C, [&paralleloids](const Point &x) -> Scalar {

...
namespace utopia {
    ...
    class GeometricMultigrid final : public IterativeSolver<typename FunctionSpace::Matrix, typename FunctionSpace::Vector>, public Configurable {
    public:
        ...
        void read(Input &in) override {
            algebraic_mg_->read(in);
        }

        void update(const std::shared_ptr<const Matrix> &op) override
        {
            algebraic_mg_->update(op);
        }

        bool apply(const Vector &rhs, Vector &sol) override
        {
            return algebraic_mg_->apply(rhs, sol);
        }
        ...
        {
            algebraic_mg_->verbose(val);
        }

        inline void max_it(const SizeType &it) override {
            algebraic_mg_->max_it(it);
        }

        inline SizeType max_it() const override {
            return algebraic_mg_->max_it();
        }

        inline void atol(const double &tol) override {
            algebraic_mg_->atol(tol);
        }

        bool init(FunctionSpace &space, const SizeType n_levels)
        {
            return init(make_ref(space), n_levels);
        }

...
namespace utopia {
    ...
    public:
        ...
        FEFunctionView(const FunctionSpaceView &space, const DataView &data) : space_(space), data_(data) {}
    ...
    public:
        ...
        FEFunction(const std::shared_ptr<Coefficient> &coeff) : space_(make_ref(coeff->space())), coeff_(coeff) {}

        FEFunction(const std::shared_ptr<FunctionSpace> &space, Vector &global_vector)
            : space_(space), coeff_(std::make_shared<Coefficient>(*space_, global_vector)) {}

        FEFunction(const std::shared_ptr<FunctionSpace> &space)
            : space_(space), coeff_(std::make_shared<Coefficient>(*space_)) {}

        FEFunction(const FunctionSpace &space, Vector &global_vector)
            : space_(make_ref(space)), coeff_(std::make_shared<Coefficient>(*space_, global_vector)) {}

        std::shared_ptr<Coefficient> coefficient() { return coeff_; }

        void update(const Vector &x) { coeff_->update(x); }

...
namespace utopia {
    ...
    public:
        ...
        bool gradient(const Vector &x, Vector &g) const override { return false; }
        ...
        bool has_upper_bound() const override { return false; }

        bool has_lower_bound() const override { return false; }

        void describe() const override {
            utopia::out() << "n_elements: " << n_elements_ << std::endl;
            utopia::out() << "n: " << n_ << std::endl;
            disp(rhs_);
        }

        inline const Matrix &laplacian() const { return laplacian_; }

        inline const Vector &rhs() const { return rhs_; }
    ...
    private:
        ...
        void assemble_matrix(Matrix &mat) {
            if (empty(mat)) {
                ...
                Dev::parallel_for(
                    n_elements_, UTOPIA_LAMBDA(const SizeType &k) {
                        auto dof_k = Kokkos::subview(dof_, k, Kokkos::ALL());
                        auto el_mat = Kokkos::subview(element_matrix_, k, Kokkos::ALL(), Kokkos::ALL());
        ...
        void init_q_rule() {
            ...
            host_q_points(1, 0) = 0.9304589153964795245728880523899, host_q_points(1, 1) = 0.5;
        ...
        void init() {
            ...
            utopia::out() << "Poisson::init() " << c << std::endl;
        }
        ...
        void init_boundary_conditions() {
            ...
            Dev::parallel_for(
                n_ + 1, UTOPIA_LAMBDA(const SizeType &i) {
                    // lower boundary
                    bcm_view.set(i, 1.0);
        ...
        void assemble_rhs(Fun fun) {
            ...
            Dev::parallel_for(
                n_elements_, UTOPIA_LAMBDA(const SizeType &e_id) {
        ...
        void assemble_laplacian() {
            ...
            utopia::out() << "Poisson::assemble_laplacian " << c << std::endl;
        ...
        void init_mesh() {
            ...
            const Scalar h = 1. / n_;
            Dev::parallel_for(
                n_, UTOPIA_LAMBDA(const SizeType &i) {

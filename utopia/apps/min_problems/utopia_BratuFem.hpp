...
namespace utopia {
    ...
    class BratuFem final : virtual public UnconstrainedExtendedTestFunction<typename FunctionSpace::Matrix,
                    ...
                    space_.element_range(), UTOPIA_LAMBDA(const SizeType &i) {
                        ...
                        for (SizeType qp = 0; qp < NQuadPoints; ++qp) {
                            for (SizeType j = 0; j < C_NDofs; ++j) {
                                ...
                                c_el_vec(j) -= inner(params_.lambda * device::exp(c[qp]), shape_test) * dx(qp);
                            }
                        }
                    ...
                    space_.element_range(), UTOPIA_LAMBDA(const SizeType &i) {
                        ...
                        for (SizeType qp = 0; qp < NQuadPoints; ++qp) {
                            for (SizeType l = 0; l < C_NDofs; ++l) {
                                ...
                                Scalar val1 = 0;
                                val1 -= inner(params_.lambda * device::exp(c[qp]), c_shape_l) * dx(qp);
                                el_mat(l, l) += val1;

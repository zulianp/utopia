...
public:
  ...
  MLSteadyState(FunctionSpace &space_coarse)
      : init_(false), n_levels_(2), n_coarse_sub_comm_(1),
        log_output_path_("rmtr_log_file.csv"), output_path_("rmtr_out"),
        save_output_(true), mprgp_smoother_(false), hjsmn_smoother_(false) {
    spaces_.resize(2);
  ...
  void run() {
    ...
    multigrid_->pre_smoothing_steps(1);
    multigrid_->post_smoothing_steps(1);

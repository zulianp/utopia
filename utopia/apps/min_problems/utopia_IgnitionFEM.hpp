...
namespace utopia {
    ...
    class IgnitionFem final : virtual public UnconstrainedExtendedTestFunction<typename FunctionSpace::Matrix,
                    ...
                    space_.element_range(), UTOPIA_LAMBDA(const SizeType &i) {
                        ...
                        for (SizeType qp = 0; qp < NQuadPoints; ++qp) {
                            for (SizeType l = 0; l < C_NDofs; ++l) {
                                ...
                                Scalar val1 = -0.5 * ((expc * c_shape_l * c_shape_l) * (1.0 + c[qp])) * dx(qp);

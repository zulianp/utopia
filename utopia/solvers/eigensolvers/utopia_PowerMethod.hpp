...
// TODO:: see if there is more appropriate interface to be used... such as
// EigenSolver...
/**
 * @brief      Power method
 * @tparam     Vector
...
class PowerMethod final : public virtual Clonable, public Configurable {
 ...
 public:
  ...
  void max_it(const SizeType &power_method_max_it) {
    max_it_ = power_method_max_it;
  }
  SizeType max_it() const { return max_it_; }

  void use_rand_vec_init(const bool &use_rand_vec_init) {
    use_rand_vec_init_ = use_rand_vec_init;
  }
 ...
 private:
  ...
  Scalar compute_max_eig(const Operator<Vector> &A) {
    ...
    while (!converged) {
      ...
      converged = ((device::abs(lambda_old - lambda) < tol_) || it > max_it_)
                      ? true
                      : false;
      if (this->verbose() && mpi_world_rank() == 0) {
        utopia::out() << "it " << it << " lambda:  " << lambda
                      << "  (lambda_old - lambda)  "

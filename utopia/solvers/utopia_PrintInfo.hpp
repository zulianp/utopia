...
class PrintInfo {
 ...
 public:
  static void print_init(const std::string& method,
                         const std::vector<std::string> status_variables) {
    if (mpi_world_rank() == 0) {
      std::cout << "  \n";
      std::cout << std::setw(10) << std::right << std::string(80, '_')
                << std::setw(15) << std::endl;
      std::cout << std::setw(45) << std::right << " utopia:: " << method
                << std::setw(15) << std::endl;
      std::cout << std::setw(10) << std::right << std::string(80, '_')
                << std::setw(15) << std::endl;
      std::cout << std::endl;
  ...
  static void print_iter_status(const std::vector<Scalar> status_variables) {
    if (mpi_world_rank() == 0) {
      for (auto status_variable : status_variables)
        std::cout << std::setw(27) << std::right << std::scientific
                  << status_variable;
      std::cout << std::endl;
    }
  }
  ...
  template <class T>
  static void print_iter_status(const T& it,
                                const std::vector<Scalar> scalar_vars) {
    if (mpi_world_rank() == 0) {

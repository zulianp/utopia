...
class ConvergenceReason {
 ...
 public:
  ...
  static void exitMessage(const long &num_it, const int &convergence_reason) {
    if (mpi_world_rank() == 0) {
      ...
      if (convergence_reason == CONVERGED_ITERATING) {
        std::cout << "\033[1;32m  LinearSolver converged in " << num_it
                  << " iterations.\033[0m\n";
      }
      if (convergence_reason == DIVERGED_MAX_IT) {
        std::cerr << "\033[1;31m [Error] Maximum number of iteration reached ("
                  << num_it << "). \033[0m\n";
      } else if (convergence_reason == CONVERGED_SNORM_RELATIVE) {
        std::cout << "\033[1;32m  LinearSolver terminated at iteration "
                  << num_it << ", (step_length< tol). \033[0m\n";
      } else if (convergence_reason == CONVERGED_TR_DELTA) {
        std::cout << "\033[1;32m  LinearSolver terminated at iteration "
                  << num_it << ", (radius < 1e-12). \033[0m\n";
      } else if (convergence_reason == CONVERGED_SNORM_RELATIVE) {
        std::cout << "\033[1;32m  LinearSolver terminated at iteration "
                  << num_it << ", no more energy reduction. \033[0m\n";
      } else if (convergence_reason < 0) {
        std::cerr << "\033[1;31m [Error] LinearSolver diverged at iteration "
                  << num_it
                  << " reason = " << diverged_reason_string(convergence_reason)
                  << ". \033[0m\n";
      } else {
  ...
  static void exitMessage_nonlinear(const long &num_it,
                                    const int &convergence_reason) {
    if (mpi_world_rank() == 0) {
      std::cout << std::endl;
      if (convergence_reason == CONVERGED_ITERATING) {
        std::cout << "\033[1;32m  NonlinearSolver converged in " << num_it
                  << " iterations.\033[0m\n";
        ...
        std::cout << "\033[1;32m  NonlinearSolver terminated at iteration "
                  << num_it << ", (step_length< tol). \033[0m\n";
      } else if (convergence_reason == CONVERGED_TR_DELTA) {
        std::cout << "\033[1;32m  NonlinearSolver terminated at iteration "
                  << num_it << ", (radius < 1e-12). \033[0m\n";
        ...
        std::cout << "\033[1;32m  NonlinearSolver terminated at iteration "
                  << num_it << ", (|| F ||/|| F_0|| < atol). \033[0m\n";
      } else if (convergence_reason == CONVERGED_FNORM_ABS) {
        std::cout << "\033[1;32m  NonlinearSolver terminated at iteration "
                  << num_it << ", (|| F || < atol). \033[0m\n";
      } else if (convergence_reason < 0) {
        std::cerr << "\033[1;31m [Error] NonlinearSolver stopped at iteration "
                  << num_it << " . \033[0m\n";
      } else {
        std::cerr << "\033[1;31m NonlinearSolver converged at iteration "
                  << num_it << " for reason = " << convergence_reason
                  << " \033[0m\n";
      }
    }
  }
  ...
  static const int CONVERGED_SNORM_RELATIVE =
  ...
  // fail
  static const int DIVERGED_FUNCTION_DOMAIN =
  ...
  static const int DIVERGED_LOCAL_MIN =
  ...
  static std::string diverged_reason_string(const int reason) {
    ...
    static const std::string string_reason[12] = {
        "KSP_CONVERGED_ITERATING",     "UNDEFINED",
        "KSP_DIVERGED_NULL",           "KSP_DIVERGED_ITS",
        "KSP_DIVERGED_DTOL",           "KSP_DIVERGED_BREAKDOWN",
        "KSP_DIVERGED_BREAKDOWN_BICG", "KSP_DIVERGED_NONSYMMETRIC",
        "KSP_DIVERGED_INDEFINITE_PC",  "KSP_DIVERGED_NANORINF",
        "KSP_DIVERGED_INDEFINITE_MAT", "KSP_DIVERGED_PCSETUP_FAILED"};

...
namespace utopia {
    ...
    public:
         ...
         */
        void reset_preconditioner() { precond_.reset(); }

        const std::shared_ptr<Preconditioner> &get_preconditioner() const { return precond_; }

        inline bool has_preconditioner() const { return static_cast<bool>(precond_); }
        ...
        PreconditionedSolver(const PreconditionedSolver &other) : IterativeSolver(other) {
            copy_preconditioner_from(other);
        }

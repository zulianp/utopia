...
class AbstractLinearSolver
    : public LinearSolver<AbstractMatrix<Scalar, SizeType>,
...
class LinearSolverWrapper
    : public AbstractLinearSolver<typename Traits<Solver>::Scalar,
                                  typename Traits<Solver>::SizeType> {
 public:
  ...
  LinearSolverWrapper(Solver *solver_ptr)
      : impl_(std::shared_ptr<Solver>(solver_ptr)) {}
  ...
  inline void print_usage(std::ostream &os) const override {
    impl_->print_usage(os);
  }
   ...
   * @return
   */
  inline bool solve(const AbstractMatrix &A, const AbstractVector &rhs,
                    AbstractVector &sol) override {
    update(make_ref(A));
    return impl_->apply(extract_vector(rhs), extract_vector(sol));
  }

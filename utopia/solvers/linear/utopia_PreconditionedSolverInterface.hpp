...
namespace utopia {
    ...
    public:
         ...
         */
        virtual void set_preconditioner(const std::shared_ptr<Preconditioner> &precond) { precond_ = precond; }

        virtual std::shared_ptr<Preconditioner> &get_preconditioner() { return precond_; }
        ...
        virtual const std::shared_ptr<Preconditioner> &get_preconditioner() const { return precond_; }

        virtual bool has_preconditioner() const { return static_cast<bool>(precond_); }

        // virtual void update(const std::shared_ptr<Operator<Vector>> &op) {
        //   precond_->update(*op);
        // }

        virtual void update(const std::shared_ptr<const Operator<Vector>> &op) { precond_->update(*op); }

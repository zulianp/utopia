...
class IdentityConstraints
    : public MultilevelVariableBoundSolverInterface<
          Matrix, Vector, IdentityConstraints<Matrix, Vector>> {
 public:
  ...
  IdentityConstraints(
    ...
    for (auto l = 0; l < transfer.size(); l++) {
      if (auto* id_transfer = dynamic_cast<IdentityTransfer<Matrix, Vector>*>(
              transfer[l].get())) {
        // utopia_error("IdentityConstraints, termination due to incorrect
        // setup. ");
      } else {
        utopia_error(
            "IdentityConstraints, termination due to incorrect setup. ");
      }
    }
  }

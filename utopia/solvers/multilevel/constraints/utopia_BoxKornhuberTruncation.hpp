...
class BoxKornhuberTruncation
    : public MultilevelVariableBoundSolverInterface<
          Matrix, Vector, BoxKornhuberTruncation<Matrix, Vector>> {
 public:
  ...
  void init_memory_impl(const std::vector<Layout> &layouts) {
    ...
    if (this->box_constraints_.has_lower_bound()) {
      constraints_memory_.active_lower[finest_level - 1] =
          *(this->box_constraints_.lower_bound());
    }

    if (this->box_constraints_.has_upper_bound()) {
      constraints_memory_.active_upper[finest_level - 1] =
          *(this->box_constraints_.upper_bound());
    }
  }
  ...
  void init_level_impl(const SizeType &level, const Vector &x_finer_level,
    ...
    // if(IPTransferNested<Matrix, Vector>* mat_transfer =
    // dynamic_cast<IPTransferNested<Matrix, Vector>* >
    // (this->transfer_[level].get()))
    {
      this->help_[finer_level] =
          constraints_memory_.active_lower[finer_level] - x_finer_level;
      // this->help_loc_[finer_level] =
      // constraints_memory_.active_upper[finer_level] - x_finer_level;
        ...
        for (auto i = rr.begin(); i != rr.end(); ++i) {
          ...
          for (auto index = 0; index < n_values; ++index) {
            ...
            if (rr_fine_level.inside(j)) {
              ...
              max_value =
                  (max_value > val_cons_fine_lb) ? max_value : val_cons_fine_lb;

              //         // Scalar val_cons_fine_ub =
              //         this->help_loc_[finer_level].get(j);
              //         // min_value = (min_value < val_cons_fine_ub) ?
              //         min_value : val_cons_fine_ub;
            } else {
              // std::cout<<"----- yes, out...\n";
              max_value = (max_value > lb_max) ? max_value : lb_max;
              //         // min_value = (min_value < ub_min) ? min_value :
              //         ub_min;
            }
          }
      ...
      constraints_memory_.active_upper[level].set(9e9);

    }  // dynamic cast test
    else {
      utopia_error(
          "TRBoundsKornhuberTruncation:: Use MatrixTruncatedTransfer! "
          "\n ");
      std::cout << "--------- error ---------- \n";
    }
  }

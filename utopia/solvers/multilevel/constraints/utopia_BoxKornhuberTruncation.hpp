...
namespace utopia {
    ...
    class BoxKornhuberTruncation : public MultilevelVariableBoundSolverInterface<Matrix, Vector, BoxKornhuberTruncation<Matrix, Vector>> {
    public:
        using Scalar = typename Traits<Vector>::Scalar;
        using SizeType = typename Traits<Vector>::SizeType;
        using Layout = typename Traits<Vector>::Layout;

        typedef utopia::MultilevelVariableBoundSolverInterface<Matrix, Vector, BoxKornhuberTruncation<Matrix, Vector>> Base;

        BoxKornhuberTruncation(const std::vector<std::shared_ptr<Transfer<Matrix, Vector>>> &transfer) : Base(transfer) {}
        ...
        void init_level_impl(const SizeType &level,
            ...
            if (auto* mat_transfer = dynamic_cast<MatrixTruncatedTransfer<Matrix, Vector>*>(this->transfer_[level].get()))

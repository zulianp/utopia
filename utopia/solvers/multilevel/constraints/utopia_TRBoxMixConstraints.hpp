 ...
 public:
  ...
  void init_level_impl(const SizeType &level, const Vector &x_finer_level,
    ...
    // intersect  lower bounds
    {
      ...
      auto d_box_lower =
          const_local_view_device(box_bounds_.active_lower(level));
      ...
      parallel_for(local_range_device(constraints_memory_.active_lower[level]),
                   UTOPIA_LAMBDA(const SizeType i) {
                     al.set(i,
                            device::max(d_tr_lower.get(i), d_box_lower.get(i)));
                   });
    }
    ...
    // intersect  upper bounds
    {
      ...
      auto d_box_upper =
          const_local_view_device(box_bounds_.active_upper(level));
      ...
      parallel_for(local_range_device(constraints_memory_.active_upper[level]),
                   UTOPIA_LAMBDA(const SizeType i) {
                     au.set(i,
                            device::min(d_tr_upper.get(i), d_box_upper.get(i)));
                   });
    }
  }
...
using TRGrattonBoxGelmanMandel =
    utopia::TRBoxMixConstraints<Matrix, Vector, TRBoundsGratton<Matrix, Vector>,
                                BoxGelmanMandel<Matrix, Vector>>;

template <class Matrix, class Vector>
using TRGrattonBoxKornhuber =
    utopia::TRBoxMixConstraints<Matrix, Vector, TRBoundsGratton<Matrix, Vector>,
                                BoxKornhuber<Matrix, Vector>>;
...
using TRKornhuberBoxGelmanMandel =
    utopia::TRBoxMixConstraints<Matrix, Vector,
                                TRBoundsKornhuber<Matrix, Vector>,
                                BoxGelmanMandel<Matrix, Vector>>;
...
template <class Matrix, class Vector>
using TRKornhuberBoxKornhuberTruncation =
    utopia::TRBoxMixConstraints<Matrix, Vector,
                                TRBoundsKornhuber<Matrix, Vector>,
                                BoxKornhuberTruncation<Matrix, Vector>>;
...
// // // // // // // // // // // // // // //
template <class Matrix, class Vector>
using TRGelmanMandelBoxGelmanMandel =
    utopia::TRBoxMixConstraints<Matrix, Vector,
...
template <class Matrix, class Vector>
using TRGelmanMandelBoxKornhuberTruncation =
    utopia::TRBoxMixConstraints<Matrix, Vector,
                                TRBoundsGelmanMandel<Matrix, Vector>,
                                BoxKornhuberTruncation<Matrix, Vector>>;

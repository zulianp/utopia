...
namespace utopia {
    ...
    class TRBoundsGelmanMandel
        : public MultilevelVariableBoundSolverInterface<Matrix, Vector, TRBoundsGelmanMandel<Matrix, Vector>> {
    public:
        using Scalar = typename Traits<Vector>::Scalar;
        using SizeType = typename Traits<Vector>::SizeType;
        using Layout = typename Traits<Vector>::Layout;

        typedef utopia::MultilevelVariableBoundSolverInterface<Matrix, Vector, TRBoundsGelmanMandel<Matrix, Vector>>
            Base;

        TRBoundsGelmanMandel(const std::vector<std::shared_ptr<Transfer<Matrix, Vector>>> &transfer) : Base(transfer) {}

        void init_memory_impl(const std::vector<Layout> &layouts) { constraints_memory_.init_memory(layouts); }
        ...
        void init_level_impl(const SizeType &level,
...

            {
                ...
                auto d_tr_lb = const_local_device_view(constraints_memory_.active_lower[finer_level]);
            ...
            Scalar lower_multiplier = 1.0 / I_inf_norm * max(this->help_[finer_level]);
            {
                ...
                parallel_for(
                    ...
                    UTOPIA_LAMBDA(const SizeType i) { d_alb.set(i, d_x.get(i) + lower_multiplier); });
            }
            ...
            {
                auto d_x_finer = const_local_device_view(x_finer_level);
                auto d_tr_ub = const_local_device_view(constraints_memory_.active_upper[finer_level]);
...

            {
                ...
                parallel_for(
                    ...
                    UTOPIA_LAMBDA(const SizeType i) { d_aub.set(i, d_x.get(i) + upper_multiplier); });
            }
        }

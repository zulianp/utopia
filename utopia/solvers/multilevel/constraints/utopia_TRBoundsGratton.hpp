...
class TRBoundsGratton : public MultilevelVariableBoundSolverInterface<
                            Matrix, Vector, TRBoundsGratton<Matrix, Vector>> {
 public:
  ...
  void init_level_impl(const SizeType &level, const Vector &x_finer_level,
...

    {
      ...
      auto d_tr_lb = const_local_view_device(
          constraints_memory_.active_lower[finer_level]);
      auto d_tr_ub = const_local_view_device(
    ...
    this->transfer_[level]->project_down_positive_negative(
        this->help_[finer_level], help_loc_[finer_level],
        constraints_memory_.active_lower[level]);
    this->transfer_[level]->project_down_positive_negative(
        help_loc_[finer_level], this->help_[finer_level],
        constraints_memory_.active_upper[level]);
  }

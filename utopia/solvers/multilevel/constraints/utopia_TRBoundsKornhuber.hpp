...
class TRBoundsKornhuber
    : public MultilevelVariableBoundSolverInterface<
          Matrix, Vector, TRBoundsKornhuber<Matrix, Vector>> {
 public:
      ...
      {
        {
          ...
          parallel_each_write(this->help_[finer_level],
          ...
          parallel_each_write(this->help_loc_[finer_level],
                              UTOPIA_LAMBDA(const SizeType i)->Scalar {
                                Scalar xi = d_x_finer.get(i);
                                auto val = xi + delta_fine;
          ...
          for (auto i = rr.begin(); i != rr.end(); ++i) {
            ...
            for (auto index = 0; index < n_values; ++index) {
              ...
              if (rr_fine_level.inside(j)) {
                ...
                max_value = (max_value > val_cons_fine_lb) ? max_value
                                                           : val_cons_fine_lb;

                Scalar val_cons_fine_ub = this->help_loc_[finer_level].get(j);
                min_value = (min_value < val_cons_fine_ub) ? min_value
                                                           : val_cons_fine_ub;
              } else {
        ...
        utopia_error(
            "TRBoundsKornhuber:: transfer operators not supported. \n ");
        std::cout << "--------- error ---------- \n";
      }

...
namespace utopia {
    ...
    class TRBoundsKornhuber
        : public MultilevelVariableBoundSolverInterface<Matrix, Vector, TRBoundsKornhuber<Matrix, Vector>> {
    public:
        ...
        void init_level_impl(const SizeType& level,
            ...
            if (finer_level == constraints_memory_.active_lower.size() - 1) {
                ...
                if (IPTransferNested<Matrix, Vector>* mat_transfer =
                        dynamic_cast<IPTransferNested<Matrix, Vector>*>(this->transfer_[level].get())) {
                    {

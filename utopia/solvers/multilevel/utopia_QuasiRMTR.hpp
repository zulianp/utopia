    ...
    public:
        ...
        void print_usage(std::ostream &os) const override {
            ...
            this->print_param_usage(
                os, "coarse-QPSolver", "MatrixFreeTRSubproblem", "Input parameters for fine level QP solvers.", "-");
            this->print_param_usage(
                os, "fine-QPSolver", "MatrixFreeTRSubproblem", "Input parameters for coarse level QP solver.", "-");
        ...
        std::string name() override { return "Quasi_RMTR"; }
        ...
        bool set_coarse_tr_strategy(const std::shared_ptr<TRSubproblem> &strategy) {
            if (tr_subproblems_.size() != this->n_levels()) tr_subproblems_.resize(this->n_levels());
    ...
    protected:
        ...
        bool delta_update(const Scalar &rho, const SizeType &level, const Vector &s_global) override {
            ...
            if (level == this->n_levels() - 1) {
                ...
                if (corr_norm <= 0.0) corr_norm = 0.0;
        ...
        Scalar criticality_measure(const SizeType &level) override { return norm2(this->ml_derivs_.g[level]); }

...
class MultiLevelBase : virtual public Configurable {
 ...
 public:
  ...
  void print_usage(std::ostream &os) const override {
    this->print_param_usage(os, "pre_smoothing_steps", "int",
                            "Number of pre-smoothing steps.", "3");
    this->print_param_usage(os, "post_smoothing_steps", "int",
                            "Number of post-smoothing steps.", "3");
    this->print_param_usage(os, "mg_type", "int", "Multigrid type.", "1");
    this->print_param_usage(os, "cycle_type", "int",
                            "Type of cycle used inside of multigrid method.",
                            "MULTIPLICATIVE_CYCLE");
    this->print_param_usage(os, "v_cycle_repetition", "int",
                            "Number of v-cycles used after one F-cycle.", "10");
    this->print_param_usage(os, "fix_semidefinite_operators", "bool",
                            "Flag to fix semidefinite operators", "false");
    this->print_param_usage(os, "num_levels", "int",
                            "Number of levels in ML hierarchy.", "-");
  }

  /**
   * @brief Returns number of levels in hierarchy.
   */
  inline SizeType n_levels() const { return num_levels_; }

  inline void n_levels(const SizeType &n_lev) { num_levels_ = n_lev; }
   ...
   */
  inline SizeType pre_smoothing_steps() const { return pre_smoothing_steps_; }

  /**
   * @return     Number of post-smoothing steps.
   */
  inline SizeType post_smoothing_steps() const { return post_smoothing_steps_; }
  ...
  inline const std::vector<TransferPtr> &transfer() const { return transfers_; }

  virtual void describe(std::ostream &os = std::cout) const { (void)os; }
  ...
  bool set_transfer_operators(
      const std::vector<std::shared_ptr<utopia::Transfer<Matrix, Vector> > >
          &transfers) {
    if (num_levels_ <= 0) {

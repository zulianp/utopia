...
class Multigrid final : public LinearMultiLevel<Matrix, Vector> {
 ...
 private:
  ...
  bool standard_cycle(const SizeType &l) {
    ...
    for (SizeType k = 0; k < this->mg_type(); k++) {
      ...
      if (new_err > err) {
        m_utopia_error_once(
            "[Error] Multigrid::standard_cycle (" + std::to_string(l) +
            "): coarse grid correction raises error " +
            std::to_string(new_err) + "<" + std::to_string(err));
      }
#endif
    }
 ...
 public:
  ...
  inline void block_size(const SizeType size) { block_size_ = size; }

...
namespace utopia {
    ...
    public:
         ...
         */
        Multigrid(const std::shared_ptr<Smoother> &smoother, const std::shared_ptr<Solver> &coarse_solver)
    ...
    private:
        ...
        bool standard_cycle(const SizeType &l) {
            ...
            for (SizeType k = 0; k < this->mg_type(); k++) {
                ...
                // assert(new_err < err)
                if (new_err > err) {
                    m_utopia_error_once("[Error] Multigrid::standard_cycle (" + std::to_string(l) +
                                        "): coarse grid correction raises error " + std::to_string(new_err) + "<" +
                                        std::to_string(err));
                }
#endif
            }

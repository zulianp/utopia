...
class RMTRParams : public TrustRegionParams<Vector> {
 public:
  ...
  void print_usage(std::ostream &os) const override {
    ...
    this->print_param_usage(os, "skip_BC_checks", "bool",
                            "Skip treatment of BC conditions.", "false");
    this->print_param_usage(os, "max_coarse_it", "int",
                            "Maximum number of coarse iterations.", "2");
    this->print_param_usage(os, "max_sucessful_smoothing_it", "int",
                            "Maximum number of smoothing steps.", "1");
    this->print_param_usage(os, "max_sucessful_coarse_it", "int",
                            "Maximum number of corse iterations.", "1");

    this->print_param_usage(
        os, "max_QP_smoothing_it", "int",
        "Maximum number of iterations for fine level QP solver.", "5");
    this->print_param_usage(
        os, "max_QP_coarse_it", "int",
        "Maximum number of iterations for coarse level QP solver.", "50");

    this->print_param_usage(
        os, "eps_delta_termination", "real",
        "Constant used for quiting recursion based on size of tr. radius.",
        "0.001");
    this->print_param_usage(os, "grad_smoothess_termination", "real",
                            "Constant used for quiting recursion based on "
                            "smoothness of the gradient on given level.",
                            "0.5");
    this->print_param_usage(
        os, "check_gradient_smoothness", "bool",
        "Flag turning on/off check for gradient smoothiness.", "true");

    this->print_param_usage(os, "use_line_search", "bool",
                            "Use line-search for coarse grid correction..",
                            "true");

    this->print_param_usage(
        os, "hessian_update_delta", "real",
        "Constant used for deciding whether to assemble fresh hessian or no.",
        "0.15");
    this->print_param_usage(
        os, "hessian_update_eta", "real",
        "Constant used for deciding whether to assemble fresh hessian or no.",
        "0.5");
    this->print_param_usage(os, "verbosity_level", "VerbosityLevel",
                            "Specifies level of verbosity.",
                            "VERBOSITY_LEVEL_NORMAL");

    this->print_param_usage(os, "deltaH_lagging", "deltaH_lagging",
                            "Use lagging of delta H term. Applies only for "
                            "Second order consistent models.",
                            "False");
    this->print_param_usage(os, "Hpost_lagging", "Hpost_lagging",
                            "Use lagging of Hessian for post-smoothing.",
                            "False");
  }

  virtual bool deltaH_lagging() const { return _deltaH_lagging; }

  virtual void deltaH_lagging(const bool &deltaH_lagging) {
  ...
  virtual void grad_smoothess_termination(
      const Scalar &grad_smoothess_termination) {
  ...
  // iterations
  virtual void max_sucessful_smoothing_it(
      const SizeType &max_sucessful_smoothing_it) {
  ...
  virtual void max_sucessful_coarse_it(
      const SizeType &max_sucessful_coarse_it) {

...
namespace utopia {
    ...
    protected:
        template <MultiLevelCoherence T = CONSISTENCY_LEVEL,
                  enable_if_t<is_any<T, FIRST_ORDER, FIRST_ORDER_MGOPT, SECOND_ORDER, GALERKIN>::value, int> = 0>
        ...
        bool get_multilevel_gradient(const Fun &fun,
                                     const SizeType &level,
                                     const Vector &s_global,
                                     const Scalar &energy_new_level_dep) {
            return ml_derivs_.compute_gradient(level, fun, memory_.x[level], s_global, energy_new_level_dep);
        }
        ...
        bool init_consistency_terms(const SizeType &level, const Scalar & /*energy_fine_level_dep*/) {
            ...
            if (!this->skip_BC_checks()) {
                this->zero_correction_related_to_equality_constrain(this->function(level - 1),
            ...
            bool smoothness_flg =
                this->check_grad_smoothness()
                    ? this->recursion_termination_smoothness(
                          this->ml_derivs_.g_diff[level - 1], this->ml_derivs_.g[level - 1], level - 1)
                    : true;
            ...
            this->memory_.energy[level - 1] = this->get_multilevel_energy(this->function(level - 1), level - 1);
        ...
        template <MultiLevelCoherence T = CONSISTENCY_LEVEL,
                  enable_if_t<is_same<T, FIRST_ORDER_ADDITIVE_MULTIPLICATIVE_DF>::value, int> = 0>
        bool init_consistency_terms(const SizeType &level, const Scalar &energy_fine_level_dep) {
            this->transfer(level - 1).restrict(this->ml_derivs_.g[level], this->ml_derivs_.g_diff_m[level - 1]);
            ...
            if (!this->skip_BC_checks()) {
                this->zero_correction_related_to_equality_constrain(this->function(level - 1),
                                                                    this->ml_derivs_.g_diff_a[level - 1]);
                this->zero_correction_related_to_equality_constrain(this->function(level - 1),
        ...
        bool init_consistency_terms(const SizeType &level, const Scalar & /*energy_fine_level_dep*/) {
            ...
            if (!this->skip_BC_checks()) {
                this->zero_correction_related_to_equality_constrain_mat(this->function(level - 1),
        ...
        template <
            ...
            enable_if_t<is_any<T, FIRST_ORDER, FIRST_ORDER_DF, FIRST_ORDER_MGOPT, SECOND_ORDER_DF>::value, int> = 0>
        ...
        bool init_deriv_loc_solve(const Fun & /*fun*/, const SizeType &level, const LocalSolveType &solve_type) {
            if (!(solve_type == PRE_SMOOTHING && level == this->n_levels() - 1)) {
                if (solve_type == PRE_SMOOTHING || solve_type == COARSE_SOLVE) {
                    // this->get_multilevel_gradient(fun, level); - as grad eval elsewhere
                    this->memory_.gnorm[level] = this->criticality_measure(level);
                }
            }
         ...
         *
         */
        virtual bool update_hessian(const Vector &g_new,
                                    const Vector &g_old,
                                    const Vector &s,
                                    const Matrix &H,
                                    const Scalar &rho,
        ...
        virtual bool update_level(const SizeType & /*level*/, const Scalar & /*energy_new_level_dep*/) { return false; }
        ...
        virtual void init_level(const SizeType &level) { this->memory_.delta[level] = this->memory_.delta[level + 1]; }
        ...
        ///////////////////////////////////////////// CONVERGENCE CHECKS
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        bool check_global_convergence(const SizeType &it,
                                      const Scalar &r_norm,
                                      const Scalar &rel_norm,
                                      const Scalar &delta) {

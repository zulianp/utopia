...
namespace utopia {
    ...
    class MultilevelDerivEval<Matrix, Vector, FIRST_ORDER_ADDITIVE_MULTIPLICATIVE_DF> final {
        using Scalar = typename Traits<Vector>::Scalar;
        using SizeType = typename Traits<Vector>::SizeType;
        using Layout = typename Traits<Vector>::Layout;

    public:
        MultilevelDerivEval(const SizeType &nl_levels) : n_levels_(nl_levels), initialized_(false), gamma_(0.5) {}

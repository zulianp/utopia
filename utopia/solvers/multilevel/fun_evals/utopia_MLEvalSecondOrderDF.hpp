...
namespace utopia {
    ...
    public:
        ...
        public:
            HessianApproxPtrConsistency(const HessianApproxPtr &fine, const HessianApproxPtr &coarse) {
        ...
        public:
            FunctionOperatorDFEval(MultilevelDerivEval &parent,
                                   const Size size,
        ...
        MultilevelDerivEval(const SizeType &nl_levels)
            : n_levels_(nl_levels), initialized_(false), switch_to_second_order_(false) {
        ...
        inline Scalar compute_energy(const SizeType &level,
            ...
            if (level < n_levels_ - 1) {
                if (switch_to_second_order_) {
                    ...
                    energy += (0.5 * dot(help1_[level], s_global)) + dot(g_diff[level], s_global);
                } else {
        ...
        inline bool compute_hessian(const SizeType & /*level*/,
            ...
            utopia::err() << "compute hessian should not be used for matrix free approach ";

            return true;
        }

        std::shared_ptr<FunctionOperatorDFEval> build_apply_H_diff(const SizeType &level) {
            std::function<void(const Vector &, Vector &)> my_func = [this, level](const Vector &x, Vector &result) {
                // std::cout << "------ inside1 ------- \n";
            ...
            return std::make_shared<FunctionOperatorDFEval>(*this, size, local_size, comm, my_func);
        }
        ...
        std::shared_ptr<FunctionOperatorDFEval> build_apply_H_plus_Hdiff(
            ...
            std::function<void(const Vector &, Vector &)> my_func = [this, &apply_B_fun, level](const Vector &x,
                                                                                                Vector &result) {
        ...
        void set_init_approx(const SizeType &level,
            ...
            hessian_approxs_init_[level].coarse_ = std::shared_ptr<HessianApprox>(approx_coarse->clone());
        }

        void switch_to_second_order(const bool &flg) { switch_to_second_order_ = flg; }

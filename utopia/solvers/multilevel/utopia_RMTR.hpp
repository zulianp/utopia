...
class RMTR_l2 final : public RMTRBase<Matrix, Vector, CONSISTENCY_LEVEL> {
 ...
 public:
  ...
  void print_usage(std::ostream &os) const override {
    ...
    this->print_param_usage(os, "coarse-QPSolver", "TRSubproblem",
                            "Input parameters for fine level QP solvers.", "-");
    this->print_param_usage(os, "fine-QPSolver", "TRSubproblem",
  ...
  bool set_tr_strategy(const std::shared_ptr<TRSubproblem> &strategy,
 ...
 private:
  ...
  bool delta_termination(const Scalar &corr_norm, const SizeType &level) {
    return (corr_norm >
            (1.0 - this->eps_delta_termination()) * this->memory_.delta[level])
               ? true
               : false;
  }
  ...
  bool solve_qp_subproblem(const SizeType &level, const bool &flg) override {
    ...
    _tr_subproblems[level]->solve(this->ml_derivs_.H[level],

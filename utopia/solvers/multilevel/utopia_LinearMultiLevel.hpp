...
class LinearMultiLevel : public MultiLevelBase<Matrix, Vector>,
 ...
 public:
  ...
  void print_usage(std::ostream &os) const override {
    ...
    this->print_param_usage(os, "must_generate_masks", "bool",
                            "Flag deciding if masks should be generated.", "-");
  }
  ...
  virtual bool set_transfer_operators(
    ...
    for (auto I = interpolation_operators.begin();
         I != interpolation_operators.end(); ++I)
  ...
  static void fix_semidefinite_operator(Matrix &A) {
...

    {
      ...
      A.read(UTOPIA_LAMBDA(const SizeType &i, const SizeType &j,
                           const Scalar &val) {
   ...
   *
   */
  inline bool set_linear_operators(
      const std::vector<std::shared_ptr<const Matrix>> &A) {
    if (this->n_levels() <= 0) {
  ...
  virtual void update_transfer(const SizeType level,

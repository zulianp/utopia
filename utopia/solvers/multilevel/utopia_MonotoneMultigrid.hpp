...
class MonotoneMultigrid final : public QPSolver<Matrix, Vector> {
  ...
  using VariableBoundSolverInterface =
      utopia::VariableBoundSolverInterface<Vector>;
...
public:
  MonotoneMultigrid(const std::shared_ptr<QPSmoother> &qp_smoother,
      ...
      : algo_(coarse_smoother, coarse_solver), qp_smoother_(qp_smoother),
  ...
  MonotoneMultigrid()
      : algo_(std::make_shared<DefaultSmoother>(),
              std::make_shared<DefaultSmoother>()),
  ...
  void set_transfer_operators(
      const std::vector<std::shared_ptr<Transfer>> &transfer_operators) {
    auto last = transfer_operators.back();

    auto tt = std::dynamic_pointer_cast<TruncatedTransfer>(last);

    if (!tt) {
      Utopia::Abort("MonotoneMultigrid::set_transfer_operators(...) last "
                    "operators must be of type TruncatedTransfer");
    }
  ...
  void init(const std::shared_ptr<QPSmoother> &qp_smoother) {
    qp_smoother_ = qp_smoother;
  }

  inline static std::shared_ptr<TruncatedTransfer>
  new_fine_level_transfer(const std::shared_ptr<Matrix> &interpolation) {
    return std::make_shared<IPTruncatedTransfer<Matrix, Vector>>(interpolation);
  }

  inline static std::shared_ptr<TruncatedTransfer>
  new_coarse_level_transfer(const std::shared_ptr<Matrix> &interpolation) {
  ...
  bool apply(const Vector &rhs, Vector &x) override {
    ...
    std::string mg_header_message =
        "MontoneMultigrid: " + std::to_string(n_levels()) + " levels";
    this->init_solver(mg_header_message, {" it. ", "|| P(x-g)-x ||",
                                          "|| u - u_old ||", "# active sets"});

    Vector x_old = x;
    Vector grad = (*this->get_operator()) * x;
    grad = grad - rhs;
    ...
    for (SizeType it = 1; it < this->max_it() && !converged; ++it) {
      ...
      help_ = x - x_old;
      Scalar x_diff = norm2(help_);

      grad = (*this->get_operator()) * x;
      grad = grad - rhs;
      g_norm = this->criticality_measure_inf(x, grad);

      if (this->verbose()) {
        PrintInfo::print_iter_status(
            it, {g_norm, x_diff, Scalar(this->active_set_->num_active_sets())});
      }

      converged = this->check_convergence(it, g_norm, 1, x_diff);
    ...
    if (use_coarse_space_) {
      if (auto *pgs_QR = dynamic_cast<ProjectedGaussSeidelQR<Matrix, Vector> *>(
        ...
        if (this->verbose()) {
          ...
          x.comm().root_print("n_active: " +
                              std::to_string(SizeType(n_active)));
        }

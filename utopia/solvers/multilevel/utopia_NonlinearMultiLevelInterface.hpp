...
class NonlinearMultiLevelInterface : public MultiLevelBase<Matrix, Vector> {
 public:
  ...
  NonlinearMultiLevelInterface(const SizeType &n_levels) {
  ...
  virtual bool set_functions(const std::vector<FunPtr> &level_functions) {
    ...
    level_functions_.insert(level_functions_.begin(), level_functions.begin(),
                            level_functions.end());
  ...
  virtual bool set_transfer_operators(
    ...
    if (this->n_levels() !=
        static_cast<SizeType>(interpolation_operators.size()) + 1) {
      utopia_error(
  ...
  virtual bool set_transfer_operators(
    ...
    for (auto I = interpolation_operators.begin(),
              ...
              P = projection_operators.begin();
         I != interpolation_operators.end() &&
         R != restriction_operators.end() && P != projection_operators.end();
         ++I, ++R, ++P)
 ...
 protected:
   ...
   */
  virtual bool zero_correction_related_to_equality_constrain(const Fun &fun,
                                                             Vector &c) {
   ...
   *
   */
  virtual bool zero_correction_related_to_equality_constrain_mat(const Fun &fun,
                                                                 Matrix &M) {

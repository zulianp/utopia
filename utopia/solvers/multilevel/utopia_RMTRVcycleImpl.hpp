...
bool RMTRBase<Matrix, Vector, CONSISTENCY_LEVEL>::solve(Vector &x_h) {
  ...
  if (!this->skip_BC_checks()) {
    this->make_iterate_feasible(this->function(fine_level),
                                this->memory_.x[fine_level]);
    this->handle_equality_constraints();
  }

  this->memory_.energy[fine_level] = this->get_multilevel_gradient_energy(
      this->function(fine_level), fine_level, this->memory_.s[fine_level]);
  ...
  if (this->verbosity_level() >= VERBOSITY_LEVEL_NORMAL &&
    ...
    std::string name_id =
        this->name() +
        "     Number of levels: " + std::to_string(fine_level + 1) +
        "   \n Fine level local dofs: " +
        std::to_string(this->local_level_layouts_.back().local_size()) + "/" +
  ...
  while (!converged) {
    ...
    if (this->verbose() && mpi_world_rank() == 0) {
      ...
      if (this->verbosity_level() > VERBOSITY_LEVEL_NORMAL &&
          this->verbose() == true) {
        this->print_init_message("RMTR OUTER SOLVE",
                                 {" it. ", "|| g ||", "   E "});
      }
    ...
    converged = this->check_global_convergence(
        this->_it_global, this->memory_.gnorm[fine_level], 9e9,
        this->memory_.delta[fine_level]);
  }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <class Matrix, class Vector, MultiLevelCoherence CONSISTENCY_LEVEL>
bool RMTRBase<Matrix, Vector, CONSISTENCY_LEVEL>::multiplicative_cycle(
  ...
  if (smoothness_flg) {
    ...
    this->memory_.s[level - 1] =
        this->memory_.x[level - 1] - this->memory_.x_0[level - 1];
    coarse_reduction -= this->memory_.energy[level - 1];

    this->transfer(level - 1).interpolate(this->memory_.s[level - 1],
    ...
    if (this->verbosity_level() >= VERBOSITY_LEVEL_VERY_VERBOSE &&
      ...
      std::string status =
          "RMTR_coarse_corr_stat, level: " + std::to_string(level);
      this->print_init_message(status, {" it. ", "   E_old     ", "   E_new",
                                        "ared   ", "  coarse_level_reduction  ",
                                        "  rho  ", "  delta ", "taken"});
      PrintInfo::print_iter_status(
          this->_it_global,
          {E_old, E_new, ared, coarse_reduction, rho,
           this->memory_.delta[level], Scalar(coarse_corr_taken)});
    }
...
template <class Matrix, class Vector, MultiLevelCoherence CONSISTENCY_LEVEL>
bool RMTRBase<Matrix, Vector, CONSISTENCY_LEVEL>::local_tr_solve(
    const SizeType &level, const LocalSolveType &solve_type) {
  ...
  while (!converged) {
    ...
    if (!std::isfinite(energy_new) || !std::isfinite(rho) ||
        has_nan_or_inf(this->memory_.x[level])) {
      rho = 0.0;
    }
    ...
    //----------------------------------------------------------------------------
    delta_converged =
        this->delta_update(rho, level, this->memory_.s_working[level]);
    ...
    converged = (delta_converged == true)
                    ? true
                    : this->check_local_convergence(it, it_success, level,
                                                    this->memory_.delta[level],
                                                    solve_type);

    if (level == this->n_levels() - 1) {
      converged =
          (converged == true || this->memory_.gnorm[level] < this->atol())
              ? true
              : false;
    }
    UTOPIA_NO_ALLOC_END();
  ...
  bool level_quit = ((this->criticality_measure_termination(level) == true) ||
                     delta_converged)
                        ? true
                        : false;

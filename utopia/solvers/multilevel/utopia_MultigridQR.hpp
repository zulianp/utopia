...
class MultigridQR final : public LinearMultiLevel<Matrix, Vector>,
 ...
 public:
  ...
  void print_usage(std::ostream &os) const override {
    ...
    this->print_param_usage(os, "perform_galerkin_assembly", "bool",
                            "Flag turning on/off galerkin assembly.", "true");
    this->print_param_usage(
        os, "use_line_search", "bool",
        "Flag turning on/off line-search after coarse grid correction.",
        "false");
    this->print_param_usage(os, "block_size", "int", "Block size for systems.",
                            "1");
    // this->print_param_usage(os, "smoother", "Smoother", "Input parameters for
    // all smoothers.", "-");
    this->print_param_usage(os, "coarse_solver", "LinearSolver",
                            "Input parameters for coarse solver.", "-");
  }
  ...
  void set_smoother(const std::shared_ptr<Smoother> &smoother,
    ...
    if (smoothers_.size() - 1 < level) {
      utopia_error(
          "MultigridQR:: set_smoother, smoothers array was not allocated.");
    }
  ...
  bool apply(const Vector &rhs, Vector &x_fine) override {
    ...
    std::string mg_header_message =
        "Multigrid: " + std::to_string(L) + " levels";
    this->init_solver(mg_header_message,
                      {" it. ", "|| r_N ||", "||x_old - x_new||"});
 ...
 private:
  bool standard_cycle(const SizeType &l) {
    ...
    for (SizeType k = 0; k < this->mg_type(); k++) {
      ...
      if (l == this->n_levels() - 1) {
        ...
        smoothing_fine(l, memory.rhs[l], memory.x[l],
                       this->pre_smoothing_steps(), true);
      } else if (l > 0) {
      ...
      if (l == this->n_levels() - 1) {
        smoothing_fine(l, memory.rhs[l], memory.x[l],
                       this->post_smoothing_steps(), false);
      } else if (l > 0) {
   ...
   * @return
   */
  inline bool smoothing(const SizeType l, const Vector &rhs, Vector &x,
                        const SizeType &nu = 1) {
  ...
  inline bool smoothing_fine(const SizeType l, const Vector &rhs, Vector &x,
                             const SizeType &nu = 1,
                             const bool &pre_sm = false) {
    assert(l != this->n_levels() - 2);

    if (auto *GS_smoother =
            dynamic_cast<ProjectedGaussSeidelQR<Matrix, Vector> *>(
                smoothers_[l].get())) {
    ...
    if (pre_sm) {
      if (auto *trunc_transfer =
        ...
        utopia_error(
            "MG_QR: requires IPRTruncatedTransfer for the finest level.");
      }
    }
    return true;
  }
  ...
  MultigridQR *clone() const override {
    return new MultigridQR(
        ...
        std::shared_ptr<Solver>(coarse_solver_->clone()), this->n_levels());
  }

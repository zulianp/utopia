    ...
    public:
        ...
        void print_usage(std::ostream &os) const override {
            ...
            this->print_param_usage(
                os, "coarse-QPSolver", "QPSolver", "Input parameters for fine level QP solvers.", "-");
            this->print_param_usage(
    ...
    public:  // nvcc requires it to be public when using lambdas
        bool solve_qp_subproblem(const SizeType &level, const bool &flg) override {
            ...
            *ub = active_upper - this->memory_.x[level];
            ...
            this->_tr_subproblems[level]->solve(
                this->ml_derivs_.H[level], this->ml_derivs_.g[level], this->memory_.s[level]);
            UTOPIA_NO_ALLOC_END();
            this->ml_derivs_.g[level] *= -1.0;

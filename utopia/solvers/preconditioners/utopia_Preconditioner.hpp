...
class LambdaOperator final : public Operator<Vector> {
 public:
  ...
  LambdaOperator(Communicator &comm, Size size, Size local_size, Fun fun)
      : comm_(comm),
        size_(std::move(size)),
        local_size_(std::move(local_size)),
        fun_(fun) {}
...
std::unique_ptr<
    LambdaOperator<Vector, std::function<bool(const Vector &, Vector &)>>>
op(typename Traits<Vector>::Communicator &comm, const Size &size,
   const Size &local_size, std::function<bool(const Vector &, Vector &)> f) {
  return utopia::make_unique<
      LambdaOperator<Vector, std::function<bool(const Vector &, Vector &)>>>(
...
class ExprPreconditioner : public Preconditioner<Vector> {
 public:
  ...
  ExprPreconditioner *clone() const override {
...
std::shared_ptr<ExprPreconditioner<Derived, Vector>> make_preconditioner(
    const Expression<Derived> &expr) {
...
class DelegatePreconditioner : public Preconditioner<Vector> {
 public:
  ...
  DelegatePreconditioner *clone() const override {

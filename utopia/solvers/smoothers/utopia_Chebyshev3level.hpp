...
class Chebyshev3level final : public OperatorBasedLinearSolver<Matrix, Vector> {
 ...
 public:
  ...
  void power_method_max_it(const SizeType &power_method_max_it) {
  ...
  void scale_max_eig(const Scalar scale_max_eig) {
    scale_max_eig_ = scale_max_eig;
  }
  Scalar scale_max_eig() const { return scale_max_eig_; }

  void scale_min_eig(const Scalar scale_min_eig) {
    scale_min_eig_ = scale_min_eig;
  }

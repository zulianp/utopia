...
class BiCGStab final : public OperatorBasedLinearSolver<Matrix, Vector> {
 public:
  ...
  inline bool solve(const Operator<Vector> &A, const Vector &b,
                    Vector &x) override {
 ...
 private:
  bool solve_preconditioned(const Operator<Vector> &A, const Vector &b,
                            Vector &x);
  bool solve_unpreconditioned(const Operator<Vector> &A, const Vector &b,
                              Vector &x);

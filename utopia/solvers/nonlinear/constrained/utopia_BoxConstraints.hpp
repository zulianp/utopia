...
template <class Vector> class BoxConstraints : public Clonable {
public:
  using Scalar = typename Traits<Vector>::Scalar;
  using SizeType = typename Traits<Vector>::SizeType;
  using Layout = typename Traits<Vector>::Layout;

  BoxConstraints(std::shared_ptr<Vector> lower_bound,
                 std::shared_ptr<Vector> upper_bound)
  ...
  BoxConstraints(const Scalar &min_value, const Scalar &max_value,
                 const Layout &lo)
  ...
  BoxConstraints(const BoxConstraints &other)
      : min_val_(other.min_val_), max_val_(other.max_val_),
  ...
  void set(const std::shared_ptr<Vector> &lower_bound,
  ...
  inline bool has_empty_bounds() const {
    return !has_lower_bound() || !has_upper_bound();
  }

  inline bool valid(const Layout &lo) {
    return !has_empty_bounds() && lo.same(layout(*lower_bound())) &&
           lo.same(layout(*upper_bound()));
  }
...
inline BoxConstraints<Vector>
make_box_constaints(const std::shared_ptr<Vector> &lower_bound,
                    const std::shared_ptr<Vector> &upper_bound) {
...
template <class Vector>
inline BoxConstraints<Vector>
make_lower_bound_constraints(const std::shared_ptr<Vector> &lower_bound) {
  return BoxConstraints<Vector>(lower_bound, nullptr);
}

template <class Vector>
inline BoxConstraints<Vector>
make_upper_bound_constraints(const std::shared_ptr<Vector> &upper_bound) {
  return BoxConstraints<Vector>(nullptr, upper_bound);
}

template <class Vector>
inline BoxConstraints<Vector>
make_uniform_box_constraints(const typename Traits<Vector>::Scalar &min_value,

...
class VariableBoundSolverInterface : virtual public MemoryInterface<Vector> {
 ...
 public:
  ...
  VariableBoundSolverInterface(const VariableBoundSolverInterface &other)
  ...
  virtual const Vector &get_upper_bound() const {
    if (!constraints_.has_upper_bound()) {
      utopia_error(
          "VariableBoundSolverInterface::upper bound does not exist. \n");
    }
  ...
  virtual const Vector &get_lower_bound() const {
    if (!constraints_.has_lower_bound()) {
      utopia_error(
          "VariableBoundSolverInterface::lower bound does not exist. \n");
    }
  ...
  virtual bool has_empty_bounds() const {
    return constraints_.has_empty_bounds();
  }
 ...
 public:
  // expose it for CUDA
  bool project(const Vector &x, const Vector &lb, const Vector &ub,
               Vector &Pc) const {
  ...
  virtual const BoxConstraints &merge_pointwise_constraints_with_uniform_bounds(
      const Vector &x_k, const Scalar &lb_uniform, const Scalar &ub_uniform) {
    ...
    if (constraints_.has_upper_bound()) {
      ...
      {
        ub_merged.transform_values(UTOPIA_LAMBDA(const Scalar &ub)->Scalar {
    ...
    if (constraints_.has_lower_bound()) {
      ...
      {
        lb_merged.transform_values(UTOPIA_LAMBDA(const Scalar &lb)->Scalar {
  ...
  virtual const BoxConstraints &build_correction_constraints(
      const Vector &x_k) {
    correction_constraints_.fill_empty_bounds(layout(x_k));

    if (constraints_.has_upper_bound()) {
      *correction_constraints_.upper_bound() =
          *constraints_.upper_bound() - x_k;
    } else {
      correction_constraints_.upper_bound()->set(9e12);
    }

    if (constraints_.has_lower_bound()) {
      *correction_constraints_.lower_bound() =
          *(constraints_.lower_bound()) - x_k;
    } else {

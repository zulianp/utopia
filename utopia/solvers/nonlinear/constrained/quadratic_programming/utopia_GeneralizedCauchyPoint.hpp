...
namespace utopia {
    ...
    public:
        ...
        GeneralizedCauchyPoint *clone() const override { return new GeneralizedCauchyPoint(*this); }
        ...
        void print_usage(std::ostream &os) const override {
            ...
            this->print_param_usage(
                os, "memory_size", "int", "Memory (in terms of breakpoints) used during computation.", "5.0");
        }

        void memory_size(const SizeType &m) { cp_memory_ = m; }

        SizeType memory_size() const { return cp_memory_; }

        void update(const Operator<Vector> &A) override {
            const auto A_layout = row_layout(A);
    ...
    private:
        bool aux_solve(const Operator<Vector> &H,
            ...
            while (it < num_uniq_break_points && !converged) {
                ...
                if (f_pp == 0 || !std::isfinite(f_pp)) return true;
        ...
        void get_breakpoints(const Vector &d, const Vector &lb, const Vector &ub, Vector &break_points) const {
            if (empty(break_points) || local_size(break_points) != local_size(d)) break_points.zeros(layout(d));
        ...
        Scalar get_next_break_point(const Vector &sorted_break_points, const SizeType &index) {
...

            {
                ...
                for (SizeType i = rr.begin(); i != rr.end(); ++i) {
                    if (i == index) value = sorted_break_points.get(i);
                }
            }

            {
                ...
                for (SizeType i = rr.begin(); i != rr.end(); ++i) t_help_.set(i, value);
            }

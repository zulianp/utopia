...
class ProjectedGaussSeidel<Matrix, Vector, PETSC>
 ...
 private:
  ...
  void apply_local_sweeps(const Matrix &A, const Vector &r, const Vector &lb,
                          const Vector &ub, Vector &c) const;

  void non_linear_jacobi_step(const Matrix &A, const Vector &b, Vector &x);

  /// residual must be computed outside
  void apply_local_sweeps_unconstrained(const Matrix &A, const Vector &r,
                                        Vector &c) const;

...
class ProjectedConjugateGradient : public QPSolver<Matrix, Vector> {
 public:
  ...
  bool apply(const Vector &b, Vector &x) override {
    if (this->verbose()) {
      this->init_solver("ProjectedConjugateGradient",
                        {" it. ", "|| u - u_old ||"});
    }
    ...
    while (!converged) {
...

      {
        ...
        parallel_for(local_range_device(x), UTOPIA_LAMBDA(const SizeType &i) {
          ...
          Scalar val = 0.;
          if (device::approxeq(elem, ub_view.get(i),
                               device::epsilon<Scalar>()) ||
              device::approxeq(elem, lb_view.get(i),
                               device::epsilon<Scalar>())) {
      ...
      if (iteration % check_s_norm_each == 0 || std::isinf(beta) ||
          std::isnan(beta)) {

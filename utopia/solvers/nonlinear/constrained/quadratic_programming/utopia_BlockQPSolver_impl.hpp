...
BlockQPSolver<Matrix, Vector, PETSC>::BlockQPSolver(
    std::shared_ptr<QPSolver> serial_solver)
...
template <class Matrix, class Vector>
BlockQPSolver<Matrix, Vector, PETSC>
    *BlockQPSolver<Matrix, Vector, PETSC>::clone() const {
...
template <class Matrix, class Vector>
bool BlockQPSolver<Matrix, Vector, PETSC>::smooth(const Vector & /*b*/,
                                                  Vector & /*x*/) {
...
bool BlockQPSolver<Matrix, Vector, PETSC>::apply(const Vector &rhs, Vector &x) {
  ...
  if (rhs.comm().size() == 1) {
    serial_solver_->set_box_constraints(
        BoxConstraints<Vector>(this->lower_bound(), this->upper_bound()));
    return serial_solver_->apply(rhs, x);
  }

  this->init_solver(
      "utopia BlockQPSolver comm.size = " + std::to_string(rhs.comm().size()),
      {" it. ", "|| u - u_old ||"});
...
bool BlockQPSolver<Matrix, Vector, PETSC>::step(const Vector &rhs, Vector &x) {
  ...
  serial_solver_->set_box_constraints(
      BoxConstraints<Vector>(local_lb_, local_ub_));
...
template <class Matrix, class Vector>
void BlockQPSolver<Matrix, Vector, PETSC>::update(
...
template <class Matrix, class Vector>
void BlockQPSolver<Matrix, Vector, PETSC>::global_to_local(const Vector &g,
                                                           Vector &l) {
  if (empty(l)) {
  ...
  parallel_for(l.range_device(), UTOPIA_LAMBDA(const SizeType &i) {
    l_view.set(i, g_view.get(i));
  });
}

template <class Matrix, class Vector>
void BlockQPSolver<Matrix, Vector, PETSC>::local_to_global(const Vector &l,
                                                           Vector &g) {
  if (empty(g)) {
    assert(false);
  ...
  parallel_for(l.range_device(), UTOPIA_LAMBDA(const SizeType &i) {
    g_view.set(i, l_view.get(i));
  });
}

template <class Matrix, class Vector>
void BlockQPSolver<Matrix, Vector, PETSC>::local_to_global_add(const Vector &l,
                                                               Vector &g) {
  if (empty(g)) {
    assert(false);
    m_utopia_error("g must be initialized outside");

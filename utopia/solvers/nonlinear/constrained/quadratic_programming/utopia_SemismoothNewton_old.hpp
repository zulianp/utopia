...
class SemismoothNewton<Matrix, Vector, INVALID_BACKEND> final
 ...
 public:
  ...
  class AdaptiveActiveTol final : public ActiveTol {
   public:
    ...
    AdaptiveActiveTol(const Scalar tol, const Range &r)
        : ActiveTol(tol),
          adaptive_tol_(r.extent(), tol),
          is_active_(r.extent(), false),
          r_begin_(r.begin()) {}
  ...
  SemismoothNewton *clone() const override {
    auto ptr = utopia::make_unique<SemismoothNewton>(
        std::shared_ptr<Solver>(linear_solver_->clone()));
    if (this->has_bound()) {
  ...
  void print_usage(std::ostream &os) const override {
    ...
    this->print_param_usage(os, "active_set_tol", "double",
                            "Numerical tolerance.", "1e-15");
    this->print_param_usage(
        os, "linear_solve_zero_initial_guess", "bool",
        "Flag to reset initial guess for each linear solve.", "true");
  }
  ...
  inline bool apply(const Vector &b, Vector &x) override {
    ...
    assert(x.empty() ||
           (x.size() == b.size() && b.comm().size() == x.comm().size()));
 ...
 private:
  bool check_constraints(const Vector &x, const bool verbose) const {
    ...
    if (this->has_upper_bound()) {
      ...
      each_read(x, [&upper_bound, &constrain_violated, verbose](
                       const SizeType i, const Scalar value) {
        const Scalar upbo = upper_bound.get(i);
        if (value > upbo + 1e-5) {
          constrain_violated = true;
          if (verbose) {
            std::cerr << "[Warning] upper bound violated at: " << i << " "
                      << value << " > " << upbo << std::endl;
          }
        }
      });
    ...
    if (this->has_lower_bound()) {
      ...
      each_read(x, [&lower_bound, &constrain_violated, verbose](
                       const SizeType i, const Scalar value) {
        const Scalar lobo = lower_bound.get(i);
        if (value < lobo - 1e-5) {
          constrain_violated = true;
          if (verbose) {
            std::cerr << "[Warning] lower bound violated at: " << i << " "
                      << value << " < " << lobo << std::endl;
          }
        }
      });
  ...
  static bool check_non_negative(const Vector &lambda, const bool verbose) {
    ...
    each_read(lambda, [&is_non_negative, verbose](const SizeType i,
                                                  const Scalar value) {
      if (value < -1e-5) {
        is_non_negative = false;
        if (verbose) {
          std::cerr << "[Warning] negative value at: " << i << " " << value
                    << std::endl;
        }
      }
    });
  ...
  bool single_bound_solve(const Matrix &A, const Vector &b, Vector &x_new) {
    ...
    if (this->verbose())
      this->init_solver("SEMISMOOTH NEWTON METHOD",
                        {" it. ", "|| u_new - u_old ||"});
    ...
    while (!converged) {
      ...
      if (!linear_solver_->solve(H, sub_g, x_new)) {
        std::cerr
            << "[Error] linear solver did not manage to solve the linear system"
            << std::endl;
        break;
      }
      ...
      if (has_nan_or_inf(x_new)) {
        ...
        std::cerr << "[Error] nan/inf entries in the solution vector"
                  << std::endl;
        converged = false;
        break;
      }
      ...
      if (!check_zero(H * x_new - sub_g)) {
        ...
        std::cerr << "[Error] non-zero residual returned by linear-solver"
                  << std::endl;
        converged = false;

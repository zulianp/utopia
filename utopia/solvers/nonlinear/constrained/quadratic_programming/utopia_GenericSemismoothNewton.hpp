...
template <class Matrix, class Vector, class Constraint,
          int Backend = Traits<Vector>::Backend>
class GenericSemismoothNewton : public IterativeSolver<Matrix, Vector> {
 public:
  ...
  GenericSemismoothNewton(const Constraint &constraint,
  ...
  GenericSemismoothNewton *clone() const override {
    return new GenericSemismoothNewton(
        constraint_, std::shared_ptr<Solver>(linear_solver_->clone()));
  }

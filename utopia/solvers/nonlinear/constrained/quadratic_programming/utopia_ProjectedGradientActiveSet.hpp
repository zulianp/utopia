...
namespace utopia {
    ...
    class ProjectedGradientActiveSet final : public OperatorBasedQPSolver<Matrix, Vector> {
    ...
    public:
        ...
        void print_usage(std::ostream &os) const override {
            ...
            this->print_param_usage(os,
                                    ...
                                    "Input parameters for Generalized Cauchy point solver.",
                                    "-");
        ...
        ProjectedGradientActiveSet *clone() const override { return new ProjectedGradientActiveSet(*this); }
        ...
        bool solve(const Operator<Vector> &A, const Vector &rhs, Vector &sol) override {
            ...
            update(A);
            // FIXME spurious allocation here (result of -1.0 * rhs)
            return aux_solve(A, -1.0 * rhs, sol, box);
        }

        void update(const Operator<Vector> &A) override {
            auto A_layout = utopia::row_layout(A);
        ...
        void set_linear_solver(const std::shared_ptr<Solver> &linear_solver) { linear_solver_ = linear_solver; }

    private:
        bool aux_solve(const Operator<Vector> &H,
                       const Vector &g,
                       Vector &s,
            ...
            if (this->verbose()) this->init_solver("ProjectedGradientActiveSet", {" "});
            ...
            {
                return false;
            } else if (size(feasible_set).get(0) ==
                       feasible_variables)  // all variables are feasible, use Newton's method to solve the system
            {
                Vector local_corr;
                ...
                if (const auto *H_matrix = dynamic_cast<const Matrix *>(&H)) {
                    ...
                    if (precond_) precond_->apply(-1.0 * grad_qp_fun, local_corr);
                }

                Vector lb_new = *lb - s;
                Vector ub_new = *ub - s;

                // alternative solution is to do full Newton step and then make iterate feasible by projecting back to
                // the boundary
        ...
        void projected_cg(const Operator<Vector> &H,
                          const Vector &grad,
                          const Vector &feasible_set,
                          const Vector &ub,
                          const Vector &lb,
                          Vector &x) {
            ...
            if (d_norm < this->stol() || !std::isfinite(d_norm)) return;
            ...
            while (!converged) {
                ...
                converged = (it > this->max_it() || r_norm < std::min(0.1, std::sqrt(r_norm0)) * r_norm0 ||
                             alpha < 1e-12 || betta < 1e-12)
                                ? true
                                : false;
                ...
                if (this->verbose()) PrintInfo::print_iter_status(it, {alpha, alpha_max, betta, r_norm});
            }
        }
        ...
        void set_GCP_memory(const SizeType &m) { cp_.set_memory_size(m); }
    ...
    private:
        SizeType get_local_size_feasible_set(const Vector &feasible_set) const {
            ...
            each_read(feasible_set, [&local_feasible_set](const SizeType /*i*/, const Scalar value) {
                if (approxeq(value, 1.0)) local_feasible_set++;
            });
        ...
        void build_feasible_set(const Vector &x, const Vector &ub, const Vector &lb, Vector &feasible_set) const {
            if (empty(feasible_set) || local_size(feasible_set) != local_size(x)) feasible_set.zeros(layout(x));
        ...
        void build_active_set(const Vector &x, const Vector &ub, const Vector &lb, Vector &active_set) const {
            if (empty(active_set) || local_size(active_set) != local_size(x)) active_set.zeros(layout(x));
            {
                Read<Vector> r_ub(ub), r_lb(lb), r_x(x);
        ...
        Scalar compute_alpha_max(const Vector &x, const Vector &lb, const Vector &ub, const Vector &d) const {
            Vector alpha_stars;
            alpha_stars.values(layout(x), 1.0);

            {
                ...
                for (SizeType i = rr.begin(); i != rr.end(); ++i) {
                    ...
                    if (di != 0.0) val = (di > 0) ? (ui - xi) / di : (li - xi) / di;

                    // checks for nans and infs, also alpha needs to be positive...
                    if (val < 1.0 && std::isfinite(val) && val > 0.0) alpha_stars.set(i, val);
                }
            }

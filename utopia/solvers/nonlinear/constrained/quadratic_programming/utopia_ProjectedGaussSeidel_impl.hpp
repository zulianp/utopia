...
template <class Matrix, class Vector>
ProjectedGaussSeidel<Matrix, Vector, PETSC>::ProjectedGaussSeidel(
    const ProjectedGaussSeidel &other)
...
template <class Matrix, class Vector>
ProjectedGaussSeidel<Matrix, Vector, PETSC>
    *ProjectedGaussSeidel<Matrix, Vector, PETSC>::clone() const {
...
template <class Matrix, class Vector>
void ProjectedGaussSeidel<Matrix, Vector, PETSC>::print_usage(
    std::ostream &os) const {
  QPSolver<Matrix, Vector>::print_usage(os);

  this->print_param_usage(os, "use_line_search", "bool",
                          "Determines if line-search should be used.", "true");
  this->print_param_usage(os, "use_symmetric_sweep", "bool",
                          "Determines if symmetric local should be used.",
                          "true");
  this->print_param_usage(os, "n_local_sweeps", "int",
                          "Number of local sweeps.", "3");
}

template <class Matrix, class Vector>
bool ProjectedGaussSeidel<Matrix, Vector, PETSC>::smooth(const Vector &b,
                                                         Vector &x) {
...
template <class Matrix, class Vector>
bool ProjectedGaussSeidel<Matrix, Vector, PETSC>::apply(const Vector &b,
                                                        Vector &x) {
  UTOPIA_TRACE_REGION_BEGIN("ProjectedGaussSeidel::apply");

  if (this->verbose()) {
    if (l1_) {
      this->init_solver("utopia ProjectedL1GaussSeidelNew",
                        {" it. ", "|| u - u_old ||"});
    } else {
      this->init_solver("utopia ProjectedGaussSeidel",
                        {" it. ", "|| u - u_old ||"});
    }
  }
...
template <class Matrix, class Vector>
void ProjectedGaussSeidel<Matrix, Vector, PETSC>::non_linear_jacobi_step(
...
void ProjectedGaussSeidel<
    Matrix, Vector, PETSC>::apply_local_sweeps_unconstrained(const Matrix &A,
                                                             const Vector &r,
                                                             Vector &c) const {
  // reset correction
  c.set(0.0);
  ...
  if (use_sweeper_) {
    ...
    /////////////////////////////////////////////
    auto sweeper = [&](const SizeType &i, const SizeType &j,
                       const Scalar &a_ij) {
      if (i != prev_i) {
        // next
...
void ProjectedGaussSeidel<Matrix, Vector, PETSC>::apply_local_sweeps(
    const Matrix &A, const Vector &r, const Vector &lb, const Vector &ub,
    Vector &c) const {
...
template <class Matrix, class Vector>
bool ProjectedGaussSeidel<Matrix, Vector, PETSC>::unconstrained_step(
    const Matrix &A, const Vector &b, Vector &x) {
  r = A * x;
  r = b - r;
...
template <class Matrix, class Vector>
bool ProjectedGaussSeidel<Matrix, Vector, PETSC>::step(const Matrix &A,
                                                       const Vector &b,
                                                       Vector &x) {
  r = A * x;
  r = b - r;
...
template <class Matrix, class Vector>
void ProjectedGaussSeidel<Matrix, Vector, PETSC>::init_memory(
    const Layout &layout) {
...
void ProjectedGaussSeidel<Matrix, Vector, PETSC>::init(const Matrix &A) {
  ...
  if (use_sweeper_) {
    if (!sweeper_) {
      sweeper_ =
          utopia::make_unique<ProjectedGaussSeidelSweep<Scalar, SizeType> >();
    }
    ...
    if (l1_) {
      ...
      A.read(UTOPIA_LAMBDA(const SizeType &i, const SizeType &,
                           const Scalar &value) {
...
template <class Matrix, class Vector>
void ProjectedGaussSeidel<Matrix, Vector, PETSC>::update(

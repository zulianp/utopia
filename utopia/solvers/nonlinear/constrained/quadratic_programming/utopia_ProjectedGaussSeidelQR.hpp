...
class ProjectedGaussSeidelQR final
    : public ProjectedGaussSeidel<Matrix, Vector> {
 public:
  ...
  ProjectedGaussSeidelQR() {
    ...
    if (mpi_world_size() > 1) {
      utopia_error(
          "ProjectedGaussSeidelQR does not support parallel computations. \n");
    }
  }
  ...
  const Vector &get_active_set() { return active_set_; }

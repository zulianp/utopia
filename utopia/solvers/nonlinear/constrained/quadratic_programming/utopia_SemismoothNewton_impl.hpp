...
SemismoothNewton<Matrix, Vector, Backend>::SemismoothNewton(
    ...
    : linear_solver_(std::move(linear_solver)),
      details_(utopia::make_unique<Details>()) {}

template <class Matrix, class Vector, int Backend>
SemismoothNewton<Matrix, Vector, Backend>::~SemismoothNewton() = default;

template <class Matrix, class Vector, int Backend>
SemismoothNewton<Matrix, Vector, Backend>
    *SemismoothNewton<Matrix, Vector, Backend>::clone() const {
  auto ptr = utopia::make_unique<SemismoothNewton>(
      std::shared_ptr<LinearSolver>(linear_solver_->clone()));
  if (this->has_bound()) {
...
template <class Matrix, class Vector, int Backend>
void SemismoothNewton<Matrix, Vector, Backend>::print_usage(
...
template <class Matrix, class Vector, int Backend>
bool SemismoothNewton<Matrix, Vector, Backend>::smooth(const Vector &,
                                                       Vector &) {
...
template <class Matrix, class Vector, int Backend>
bool SemismoothNewton<Matrix, Vector, Backend>::apply(const Vector &b,
                                                      Vector &x) {
  ...
  if (this->verbose()) {
    this->init_solver(
        "SemismoothNewton comm.size = " + std::to_string(b.comm().size()),
        {" it. ", "      || x_k - x_{k-1} ||"});
  }
  ...
  while (!converged) {
    ...
    if (!linear_solver_->solve(mat, residual, correction)) {
      utopia::err()
          << "SemismoothNewton: Unable to solve linear system for subgradient"
          << std::endl;

      if (this->details_->fallback_solver) {
        correction.set(0.0);
        if (!this->details_->fallback_solver->solve(mat, residual,
                                                    correction)) {
          utopia::err() << "SemismoothNewton: fallback could not solve"
                        << std::endl;
          assert(false);
          return false;
        }
      ...
      if (norm_rhs < norm_r) {
        utopia::err()
            << "SemismoothNewton: linear solver diverged. Norm residual: "
            << norm_r << " > " << norm_rhs << std::endl;
        assert(false);
        return false;
      }
    }
...
template <class Matrix, class Vector, int Backend>
void SemismoothNewton<Matrix, Vector, Backend>::init_memory(
    const Layout &layout) {
...
template <class Matrix, class Vector, int Backend>
void SemismoothNewton<Matrix, Vector, Backend>::update(

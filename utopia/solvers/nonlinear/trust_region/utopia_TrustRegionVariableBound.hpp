...
class TrustRegionVariableBound final
 ...
 public:
  ...
  bool solve(Function<Matrix, Vector> &fun, Vector &x_k) override {
    ...
    while (!converged) {
      ...
      if (auto *tr_subproblem =
              dynamic_cast<QPSolver *>(this->linear_solver_.get())) {
        ...
        utopia_warning(
            "TrustRegionVariableBound::Set suitable TR subproblem.... \n ");
      }
  ...
  void set_trust_region_strategy(
      const std::shared_ptr<QPSolver> &tr_linear_solver) {

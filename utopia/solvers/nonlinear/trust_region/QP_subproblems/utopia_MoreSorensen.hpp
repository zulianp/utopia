...
class MoreSorensenEigen final : public TRSubproblem<Matrix, Vector> {
 ...
 public:
  ...
  void print_usage(std::ostream &os) const override {
    ...
    this->print_param_usage(os, "kappa_easy", "double",
                            "Treshold for kappa estimation.", "1e-10");
    this->print_param_usage(os, "lambda_eps", "double",
                            "Treshold for min. eigenvalue.", "1e-5");

    this->print_param_usage(os, "linear-solver", "LinearSolver",
                            "Input parameters for linear solver.", "-");
    this->print_param_usage(os, "eigen-solver", "EigenSolver",
                            "Input parameters for eigen solver.", "-");
  }
 ...
 private:
  bool aux_solve(const Matrix &H, const Vector &g, Vector &s_k) {
    ...
    if (s_norm <= this->current_radius()) {
      if (lambda == 0.0 || s_norm == this->current_radius())
        ...
        Scalar alpha = this->quadratic_function(
            1.0, 2.0 * s_k_eigen,
            sk_sk - (this->current_radius() * this->current_radius()));
    ...
    for (auto it = 0; it < this->max_it(); it++) {
      if (std::abs(s_norm - this->current_radius()) <=
          kappa_easy_ * this->current_radius())
        return true;

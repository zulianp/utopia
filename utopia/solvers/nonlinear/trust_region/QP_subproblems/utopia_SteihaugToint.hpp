...
class SteihaugToint final : public OperatorBasedTRSubproblem<Matrix, Vector> {
 ...
 public:
  ...
  void update(const Operator<Vector> &A) override {
    ...
    if (this->precond_) {
      auto ls_ptr =
          dynamic_cast<LinearSolver<Matrix, Vector> *>(this->precond_.get());
      if (ls_ptr) {
 ...
 private:
  bool unpreconditioned_solve(const Operator<Vector> &B, const Vector &g,
                              Vector &corr) {
    r = -1.0 * g;
    v_k = r;
  ...
  bool preconditioned_solve(const Operator<Vector> &B, const Vector &g,
                            Vector &s_k) {
    bool converged = false;
    SizeType it = 0;
    ...
    this->init_solver(" Precond-ST-CG ",
                      {"it. ", "||g||", "||s||", "||p||", "sMp"});
    if (this->verbose()) {
      PrintInfo::print_iter_status(it, {g_norm});
    }
    ...
    if (!std::isfinite(p_norm)) {
      ...
      if (r2 >= g_norm) {
        alpha_termination =
            1.0;  // grad. step is inside of tr boundary, just take it
      } else {
    ...
    while (!converged) {
      ...
      if (s_norm_new >= r2) {
        ...
        if (this->verbose() && mpi_world_rank() == 0) {
          utopia::out()
              << "termination due to correction exceeding TR radius... \n";
        }

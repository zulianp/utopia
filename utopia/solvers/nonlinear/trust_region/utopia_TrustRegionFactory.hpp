...
namespace utopia {
    ...
    public:
        TRStrategyFactory() { static_assert(Backend < HOMEMADE, "TRStrategyFactory not implemented for this backend"); }
    };
     ...
     *
     *
     * 				Note that TR solver can be used in 2 different settings: minimization of the objective function, solving
     * nonlinear eq. \n Setting Function and relative derivatives properly determines which strategy to use. \n Using
     * LeastSquaresFunction helps to solve arising LS in "smart" way, without explicitly forming hessian matrices with
     * doubled conditioned number.  \n Currently, we have available:
     * 				-# <a href="http://www.mcs.anl.gov/~anitescu/CLASSES/2012/LECTURES/S310-2012-lect5.pdf">Cauchy
     * point</a>
     * 				-# <a
     ...
     * 				-# <a href="http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.49.9338&rep=rep1&type=pdf">Generalized
     * Lanczos</a>
     * 				-# <a href= "http://www.ii.uib.no/~trond/publications/papers/trust.pdf"> Toint</a>
     * 				-# <a href="\http://epubs.siam.org/doi/abs/10.1137/0613049?journalCode=sjmael">CGNE</a>, which is CG for
     * normal eq.
    ...
    const SolutionStatus &trust_region_solve(Function<Matrix, Vector> &fun,
                                             Vector &x,
                                             const SolverType &solve_type,
                                             Input &in) {

...
class TrustRegion final : public NewtonBase<Matrix, Vector>,
 ...
 public:
  TrustRegion(const std::shared_ptr<TRSubproblem> &tr_subproblem =
                  std::make_shared<SteihaugToint<Matrix, Vector>>())
...
#define DEBUG_mode
    ...
    if (this->verbose_) {
      this->init_solver(
          "TRUST_REGION_BASE",
          {" it. ", "|| g ||", "r_norm", "<g, dx>", "J_k", "J_{k+1}", "ared",
           "pred", "rho", "delta_k", "|| p_k || "});
    ...
    while (!converged) {
      ...
      if (auto *tr_subproblem =
              dynamic_cast<TRSubproblem *>(this->linear_solver().get())) {
        if (empty(p_k)) {
          p_k = 0.0 * x_k;
        } else {
          p_k.set(0.0);
...
#ifdef DEBUG_mode
      if (this->verbose_)
        PrintInfo::print_iter_status(it, {g_norm, r_norm, product, E_k, E_k1,
                                          ared, pred, rho, delta, s_norm});
#else
      if (this->verbose_)
        PrintInfo::print_iter_status(
            it, {g_norm, r_norm, E_k, E_k1, rho, delta, s_norm});
#endif
  ...
  void set_trust_region_strategy(
      const std::shared_ptr<TRSubproblem> &tr_linear_solver) {
    NonLinearSolver::set_linear_solver(tr_linear_solver);
  }

  void set_linear_solver(const std::shared_ptr<LinearSolver<Matrix, Vector>>
                             & /*linear_solver*/) override {
    utopia_error(
        "TrustRegion:: do not use set_linear solver. Use "

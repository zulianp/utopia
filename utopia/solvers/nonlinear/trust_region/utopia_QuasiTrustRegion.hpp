...
class QuasiTrustRegion final : public TrustRegionBase<Vector>,
    ...
    while (!converged) {
      ...
      if (this->verbose_)
        PrintInfo::print_iter_status(
            it, {g_norm, r_norm, product, E_taken, E_old, E_new, ared, pred,
                 rho, delta, s_norm});
#else
      if (this->verbose_)
        PrintInfo::print_iter_status(
            it, {g_norm, r_norm, E_taken, E_new, rho, delta, s_norm});
#endif

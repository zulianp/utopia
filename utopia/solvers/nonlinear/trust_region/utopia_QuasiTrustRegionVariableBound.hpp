...
class QuasiTrustRegionVariableBound final
 ...
 public:
  ...
  bool solve(FunctionBase<Vector> &fun, Vector &x_k) override {
    ...
    if (this->verbose_) {
      this->init_solver("QUasiTrustRegionVariableBound",
                        {" it. ", "||P_c(x-g)-x||", "J_k", "J_{k+1}", "ared",
                         "pred", "rho", "delta_k", "|| p_k ||_{inf} "});
    ...
    while (!converged) {
      ...
      if (auto *tr_subproblem =
              dynamic_cast<MatrixFreeQPSolver *>(this->linear_solver().get())) {
        p_k.set(0.0);
        auto box = this->merge_pointwise_constraints_with_uniform_bounds(
            x_k, -1.0 * delta, delta);
      ...
      if (this->verbose_) {
        PrintInfo::print_iter_status(
            it, {g_norm, E_old, E_new, ared, pred, rho, delta, s_norm});
      }
  ...
  void set_trust_region_strategy(
      const std::shared_ptr<MatrixFreeQPSolver> &tr_linear_solver) {

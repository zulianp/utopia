 ...
 */
class LeastSquaresTrustRegion
    : public NonLinearLeastSquaresSolver<Matrix, Vector>,
      public TrustRegionBase<Vector> {
  using Scalar = typename utopia::Traits<Vector>::Scalar;
  typedef utopia::LinearSolver<Matrix, Vector> Solver;
  typedef utopia::TRSubproblem<Matrix, Vector> TRSubproblem;
  typedef utopia::NonLinearLeastSquaresSolver<Matrix, Vector>
 ...
 public:
  LeastSquaresTrustRegion(const std::shared_ptr<TRSubproblem> &linear_solver)
      : NonLinearLeastSquaresSolver(linear_solver), TrustRegionBase() {}
  ...
  bool solve(LeastSquaresFunction<Matrix, Vector> &fun, Vector &x_k) override {
    ...
    Scalar it = 0;
    bool rad_flg = false;
    Scalar g_norm, g0_norm, r_norm,
        s_norm = std::numeric_limits<Scalar>::infinity();
...
// print out - just to have idea how we are starting
    ...
    if (this->verbose_) {
      this->init_solver(
          "TRUST_NORMAL_EQ",
          {" it. ", "|| g ||", "r_norm", "<g, dx>", "J_k", "J_{k+1/2}",
           "J_{k+1}", "ared", "pred", "rho", "delta_k", "|| p_k || "});
    ...
    while (!converged) {
      ...
      //----------------------------------------------------------------------------
      if (auto *tr_subproblem =
              dynamic_cast<TRSubproblem *>(this->linear_solver_.get()))
...
#ifdef DEBUG_mode
      if (this->verbose_)
        PrintInfo::print_iter_status({it, g_norm, r_norm, product, E, E_k, E_k1,
                                      ared, pred, rho, delta, s_norm});
#else
      if (this->verbose_)
        PrintInfo::print_iter_status(
            {it, g_norm, r_norm, E, E_k1, rho, delta, s_norm});
#endif

      converged = TrustRegionBase::check_convergence(
          *this, tol, this->max_it(), it, g_norm, r_norm, s_norm, delta);
 ...
 protected:
   ...
   * @return
   */
  void delta_update(const Scalar &rho, const Vector & /*p_k*/, Scalar &delta,
                    const bool /*flg=false*/) override {
    if (rho < this->eta1()) {

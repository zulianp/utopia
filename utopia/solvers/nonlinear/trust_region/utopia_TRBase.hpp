...
class TrustRegionParams : public virtual Configurable {
 ...
 public:
  ...
  void print_usage(std::ostream &os) const override {
    this->print_param_usage(os, "delta_max", "real",
                            "Maximum value of tr. radius.", "1e14");
    this->print_param_usage(os, "delta_min", "real",
                            "Minimum value of tr. radius.", "1e-14");
    this->print_param_usage(os, "delta0", "real",
                            "Initial value of tr. radius.", "1.0");

    this->print_param_usage(os, "gamma1", "real",
                            "Factor use to shrink tr. radius.", "0.5");
    this->print_param_usage(os, "gamma2", "real",
                            "Factor use to enlarge tr. radius.", "2.0");

    this->print_param_usage(os, "eta1", "real",
                            "Threshold for rho to shrink tr. radius.", "0.25");
    this->print_param_usage(os, "eta2", "real",
                            "Threshold for rho to enlarge tr. radius.", "0.75");

    this->print_param_usage(os, "rho_tol", "real",
                            "Threshold for rho to take trial point.", "0.005");
...
class TrustRegionBase : public TrustRegionParams<Vector> {
 ...
 protected:
  ...
  virtual Scalar get_pred(const Vector &g, const Operator<Vector> &B,
                          const Vector &p_k) {
  ...
  virtual bool check_convergence(Monitor<Vector> &monitor,
                                 ...
                                 const SizeType max_it, const SizeType &it,
                                 const Scalar &g_norm, const Scalar &r_norm,
                                 const Scalar &s_norm,
  ...
  \param x_k1           - new iterate
    */
  virtual bool trial_point_acceptance(const Scalar &rho, Scalar &E,
                                      const Scalar &E_k, const Scalar &E_k1,
                                      const Vector &p_k, const Vector &x_k,
                                      Vector &x_k1) {
   ...
   */
  virtual bool trial_point_acceptance(const Scalar &rho, const Vector &x_trial,
                                      Vector &x_k) {
    // good reduction, accept trial point
    if (rho >= this->rho_tol()) {
      x_k = x_trial;
  ...
  \param p_k            - iterate step
    */
  virtual void delta_update(const Scalar &rho, const Vector &p_k,
                            Scalar &radius, const bool inf_flg = false) {
    if (inf_flg == false) {
      if (rho < this->eta1()) {
        radius =
            std::max(Scalar(this->gamma1() * norm2(p_k)), this->delta_min());
      } else if (rho > this->eta2()) {
        Scalar intermediate =
            std::max(Scalar(this->gamma2() * norm2(p_k)), radius);
    ...
    } else  // computing update for L_inf norm
    {
      if (rho < this->eta1()) {
        ...
        Scalar intermediate =
            std::max(Scalar(this->gamma2() * norm_infty(p_k)), radius);
  ...
  virtual void delta_update_inf(const Scalar &rho, const Vector &p_k,
                                Scalar &radius) {
    if (rho >= this->eta2()) {
  ...
  \param radius              - tr. radius
    */
  virtual Scalar delta_init(const Vector &x_k, const Scalar &radius0,
                            bool &rad_flg) {

...
class HessianApproximation : public virtual Clonable,
 ...
 public:
  ...
  virtual bool update(const Vector & /* s  */, const Vector & /* y */,
                      const Vector & /* x */, const Vector & /* g */) = 0;
  ...
  void print_usage(std::ostream &os) const override {
    this->print_param_usage(os, "num_tol", "double", "Numerical tolerance.",
                            "1e-12");
  }
  ...
  std::shared_ptr<FunctionOperator> build_apply_H() {
    std::function<void(const Vector &, Vector &)> my_func =
        [this](const Vector &x, Vector &result) { this->apply_H(x, result); };

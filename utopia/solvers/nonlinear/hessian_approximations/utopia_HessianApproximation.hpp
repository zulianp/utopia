...
namespace utopia {
    ...
    public:
        ...
        public:
            FunctionOperator(HessianApproximation &parent,
        ...
        virtual SizeType memory_size() const { return 0; }
        ...
        virtual bool update(const Vector & /* s  */,
                            const Vector & /* y */,
                            const Vector & /* x */,
                            const Vector & /* g */) = 0;

        virtual bool replace_at_update_inv(const SizeType & /*index*/, const Vector & /*s*/, const Vector & /*y*/) {
            utopia_error("replace_at_update_inv  not implemented") return false;
        }
        ...
        Scalar num_tol() const { return num_tol_; }
        ...
        std::shared_ptr<FunctionOperator> build_apply_Hinv() {
            std::function<void(const Vector &, Vector &)> my_func = [this](const Vector &x, Vector &result) {
        ...
        std::shared_ptr<FunctionOperator> build_compute_uHinvv_dot() {
            std::function<Scalar(const Vector &, const Vector &)> my_func =
                [this](const Vector &x, const Vector &result) { return this->compute_uHinvv_dot(x, result); };
        ...
        std::shared_ptr<FunctionOperator> build_compute_uHv_dot() {
            std::function<Scalar(const Vector &, const Vector &)> my_func =
                [this](const Vector &x, const Vector &result) { return this->compute_uHv_dot(x, result); };

            return std::make_shared<FunctionOperator>(*this, my_func);
        }

        std::shared_ptr<FunctionOperator> build_compute_uHu_dot() {
            std::function<Scalar(const Vector &)> my_func = [this](const Vector &x) {

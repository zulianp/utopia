...
class LBFGS final : public HessianApproximation<Vector> {
 ...
 public:
  ...
  bool update(const Vector &s, const Vector &y, const Vector & /*x*/,
    ...
    if (current_m_ < m_) {
      ...
      if (scaling_tech_ == LBFGSScalingTechnique::FORBENIUS) {
        ...
        std::rotate(SY_point_wise_.begin(), SY_point_wise_.begin() + 1,
                    SY_point_wise_.end());
        std::rotate(YY_point_wise_.begin(), YY_point_wise_.begin() + 1,
                    YY_point_wise_.end());
      }
      // UTOPIA_NO_ALLOC_END();
    }
  ...
  void print_usage(std::ostream &os) const override {
    ...
    this->print_param_usage(os, "memory_size", "int", "History/Memory size.",
                            "5");
  }

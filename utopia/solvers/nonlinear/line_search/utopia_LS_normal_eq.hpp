...
class LeastSquaresNewton final
 ...
 public:
  LeastSquaresNewton(const std::shared_ptr<Solver> &linear_solver =
  ...
  bool solve(LeastSquaresFunction<Matrix, Vector> &fun, Vector &x_k) {
    ...
    Scalar alpha_k, r_norm, r0_norm, rel_norm,
        s_norm = std::numeric_limits<Scalar>::max();
    SizeType it = 0;
    ...
    if (this->verbose_) PrintInfo::print_iter_status(it, {r_norm, 1, 0, 1});
    it++;

...
class SimpleBacktracking final : public LSStrategy<Vector> {
 ...
 public:
  ...
  bool get_alpha(LeastSquaresFunctionBase<Vector> &fun, const Vector &g,
                 const Vector &x, const Vector &d, Scalar &alpha) override {
    return get_alpha_aux(fun, g, x, d, alpha);
  }

  bool get_alpha(FunctionBase<Vector> &fun, const Vector &g, const Vector &x,
                 const Vector &d, Scalar &alpha) override {
    return get_alpha_aux(fun, g, x, d, alpha);
  }

 private:
  template <class FunctionT>
  bool get_alpha_aux(FunctionT &fun, const Vector &g, const Vector &x,
                     const Vector &p_k, Scalar &alpha_k) {
    Scalar E_k, E_k1, g_p;
    ...
    if (this->verbose())
      PrintInfo::print_init("SIMPLE_BACKTRACKING_LS_INNER_ITERATIONS",
                            {" it. ", "|| E_k1 ||"});

    // Wolfe conditions
    while (E_k1 > (E_k + (this->c1() * alpha_k * g_p)) && it < this->max_it() &&
           alpha_k > this->alpha_min()) {
      alpha_k *= this->rho();
      x_k = x + alpha_k * p_k;
      fun.value(x_k, E_k1);
      it++;
 ...
 public:
  void init_memory(const Layout &layout) override {
    if (empty(x_k)) {
      x_k.zeros(layout);
    } else if (!x_k.comm().conjunction(layout.local_size() ==
                                       local_size(x_k).get(0))) {
      x_k.zeros(layout);
    }
  }

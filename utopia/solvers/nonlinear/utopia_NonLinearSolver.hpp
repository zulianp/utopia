...
class NonLinearSolver : public Monitor<Vector>, virtual public Configurable {
 public:
  ...
  void print_usage(std::ostream &os) const override {
    this->print_param_usage(os, "atol", "real", "Absolute tolerance.",
                            std::to_string(atol()));
    this->print_param_usage(os, "rtol", "real", "Relative tolerance.",
                            std::to_string(rtol()));
    this->print_param_usage(os, "stol", "real", "Step size tolerance.",
                            std::to_string(stol()));

    this->print_param_usage(os, "max-it", "int",
                            "Maximum number of iterations.",
                            std::to_string(max_it()));
    this->print_param_usage(os, "verbose", "bool", "Turn on/off output.",
                            std::to_string(verbose_));
    this->print_param_usage(os, "time-statistics", "bool",
                            "Collect time-statistics.",
                            std::to_string(time_statistics_));
  }
 ...
 protected:
   ...
   */
  void init_solver(const std::string &method,
  ...
  virtual void print_init_message(
   ...
   */
  void exit_solver(const SizeType &num_it,
                   const Scalar &convergence_reason) override {
    _time.stop();

    if (mpi_world_rank() == 0 && verbose_) {
      ConvergenceReason::exitMessage_nonlinear(num_it, convergence_reason);
      utopia::out() << "  Walltime of solve: " << _time.get_seconds()
                    << " seconds. \n";
    }
   ...
   */
  bool check_convergence(const SizeType &it, const Scalar &g_norm,
                         const Scalar &r_norm, const Scalar &s_norm) override {
    ...
    // step size so small that we rather exit than wait for nan's
      ...
      this->solution_status_.reason =
          ConvergenceReason::CONVERGED_SNORM_RELATIVE;
      converged = true;
    }
    ...
    // step size so small that we rather exit than wait for nan's
      ...
      this->solution_status_.reason =
          ConvergenceReason::CONVERGED_FNORM_RELATIVE;
      converged = true;
    }
 ...
 public:
  ...
  void time_statistics(const bool &time_statistics_in) {
    time_statistics_ = time_statistics_in;
  };

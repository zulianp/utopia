...
namespace utopia {
    ...
    public:
        ...
        public:
            ...
            Scalar dot(const Vector &left, const Vector &right) const override { return utopia::dot(left, right); }
        ...
        public:
            AHilbertFunction(const std::shared_ptr<LinearSolverT> &linear_solver) : M_inv(linear_solver) {}
            ...
            void transform_gradient(Vector &in, Vector &out) override { M_inv->apply(in, out); }

            Scalar norm(const Vector &x) const override { return std::sqrt(utopia::dot(x, (*M) * x)); }
        ...
        public:
            ASquaredHilbertFunction(const std::shared_ptr<LinearSolverT> &linear_solver) : M_inv(linear_solver) {}
            ...
            Scalar norm(const Vector &x) const override { return utopia::norm2((*M) * x); }
        ...
        MSSolver(const std::shared_ptr<LinearSolverT> &linear_solver)
            ...
            normed_[A_NORM] =
                std::make_shared<AHilbertFunction>(std::shared_ptr<LinearSolverT>(linear_solver->clone()));
            normed_[A_SQUARED_NORM] =
                std::make_shared<ASquaredHilbertFunction>(std::shared_ptr<LinearSolverT>(linear_solver->clone()));
        }

        void set_convex_hull_n_gradients(const SizeType n) { convex_hull_n_gradients_ = n; }

        inline void set_norm_type(const NormType norm_type) { norm_type_ = norm_type; }
        ...
        bool line_search(Function<Matrix, Vector> &fun, const Vector &x, const Vector &dir, Scalar &alpha) {
            x_new = x + alpha * dir;
            Scalar f_x = 0.;
            Scalar f_x_new = 0.;

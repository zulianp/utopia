...
class HilbertFunction {
 public:
  ...
  virtual void gradient(Function<Matrix, Vector> &function, const Vector &x,
                        Vector &gradient) = 0;
...
class IMSConvexHullSolver {
 public:
  ...
  virtual void solve(HilbertFunction<GlobalMatrix, GlobalVector> &normed,
                     const Scalar a_norm2, std::vector<GlobalVector> &gradients,
                     GlobalVector &in_out_b_g) = 0;
};

template <class GlobalMatrix, class GlobalVector, class Matrix, class Vector>
class MSConvexHullSolver final
    : public IMSConvexHullSolver<GlobalMatrix, GlobalVector> {
 public:
  ...
  void solve(HilbertFunction<GlobalMatrix, GlobalVector> &normed,
    ...
    Vector lambda(vec_layout, 0.0), r_k(vec_layout, 0), p_k(vec_layout, 0),
        Cp_k(vec_layout, 0), val_newton_func(vec_layout, 0),
    ...
    lambda.set(
        n_gradients - 1,
        std::min(1., std::max(0., (C.get(n_gradients, n_gradients) -
                                   C.get(n_gradients - 1, n_gradients)) /
                                      (C.get(n_gradients - 1, n_gradients - 1) -
                                       2 * C.get(n_gradients - 1, n_gradients) +
                                       C.get(n_gradients, n_gradients)))));
    ...
    lambda.set(n_gradients,
               1 - lambda.get(n_gradients -
                              1));  // xx is wrong in the alternative!!!!!
    ...
    for (std::size_t i = 0; i < n_gp1; i++) {
      ...
      val_newton_func.set(
          n_gp1, lambda.get(i) + val_newton_func.get(n_gp1));  // xx better?
      for (std::size_t j = 0; j < n_gp1; j++) {
        val_newton_func.set(i, C.get(i, j) * lambda.get(j) +
                                   val_newton_func.get(i));  // xx better?
      }
      val_newton_func.set(
          i + n_gp2,
          lambda.get(i) - std::max(0., lambda.get(i) - lambda.get(i + n_gp2)));
    ...
    while (dot(val_newton_func, val_newton_func) > .000000000001 &&
           counter1 <
               500) {  // Begin Semismooth Newton Method to solve F(lambda)=0
      ...
      p_k = r_k;
      res_k = dot(r_k, r_k);
      while (res_k > .000000000001 &&
             counter2 < 500)  // Begin loop of the CG-method xx
      {
        counter2++;
      ...
      for (std::size_t i = 0; i < n_gp1; i++) {
        ...
        val_newton_func.set(
            i + n_gp2, lambda.get(i) -
                           std::max(0., lambda.get(i) - lambda.get(i + n_gp2)));
        if (lambda.get(i) > lambda.get(i + n_gp2)) {
          C.set(i + n_gp2, i, 0);
          C.set(i + n_gp2, i + n_gp2, 1);
        } else {
 ...
 public:
  ...
  MSSolver(const std::shared_ptr<LinearSolverT> &linear_solver)
      : NewtonBase<Matrix, Vector>(linear_solver),
        ...
        delta_prime_(0.35),
        radius0_(3.),
        T1_([this](const Scalar &radius) -> Scalar {
          return radius / radius0_;
        }),
        T2_([](const Scalar &radius) -> Scalar { return 0.35 * radius; }),
        G_([](const Scalar & /*radius*/, const Scalar &radius0) -> Scalar {
          return radius0;
        }),
        norm_type_(L2_NORM),
    ...
    normed_[A_NORM] = std::make_shared<AHilbertFunction>(
        std::shared_ptr<LinearSolverT>(linear_solver->clone()));
    normed_[A_SQUARED_NORM] = std::make_shared<ASquaredHilbertFunction>(
        std::shared_ptr<LinearSolverT>(linear_solver->clone()));
  }
  ...
  bool B(Function<Matrix, Vector> &fun, HilbertFunction &normed,
         const Vector &left_in, const Vector &right_in, const Vector &dir,
         const Scalar &tol, const Scalar & /*step_size*/,  // not needed
         const Scalar &val_left_in, const Scalar &val_right_in, Vector &h_g) {
  ...
  bool line_search(Function<Matrix, Vector> &fun, const Vector &x,
                   const Vector &dir, Scalar &alpha) {
    x_new = x + alpha * dir;
    Scalar f_x = 0.;
    Scalar f_x_new = 0.;
        ...
        while (a_norm > T1_(radiusk)) {
          ...
          if (diff_val < (-delta_ * a_norm * radiusk)) {
            ...
            if (!convex_hull_solver_ || convex_hull_n_gradients_ == 2) {
              ...
              const auto lambda =
                  (n_bg2 - dot_bg_fg) / (n_bg2 + a_norm2 - 2. * dot_bg_fg);
              assert(!is_nan_or_inf(lambda));

              // add small elements to list
              gradients_.back() = (lambda * first_g + (1. - lambda) * b_g);
              assert(!gradients_.back().has_nan_or_inf());
            } else {
              convex_hull_solver_->solve(*normed, a_norm2, gradients_, b_g);
              assert(!b_g.has_nan_or_inf());

              if (static_cast<SizeType>(gradients_.size()) >=
                  convex_hull_n_gradients_) {
                gradients_.erase(gradients_.begin());
              }
            }

            a_norm = normed->norm(gradients_.back());
            assert(!is_nan_or_inf(a_norm));
  ...
  void set_convex_hull_solver(
      const std::shared_ptr<IMSConvexHullSolver<Matrix, Vector>> &solver) {

...
class QuasiNewtonBase : public MatrixFreeNonLinearSolver<Vector> {
 ...
 public:
  ...
  QuasiNewtonBase(const std::shared_ptr<MFSolver> &solver)
  ...
  void print_usage(std::ostream &os) const override {
    ...
    this->print_param_usage(os, "dumping", "double", "Default step size.",
                            "1.0");
    ...
    this->print_param_usage(
        os, "hessian-approx-strategy", "HessianApproxStrategy",
        "Input parameters for hessian-approximation strategy.", "-");
  }

  virtual bool set_hessian_approximation_strategy(
  ...
  virtual void set_linear_solver(
      const std::shared_ptr<MFSolver> &linear_solver) {
  ...
  virtual void update(const Vector &s, const Vector &y, const Vector &x,
                      const Vector &g) {
    if (hessian_approx_strategy_) {
  ...
  virtual void set_line_search_strategy(
      const std::shared_ptr<LSStrategy> &strategy) {
 ...
 protected:
  ...
  inline Scalar get_alpha(FunctionBase<Vector> &fun, const Vector &g,

...
namespace utopia {
    ...
    public:
        RosenbrockTrustRegion(const std::shared_ptr<Solver> &linear_solver,
            ...
            : NewtonBase<Matrix, Vector>(linear_solver), eigen_solver_(eigen_solver), eps_(1e-12), tau_(1e-4) {}

        Scalar eps() const { return eps_; }

        Scalar tau() const { return tau_; }

        void tau(const Scalar &tau) { tau_ = tau; }
        ...
        bool solve(Function<Matrix, Vector> &fun, Vector &x) override {
            ...
            if (this->verbose())
                this->init_solver("RosenbrockTrustRegion",
                                  {" it. ", "|| g ||", " E ", "rho", "lambda", "|| Delta x || "});
            ...
            Matrix I;
            I.identity(square_matrix_layout(layout(x)));
            ...
            if (this->verbose()) PrintInfo::print_iter_status(it, {g_norm, energy_old, 0.0, lambda, 0.0});
            ...
            while (!converged) {
                ...
                if (this->verbose()) PrintInfo::print_iter_status(it, {g_norm, energy, rho, lambda, s_norm});
        ...
        void print_usage(std::ostream &os) const override {
            ...
            this->print_param_usage(
                os, "tau", "real", "Constant defining lower bound on predicted reduction.", "1e-12");
        }

...
class QuadraticFunction final : public Function<Matrix, Vector, Backend> {
 public:
  ...
  QuadraticFunction(const std::shared_ptr<Matrix> &H,
...
class QuadraticExtendedFunction final
    : public ExtendedFunction<Matrix, Vector> {
 public:
  using SizeType = typename utopia::Traits<Vector>::SizeType;
  using Scalar = typename utopia::Traits<Vector>::Scalar;

  QuadraticExtendedFunction(const Matrix &H, const Vector &rhs,
                            const Vector &x_0, const Vector &bc_marker,

 ...
 * @tparam     Vector
 */
template <class Vector> class FunctionBase : public Configurable {
public:
...
public:
  ...
  virtual bool hessian(const Vector & /*point*/, Matrix & /*result*/,
                       Matrix & /*preconditioner*/) const {
    return false;
  }

  /// If both Hessian and gradient are required at the same time use this method
  /// (it can be used to be more efficient)
  virtual bool hessian_and_gradient(const Vector &x, Matrix &H,
                                    Vector &g) const {
    return this->gradient(x, g) && this->hessian(x, H);
  }

  virtual bool hessian_and_gradient(const Vector &x, Matrix &H,
                                    Matrix &preconditioner, Vector &g) const {
    return this->gradient(x, g) && this->hessian(x, H, preconditioner);
  }

  virtual bool value_grad(const Vector &x, typename Vector::Scalar &result,
                          Vector &grad) const {
    return this->gradient(x, grad) && this->value(x, result);
  }

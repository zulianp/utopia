    ...
    public:
        ...
        void initialize() override {
            ...
            for (SizeType i = 0; i < n_instances; ++i) {
                ...
                this->register_experiment("allocation_" + std::to_string(i), [ml]() {
                    Matrix A;
                    A.sparse(ml, 3, 2);
                });

                // measure assembly time of the operator
                this->register_experiment("assembly_" + std::to_string(i), [ml]() {
                    Matrix A;
                    A.sparse(ml, 3, 2);
                ...
                // matrix-vector mult
                this->register_experiment("mat_mat_mult_" + std::to_string(i), [ml]() {
                    Matrix A;
                    A.sparse(ml, 3, 2);
                    assemble_laplacian_1D(A);
                    // matrix copy

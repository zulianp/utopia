...
namespace utopia {
    ...
    Binary<Multiply<Tensor<L, 2>, Tensor<R, 1>>, Tensor<Left, 1>, Plus> operator+(
        const Tensor<Left, 1> &left,
        const Multiply<Tensor<L, 2>, Tensor<R, 1>> &right) {
     ...
     */
    template <class Derived>
    auto operator/(const typename Derived::Scalar &left, const Expression<Derived> &expr)
        -> decltype(transform(expr, Reciprocal<typename Derived::Scalar>(left))) {
        return transform(expr, Reciprocal<typename Derived::Scalar>(left));
    }

    template <class LDerived, class RDerived>
    Binary<LDerived, RDerived, Divides> operator/(const Expression<LDerived> &left, const Expression<RDerived> &right) {
    ...
    Binary<Number<typename Expr::Scalar>, Reduce<Expr, Operation>, Divides> operator/(
        const typename Expr::Scalar &left,
        const Reduce<Expr, Operation> &right) {
        return Binary<Number<typename Expr::Scalar>, Reduce<Expr, Operation>, Divides>(left, right);
    }

    template <class Derived, class Expr, class Operation>
    Binary<Derived, Reduce<Expr, Operation>, Divides> operator/(const Expression<Derived> &left,
                                                                const Reduce<Expr, Operation> &right) {
        return Binary<Derived, Reduce<Expr, Operation>, Divides>(left.derived(), right);
    }

    template <class Expr, int Type>
    Binary<Number<typename Expr::Scalar>, Norm<Expr, Type>, Divides> operator/(const typename Expr::Scalar &left,
                                                                               const Norm<Expr, Type> &right) {
        return Binary<Number<typename Expr::Scalar>, Norm<Expr, Type>, Divides>(left, right);
    }

    template <class LExpr, class LOp, class RExpr, class ROp>
    Binary<Reduce<LExpr, LOp>, Reduce<RExpr, ROp>, Divides> operator/(const Reduce<LExpr, LOp> &left,
                                                                      const Reduce<RExpr, ROp> &right) {
        return Binary<Reduce<LExpr, LOp>, Reduce<RExpr, ROp>, Divides>(left, right);
    }
    ...
    inline Binary<Left, Right, EMultiplies> e_mul(const Expression<Left> &left, const Expression<Right> &right) {
        return Binary<Left, Right, EMultiplies>(left.derived(), right.derived());
    }
     ...
     */
    template <class Left, class Right, int Order>
    inline Binary<Left, Number<Right>, Min> min(const Expression<Left> &left,
                                                const Factory<Values<Right>, Order> &right) {
     ...
     */
    template <class Left, class Right, int Order>
    inline Binary<Left, Number<Right>, Min> min(const Factory<Values<Right>, Order> &right,
     ...
     */
    template <class Left, class Right, int Order>
    inline Binary<Left, Number<Right>, Max> max(const Expression<Left> &left,
                                                const Factory<Values<Right>, Order> &right) {
     ...
     */
    template <class Left, class Right, int Order>
    inline Binary<Left, Number<Right>, Max> max(const Factory<Values<Right>, Order> &right,
                                                const Expression<Left> &left) {
        return Binary<Left, Number<Right>, Max>(left.derived(), right.type().value());
    }

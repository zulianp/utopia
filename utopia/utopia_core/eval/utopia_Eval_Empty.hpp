...
namespace utopia {
    ...
    public:
        inline static const Derived &apply(const Tensor<Derived, Order> &expr) { return expr.derived(); }

        inline static Derived &apply(Tensor<Derived, Order> &expr) { return expr.derived(); }

        inline static Derived &&apply(Tensor<Derived, Order> &&expr) { return std::move(expr.derived()); }

        inline static void apply(Tensor<Derived, Order> &&expr, Derived &result) { result = std::move(expr.derived()); }

        inline static void apply(const Tensor<Derived, Order> &expr, Derived &result) { result = expr.derived(); }
    };
    ...
    public:
        inline static const Derived &apply(const Tensor<const Derived &, Order> &expr) { return expr.derived(); }
    };

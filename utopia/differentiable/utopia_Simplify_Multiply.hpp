...
namespace utopia {
    ...
    public:
        ...
        inline static const Type &make(const Type &expr) { return expr; }
    };
    ...
    public:
        ...
        static UTOPIA_STORE_CONST(Type) make(const Multiply<Factory<Zeros, Order>, Expr> &expr) { return expr.left(); }
    };
    ...
    public:
        ...
        static UTOPIA_STORE_CONST(Type) make(const Multiply<Left, Right> &expr) {
            return Sim::make(utopia::Simplify<Left>::make(expr.left()) * utopia::Simplify<Right>::make(expr.right()));
        }
    };
}  // namespace utopia

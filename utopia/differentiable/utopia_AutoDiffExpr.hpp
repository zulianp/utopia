...
namespace utopia {
    ...
    public:
        ...
        static Type make(const Differentiable<Expr> &expr) { return Type(size(expr)); }
    };
    ...
    public:
        using Scalar = typename InnerExpr::Scalar;
        //[new backend concept]
        ...
        static UTOPIA_STORE_CONST(Type) make(const InnerExpr &expr, const Pow2 &) { return diag(Scalar(2.0) * expr); }
    };
    ...
    public:
        ...
        static UTOPIA_STORE_CONST(Type) make(const InnerExpr &expr, const Sqrt &) { return Scalar(.5) / sqrt(expr); }
    };
    ...
    public:
        ...
        using Left = typename Diff::Type;
        using Right = typename AutoDiffExpr<InnerExpr>::Type;
        ...
        static UTOPIA_STORE_CONST(Type) make(const Unary<InnerExpr, Operation> &expr) {
            return utopia::Simplify<ComplexType>::make(ComplexType(
                ...
                Diff::make(expr.expr(), expr.operation()), AutoDiffExpr<InnerExpr>::make(expr.expr())));
        }
    };

...
class TrilinosTest {
 public:
  ...
  void build_rectangular_matrix(const SizeType &n, const SizeType &m,
                                Matrix &mat) const {
    using TraitsT = utopia::Traits<Matrix>;
    // mat  = local_sparse(n, m, 2);
    mat.sparse(layout(comm_, n, m, TraitsT::determine(), TraitsT::determine()),
               2, 2);
  ...
  template <class Matrix>
  void build_rectangular_matrix_2(const SizeType &n, const SizeType &m,
                                  Matrix &mat) const {
    using TraitsT = utopia::Traits<Matrix>;
    // mat  = local_sparse(n, m, 2);
    mat.sparse(layout(comm_, n, m, TraitsT::determine(), TraitsT::determine()),
  ...
  void trilinos_build() {
    auto n = 10;
    TpetraMatrixd m;
    m.sparse(layout(comm_, n, n, Traits::determine(), Traits::determine()), 3,
             2);
  ...
  void trilinos_build_identity() {
    auto n = 10;
    TpetraMatrixd id;
    id.identity(layout(comm_, n, n, Traits::determine(), Traits::determine()),
                1.0);
  ...
  void trilinos_matrix_access() {
    auto n = 10;
    TpetraMatrixd Y;
    Y.sparse(layout(comm_, n, n, Traits::determine(), Traits::determine()), 3,
             2);
  ...
  void trilinos_mat_axpy() {
    auto n = 10;
    TpetraMatrixd X;
    X.sparse(layout(comm_, n, n, Traits::determine(), Traits::determine()), 3,
             2);
  ...
  void trilinos_apply_transpose() {
    ...
    TpetraMatrixd A;
    A.sparse(
        layout(comm_, rows, cols, Traits::determine(), Traits::determine()), 2,
        2);
    ...
    each_read(At_v, [](const SizeType &, const double val) {
  ...
  void trilinos_transpose() {
    ...
    TpetraMatrixd id;
    id.identity(
        layout(comm_, rows, cols, Traits::determine(), Traits::determine()),
        1.0);
  ...
  void trilinos_mm() {
    auto n = 10;
    TpetraMatrixd A;
    A.sparse(layout(comm_, n, n, Traits::determine(), Traits::determine()), 3,
             2);
  ...
  void trilinos_diag() {
    ...
    if (!approxeq(val_3, size(A).get(1) * 1.)) {
      m_utopia_error(
          "diag does not work on for tpetra rectangular matrices in parallel "
          "(nor serial)");
    }
  }
  ...
  void trilinos_diag_rect_matrix() {
    ...
    auto m = 3;
    TpetraMatrixd A;
    A.identity(layout(comm_, n, m, Traits::determine(), Traits::determine()),
               1.0);
    TpetraVectord d;
    d = diag(A);
    const double val = norm1(d);
  ...
  void test_ptap(const int n, const int m) {
    ...
    PetscMatrix A_petsc;
    A_petsc.sparse(
        layout(comm_, n, n, PetscTraits::determine(), PetscTraits::determine()),
        3, 2);
  ...
  void test_mg() {
    ...
    // using IPRTransferT = utopia::IPRTransfer<Matrix, Vector>;

    const static bool verbose = false;
    const static bool use_masks = false;

    using ProblemType = utopia::Poisson1D<Matrix, Vector>;
    MultiLevelTestProblem1D<Matrix, Vector, ProblemType> ml_problem(4, 10,
                                                                    !use_masks);
    // ml_problem.describe();
    ...
    // smoother->set_preconditioner(std::make_shared<
    // InvDiagPreconditioner<Matrix, Vector> >());
    coarse_solver->set_preconditioner(
        std::make_shared<InvDiagPreconditioner<Matrix, Vector>>());
  ...
  void trilinos_test_read() {
    ...
    */
    ...
    Scalar val = 0.0;
    m.read(
        [&](const SizeType &, const SizeType &, const Scalar &v) { val += v; });
  ...
  void trilinos_mg() {
    ...
    // FIXME needs trilinos formats but for the moment lets use petsc's
    {
      ...
      const std::string folder =
          Utopia::instance().get("data_path") + "/laplace/matrices_for_petsc";
      ...
      double acc = 0.0;
      petsc_A.read([&acc](const SizeType &, const SizeType &,
                          const Scalar &val) { acc += std::abs(val); });
  ...
  void trilinos_row_view() {
    TpetraMatrixd A;
    A.sparse(layout(comm_, 4, 4, Traits::determine(), Traits::determine()), 3,
             2);
    assemble_laplacian_1D(A);
  ...
  void trilinos_row_view_and_loops() {
    ...
    SizeType count = 0;
    each_read(P,
              [&count](const SizeType &, const SizeType &, const double val) {
  ...
  void trilinos_read() {
    TpetraMatrixd m;
    auto path = Utopia::instance().get("data_path") +
                "/matrixmarket/gre_343_343_crg.mm";
    bool ok = read(path, m);
    utopia_test_assert(ok);
  }
  ...
  void trilinos_bratu_1D() {
    ...
    auto fun_tpetra =
        std::make_shared<Bratu1D<TpetraMatrixd, TpetraVectord>>(n);
  ...
  void rmtr_test() {
    ...
    auto problem =
        MultiLevelTestProblem1D<Matrix, Vector, ProblemType>(2, 10, true);
    ...
    auto tr_strategy_coarse =
        std::make_shared<utopia::SteihaugToint<Matrix, Vector, HOMEMADE>>();
    tr_strategy_coarse->set_preconditioner(
        std::make_shared<IdentityPreconditioner<Vector>>());
    tr_strategy_coarse->atol(1e-12);
    tr_strategy_coarse->rtol(1e-12);

    auto tr_strategy_fine =
        std::make_shared<utopia::SteihaugToint<Matrix, Vector, HOMEMADE>>();
    tr_strategy_fine->set_preconditioner(
        std::make_shared<IdentityPreconditioner<Vector>>());
    tr_strategy_fine->atol(1e-12);
    tr_strategy_fine->rtol(1e-12);

    auto rmtr =
        std::make_shared<RMTR_l2<Matrix, Vector, GALERKIN>>(problem.n_levels());
  ...
  void trilinos_replace_value() {
    TpetraMatrixd A;
    A.sparse(layout(comm_, 2, 3, Traits::determine(), Traits::determine()), 4,
             4);
  ...
  void trilinos_copy_write() {
...

    {
      ...
      each_read(P, [&P2](const SizeType i, const SizeType j,
                         const double value) { P2.set(i, j, value * 2.); });
    }

    TpetraMatrixd A;
    MultiLevelTestProblem1D<TpetraMatrixd, TpetraVectord,
                            Poisson1D<TpetraMatrixd, TpetraVectord>>
        ml_problem(10, 2);
    // A = *ml_problem.interpolators[0];
    auto transfer = ml_problem.get_transfer();

    if (dynamic_cast<IPRTransfer<TpetraMatrixd, TpetraVectord> *>(
            transfer.back().get()) != nullptr) {
      auto *mat_transfer =
          dynamic_cast<IPRTransfer<TpetraMatrixd, TpetraVectord> *>(
              transfer.back().get());
      A = mat_transfer->I();
    } else {
...

    {
      ...
      each_read(A, [&A2](const SizeType i, const SizeType j, const double &) {
        A2.set(i, j, 1.);
      });
    }
  }
  ...
  void trilinos_matrix_norm() {
    TpetraMatrixd m;
    m.identity(layout(comm_, 10, 10, Traits::determine(), Traits::determine()),
               1.0);
  ...
  void trilinos_belos() {
    {
      m_utopia_warning(
          "TrilinosTest::trilinos_belos commented out because of excpetion. "
          "Fix and remove this fallback.");
      return;
    }

    std::string xml_file =
        Utopia::instance().get("data_path") + "/xml/UTOPIA_belos.xml";

    BelosSolver<TpetraMatrixd, TpetraVectord> solver;
    // solver.read_xml(xml_file);
    solver.import("linear-solver",
                  Utopia::instance().get("data_path") + "/json/belos.json");
  ...
  void trilinos_amesos2() {
    std::string xml_file =
        Utopia::instance().get("data_path") + "/xml/UTOPIA_amesos.xml";
  ...
  void trilinos_transform() {
    ...
    each_transform(
        P, [](const SizeType &, const SizeType j, const double &) -> double {
          return j;
        });
  ...
  void run() {
    ...
    // else {
    //     m_utopia_warning_once("several tests left out for parallel
    //     execution");
    // }

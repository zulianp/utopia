...
namespace utopia 
{
	...
	public:
		...
		UnconstrainedOptimizationBenchmark(const SizeType & n = 200) : n_(n), verbose_(true)
		{
...
// 			);					
	...
	private:
		...
		template<class TRSolver, class Fun>
		static void run_tr(std::vector<std::shared_ptr<Fun> > & test_functions, TRSolver &solver, const std::string & solv_name,  const bool & exp_verbose = false) 
		{
	    	...
	    	for(size_t i =0; i < test_functions.size(); i++)
	    	{
				...
				NonEllipse2D<Matrix, Vector> * fun_poisson2D = dynamic_cast<NonEllipse2D<Matrix, Vector> *>(test_functions[i].get());
				fun_poisson2D->output_to_VTK(x_init, "NonEllipse22D.vtk");

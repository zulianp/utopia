...
class QuasiTRTest : public Benchmark {
 public:
  ...
  explicit QuasiTRTest(const SizeType &n, const bool verbose)
      : n_(n), verbose_(verbose) {
    if (mpi_world_size() == 1) {
      test_functions_parallel_.resize(4);
      test_functions_parallel_[0] =
          std::make_shared<Poisson1D<Matrix, Vector> >(n_);
      test_functions_parallel_[1] =
          std::make_shared<Bratu1D<Matrix, Vector> >(n_);
      test_functions_parallel_[2] =
          std::make_shared<Morebv1D<Matrix, Vector> >(n_ * mpi_world_size());
      test_functions_parallel_[3] =
          std::make_shared<NonEllipse2D<Matrix, Vector> >(n_ *
                                                          mpi_world_size());

      test_functions_.resize(4);
      test_functions_[0] = std::make_shared<Rosenbrock01<Matrix, Vector> >();
      test_functions_[1] =
          std::make_shared<QPTestFunction_2D<Matrix, Vector> >();
      ...
      test_functions_constrained_[0] =
          std::make_shared<Beale05Constrained<Matrix, Vector> >();
      test_functions_constrained_[1] =
          std::make_shared<Rosenbrock21Constrained<Matrix, Vector> >();
    } else {
      test_functions_parallel_.resize(3);
      test_functions_parallel_[0] =
          std::make_shared<Poisson1D<Matrix, Vector> >(n_ * mpi_world_size());
      test_functions_parallel_[1] =
          std::make_shared<Bratu1D<Matrix, Vector> >(n_ * mpi_world_size());
      // Only petsc
      test_functions_parallel_[2] =
          std::make_shared<NonEllipse2D<Matrix, Vector> >(n_ *
                                                          mpi_world_size());
    ...
    test_functions_parallel_constrained_[3] =
        std::make_shared<NonEllipse2D<Matrix, Vector> >(n_ * mpi_world_size(),
                                                        2);
  }
  ...
  void initialize() override {
    this->register_experiment("QuasiNewton_InvApprox_LBFGS", [this]() {
      ...
      auto lsolver =
          std::make_shared<EmptyPrecondMatrixFreeLinearSolver<Vector> >();
      ...
      run_unconstr(this->test_functions_parallel_, solver,
                   "QuasiNewton_InvApprox_LBFGS", this->verbose_);
      run_unconstr(this->test_functions_, solver, "QuasiNewton_InvApprox_LBFGS",
                   this->verbose_);
    });
    ...
    this->register_experiment("QuasiNewton_InvApprox_BFGS", [this]() {
      ...
      run_unconstr(this->test_functions_parallel_, solver,
                   "QuasiNewton_InvApprox_BFGS", this->verbose_);
      run_unconstr(this->test_functions_, solver, "QuasiNewton_InvApprox_BFGS",
                   this->verbose_);
    });
    ...
    this->register_experiment("QuasiTR_LBFGS", [this]() {
      ...
      subproblem->set_preconditioner(
          std::make_shared<IdentityPreconditioner<Vector> >());
      QuasiTrustRegion<Vector> solver(hessian_approx, subproblem);

      run_unconstr(this->test_functions_parallel_, solver, "QuasiTR_LBFGS",
                   this->verbose_);
      run_unconstr(this->test_functions_, solver, "QuasiTR_LBFGS",
                   this->verbose_);
    });
    ...
    this->register_experiment("QuasiTR_LSR1", [this]() {
      ...
      run_unconstr(this->test_functions_parallel_, solver, "QuasiTR_LSR1",
                   this->verbose_);
      run_unconstr(this->test_functions_, solver, "QuasiTR_LSR1",
                   this->verbose_);
    });
    ...
    this->register_experiment("QuasiNewton_Bound_LBFGS", [this]() {
      ...
      run_constrained(this->test_functions_constrained_, solver,
                      "QuasiNewton_Bound_LBFGS", this->verbose_);
      run_unconstr(this->test_functions_, solver, "QuasiNewton_Bound_LBFGS",
                   this->verbose_);
    });
    ...
    this->register_experiment("QuasiTR_Bound_LBFGS", [this]() {
      ...
      run_unconstr(this->test_functions_parallel_, solver,
                   "QuasiNewton_Bound_LBFGS", this->verbose_);
      run_constrained(this->test_functions_parallel_constrained_, solver,
                      "QuasiNewton_Bound_LBFGS", this->verbose_);
    });
    ...
    this->register_experiment("QuasiTR_Bound_LSR1", [this]() {
      ...
      run_unconstr(this->test_functions_parallel_, solver, "QuasiTR_Bound_LSR1",
                   this->verbose_);
      run_constrained(this->test_functions_parallel_constrained_, solver,
                      "QuasiTR_Bound_LSR1", this->verbose_);
    });
  }

 private:
  template <class Fun, class NonlinearSolver>
  static void run_constrained(
      std::vector<std::shared_ptr<Fun> > &test_functions,
      NonlinearSolver &solver, const std::string &solv_name,
    ...
    for (size_t i = 0; i < test_functions.size(); i++) {
      ...
      if (dynamic_cast<ConstrainedTestFunction<Matrix, Vector> *>(
              test_functions[i].get())) {
        auto *fun_constrained =
            dynamic_cast<ConstrainedTestFunction<Matrix, Vector> *>(
                test_functions[i].get());
      ...
      if (exp_verbose && mpi_world_rank() == 0) {
        std::cout << i << std::setw(5 - std::to_string(i).size()) << " : "
                  << test_functions[i]->name() << "_" << dim << std::right
                  << std::setw(60 - std::to_string(dim).size() -
                               test_functions[i]->name().size())
                  << std::right << "its:  " << num_its
                  << std::setw(5 - std::to_string(num_its).size()) << "  \n";
 ...
 private:
  std::vector<
      std::shared_ptr<UnconstrainedExtendedTestFunction<Matrix, Vector> > >
      test_functions_parallel_;
  std::vector<std::shared_ptr<UnconstrainedTestFunction<Matrix, Vector> > >
      test_functions_;

  std::vector<std::shared_ptr<ConstrainedTestFunction<Matrix, Vector> > >
      test_functions_constrained_;
  std::vector<
      std::shared_ptr<ConstrainedExtendedTestFunction<Matrix, Vector> > >

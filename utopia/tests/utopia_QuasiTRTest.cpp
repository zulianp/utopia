...
namespace utopia 
{
	...
	public:
		...
		QuasiTRTest(const SizeType & n = 10, const bool verbose = false): n_(n), verbose_(verbose)
		{
			...
			test_functions_parallel_constrained_[0] = std::make_shared<Poisson1D<Matrix, Vector> >(n_*mpi_world_size(), 2);
			test_functions_parallel_constrained_[1] = std::make_shared<Poisson1D<Matrix, Vector> >(n_*mpi_world_size(), 3);

			// only petsc 
			test_functions_parallel_constrained_[2] = std::make_shared<Membrane2D<Matrix, Vector> >(n_*mpi_world_size());
			test_functions_parallel_constrained_[3] = std::make_shared<NonEllipse2D<Matrix, Vector> >(n_*mpi_world_size(), 2);				
		}
		...
		void initialize() override
		{
			...
			this->register_experiment("QuasiTR_Bound_LBFGS",
				[this]() {
					...
					run_constrained(this->test_functions_parallel_constrained_, solver, "QuasiNewton_Bound_LBFGS", this->verbose_);
				}
			);

			this->register_experiment("QuasiTR_Bound_LSR1",
				[this]() {
	                auto hessian_approx   = std::make_shared<LSR1<Vector> >(7);
	                auto qp_solver = std::make_shared<MPGRP<Matrix, Vector> >();
	                qp_solver->max_it(1000);
					...
					run_constrained(this->test_functions_parallel_constrained_, solver, "QuasiTR_Bound_LSR1", this->verbose_);
				}
			);			
	...
	private:
		...
		static void run_constrained(std::vector<std::shared_ptr<Fun> > & test_functions, NonlinearSolver &solver, const std::string & solv_name,  const bool & exp_verbose = false) 
		{
			InputParameters in;
			in.set("atol", 1e-5);
			in.set("rtol", 1e-11);
		...
		template<class Fun, class NonlinearSolver>
		static void run_unconstr(std::vector<std::shared_ptr<Fun> > & test_functions, NonlinearSolver &solver, const std::string & solv_name,  const bool & exp_verbose = false) 
		{
			InputParameters in;
			in.set("atol", 1e-5);
			in.set("rtol", 1e-11);
	...
	private:
		...
		std::vector<std::shared_ptr<ConstrainedExtendedTestFunction<Matrix, Vector> > >  test_functions_parallel_constrained_;

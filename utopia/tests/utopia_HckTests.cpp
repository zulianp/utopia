...
class HckTests {
 public:
  ...
  explicit HckTests(const SizeType &n, const SizeType &n_levels,
                    const Scalar &lambda, const bool verbose,
  ...
  void QP_solve(QPSolverTemp &qp_solver) const {
    ...
    if (dynamic_cast<QPSolver<Matrix, Vector> *>(qp_solver.get())) {
      ...
      qp_box->set_box_constraints(
          make_box_constaints(make_ref(lb), make_ref(ub)));
  ...
  void Poisson_test() {
    ...
    auto subproblem =
        std::make_shared<utopia::KSP_TR<Matrix, Vector>>("stcg", "lu", false);
    ...
    PetscMultilevelTestProblem<Matrix, Vector, Poisson3D<Matrix, Vector>>
    ...
    auto direct_solver =
        std::make_shared<utopia::RedundantLinearSolver<Matrix, Vector>>();
  ...
  void RMTR_unconstrained() {
    PetscMultilevelTestProblem<Matrix, Vector, Bratu2D<Matrix, Vector>>
        multilevel_problem(2, n_levels_, n_);
    ...
    auto tr_strategy_coarse =
        std::make_shared<utopia::KSP_TR<Matrix, Vector>>("stcg", "lu", true);
    auto tr_strategy_fine =
        std::make_shared<utopia::Lanczos<Matrix, Vector>>("sor");

    // auto rmtr = std::make_shared<RMTR<Matrix, Vector, SECOND_ORDER>
    // >(n_levels_);
    auto rmtr =
        std::make_shared<RMTR_l2<Matrix, Vector, SECOND_ORDER>>(n_levels_);
  ...
  void RMTR_l2_linear() {
    ...
    // >(n_levels_);
    auto rmtr =
        std::make_shared<RMTR_l2<Matrix, Vector, FIRST_ORDER>>(n_levels_);
  ...
  void TR_tril_test() {
    ...
    QuadraticExtendedFunction<PetscMatrix, PetscVector> fun_QP(
        H, g, x_eq, x_bc_marker, empty_rhs);
    ...
    TrustRegionVariableBound<Matrix, Vector> tr_solver(QP_solver);
    tr_solver.set_box_constraints(
        make_box_constaints(make_ref(lb_tril), make_ref(ub_tril)));
    tr_solver.read(input_params_);
    tr_solver.verbose(verbose_);
  ...
  std::shared_ptr<QuadraticExtendedFunction<Matrix2, Vector2>>
  copy_QPfun_to_tril(const Poisson3D<Matrix1, Vector1> &fun) {
    Matrix1 H;
  ...
  template <class Matrix1, class Vector1, class Matrix2, class Vector2>
  std::shared_ptr<Bratu3D<Matrix2, Vector2>> copy_Bratufun_to_tril(
      const Poisson3D<Matrix1, Vector1> &fun, const Scalar &lambda) {
    Matrix1 H;
    Vector1 g, x_eq, x_bc_marker;
    ...
    return std::make_shared<Bratu3D<Matrix2, Vector2>>(
        H_tril, x_eq_tril, x_bc_marker_tril, empty_rhs_tril, lambda, dx, dx,
        dx);
  }

  template <class Matrix1, class Vector1, class ProblemType>
  void get_ML_problem(
      std::vector<std::shared_ptr<Transfer<Matrix1, Vector1>>> &transfers_tril,
      std::vector<std::shared_ptr<ExtendedFunction<Matrix1, Vector1>>>
          &level_functions_tril,
      Vector1 &x_fine) {
    PetscMultilevelTestProblem<PetscMatrix, PetscVector,
                               Poisson3D<PetscMatrix, PetscVector>>
        multilevel_problem(3, n_levels_, n_);
    ...
    for (auto i = 0; i < multilevel_problem.n_levels(); i++) {
      auto *mat_transfer =
          dynamic_cast<MatrixTransfer<PetscMatrix, PetscVector> *>(
              transfers[i].get());

      Matrix I_tril;
      convert(mat_transfer->I(), I_tril);
      transfers_tril.push_back(std::make_shared<MatrixTransfer<Matrix, Vector>>(
          std::make_shared<Matrix>(I_tril)));
    }
    ...
    for (auto i = 0; i < multilevel_problem.n_levels(); i++) {
      ...
      if (std::is_same<ProblemType, Bratu3D<Matrix1, Vector1>>::value) {
        level_functions_tril[i] =
            copy_Bratufun_to_tril<PetscMatrix, PetscVector, Matrix, Vector>(
                *fun_Laplace);
      } else {
        level_functions_tril[i] =
            copy_QPfun_to_tril<PetscMatrix, PetscVector, Matrix, Vector>(
                *fun_Laplace);
      }
    }
  }
  ...
  void RMTR_inf_test() {
    ...
    get_ML_problem<Matrix, Vector, Bratu3D<Matrix, Vector>>(
        transfers_tril, level_functions_tril, x_fine);
  ...
  void RMTR_l2_test() {
    ...
    // level_functions_tril, x_fine);
    get_ML_problem<Matrix, Vector, Poisson3D<Matrix, Vector>>(
        transfers_tril, level_functions_tril, x_fine);

    auto tr_strategy_fine =
        std::make_shared<utopia::SteihaugToint<Matrix, Vector>>();
...
void hck() {
  ...
  HckTests<PetscMatrix, PetscVector>(coarse_dofs, n_levels, 1.0, false, true)
      .run_petsc();
  HckTests<PetscMatrix, PetscVector>(coarse_dofs, n_levels, 1.0, verbose, true)
      .run_trilinos();

#ifdef UTOPIA_WITH_TRILINOS
  HckTests<TpetraMatrixd, TpetraVectord>(coarse_dofs, n_levels, 1.0, verbose,
                                         true)
      .run_trilinos();
#endif
}

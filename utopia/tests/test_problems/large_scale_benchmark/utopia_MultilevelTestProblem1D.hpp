...
namespace utopia {
    ...
    public:
        ...
        void n_dofs(const SizeType &level, const SizeType &dofs_l) { n_dofs_[level] = dofs_l; }

        bool remove_bc() const { return remove_bc_; }

        const std::vector<std::shared_ptr<Transfer<Matrix, Vector> > > &get_transfer() const { return transfers_; }
    ...
    public:
        ...
        MultiLevelTestProblem1D(const SizeType &n_levels,
            ...
            : MultilevelTestProblemBase<Matrix, Vector>(n_levels, n_coarse_elements, remove_bc) {
            ...
            for (SizeType i = 0; i < n_levels - 1; ++i) {
                ...
                Matrix I;
                I.sparse(layout(comm, Traits::decide(), Traits::decide(), n_fine, n_coarse), 2, 2);

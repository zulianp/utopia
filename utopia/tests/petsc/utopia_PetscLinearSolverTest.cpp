...
// FIXME make it work also without mumps
  ...
  void petsc_mg_1D() {
    ...
    MultiLevelTestProblem1D<PetscMatrix, PetscVector,
                            Poisson1D<PetscMatrix, PetscVector>>
        ml_problem(4, 10, true);
  ...
  void petsc_mg_exp() {
    ...
    Multigrid<PetscMatrix, PetscVector, PETSC_EXPERIMENTAL>
        multigrid;  //(smoother, linear_solver);
  ...
  void petsc_bicgstab() {
    PetscMatrix mat;
    mat.identity(layout(comm_, Traits::decide(), Traits::decide(), n_, n_),
                 1.0);
  ...
  void petsc_redundant_test() {
    ...
    auto solver = std::make_shared<
        utopia::RedundantLinearSolver<PetscMatrix, PetscVector>>();
  ...
  void petsc_cg_mg() {
    ...
    ConjugateGradient<PetscMatrix, PetscVector, HOMEMADE>
        cg;  // with the HOMEMADE works in parallel
    ...
    cg.set_preconditioner(
        std::make_shared<InvDiagPreconditioner<PetscMatrix, PetscVector>>());
  ...
  void petsc_superlu_mg() {
    ...
    if (diff > 1e-6) {
      utopia_error(
          "petsc_superlu_mg fails. Known problem that needs to be fixed!");
    }
  ...
  void petsc_factorization() {
    ...
    auto cholesky_factorization =
        std::make_shared<Factorization<PetscMatrix, PetscVector>>();
    cholesky_factorization->set_type(Solver::petsc(),
                                     Solver::lu_decomposition());

    if (cholesky_factorization->solve(A, rhs, x)) {
      double diff = norm2(rhs - A * x);
      if (diff > 1e-6) {
        utopia_error(
            "petsc_factorization fails. Known problem that needs to be fixed!");
      }

...
namespace utopia {
    ...
    public:
        ...
        void interior_point_qp_solver_test() {
            ...
            Matrix H;
            Vector g(layout(b), 0.), delta_x(layout(b), 0.), delta_lambda, delta_s, rd, rp, rs, rs_aff;

            // Missing transfer op
            lambda = A * x - b;
            slack = x - *box.upper_bound();
            slack.e_max(1e-8);
            lambda.e_max(1e-8);
            ...
            for (int k = 0; k < 10; ++k) {
                ...
                delta_s = -e_mul(1. / lambda, rs + e_mul(slack, delta_lambda));
                ...
                buff.set(sigma * mu);
                rs_aff = e_mul(slack, lambda) + e_mul(delta_s, delta_lambda) - buff;
                ...
                delta_s = -e_mul(1. / lambda, rs_aff + e_mul(slack, delta_lambda));

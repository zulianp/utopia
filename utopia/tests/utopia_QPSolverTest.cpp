...
template <class Matrix, class Vector> class QPSolverTest {
public:
  ...
  template <class QPSolver> void run_qp_solver(QPSolver &qp_solver) const {
  ...
  static void create_symm_lapl_test_data(Comm &comm, Matrix &A, Vector &b,
  ...
  void log_barrier_qp_solver_test(const std::string &barrier_function_type,
                                  const bool verbose) {
    auto &&comm = Comm::get_default();
  ...
  void interior_point_qp_solver_test() {
    ...
    linear_solver.set_preconditioner(
        std::make_shared<InvDiagPreconditioner<Matrix, Vector>>());
  ...
  void MG_QR_test() {
    ...
    auto fine_smoother =
        std::make_shared<ProjectedGaussSeidelQR<Matrix, Vector>>();
    fine_smoother->set_R(R); // Monotone

    auto coarse_smoother = std::make_shared<GaussSeidel<Matrix, Vector>>();
    auto direct_solver =
        std::make_shared<Factorization<Matrix, Vector>>("mumps", "lu");
    // MultigridQR<Matrix, Vector> multigrid(fine_smoother, coarse_smoother,
    // direct_solver, num_levels);  // QR
    MonotoneMultigrid<Matrix, Vector> multigrid(fine_smoother, coarse_smoother,
                                                direct_solver); // Monotone

    std::vector<std::shared_ptr<Transfer<Matrix, Vector>>>
        interpolation_operators;
    interpolation_operators.resize(num_levels - 1);
    interpolation_operators[1] =
        std::make_shared<IPTruncatedTransfer<Matrix, Vector>>(
            std::make_shared<Matrix>(QtIh));
    interpolation_operators[0] = std::make_shared<IPTransfer<Matrix, Vector>>(
...
template <class Matrix, class Vector> class PQPSolverTest {
public:
  using Traits = utopia::Traits<Matrix>;
  using Scalar = typename Traits::Scalar;
  using SizeType = typename Traits::SizeType;
  ...
  void MPRGP_DD() {
    ...
    if (true) {
      ...
      SizeType n = 1e3;
      QPSolverTest<Matrix, Vector>::create_symm_lapl_test_data(comm, A, b, box,
                                                               n, true);
...
template <class Matrix, class Vector> class MonotoneMGTest {
public:
  using Traits = utopia::Traits<Matrix>;
  using Scalar = typename Traits::Scalar;
  using SizeType = typename Traits::SizeType;
  ...
  void monotone_amg_test() {
    ...
    MonotoneAlgebraicMultigrid<Matrix, Vector> amg(qp_smoother, fine_smoother,
                                                   direct_solver, agglomerator);
    amg.set_n_levels(n_levels);

    using ProblemType = utopia::Poisson1D<Matrix, Vector>;
    MultiLevelTestProblem1D<Matrix, Vector, ProblemType> ml_problem(
        n_levels, n_coarse, !use_masks);
  ...
  void monotone_mg_test() {
    ...
    auto direct_solver = std::make_shared<Factorization<Matrix, Vector>>();
    // auto direct_solver = std::make_shared<ProjectedGaussSeidel<Matrix,
    // Vector>>();
    ...
    auto t = std::static_pointer_cast<MatrixTransfer<Matrix, Vector>>(
        transfers[n_levels - 2]);
    interpolation_operators[n_levels - 2] =
        multigrid.new_fine_level_transfer(std::make_shared<Matrix>(t->I()));

    // std::make_shared<IPRTruncatedTransfer<Matrix,
    // Vector>>(std::make_shared<Matrix>(t->I()));
    ...
    // avoids flip-floping of active nodes (and Galerkin assembly when nothing
    // changes)
...
static void qp_solver() {
  ...
  QPSolverTest<BlasMatrixd, BlasVectord>()

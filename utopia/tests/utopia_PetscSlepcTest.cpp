...
namespace utopia
{
	...
	public:
		...
		void petsc_slepc_generalized_eigen_test()
		{
			...
			PetscMatrix B = 99 * local_identity(local_size(A)); 
			bool verbose = false; 
	    ...
	    void tr_more_sorensen_eigen_test()
	    {
	    	if(mpi_world_size() != 1)
	       	...
	       	PetscVector x_w1  = values(4, 10);
			PetscVector expected_woods = values(4, 1);

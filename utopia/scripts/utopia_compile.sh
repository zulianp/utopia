...
function display_help() {
    printf "This is the general utopia build script:\n ARGUMENTS:\n\t-h: Displays this information.\n\t-b <build_type>: Run the preferred build type.\n\t\t<build_type>: \n\t\tbasic: Compile utopia with only blas, lapack, umfpack\n\t\tall: Compile utopia with petsc, trilinos, blas, lapack, umfpack, yaml-cpp and other options.\n\t\tfluya: Compile utopia in fluya mode.\n\t\tlocal: Compile utopia and download/install petsc and trilinos locally in utopia/external/.\n\t-j <n_jobs>: Number of jobs to run in make.\n\t-p <install_prefix>: Path for utopia installation. Default installation path is \"/usr/local/utopia\" on UNIX, \"C:/Program Files/utopia\" on Windows.\n"
}
...
if [ $# -eq 0 ]; then
    echo "No arguments provided, try using ./utopia_compile -h."
    exit 1
fi

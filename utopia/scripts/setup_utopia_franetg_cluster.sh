...
mkdir build && cd build
cmake .. -DCMAKE_INSTALL_PREFIX=$INSTALL_DIR/mars -DMARS_ENABLE_ADIOS2=ON -DADIOS2_DIR=$ADIOS2_DIR -DMARS_ENABLE_CUDA=ON -DMARS_ENABLE_CUDAUVM=ON
make -j10 && make install
...
cmake .. -DCMAKE_INSTALL_PREFIX=$INSTALL_DIR/utopia_franetg_gpu -DUTOPIA_ENABLE_FRANETG_MODE=ON -DCMAKE_CXX_COMPILER=$TRILINOS_DIR/bin/nvcc_wrapper
make yaml-cpp
...
cd utopia/utopia_fe
mkdir build_franetg && cd build_franetg
cmake .. -DUTOPIA_ENABLE_MARS=ON -DCMAKE_INSTALL_PREFIX=$INSTALL_DIR/utopia_fe_franetg_gpu -DUTOPIA_ENABLE_LIBMESH=OFF -DUTOPIA_ENABLE_STK=OFF -DMars_DIR=$INSTALL_DIR/mars -DCMAKE_CXX_COMPILER=$TRILINOS_DIR/bin/nvcc_wrapper -DUTOPIA_ENABLE_MOONOLITH=OFF
make -j10
make install

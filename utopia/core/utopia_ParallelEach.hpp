...
namespace utopia {
    ...
    inline void parallel_each_read(const Tensor &v, Fun fun, const std::string &name = "parallel_each_read")
    {
        ParallelEach<Tensor>::apply_read(v, fun, name);
    }

    template<class Tensor, class Fun>
    inline void parallel_each_write(Tensor &v, Fun fun, const std::string &name = "parallel_each_write")
    {

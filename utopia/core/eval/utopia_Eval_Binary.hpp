...
namespace utopia {
    ...
    public:
        ...
        static void apply(Left &&l, Right &&r, const Operation &op, Tensor<Res, Order> &result) {
            EvalBinaryAux<Tensor<Res, Order>>::apply(
                std::forward<Left>(l), std::forward<Right>(r), op, result.derived());
        }
        ...
        static void apply(Left &&l, Right &&r, const Operation &op, Number<Res> &result) {
            EvalBinaryAux<Number<Res>>::apply(std::forward<Left>(l), std::forward<Right>(r), op, result);
        }
    };
    ...
    public:
        ...
        template <class Left, class Right>
        static void apply(const Tensor<Left, 2> &left,
                          const Tensor<Right, 1> &right,
                          const Multiplies &,
                          Result &result) {
    ...
    public:
        template <class Op>
        static void apply(const Number<Scalar> &left,
                          const Number<Scalar> &right,
                          const Op &op,
    ...
    public:
        ...
        inline static void apply(const Expr &expr, Result &result) {
            ...
            apply_aux(Eval<Left, Traits>::apply(expr.left()),
        ...
        inline static void apply_aux(L &&l, R &&r, const Operation &op, Tensor<Res, Order> &result) {
            EvalBinaryAux<Tensor<Res, Order>>::apply(std::forward<L>(l), std::forward<R>(r), op, result.derived());
        }

        template <class L, class R, class Res>
        inline static void apply_aux(L &&l, R &&r, const Operation &op, Number<Res> &result) {
            EvalBinaryAux<Number<Res>>::apply(std::forward<L>(l), std::forward<R>(r), op, result);
        }
    };

    template <typename T, class Op>
    using TensorAssignElWise = Assign<Tensor<T, 1>, Binary<Tensor<T, 1>, Tensor<T, 1>, Op>>;
    ...
    public:
        ...
        inline static void apply(const Expr &expr) {
            ...
            apply(Eval<TensorT, Traits>::apply(expr.right().left()),
        ...
        inline static void apply(TL &&l, TR &&r, const Op &op, Res &result) {
            if (r.is_alias(result)) {
                if (!utopia::is_commutative<Op>::value) {
                    ...
                    EvalBinaryAux<TensorT>::apply(std::forward<TR>(r), std::forward<TL>(l), op, result.derived());
                }
            } else {
                EvalBinaryAux<TensorT>::apply(std::forward<TL>(l), std::forward<TR>(r), op, result.derived());
            }
        }
    };
    ...
    public:
        ...
        inline static void apply(const Expr &expr, T &result) {
            EvalAssignElWise<Tensor<T, 1>, Op, Traits>::apply(Eval<Tensor<T, 1>, Traits>::apply(expr.left()),

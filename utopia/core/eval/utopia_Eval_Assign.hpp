...
namespace utopia {
    ...
    public:
        ...
        inline static bool apply(const Expr &expr) {
            ...
            Eval<LeftExpr, Traits>::apply(expr.left()).assign(Eval<RightExpr, Traits>::apply(expr.right()));
    ...
    public:
        inline static bool apply(const Assign<Left, Unary<Right, Minus>> &expr) {
            ...
            l.construct(Eval<Right, Traits>::apply(expr.right().expr()));
    ...
    class Eval<Assign<Left, Binary<Number<T>, Unary<Right, Op>, Multiplies>>, Traits, Backend> {
    public:
        ...
        inline static void apply(const Assign<Left, Binary<Number<T>, Unary<Right, Op>, Multiplies>> &expr) {
            ...
            l.construct(Eval<Right, Traits>::apply(expr.right().right().expr()));
    ...
    using MatVecOpVec = utopia::Binary<MatVecMult<Matrix, Vector>, Tensor<Vector, 1>, Op>;

    template <class Matrix, class Vector, class Op>
    using AssignMatVecOpVec = utopia::Assign<Tensor<Vector, 1>, MatVecOpVec<Matrix, Vector, Op>>;
    ...
    public:
        ...
        inline static bool apply(const Expr &expr) {
            ...
            left.construct(Eval<Right, Traits>::apply(expr.right().expr()));
    ...
    public:
        ...
        template <typename T>
        static void apply(const ScaledVecExpr<T, Vector> &left_expr,
                          const Op &op,
                          const Matrix &A,
                          const Vector &x,
                          Vector &result) {
    ...
    public:
        ...
        static void apply(const Expr &expr) {
            ...
            EvalAssignScaledVecOpMatVecMult<Matrix, Vector, Plus>::apply(expr.right().left(), op, A, right, result);
    ...
    public:
        ...
        static void apply(const Expr &expr) {
            ...
            EvalAssignScaledVecOpMatVecMult<Matrix, Vector, Minus>::apply(expr.right().left(), op, A, right, result);
    ...
    class Eval<Assign<Left, Factory<CRS<RowPtr, ColIndex, Values>, 2>>, Traits, Backend> {
    public:

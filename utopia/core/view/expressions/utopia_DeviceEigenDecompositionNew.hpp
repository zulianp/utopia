...
namespace utopia {
	...
	private:
		...
		{
			const Scalar t0 = Scalar(0.5) * device::sqrt( device::squared(m(0,0)-m(1,1)) + Scalar(4)*device::squared(m(1,0)));
		...
		UTOPIA_INLINE_FUNCTION static void apply_2(
			const Expr &mat,
            ...
            EigenVectors &eigen_vectors)
		{
			...
			else
			{
				...
				if(a2>c2)
				{
					const Scalar norm_col = device::sqrt(a2+b2); UTOPIA_DEVICE_ASSERT(norm_col > 0);
					eigen_vectors(0, 1)   = -scaled_mat(1,0)/norm_col;
					eigen_vectors(1, 1)   =  scaled_mat(0,0)/norm_col;
				}
				else
				{
					const Scalar norm_col = device::sqrt(c2+b2); UTOPIA_DEVICE_ASSERT(norm_col > 0);
		...
		//////////////////////////////////////////////////////////////
		 ...
		 static UTOPIA_INLINE_FUNCTION void eigenvalues_3(const MatT& m, VecT& roots)
		 {
		    ...
		    Scalar theta     = device::atan2(device::sqrt(q),half_b)*s_inv3;  // since sqrt(q) > 0, atan2 is in [0, pi] and theta is in [0, pi/3]
		  ...
		  static inline void extract_kernel_3(const MatT& mat, VecT &res, VecT &representative)
		  {
		  	...
		  	if(n0>n1) res = c0/device::sqrt(n0);
		  	else      res = c1/device::sqrt(n1);
		  }

		template<class EigenValues, class EigenVectors>
		UTOPIA_INLINE_FUNCTION static void apply_3(
			const Expr &mat,
            EigenValues &eigen_values,
            EigenVectors &eigen_vectors)
		{

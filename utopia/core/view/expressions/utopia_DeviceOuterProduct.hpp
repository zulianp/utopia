...
namespace utopia {
    ...
    public:
        ...
        UTOPIA_INLINE_FUNCTION DeviceOuterProduct(const Left &left, const Right &right) : left_(left), right_(right) {}

        UTOPIA_INLINE_FUNCTION Scalar operator()(const SizeType &i, const SizeType &j) const {
            return left_[i] * right_[j];
        }

        UTOPIA_INLINE_FUNCTION SizeType rows() const { return left_.size(); }

        UTOPIA_INLINE_FUNCTION SizeType cols() const { return right_.size(); }

        friend UTOPIA_INLINE_FUNCTION SizeType rows(const DeviceOuterProduct &expr) { return expr.rows(); }

        friend UTOPIA_INLINE_FUNCTION SizeType cols(const DeviceOuterProduct &expr) { return expr.cols(); }

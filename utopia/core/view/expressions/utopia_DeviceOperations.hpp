...
namespace utopia {
    ...
    template <class Expr>
    UTOPIA_INLINE_FUNCTION DeviceBinary<Expr, DeviceNumber<double>, Minus> operator-(const DeviceExpression<Expr> &left,
                                                                                     const double &right) {
        return DeviceBinary<Expr, DeviceNumber<double>, Minus>(left, right);
    }

    template <class Expr>
    UTOPIA_INLINE_FUNCTION constexpr double operator-(const DeviceNumber<double> &left,
                                                      const DeviceNumber<double> &right) {
    ...
    UTOPIA_INLINE_FUNCTION constexpr DeviceBinary<Left, Right, Minus> operator-(const DeviceExpression<Left> &left,
                                                                                const DeviceExpression<Right> &right) {
        return DeviceBinary<Left, Right, Minus>(left.derived(), right.derived());
    }

    template <class Left, class Right>
    UTOPIA_INLINE_FUNCTION DeviceBinary<Left, Right, Plus> operator+(const DeviceExpression<Left> &left,
                                                                     const DeviceExpression<Right> &right) {
    ...
    UTOPIA_INLINE_FUNCTION DeviceBinary<DeviceMultiply<TensorView<L, 2>, TensorView<R, 1>>, TensorView<Left, 1>, Plus>
    operator+(const TensorView<Left, 1> &left, const DeviceMultiply<TensorView<L, 2>, TensorView<R, 1>> &right) {
        return DeviceBinary<DeviceMultiply<TensorView<L, 2>, TensorView<R, 1>>, TensorView<Left, 1>, Plus>(
            right.derived(), left.derived());
    }

    template <class Left, class Right>
    UTOPIA_INLINE_FUNCTION DeviceMultiply<Left, Right> operator*(const DeviceExpression<Left> &left,
                                                                 const DeviceExpression<Right> &right) {
    ...
    UTOPIA_INLINE_FUNCTION DeviceUnary<Derived, Operation> transform(const DeviceExpression<Derived> &expr,
    ...
    template <class Derived>
    UTOPIA_INLINE_FUNCTION DeviceUnary<Derived, Pow> power(const DeviceExpression<Derived> &expr,
                                                           const typename Traits<Derived>::Scalar &a) {
        return DeviceBinary<Derived, DeviceNumber<typename Traits<Derived>::Scalar>, Pow>(expr.derived(), a);
    }
     ...
     */
    template <class Derived>
    UTOPIA_INLINE_FUNCTION DeviceBinary<DeviceNumber<typename Derived::Scalar>, Derived, Divides> operator/(
        const typename Derived::Scalar &left,
        const DeviceExpression<Derived> &expr) {
        return DeviceBinary<DeviceNumber<typename Derived::Scalar>, Derived, Divides>(left, expr.derived());
    }
    ...
    template <class Left, class Right>
    UTOPIA_INLINE_FUNCTION typename Traits<Right>::Scalar dot(const DeviceExpression<Left> &left,
                                                              const DeviceExpression<Right> &right) {
        return DeviceDot<Left, Right, Traits<Left>::Order>::apply(left.derived(), right.derived());
    }

    template <class Left, class Right>
    UTOPIA_INLINE_FUNCTION typename Traits<Right>::Scalar inner(const DeviceExpression<Left> &left,
                                                                const DeviceExpression<Right> &right) {
    ...
    template <class Left, class Right>
    UTOPIA_INLINE_FUNCTION DeviceOuterProduct<Left, Right> outer(const DeviceExpression<Left> &left,
                                                                 const DeviceExpression<Right> &right) {
        return DeviceOuterProduct<Left, Right>(left.derived(), right.derived());
    }

    template <class Left, class Right>
    UTOPIA_INLINE_FUNCTION DeviceCrossProduct<Left, Right> cross(const DeviceExpression<Left> &left,
                                                                 const DeviceExpression<Right> &right) {
        return DeviceCrossProduct<Left, Right>(left.derived(), right.derived());
    }

    UTOPIA_INLINE_FUNCTION constexpr double inner(const double &left, const double &right) { return left * right; }
    ...
    template <class Expr>
    UTOPIA_INLINE_FUNCTION typename Traits<Expr>::Scalar norm1(const DeviceExpression<Expr> &expr) {
        return DeviceNorm<Expr, 1>::apply(expr.derived());
    }

    template <class Expr>
    UTOPIA_INLINE_FUNCTION typename Traits<Expr>::Scalar norm2(const DeviceExpression<Expr> &expr) {
        return DeviceNorm<Expr, 2>::apply(expr.derived());
    }

    template <class Expr>
    UTOPIA_INLINE_FUNCTION typename Traits<Expr>::Scalar norm_infty(const DeviceExpression<Expr> &expr) {
     ...
     */
    template <class Left, class Right>
    UTOPIA_INLINE_FUNCTION DeviceBinary<Left, Right, EMultiplies> e_mul(const DeviceExpression<Left> &left,
                                                                        const DeviceExpression<Right> &right) {
        return DeviceBinary<Left, Right, EMultiplies>(left.derived(), right.derived());
    }
     ...
     */
    template <class Left, class Right>
    UTOPIA_INLINE_FUNCTION DeviceBinary<Left, Right, Min> min(const DeviceExpression<Left> &left,
                                                              const DeviceExpression<Right> &right) {
    ...
    // UTOPIA_INLINE_FUNCTION DeviceBinary<Left, DeviceNumber<Right>, Min> min(const DeviceExpression<Left> &left, const
    // Factory<Values<Right>, Order> &right) {
     ...
     */
    // template<class Left, class Right, int Order>
    // UTOPIA_INLINE_FUNCTION DeviceBinary<Left, DeviceNumber<Right>, Min> min(const Factory<Values<Right>, Order>
     ...
     */
    template <class Left, class Right>
    UTOPIA_INLINE_FUNCTION DeviceBinary<Left, Right, Max> max(const DeviceExpression<Left> &left,
                                                              const DeviceExpression<Right> &right) {
     ...
     */
    // template<class Left, class Right, int Order>
    // UTOPIA_INLINE_FUNCTION DeviceBinary<Left, DeviceNumber<Right>, Max> max(const DeviceExpression<Left> &left, const
    // Factory<Values<Right>, Order> &right) {
     ...
     */
    // template<class Left, class Right, int Order>
    // UTOPIA_INLINE_FUNCTION DeviceBinary<Left, DeviceNumber<Right>, Max> max(const Factory<Values<Right>, Order>
    // &right, const DeviceExpression<Left> &left) {
    //     return DeviceBinary<Left, DeviceNumber<Right>, Max>(left.derived(), right.type().value());
    // }
    ...
    UTOPIA_INLINE_FUNCTION void eig(const DeviceExpression<Expr> &expr, Result &eigen_values) {
        DeviceEigenValues<Expr>::apply(expr.derived(), eigen_values);
    }

    template <class Expr, class Values, class Vectors>
    UTOPIA_INLINE_FUNCTION void eig(const DeviceExpression<Expr> &expr, Values &eigen_values, Vectors &eigen_vectors) {
    ...
    template <class Expr, class Result>
    UTOPIA_INLINE_FUNCTION void sv(const DeviceExpression<Expr> &expr, Result &singular_values) {

...
namespace utopia {
    namespace device {
        ...
        UTOPIA_INLINE_FUNCTION void copy(const InputIterator &in_begin,
        ...
        UTOPIA_INLINE_FUNCTION auto extent(const NDimArray &in, const S &s)
            -> decltype(Extent<NDimArray>::apply(in, s)) {
        ...
        class Zero {
        public:
            UTOPIA_INLINE_FUNCTION static constexpr T value() noexcept { return T(0); }
        };

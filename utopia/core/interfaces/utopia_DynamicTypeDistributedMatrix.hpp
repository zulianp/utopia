...
namespace utopia {
     ...
     */
    template <typename Scalar, typename SizeType, class DerivedMatrixType, class ConcreteVectorType>
    class DynamicTypeDistributedMatrix : public DistributedMatrix<Scalar, SizeType>,
    ...
    public:
        ...
        void swap(DistributedMatrix &x) override { derived_().swap(static_cast<DerivedMatrixType &>(x)); }

        void copy(const DistributedMatrix &x) override { derived_().copy(static_cast<const DerivedMatrixType &>(x)); }
        ...
        void multiply(const Scalar &alpha, const DistributedMatrix &B, DistributedMatrix &C) const override {
            derived_().axpy(
                alpha, static_cast<const DerivedMatrixType &>(B), static_cast<const DerivedMatrixType &>(C));
        }
        ...
        void gemv(const bool transpose,
                  ...
                  const Scalar &beta,
                  DistributedVector &y) const override {
            derived_().gemv(transpose,
                            alpha,
    ...
    private:
        DerivedMatrixType &derived_() { return static_cast<DerivedMatrixType &>(*this); }

        const DerivedMatrixType &derived_() const { return static_cast<const DerivedMatrixType &>(*this); }
    };

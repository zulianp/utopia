...
namespace utopia {
    ...
    public:
        ...
        /// y = A * x
        virtual void multiply(const Vector &x, Vector &y) const { multiply(1.0, x, y); }

        /// y = alpha * A * x
        virtual void multiply(const Scalar &alpha, const Vector &x, Vector &y) const { gemv(false, alpha, x, 0.0, y); }

        /// y = alpha * A * x
        virtual void transpose_multiply(const Vector &x, Vector &y) const { gemv(false, 1.0, x, 0.0, y); }
        ...
        virtual void gemv(const bool transpose,
                          const Scalar &alpha,
                          const Vector &x,
                          const Scalar &beta,
                          Vector &y) const = 0;
    ...
    public:
        ...
        virtual void multiply(const Matrix &B, Matrix &C) const { multiply(1.0, B, C); }

        /// C := alpha * A * B
        virtual void multiply(const Scalar &alpha, const Matrix &B, Matrix &C) const = 0;

        /// C := A^T * B
        virtual void transpose_multiply(const Matrix &B, Matrix &C) const { multiply(true, false, B, C); }

        /// C := A * B^T
        virtual void multiply_transpose(const Matrix &B, Matrix &C) const { multiply(false, true, B, C); }

        /// C := alpha * op(A) * op(B)
        virtual void multiply(const bool transpose_A, const bool transpose_B, const Matrix &B, Matrix &C) const = 0;
    ...
    public:
        ...
        /// C := alpha * op(A) * op(B)
        void multiply(const bool transpose_A, const bool transpose_B, const Matrix &B, Matrix &C) const override {
            gemm(transpose_A, 1.0, transpose_B, B, 0.0, C);
        }

        // <Scalar>GEMM - matrix matrix multiply  C := alpha*op( A )*op( B ) + beta*C
        virtual void gemm(const bool transpose_A,
                          const Scalar alpha,

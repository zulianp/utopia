...
namespace utopia {
    ...
    class Wrapper<Matrix, 2>
        : public AbstractMatrix<typename Traits<Matrix>::Scalar, typename Traits<Matrix>::SizeType> {
    public:
        ...
        // basic mutators
        inline void set(const SizeType &i, const SizeType &j, const Scalar &value) override { impl_->set(i, j, value); }

        inline void add(const SizeType &i, const SizeType &j, const Scalar &value) override { impl_->add(i, j, value); }

        // print function
        inline void describe() const override { impl_->describe(); }

        // utility functions
        inline bool empty() const override { return impl_->empty(); }

        inline void clear() override { return impl_->clear(); }

        inline SizeType rows() const override { return impl_->rows(); }

        inline SizeType cols() const override { return impl_->cols(); }
        ...
        inline Range row_range() const override { return impl_->row_range(); }
        inline Range col_range() const override { return impl_->col_range(); }

        inline SizeType local_rows() const override { return impl_->local_rows(); }
        inline SizeType local_cols() const override { return impl_->local_cols(); }

        inline Size local_size() const override { return impl_->local_size(); }

        inline Size size() const override { return impl_->size(); }
        ...
        inline SizeType nnz(const Scalar tol = 0.0) const override { return impl_->nnz(tol); }

        inline Scalar trace() const override { return impl_->trace(); }
        ...
        /// y := alpha * A * x + beta * y
        inline void multiply_add(const Scalar &alpha,
                                 const AbstractVector &x,
                                 const Scalar &beta,
                                 AbstractVector &y) const override {
            auto &x_w = static_cast<const VectorWrapper &>(x);
            auto &y_w = static_cast<VectorWrapper &>(y);
        ...
        /// y := alpha * A' * x + beta * y
        inline void transpose_multiply_add(const Scalar &alpha,
                                           const AbstractVector &x,
                                           const Scalar &beta,
                                           AbstractVector &y) const override {
            auto &x_w = static_cast<const VectorWrapper &>(x);
        ...
        inline void gemv(const bool transpose,
                         const Scalar &alpha,
                         const AbstractVector &x,
                         const Scalar &beta,
                         AbstractVector &y) const override {
        ...
        /// C := alpha * op(A) * op(B)
        inline void multiply(const bool transpose_A,
        ...
        Matrix &get() { return *impl_; }

        const Matrix &get() const { return *impl_; }

        std::shared_ptr<Matrix> ptr() { return impl_; }

        std::shared_ptr<const Matrix> ptr() const { return impl_; }

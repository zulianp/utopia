...
namespace utopia {
    ...
    class Fill2String {
    public:
        constexpr static const char *Value() { return "Undefined"; }
    };

    template <>
    class Fill2String<FillType::SPARSE> {
    public:
        constexpr static const char *Value() { return "Sparse"; }
    };

    template <>
    class Fill2String<FillType::DENSE> {
    public:
        constexpr static const char *Value() { return "Dense"; }
    };

    template <>
    class Fill2String<FillType::DELEGATE> {
    public:
        constexpr static const char *Value() { return ""; }
    };

    template <>
    class Fill2String<FillType::SCALAR> {
    public:
        constexpr static const char *Value() { return "Scalar"; }
    };

    template <>
    class Fill2String<FillType::POLYMORPHIC> {
    public:
        constexpr static const char *Value() { return "Polymorphic"; }
    };
    ...
    // #define WRAPPER_TYPE(Traits, Expr) utopia::Wrapper<typename utopia::TensorQuery<Traits, Expr::Order,
    // FillTypeQuery<Expr>::value>::Type, Expr::Order>
    ...
    public:
        ...
        enum { FILL_TYPE = FillType::DELEGATE };
    };
    ...
    struct is_sparse {
        enum { value = TraitsT::FILL_TYPE == FillType::SPARSE || TraitsT::FILL_TYPE == FillType::POLYMORPHIC };
    };

    template <class Tensor, class TraitsT = Traits<Tensor>>
    struct is_polymorhic {
        enum { value = TraitsT::FILL_TYPE == FillType::POLYMORPHIC };
    };

    template <class Tensor, class TraitsT = Traits<Tensor>>
    struct is_dense {
        enum { value = TraitsT::FILL_TYPE == FillType::DENSE };
    };

    template <class Tensor, class TraitsT = Traits<Tensor>>
    struct is_dense_or_polymorphic {
        enum { value = is_dense<Tensor>::value || is_polymorhic<Tensor>::value };
    };
...
#define EXPR_TYPE(ImplementationTraits, Expr) typename utopia::TypeAndFill<ImplementationTraits, Expr>::Type
    ...
    public:
        enum { FILL_TYPE = FillType::SPARSE };
    };

    class DefaultPolymorphicTraits {
    public:
        enum { FILL_TYPE = FillType::POLYMORPHIC };
    };

#define UTOPIA_MAKE_TRAITS(TensorType, TraitsType, Order)                                                             \
    template <>                                                                                                       \
    class Traits<TensorType> : public TraitsType, public DefaultDelegateTraits, public DefaultTensorTraits<Order> {}; \
    template <>                                                                                                       \
    class Traits<const TensorType &>                                                                                  \
        : public TraitsType, public DefaultDelegateTraits, public DefaultTensorTraits<Order> {};                      \
    template <>                                                                                                       \
    class Traits<TensorType &> : public TraitsType, public DefaultDelegateTraits, public DefaultTensorTraits<Order> {}

#define UTOPIA_MAKE_TRAITS_SPARSE(TensorType, TraitsType, Order)                                                    \
    template <>                                                                                                     \
    class Traits<TensorType> : public TraitsType, public DefaultSparseTraits, public DefaultTensorTraits<Order> {}; \
    template <>                                                                                                     \
    class Traits<const TensorType &>                                                                                \
        : public TraitsType, public DefaultSparseTraits, public DefaultTensorTraits<Order> {};                      \
    template <>                                                                                                     \
    class Traits<TensorType &> : public TraitsType, public DefaultSparseTraits, public DefaultTensorTraits<Order> {}

#define UTOPIA_MAKE_TRAITS_DENSE(TensorType, TraitsType, Order)                                                    \
    template <>                                                                                                    \
    class Traits<TensorType> : public TraitsType, public DefaultDenseTraits, public DefaultTensorTraits<Order> {}; \
    template <>                                                                                                    \
    class Traits<const TensorType &>                                                                               \
        : public TraitsType, public DefaultDenseTraits, public DefaultTensorTraits<Order> {};                      \
    template <>                                                                                                    \
    class Traits<TensorType &> : public TraitsType, public DefaultDenseTraits, public DefaultTensorTraits<Order> {}

#define UTOPIA_MAKE_TRAITS_POLYMORPHIC(TensorType, TraitsType, Order)                               \
    template <>                                                                                     \
    class Traits<TensorType>                                                                        \
        : public TraitsType, public DefaultPolymorphicTraits, public DefaultTensorTraits<Order> {}; \
    template <>                                                                                     \
    class Traits<const TensorType &>                                                                \
        : public TraitsType, public DefaultPolymorphicTraits, public DefaultTensorTraits<Order> {}; \
    template <>                                                                                     \
...
#define UTOPIA_MAKE_TRAITS_TPL_1(TensorType, TraitsType, Order)                                     \
    template <typename T>                                                                           \
    class Traits<TensorType<T>>                                                                     \
        : public TraitsType<T>, public DefaultDelegateTraits, public DefaultTensorTraits<Order> {}; \
    template <typename T>                                                                           \
    class Traits<const TensorType<T> &>                                                             \
        : public TraitsType<T>, public DefaultDelegateTraits, public DefaultTensorTraits<Order> {}; \
    template <typename T>                                                                           \
    class Traits<TensorType<T> &>                                                                   \
        : public TraitsType<T>, public DefaultDelegateTraits, public DefaultTensorTraits<Order> {}
...
#define UTOPIA_MAKE_TRAITS_SPARSE_TPL_1(TensorType, TraitsType, Order)                            \
    template <typename T>                                                                         \
    class Traits<TensorType<T>>                                                                   \
        : public TraitsType<T>, public DefaultSparseTraits, public DefaultTensorTraits<Order> {}; \
    template <typename T>                                                                         \
    class Traits<const TensorType<T> &>                                                           \
        : public TraitsType<T>, public DefaultSparseTraits, public DefaultTensorTraits<Order> {}; \
    template <typename T>                                                                         \
...
#define UTOPIA_MAKE_TRAITS_DENSE_TPL_1(TensorType, TraitsType, Order)                            \
    template <typename T>                                                                        \
    class Traits<TensorType<T>>                                                                  \
        : public TraitsType<T>, public DefaultDenseTraits, public DefaultTensorTraits<Order> {}; \
    template <typename T>                                                                        \
    class Traits<const TensorType<T> &>                                                          \
        : public TraitsType<T>, public DefaultDenseTraits, public DefaultTensorTraits<Order> {}; \
    template <typename T>                                                                        \
...
#define UTOPIA_MAKE_PARALLEL_TRAITS(TensorType) \
    template <>                                 \

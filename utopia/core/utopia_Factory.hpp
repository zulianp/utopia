...
namespace utopia {
    ...
    public:
        ...
        inline const T &nnz() const { return _nnz; }
        inline void setSparse(T nnz) { _nnz = nnz; }
    ...
    class NNZXRow {
    public:
        NNZXRow(const std::vector<SizeType> &d_nnz, const std::vector<SizeType> &o_nnz) : d_nnz(d_nnz), o_nnz(o_nnz) {}
    ...
    public:
        ...
        const Size::SizeType &local_size() const { return local_size_; }

        const Size::SizeType &global_size() const { return global_size_; }
    ...
    inline Factory<CRS<_SizeType, _IntType, _Scalar>, 2> crs(const Size::SizeType rows,
                                                             const Size::SizeType cols,
                                                             _SizeType &rowPtr,
                                                             _IntType &crs_columns,
                                                             _Scalar &values) {
        return Factory<CRS<_SizeType, _IntType, _Scalar>, 2>(
            Size({rows, cols}), CRS<_SizeType, _IntType, _Scalar>(rowPtr, crs_columns, values));
    }

    // FIXME make proper version
    template <class Index>
    UTOPIA_DEPRECATED_MSG("Use the object variant with 1st argument as the communicator.")
    inline Ghosts<Index> ghosted(const Size::SizeType &local_size, const Size::SizeType &global_size, Index &&index) {

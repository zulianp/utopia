...
namespace utopia {
    ...
    public:
        ...
        inline const Operation &operation() const { return operation_; }
    ...
    inline Reduce<Binary<Left, Right, EMultiplies>, Plus> dot(const Expression<Left> &left,
                                                              const Expression<Right> &right) {
        return sum(Binary<Left, Right, EMultiplies>(left.derived(), right.derived()));
    }

    template <class Left, class Right>
    inline Reduce<Binary<Left, Right, EMultiplies>, Plus> inner(const Expression<Left> &left,
                                                                const Expression<Right> &right) {
        return sum(Binary<Left, Right, EMultiplies>(left.derived(), right.derived()));
    }

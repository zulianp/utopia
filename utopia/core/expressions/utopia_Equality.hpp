...
namespace utopia {
    ...
    public:
        ...
        Equality(const Left &left, const Right &right) : left_(left), right_(right) {}
        ...
        std::string get_class() const { return "Equality<" + left().get_class() + ", " + right().get_class() + ">"; }
    ...
    template <class Left, class Right>
    inline Equality<Left, Right> operator==(const Expression<Left> &left, const Expression<Right> &right) {
        return Equality<Left, Right>(left.derived(), right.derived());
    }

    template <class Left, class Right>
    inline Equality<Left, Number<Right> > operator==(const Expression<Left> &left, const Number<Right> &right) {
        return Equality<Left, Number<Right> >(left.derived(), right);
    }

    /// If there is a scalar argument, then it is always place on the right
    template <class Left, class Right>
    inline Equality<Left, Number<Right> > operator==(const Number<Right> &right, const Expression<Left> &left) {
        return Equality<Left, Number<Right> >(left.derived(), right);
    }
}  // namespace utopia

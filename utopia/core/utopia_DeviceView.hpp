...
namespace utopia {
    ...
    public:
        ...
        inline void set(const SizeType &idx, const Scalar &value) const { tensor_.set(idx, value); }

        inline void add(const SizeType &idx, const Scalar &value) const { tensor_.add(idx, value); }
        ...
        DeviceView(T &tensor, WriteMode wm = utopia::AUTO)
            : tensor_(tensor), lock_(std::make_shared<ReadAndWrite<T>>(tensor, wm)) {}
    ...
    public:
        ...
        inline void set(const SizeType &idx, const Scalar &val) const { return tensor_.l_set(idx, val); }

        inline void add(const SizeType &idx, const Scalar &val) const { return tensor_.l_add(idx, val); }

        // FIXME
        inline void atomic_add(const SizeType &idx, const Scalar &val) const { return tensor_.l_add(idx, val); }

        inline ArrayView<Scalar> array() const { return tensor_.array_view_write(); }
    ...
    public:
        ...
        DeviceView(T &tensor, WriteMode wm = utopia::AUTO)
            : tensor_(tensor), lock_(std::make_shared<Write<T>>(tensor, wm)) {}

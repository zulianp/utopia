...
namespace utopia {
    ...
    public:
        ...
        inline static void eval(const InPlace<Left, Right, Operation> &expr) {
            ...
            eval(TransformedExpr(expr.left(), expr.right(), expr.operation()), expr.left());
        }
        ...
        inline static Scalar eval_at(const Binary<Left, Right, Operation> &expr, const SizeType i, const SizeType j) {
            return Operation::template apply<Scalar>(eval_at(expr.left(), i, j), eval_at(expr.right(), i, j));
        }
        ...
        template <class Left, class Right, class Operation>
        inline static Scalar eval_at(const Binary<Number<Left>, Right, Operation> &expr,
                                     const SizeType i,
                                     const SizeType j) {
            return Operation::template apply<Scalar>(static_cast<Left>(expr.left()), eval_at(expr.right(), i, j));
        }

        template <class Left, class Right, class Operation>
        inline static Scalar eval_at(const Binary<Left, Number<Right>, Operation> &expr,
                                     const SizeType i,
                                     const SizeType j) {
            return Operation::template apply<Scalar>(eval_at(expr.left(), i, j), static_cast<Right>(expr.right()));
        }
        ...
        inline static Scalar eval_at(const Binary<Left, Right, Operation> &expr, const SizeType i) {
            return Operation::template apply<Scalar>(eval_at(expr.left(), i), eval_at(expr.right(), i));
        }

        template <class Left, class Right, class Operation>
        inline static Scalar eval_at(const Binary<Number<Left>, Right, Operation> &expr, const SizeType i) {
            return Operation::template apply<Scalar>(expr.left(), eval_at(expr.right(), i));
        }

        template <class Left, class Right, class Operation>
        inline static Scalar eval_at(const Binary<Left, Number<Right>, Operation> &expr, const SizeType i) {
            return Operation::template apply<Scalar>(eval_at(expr.left(), i), expr.right());
        }
        ...
        template <class Left, class Right>
        inline static Scalar eval_at(const Multiply<Left, Right> &expr, const SizeType i, const SizeType j) {
        ...
        template <class Left, class Right>
        inline static Scalar eval_at(const Multiply<Left, Right> &expr, const SizeType i) {
            // Not implemented in parallel
            // FIXME implement range(expr)
        ...
        inline static Scalar eval_at(const Binary<Reduce<InnerExpr, ReduceOp>, Number<T>, Operation> &expr,
                                     const int i = 0,
                                     const int j = 0) {
            assert(i == 0 && j == 0);
            return Operation::template apply<Scalar>(static_cast<Scalar>(eval_at(expr.left())),
                                                     static_cast<T>(expr.right()));
        }

        template <class InnerExpr, class ReduceOp, typename T, class Operation>
        inline static Scalar eval_at(const Binary<Number<T>, Reduce<InnerExpr, ReduceOp>, Operation> &expr,
                                     const int i = 0,
                                     const int j = 0) {
            assert(i == 0 && j == 0);
            return Operation::template apply<Scalar>(static_cast<T>(expr.left()),
                                                     static_cast<Scalar>(eval_at(expr.right())));
        }

        template <class Left, class ReduceOpLeft, class Right, class ReduceOpRight, class Operation>
        inline static Scalar eval_at(
            const Binary<Reduce<Left, ReduceOpLeft>, Reduce<Right, ReduceOpRight>, Operation> &expr,
            const int i = 0,
            const int j = 0) {
            assert(i == 0 && j == 0);
